...
def get_pretrained(model, weights, out_features: int = None, num_train_layers: int = 0):
	"""
	根据指定的模型和权重创建一个预训练的模型，并修改其全连接层以适应新的分类任务。
	能用的模型包括：
	- ResNet: resnet18, ResNet18_Weights
	- VGG: vgg16, VGG16_Weights, VGG16_BN_Weights
	...
	weights: 预训练权重，通常来自ImageNet数据集，用于初始化模型。
...
# PIL图像转tensor
def pil_to_tensor(img: Image.Image, img_size: Union[int, tuple[int, int]]):
