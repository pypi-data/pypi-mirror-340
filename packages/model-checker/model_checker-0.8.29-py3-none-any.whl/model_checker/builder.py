...
class BuildProject:
    ...
    CORE_FILES = [
        ...
        "operators.py",
        "semantic.py",
        "semantics.py",  # Include both versions to handle different naming conventions
        "README.md"
    ]
    ...
    def _validate_source_directory(self):
        ...
        for file in self.ESSENTIAL_FILES:
            if not any(f.endswith(file) for f in os.listdir(self.source_dir)):
    ...
    def _validate_essential_files(self):
        ...
        missing_files = []
        for file in self.ESSENTIAL_FILES:
            if not any(f.endswith(file) for f in os.listdir(self.destination_dir)):
                missing_files.append(file)
