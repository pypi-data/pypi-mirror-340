...
class User(models.Model):
    ...
    def get_or_new(cls, username: str, defaults: dict = {}):
        ...
        v_user, created = cls.objects.get_or_create(
            username=username,
            defaults=defaults,
        )

        if created:
            log.success(f"👩‍🔬 Created NEW user entry: [{v_user.username}] ✨")
        else:
            log.debug(f"👩‍🔬 User already known: [{v_user.username}] 🆗")

        return v_user, created

    @classmethod
    def is_up_to_date_from_ad(cls, user_details, accessgroup_member):
        """Wrapper using an AD query response for creating / updating a user.

        Parameters
        ----------
        user_details : Bix
            A Box object with user details as returned by `user_details_from_dn`
            from `diop.activedirectory.ActiveDirectory`.
    ...
    def is_up_to_date(
        ...
        v_user, created = cls.get_or_new(username=username, defaults=defaults)

        if created or (
            v_user.email == email
            and v_user.fullname == fullname
            and v_user.department == department
            and v_user.enabled == enabled
            and v_user.accessgroup_member
        ):
            log.debug(f"👩‍🔬 No further user changes: [{v_user.username}] 🆗")
            return created, updated

        updated = True
        v_user.email = email
        v_user.fullname = fullname
        v_user.department = department
        v_user.enabled = enabled
        v_user.accessgroup_member = True
        v_user.save()
    ...
    def from_psy_session(cls, psy_session: dict):
        ...
        v_user, _ = cls.get_or_new(username, defaults={"email": upn})
