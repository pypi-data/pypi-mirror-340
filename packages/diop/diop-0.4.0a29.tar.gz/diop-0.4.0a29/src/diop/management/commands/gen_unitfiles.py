...
class Command(BaseCommand):
    ...
    def q2_unit(desc, cluster, workingdir=None, loglevel="SUCCESS"):
        ...
        settings = Path(env.str("SETTINGS_ENV")).resolve()
        venv = Path(env.str("VIRTUAL_ENV")).resolve()
        ...
        Environment=SETTINGS_ENV={settings}
        Environment=LOGURU_LEVEL={loglevel}
        Environment=Q_CLUSTER_NAME={cluster}
        ExecStartPre={venv}/bin/diop-manage migrate
        ExecStart={venv}/bin/diop-manage qcluster
    ...
    def diop_web_unit(workers=2, bind=None, workingdir=None, loglevel="SUCCESS"):
        ...
        Environment=LOGURU_LEVEL={loglevel}
        ExecStartPre={venv}/bin/diop-manage migrate
        ExecStart={venv}/bin/gunicorn --workers {workers} --bind {bind} diop_core.wsgi
