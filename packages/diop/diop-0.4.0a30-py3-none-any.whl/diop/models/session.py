...
class Session(models.Model):
    ...
    def cur_properties(self):
        """Shorthand for the current (=newest) "SessionProperties" element."""
        return self.sessionproperties_set.order_by("t_change").last()

    @property
    def cur_state(self):
        """Shorthand to get the current session state."""
        if self.t_end:
            return "ended"
    ...
    def get_or_new(cls, uid, machine, user, t_start):
        ...
        """
        v_session, created = Session.objects.get_or_create(
            uid=uid,
            machine=machine,
            user=user,
        ...
        if created:
            log.success(f"✨🎬 Created NEW session entry: {v_session}")
        else:
            log.debug(f"🎬 Session already known to DB: {v_session} 🆗")
        return v_session, created
