...
def status_update_task(func):
    ...
    *WARNING*: decorated functions will have any exception "silenced" explicitly
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            v_task = Task.objects.get(name=task_name)
            v_task.t_start = make_aware(datetime.now())
        except Task.DoesNotExist:
            v_task = Task(name=task_name, t_start=make_aware(datetime.now()))

        # use the first line of the function's docstring as task description:
        v_task.description = func.__doc__.split("\n")[0]
        v_task.save()
        ...
        try:
            ...
            v_task.t_end = make_aware(datetime.now())
            v_task.failed += 1
            v_task.save()
            v_issue = Issue(
            ...
            log.warning(f"❗📒  Registered issue {v_issue} in DB.")
