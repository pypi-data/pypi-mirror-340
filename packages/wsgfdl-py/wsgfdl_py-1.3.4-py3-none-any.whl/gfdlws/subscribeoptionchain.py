...
def get(ws, exchange,product,expiry, strikeprice,depth,optiontype=None, Unsubscribe=None):
    ...
    if strikeprice == "":
        return "strikeprice is mandatory."
        
    else:
        srp = strikeprice
    if depth == "":
        return "depth is mandatory."
    ...
    rmsg = asyncio.get_event_loop().run_until_complete(
        mass_subscribe_n_stream(ws, exg,  prd, exp, otp, srp,dth, oa))
    return rmsg


async def mass_subscribe_n_stream(ws, exg,  prd, exp, otp, srp,dth, oa):
    global printed_once
    try:
        req_msg = '{"MessageType":"SubscribeOptionChain",'
        if exg is not None:
...
async def get_msg(ws):
    while True:
        try:
            ...
            if rslt["MessageType"] == 'RealtimeOptionChainResult':
                return message
