...
def train_and_predict_catboost(year, df, feature_importances=[]):
    ...
    features = [col for col in df_train.columns if
                col not in ['yield', 'yield_anomaly', 'county_trend', 'Year', 'FIPS']]
    X_train = df_train[features]
    y_train = df_train['yield_anomaly']
    X_test = df_test[features]

    try:
        model = CatBoostRegressor(iterations=3500, depth=6, learning_rate=0.02, loss_function='RMSE', verbose=0,
                                  random_state=42)
...
def calculate_metrics(out_df):
    ...
    out_df['predicted_yield'] = out_df.apply(
        lambda row: row['fit'] + row['county_trend'] if pd.notna(row['county_trend']) else np.nan, axis=1)
    out_df['actual_yield'] = out_df.apply(
        lambda row: row['yield_anomaly'] + row['county_trend'] if pd.notna(row['county_trend']) else np.nan, axis=1)
    breakpoint()
    metrics = []
    for (year, model), group in out_df.groupby(['Year', 'model']):
        ...
        metrics.append({'Year': year, 'Model': model, 'State': state_name, 'FIPS': fips_code, 'MAPE': mape, 'MSE': mse,
                        'R-squared': r_squared})
    ...
    metrics_df = metrics_df.groupby(['Year', 'Model', 'State'])[numeric_cols].mean().reset_index()
...
def main():
    ...
    gam_results = [train_and_predict_r_gam(year, df, formula_gam, mgcv) for year in
                   tqdm(years, desc="Training GAM Model")]

    catboost_results = [train_and_predict_catboost(year, df) for year in tqdm(years, desc="Training CatBoost Model")]
    ...
    metrics_df.to_csv(r'D:\Users\ritvik\projects\GEOGLAM\Output\ml\analysis\model_results.csv', index=False)
