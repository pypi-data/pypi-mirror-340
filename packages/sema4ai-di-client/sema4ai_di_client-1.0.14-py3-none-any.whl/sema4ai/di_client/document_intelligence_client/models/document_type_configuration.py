...
class DocumentTypeConfiguration(BaseModel):
    """
    DocumentTypeConfiguration
    """ # noqa: E501
    source_location: StrictStr = Field(description="The S3 location of the document.")
    description: StrictStr = Field(description="A brief description of the document Type or use case.")
    validation_expressions: List[StrictStr] = Field(description="List of validation expressions for the document.")
    non_tbl_fields: List[DocumentTypeConfigurationNonTblFieldsInner] = Field(description="List of fields relevant to the document.")
    tbl_fields: List[DocumentTypeConfigurationTblFieldsInner] = Field(description="List of tables related to the document.")
    config: List[StrictStr] = Field(description="List of custom config keys for the doc type")
    processing_workflow: List[DocumentProcessingWorkflow] = Field(description="The workflow stages for processing this document type.")
    __properties: ClassVar[List[str]] = ["source_location", "description", "validation_expressions", "non_tbl_fields", "tbl_fields", "config", "processing_workflow"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of DocumentTypeConfiguration from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of DocumentTypeConfiguration from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "tbl_fields": [DocumentTypeConfigurationTblFieldsInner.from_dict(_item) for _item in obj["tbl_fields"]] if obj.get("tbl_fields") is not None else None,
