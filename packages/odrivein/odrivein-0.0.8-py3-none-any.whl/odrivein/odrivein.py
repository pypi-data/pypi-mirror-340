...
class Motor:
    ...
    def setPositionControl(self):
        self.odrvAxis.controller.input_pos = self.odrvAxis.encoder.pos_estimate
        time.sleep(0.5)
        self.odrvAxis.requested_state = AxisState.CLOSED_LOOP_CONTROL
        time.sleep(0.2)
    ...
    def getPosition(self):
        return (self.odrvAxis.encoder.pos_estimate - self.zeroPosition) / self.GEARBOX_RATIO * 360 * self.direction
    ...
    def setPositionAndWait(self, positionSetting:float):
        ...
        self.odrvAxis.controller.input_pos = ((positionSetting / 360) + self.zeroPosition) * self.GEARBOX_RATIO  * self.direction
        while abs(((positionSetting / 360) + self.zeroPosition) * self.GEARBOX_RATIO  * self.direction - self.odrvAxis.encoder.pos_estimate) > 1 * self.GEARBOX_RATIO / 360 :
            time.sleep(0.05)
