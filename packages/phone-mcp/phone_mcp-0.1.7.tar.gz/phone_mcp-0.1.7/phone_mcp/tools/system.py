...
async def get_current_window(mcp=None):
    ...
    try:
        ...
        if success and output.strip():
            ...
            for line in lines:
                ...
                if "mCurrentFocus" in line:
                    window_info["current_focus"] = line.replace("mCurrentFocus=", "").strip()
                    ...
                    window_info["focused_app"] = line.replace("mFocusedApp=", "").strip()
        
        # Method 2: Get current resumed activity 
        cmd = "adb shell dumpsys activity activities | grep -E 'ResumedActivity|mResumedActivity'"
        success, output = await run_command(cmd)
        ...
        if not success or "Error" in output:
            ...
            cmd = "adb shell cmd activity activities | grep -E 'ACTIVITY|topResumedActivity'"
            success, output = await run_command(cmd)
        ...
        if success and output.strip():
            success_count += 1
            window_info["recent_activity"] = output.strip()
        ...
        cmd = "adb shell dumpsys power | grep -E 'mWakefulness=|Display Power'"
        success, output = await run_command(cmd)
        if success and output.strip():
            window_info["screen_state"] = "on" if "ON" in output or "AWAKE" in output else "off"
        ...
        if success_count > 0:
            ...
            if success:
                return json.dumps({"basic_info": "Unable to get window details", 
                                 "device_model": model.strip()}, indent=2)
            else:
                return "Failed to retrieve current window information"
    except Exception as e:
        return f"Error retrieving window information: {str(e)}"

async def launch_activity(mcp=None, package_component=None, action=None, extra_args=None):
...
async def get_app_shortcuts(mcp=None, package_name=None):
    ...
    try:
        ...
        if package_name:
            cmd += f" | grep -A 50 'Package: {package_name}'"
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith("Package:"):
                ...
                parts = line.split(':', 1)
                if len(parts) > 1:
                    current_package = parts[1].strip().split()[0]  # Get package name without UID
