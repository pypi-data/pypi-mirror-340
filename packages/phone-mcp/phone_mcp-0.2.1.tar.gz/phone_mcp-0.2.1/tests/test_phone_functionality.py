...
from phone_mcp.tools.messaging import send_text_message, receive_text_messages, get_raw_messages
...
class TestPhoneFunctionality:
    ...
    async def test_call_number_success(self, adb_mock):
        ...
        with patch('phone_mcp.tools.call.run_command', new_callable=AsyncMock) as mock_call:
            # 设置模拟的命令执行返回值
    ...
    async def test_send_text_message_success(self, adb_mock):
        ...
        with patch('phone_mcp.tools.messaging.run_command', new_callable=AsyncMock) as mock_msg:
            # 模拟多个命令调用的成功情况
    ...
    async def test_receive_text_messages_success(self, adb_mock):
        ...
        with patch('phone_mcp.core.check_device_connection', new_callable=AsyncMock) as mock_check:
            # 模拟设备连接正常
            ...
            with patch('phone_mcp.tools.messaging.run_command', new_callable=AsyncMock) as mock_msg:
                # 模拟短信查询返回正确的短信内容
                mock_msg.return_value = (True, """Row: 0 address=13800138000, body=你好，这是测试短信, date=1628151234567
Row: 1 address=13900139000, body=这是另一条测试短信, date=1628151234000""")
    ...
    async def test_get_raw_messages_success(self, adb_mock):
        ...
        with patch('phone_mcp.core.check_device_connection', new_callable=AsyncMock) as mock_check:
            ...
            with patch('phone_mcp.tools.messaging.run_command', new_callable=AsyncMock) as mock_msg:
                # 模拟短信查询返回正确的短信内容
                mock_msg.return_value = (True, """Row: 0 address=13800138000, body=Hello, date=1628151234567
Row: 1 address=13900139000, body=世界, date=1628151234000""")
            ...
            assert any(text in result for text in ["Hello", "世界", "13800138000", "13900139000"])
