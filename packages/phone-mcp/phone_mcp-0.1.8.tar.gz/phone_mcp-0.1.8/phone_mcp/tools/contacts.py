...
async def get_contacts(mcp=None, limit=20):
    ...
    try:
        ...
        if not success or "usage:" in output:
            cmd = "adb shell \"sqlite3 /data/data/com.android.providers.contacts/databases/contacts2.db 'SELECT display_name, data1 FROM raw_contacts JOIN data ON (raw_contacts._id = data.raw_contact_id) WHERE mimetype_id = (SELECT _id FROM mimetypes WHERE mimetype = \"vnd.android.cursor.item/phone_v2\") LIMIT " + str(limit) + ";'\""
            success, output = await run_command(cmd)
