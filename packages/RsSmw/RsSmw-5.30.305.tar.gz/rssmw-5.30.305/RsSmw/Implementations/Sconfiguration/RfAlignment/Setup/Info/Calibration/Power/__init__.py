...
class PowerCls:
	...
	def get_mode(self) -> enums.RfPortValType:
		...
		Snippet: value: enums.RfPortValType = driver.sconfiguration.rfAlignment.setup.info.calibration.power.get_mode() \n
		...
		"""
		response = self._core.io.query_str('SCONfiguration:RFALignment:SETup:INFO:CALibration:POWer:MODE?')
	...
	def get_step(self) -> float:
		...
		Snippet: value: float = driver.sconfiguration.rfAlignment.setup.info.calibration.power.get_step() \n
		Queries the frequency/PEP step size with that the calibration values are selected from the defined value range. \n
			:return: pep_step: float
		"""
		response = self._core.io.query_str('SCONfiguration:RFALignment:SETup:INFO:CALibration:POWer:STEP?')
