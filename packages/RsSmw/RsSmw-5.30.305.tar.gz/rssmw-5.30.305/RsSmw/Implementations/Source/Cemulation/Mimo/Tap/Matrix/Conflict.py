...
class ConflictCls:
	...
	def get(self, mimoTap=repcap.MimoTap.Default) -> bool:
		"""SCPI: [SOURce<HW>]:CEMulation:MIMO:TAP<CH>:MATRix:CONFlict \n
		Snippet: value: bool = driver.source.cemulation.mimo.tap.matrix.conflict.get(mimoTap = repcap.MimoTap.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:CEMulation:MIMO:TAP{mimoTap_cmd_val}:MATRix:CONFlict?')
