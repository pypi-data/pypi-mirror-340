...
class VhCls:
	...
	def set(self, phase_vh: float, mimoTap=repcap.MimoTap.Default, subPath=repcap.SubPath.Default) -> None:
		"""SCPI: [SOURce<HW>]:CEMulation:SCM:TAP<ST>:SUBPath<DI>:PHASe:VH \n
		Snippet: driver.source.cemulation.scm.tap.subPath.phase.vh.set(phase_vh = 1.0, mimoTap = repcap.MimoTap.Default, subPath = repcap.SubPath.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:CEMulation:SCM:TAP{mimoTap_cmd_val}:SUBPath{subPath_cmd_val}:PHASe:VH {param}')

	def get(self, mimoTap=repcap.MimoTap.Default, subPath=repcap.SubPath.Default) -> float:
		"""SCPI: [SOURce<HW>]:CEMulation:SCM:TAP<ST>:SUBPath<DI>:PHASe:VH \n
		Snippet: value: float = driver.source.cemulation.scm.tap.subPath.phase.vh.get(mimoTap = repcap.MimoTap.Default, subPath = repcap.SubPath.Default) \n
		No command help available \n
			:param mimoTap: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Tap')
			:param subPath: optional repeated capability selector. Default value: Nr1 (settable in the interface 'SubPath')
		...
		mimoTap_cmd_val = self._cmd_group.get_repcap_cmd_value(mimoTap, repcap.MimoTap)
		subPath_cmd_val = self._cmd_group.get_repcap_cmd_value(subPath, repcap.SubPath)
		response = self._core.io.query_str(f'SOURce<HwInstance>:CEMulation:SCM:TAP{mimoTap_cmd_val}:SUBPath{subPath_cmd_val}:PHASe:VH?')
