...
class ProfileCls:
	...
	def get_catalog(self) -> List[str]:
		"""SCPI: [SOURce<HW>]:CEMulation:DSSimulation:USER:PROFile:CATalog \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:CEMulation:DSSimulation:USER:PROFile:CATalog?')
		return Conversions.str_to_str_list(response)

	def get_select(self) -> str:
		"""SCPI: [SOURce<HW>]:CEMulation:DSSimulation:USER:PROFile:SELect \n
		Snippet: value: str = driver.source.cemulation.dsSimulation.user.profile.get_select() \n
		No command help available \n
			:return: prof_select: No help available
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:CEMulation:DSSimulation:USER:PROFile:SELect?')
	...
	def set_select(self, prof_select: str) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:CEMulation:DSSimulation:USER:PROFile:SELect {param}')

	# noinspection PyTypeChecker
	def get_source(self) -> enums.FadDssUsrProfSour:
		"""SCPI: [SOURce<HW>]:CEMulation:DSSimulation:USER:PROFile:SOURce \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:CEMulation:DSSimulation:USER:PROFile:SOURce?')
	...
	def set_source(self, prof_source: enums.FadDssUsrProfSour) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:CEMulation:DSSimulation:USER:PROFile:SOURce {param}')
