...
class DelPyCls:
	...
	def get_state(self) -> bool:
		"""SCPI: [SOURce<HW>]:CEMulation:DEL:STATe \n
		Snippet: value: bool = driver.source.cemulation.delPy.get_state() \n
		No command help available \n
			:return: state: No help available
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:CEMulation:DEL:STATe?')
		return Conversions.str_to_bool(response)

	def set_state(self, state: bool) -> None:
		"""SCPI: [SOURce<HW>]:CEMulation:DEL:STATe \n
		Snippet: driver.source.cemulation.delPy.set_state(state = False) \n
		No command help available \n
			:param state: No help available
		"""
		param = Conversions.bool_to_str(state)
		self._core.io.write(f'SOURce<HwInstance>:CEMulation:DEL:STATe {param}')
