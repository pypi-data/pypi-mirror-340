...
class TrajectoryCls:
	...
	def get_value(self) -> enums.FadDssUsrTraj:
		"""SCPI: [SOURce<HW>]:CEMulation:DSSimulation:USER:RX:TRAJectory \n
		Snippet: value: enums.FadDssUsrTraj = driver.source.cemulation.dsSimulation.user.rx.trajectory.get_value() \n
		No command help available \n
			:return: trajectory: No help available
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:CEMulation:DSSimulation:USER:RX:TRAJectory?')
		return Conversions.str_to_scalar_enum(response, enums.FadDssUsrTraj)

	def set_value(self, trajectory: enums.FadDssUsrTraj) -> None:
		"""SCPI: [SOURce<HW>]:CEMulation:DSSimulation:USER:RX:TRAJectory \n
		Snippet: driver.source.cemulation.dsSimulation.user.rx.trajectory.set_value(trajectory = enums.FadDssUsrTraj.EPHemeris) \n
		No command help available \n
			:param trajectory: No help available
		"""
		param = Conversions.enum_scalar_to_str(trajectory, enums.FadDssUsrTraj)
		self._core.io.write(f'SOURce<HwInstance>:CEMulation:DSSimulation:USER:RX:TRAJectory {param}')
