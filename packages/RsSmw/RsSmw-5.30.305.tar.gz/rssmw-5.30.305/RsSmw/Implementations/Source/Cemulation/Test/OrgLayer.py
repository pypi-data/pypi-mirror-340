...
class OrgLayerCls:
	...
	def get(self, test_arg: str) -> str:
		"""SCPI: [SOURce<HW>]:CEMulation:TEST:ORGLayer \n
		Snippet: value: str = driver.source.cemulation.test.orgLayer.get(test_arg = 'abc') \n
		No command help available \n
			:param test_arg: No help available
			:return: test_answer: No help available"""
		param = Conversions.value_to_quoted_str(test_arg)
		response = self._core.io.query_str(f'SOURce<HwInstance>:CEMulation:TEST:ORGLayer? {param}')
