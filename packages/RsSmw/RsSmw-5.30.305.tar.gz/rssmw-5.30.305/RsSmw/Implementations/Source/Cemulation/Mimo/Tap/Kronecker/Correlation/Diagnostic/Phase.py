...
class PhaseCls:
	...
	def get(self, mimoTap=repcap.MimoTap.Default) -> float:
		"""SCPI: [SOURce<HW>]:CEMulation:MIMO:TAP<CH>:KRONecker:CORRelation:DIAG:PHASe \n
		Snippet: value: float = driver.source.cemulation.mimo.tap.kronecker.correlation.diagnostic.phase.get(mimoTap = repcap.MimoTap.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:CEMulation:MIMO:TAP{mimoTap_cmd_val}:KRONecker:CORRelation:DIAG:PHASe?')
