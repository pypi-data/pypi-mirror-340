...
class RdelayCls:
	...
	def get(self, fadingGroup=repcap.FadingGroup.Default, path=repcap.Path.Default) -> float:
		"""SCPI: [SOURce<HW>]:CEMulation:MDELay:DEL30:GROup<ST>:PATH<CH>:RDELay \n
		Snippet: value: float = driver.source.cemulation.mdelay.del30.group.path.rdelay.get(fadingGroup = repcap.FadingGroup.Default, path = repcap.Path.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:CEMulation:MDELay:DEL30:GROup{fadingGroup_cmd_val}:PATH{path_cmd_val}:RDELay?')
