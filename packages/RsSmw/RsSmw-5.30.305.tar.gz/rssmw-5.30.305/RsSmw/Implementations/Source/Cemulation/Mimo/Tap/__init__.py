...
class TapCls:
	...
	def get_value(self) -> enums.FadMimoTap:
		"""SCPI: [SOURce<HW>]:CEMulation:MIMO:TAP \n
		Snippet: value: enums.FadMimoTap = driver.source.cemulation.mimo.tap.get_value() \n
		No command help available \n
			:return: tap: No help available
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:CEMulation:MIMO:TAP?')
		return Conversions.str_to_scalar_enum(response, enums.FadMimoTap)

	def set_value(self, tap: enums.FadMimoTap) -> None:
		"""SCPI: [SOURce<HW>]:CEMulation:MIMO:TAP \n
		Snippet: driver.source.cemulation.mimo.tap.set_value(tap = enums.FadMimoTap.TAP1) \n
		No command help available \n
			:param tap: No help available
		"""
		param = Conversions.enum_scalar_to_str(tap, enums.FadMimoTap)
		self._core.io.write(f'SOURce<HwInstance>:CEMulation:MIMO:TAP {param}')
