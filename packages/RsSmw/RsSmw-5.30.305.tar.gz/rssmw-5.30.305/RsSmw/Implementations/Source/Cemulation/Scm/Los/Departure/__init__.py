...
class DepartureCls:
	...
	def get_angle(self) -> float:
		"""SCPI: [SOURce<HW>]:CEMulation:SCM:LOS:DEParture:[ANGLe] \n
		Snippet: value: float = driver.source.cemulation.scm.los.departure.get_angle() \n
		No command help available \n
			:return: angle: No help available
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:CEMulation:SCM:LOS:DEParture:ANGLe?')
		return Conversions.str_to_float(response)

	def set_angle(self, angle: float) -> None:
		"""SCPI: [SOURce<HW>]:CEMulation:SCM:LOS:DEParture:[ANGLe] \n
		Snippet: driver.source.cemulation.scm.los.departure.set_angle(angle = 1.0) \n
		No command help available \n
			:param angle: No help available
		"""
		param = Conversions.decimal_value_to_str(angle)
		self._core.io.write(f'SOURce<HwInstance>:CEMulation:SCM:LOS:DEParture:ANGLe {param}')
