...
class MatrixCls:
	...
	def get(self, mimoTap=repcap.MimoTap.Default) -> float:
		"""SCPI: [SOURce<HW>]:CEMulation:MIMO:TAP<CH>:ANTenna:POLarization:MATRix \n
		Snippet: value: float = driver.source.cemulation.mimo.tap.antenna.polarization.matrix.get(mimoTap = repcap.MimoTap.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:CEMulation:MIMO:TAP{mimoTap_cmd_val}:ANTenna:POLarization:MATRix?')
