...
class GainCls:
	...
	def set(self, gain: float, mimoTap=repcap.MimoTap.Default, ray=repcap.Ray.Default) -> None:
		"""SCPI: [SOURce<HW>]:CEMulation:MIMO:TAP<CH>:TGN:RAY<ST>:GAIN \n
		Snippet: driver.source.cemulation.mimo.tap.tgn.ray.gain.set(gain = 1.0, mimoTap = repcap.MimoTap.Default, ray = repcap.Ray.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:CEMulation:MIMO:TAP{mimoTap_cmd_val}:TGN:RAY{ray_cmd_val}:GAIN {param}')

	def get(self, mimoTap=repcap.MimoTap.Default, ray=repcap.Ray.Default) -> float:
		"""SCPI: [SOURce<HW>]:CEMulation:MIMO:TAP<CH>:TGN:RAY<ST>:GAIN \n
		Snippet: value: float = driver.source.cemulation.mimo.tap.tgn.ray.gain.get(mimoTap = repcap.MimoTap.Default, ray = repcap.Ray.Default) \n
		No command help available \n
			:param mimoTap: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Tap')
			:param ray: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ray')
		...
		mimoTap_cmd_val = self._cmd_group.get_repcap_cmd_value(mimoTap, repcap.MimoTap)
		ray_cmd_val = self._cmd_group.get_repcap_cmd_value(ray, repcap.Ray)
		response = self._core.io.query_str(f'SOURce<HwInstance>:CEMulation:MIMO:TAP{mimoTap_cmd_val}:TGN:RAY{ray_cmd_val}:GAIN?')
