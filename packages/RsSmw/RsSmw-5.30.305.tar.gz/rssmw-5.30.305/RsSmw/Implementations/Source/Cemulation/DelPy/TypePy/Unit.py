...
class UnitCls:
	...
	def set(self, delay_unit: enums.UnitTimeSecMsUsNsPs, typePy=repcap.TypePy.Default) -> None:
		"""SCPI: [SOURce<HW>]:CEMulation:DEL:TYPE<CH>:UNIT \n
		Snippet: driver.source.cemulation.delPy.typePy.unit.set(delay_unit = enums.UnitTimeSecMsUsNsPs.MS, typePy = repcap.TypePy.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:CEMulation:DEL:TYPE{typePy_cmd_val}:UNIT {param}')

	# noinspection PyTypeChecker
	def get(self, typePy=repcap.TypePy.Default) -> enums.UnitTimeSecMsUsNsPs:
		"""SCPI: [SOURce<HW>]:CEMulation:DEL:TYPE<CH>:UNIT \n
		Snippet: value: enums.UnitTimeSecMsUsNsPs = driver.source.cemulation.delPy.typePy.unit.get(typePy = repcap.TypePy.Default) \n
		No command help available \n
			:param typePy: optional repeated capability selector. Default value: Nr1 (settable in the interface 'TypePy')
			:return: delay_unit: No help available"""
		typePy_cmd_val = self._cmd_group.get_repcap_cmd_value(typePy, repcap.TypePy)
		response = self._core.io.query_str(f'SOURce<HwInstance>:CEMulation:DEL:TYPE{typePy_cmd_val}:UNIT?')
