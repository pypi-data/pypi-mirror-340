...
class SpeedCls:
	...
	def set(self, speed: float, fadingGroup=repcap.FadingGroup.Default, path=repcap.Path.Default) -> None:
		"""SCPI: [SOURce<HW>]:CEMulation:MDELay:DEL30:GROup<ST>:PATH<CH>:SPEed \n
		Snippet: driver.source.cemulation.mdelay.del30.group.path.speed.set(speed = 1.0, fadingGroup = repcap.FadingGroup.Default, path = repcap.Path.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:CEMulation:MDELay:DEL30:GROup{fadingGroup_cmd_val}:PATH{path_cmd_val}:SPEed {param}')

	def get(self, fadingGroup=repcap.FadingGroup.Default, path=repcap.Path.Default) -> float:
		"""SCPI: [SOURce<HW>]:CEMulation:MDELay:DEL30:GROup<ST>:PATH<CH>:SPEed \n
		Snippet: value: float = driver.source.cemulation.mdelay.del30.group.path.speed.get(fadingGroup = repcap.FadingGroup.Default, path = repcap.Path.Default) \n
		No command help available \n
			:param fadingGroup: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Group')
			:param path: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Path')
		...
		fadingGroup_cmd_val = self._cmd_group.get_repcap_cmd_value(fadingGroup, repcap.FadingGroup)
		path_cmd_val = self._cmd_group.get_repcap_cmd_value(path, repcap.Path)
		response = self._core.io.query_str(f'SOURce<HwInstance>:CEMulation:MDELay:DEL30:GROup{fadingGroup_cmd_val}:PATH{path_cmd_val}:SPEed?')
