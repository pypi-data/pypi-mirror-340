...
class GainCls:
	...
	def set(self, gain: float, mimoTap=repcap.MimoTap.Default) -> None:
		"""SCPI: [SOURce<HW>]:CEMulation:MIMO:TAP<CH>:GVECtor:DH:GAIN \n
		Snippet: driver.source.cemulation.mimo.tap.gvector.dh.gain.set(gain = 1.0, mimoTap = repcap.MimoTap.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:CEMulation:MIMO:TAP{mimoTap_cmd_val}:GVECtor:DH:GAIN {param}')

	def get(self, mimoTap=repcap.MimoTap.Default) -> float:
		"""SCPI: [SOURce<HW>]:CEMulation:MIMO:TAP<CH>:GVECtor:DH:GAIN \n
		Snippet: value: float = driver.source.cemulation.mimo.tap.gvector.dh.gain.get(mimoTap = repcap.MimoTap.Default) \n
		No command help available \n
			:param mimoTap: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Tap')
			:return: gain: No help available"""
		mimoTap_cmd_val = self._cmd_group.get_repcap_cmd_value(mimoTap, repcap.MimoTap)
		response = self._core.io.query_str(f'SOURce<HwInstance>:CEMulation:MIMO:TAP{mimoTap_cmd_val}:GVECtor:DH:GAIN?')
