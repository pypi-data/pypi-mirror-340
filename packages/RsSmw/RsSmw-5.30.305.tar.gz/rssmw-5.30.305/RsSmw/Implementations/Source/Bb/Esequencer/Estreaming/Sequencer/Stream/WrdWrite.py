...
class WrdWriteCls:
	...
	def get(self, sequencer=repcap.Sequencer.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:ESEQuencer:ESTReaming:[SEQuencer<ST>]:STReam:WRDWrite \n
		Snippet: value: float = driver.source.bb.esequencer.estreaming.sequencer.stream.wrdWrite.get(sequencer = repcap.Sequencer.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ESEQuencer:ESTReaming:SEQuencer{sequencer_cmd_val}:STReam:WRDWrite?')
