...
class PatternCls:
	...
	def set(self, pattern: str, satelliteSvid=repcap.SatelliteSvid.Default) -> None:
		...
		Snippet: driver.source.bb.gnss.svid.galileo.signal.l5Band.e5B.data.nmessage.pattern.set(pattern = rawAbc, satelliteSvid = repcap.SatelliteSvid.Default) \n
	...
	def get(self, satelliteSvid=repcap.SatelliteSvid.Default) -> str:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:GALileo:SIGNal:L5Band:E5B:DATA:NMESsage:PATTern \n
		Snippet: value: str = driver.source.bb.gnss.svid.galileo.signal.l5Band.e5B.data.nmessage.pattern.get(satelliteSvid = repcap.SatelliteSvid.Default) \n
		Sets a bit pattern as data source. \n
			:param satelliteSvid: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
