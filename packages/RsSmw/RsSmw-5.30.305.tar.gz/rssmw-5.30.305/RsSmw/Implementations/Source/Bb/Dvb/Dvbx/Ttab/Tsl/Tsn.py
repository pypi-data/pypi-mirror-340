...
class TsnCls:
	...
	def set(self, tsn: str, bitcount: int, channelNull=repcap.ChannelNull.Default) -> None:
		...
		Snippet: driver.source.bb.dvb.dvbx.ttab.tsl.tsn.set(tsn = rawAbc, bitcount = 1, channelNull = repcap.ChannelNull.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:DVB:DVBX:TTAB:TSL{channelNull_cmd_val}:TSN {param}'.rstrip())
	...
	def get(self, channelNull=repcap.ChannelNull.Default) -> TsnStruct:
		"""SCPI: [SOURce<HW>]:BB:DVB:DVBX:TTAB:TSL<CH0>:TSN \n
		Snippet: value: TsnStruct = driver.source.bb.dvb.dvbx.ttab.tsl.tsn.get(channelNull = repcap.ChannelNull.Default) \n
		Sets the 8-bit sized time-slice number in hexadecimal representation. Syntax to enter the hexadecimal value of the time
		slice number:#H<value>,8 \n
			:param channelNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Tsl')
			:return: structure: for return value, see the help for TsnStruct structure arguments."""
		channelNull_cmd_val = self._cmd_group.get_repcap_cmd_value(channelNull, repcap.ChannelNull)
		return self._core.io.query_struct(f'SOURce<HwInstance>:BB:DVB:DVBX:TTAB:TSL{channelNull_cmd_val}:TSN?', self.__class__.TsnStruct())
