...
class PilotsCls:
	...
	def set(self, pilots: bool, channelNull=repcap.ChannelNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:DVB:DVBS:TTAB:TSL<CH0>:PILots \n
		Snippet: driver.source.bb.dvb.dvbs.ttab.tsl.pilots.set(pilots = False, channelNull = repcap.ChannelNull.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:DVB:DVBS:TTAB:TSL{channelNull_cmd_val}:PILots {param}')

	def get(self, channelNull=repcap.ChannelNull.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:DVB:DVBS:TTAB:TSL<CH0>:PILots \n
		Snippet: value: bool = driver.source.bb.dvb.dvbs.ttab.tsl.pilots.get(channelNull = repcap.ChannelNull.Default) \n
		Enables or disables the pilots in the FEC frame of the respective time slice. \n
			:param channelNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Tsl')
			:return: pilots: 1| ON| 0| OFF"""
		channelNull_cmd_val = self._cmd_group.get_repcap_cmd_value(channelNull, repcap.ChannelNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:DVB:DVBS:TTAB:TSL{channelNull_cmd_val}:PILots?')
