...
class EphemerisCls:
	...
	def iodNav(self):
		...
		if not hasattr(self, '_iodNav'):
			from .IodNav import IodNavCls
			self._iodNav = IodNavCls(self._core, self._cmd_group)
		return self._iodNav
	...
	def sar(self):
		...
		if not hasattr(self, '_sar'):
			from .Sar import SarCls
			self._sar = SarCls(self._core, self._cmd_group)
		return self._sar
	...
	def sisa(self):
		...
		if not hasattr(self, '_sisa'):
			from .Sisa import SisaCls
			self._sisa = SisaCls(self._core, self._cmd_group)
		return self._sisa
