...
class RxdBytesCls:
	...
	def get(self, index=repcap.Index.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:ESEQuencer:ESTReaming:STReam:RXDBytes<CH2> \n
		Snippet: value: int = driver.source.bb.esequencer.estreaming.stream.rxdBytes.get(index = repcap.Index.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ESEQuencer:ESTReaming:STReam:RXDBytes{index_cmd_val}?')
