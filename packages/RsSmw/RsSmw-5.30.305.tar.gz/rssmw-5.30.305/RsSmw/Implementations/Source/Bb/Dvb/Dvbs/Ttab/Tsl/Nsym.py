...
class NsymCls:
	...
	def get(self, channelNull=repcap.ChannelNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:DVB:DVBS:TTAB:TSL<CH0>:NSYM \n
		Snippet: value: int = driver.source.bb.dvb.dvbs.ttab.tsl.nsym.get(channelNull = repcap.ChannelNull.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:DVB:DVBS:TTAB:TSL{channelNull_cmd_val}:NSYM?')
