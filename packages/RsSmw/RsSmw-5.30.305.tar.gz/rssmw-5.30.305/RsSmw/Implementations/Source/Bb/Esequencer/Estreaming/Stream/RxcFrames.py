...
class RxcFramesCls:
	...
	def get(self, indexNull=repcap.IndexNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:ESEQuencer:ESTReaming:STReam:RXCFrames<CH0> \n
		Snippet: value: int = driver.source.bb.esequencer.estreaming.stream.rxcFrames.get(indexNull = repcap.IndexNull.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ESEQuencer:ESTReaming:STReam:RXCFrames{indexNull_cmd_val}?')
