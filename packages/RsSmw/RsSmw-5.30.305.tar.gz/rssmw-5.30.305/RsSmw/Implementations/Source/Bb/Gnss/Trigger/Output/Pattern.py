...
class PatternCls:
	...
	def set(self, pattern: str, output=repcap.Output.Default) -> None:
		...
		Snippet: driver.source.bb.gnss.trigger.output.pattern.set(pattern = rawAbc, output = repcap.Output.Default) \n
	...
	def get(self, output=repcap.Output.Default) -> str:
		"""SCPI: [SOURce<HW>]:BB:GNSS:TRIGger:OUTPut<CH>:PATTern \n
		Snippet: value: str = driver.source.bb.gnss.trigger.output.pattern.get(output = repcap.Output.Default) \n
		Defines the bit pattern used to generate the marker signal. \n
			:param output: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')
