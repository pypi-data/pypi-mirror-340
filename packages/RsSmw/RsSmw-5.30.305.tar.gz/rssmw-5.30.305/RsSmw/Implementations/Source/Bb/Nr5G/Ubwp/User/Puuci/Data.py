...
class DataCls:
	...
	def set(self, data_source: enums.DataSourceA, userNull=repcap.UserNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:NR5G:UBWP:USER<US(CH0)>:PUUCi:DATA \n
		Snippet: driver.source.bb.nr5G.ubwp.user.puuci.data.set(data_source = enums.DataSourceA.DLISt, userNull = repcap.UserNull.Default) \n
		Selects the data source for the PUSCH UCI.
			INTRO_CMD_HELP: Prerequisites for this command \n
			- Allow selection of data source ([:SOURce<hw>]:BB:NR5G:UBWP:USER<us>:PUUCi:DSUNique) . \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:NR5G:UBWP:USER{userNull_cmd_val}:PUUCi:DATA {param}')

	# noinspection PyTypeChecker
	def get(self, userNull=repcap.UserNull.Default) -> enums.DataSourceA:
		"""SCPI: [SOURce<HW>]:BB:NR5G:UBWP:USER<US(CH0)>:PUUCi:DATA \n
		Snippet: value: enums.DataSourceA = driver.source.bb.nr5G.ubwp.user.puuci.data.get(userNull = repcap.UserNull.Default) \n
		Selects the data source for the PUSCH UCI.
			INTRO_CMD_HELP: Prerequisites for this command \n
			- Allow selection of data source ([:SOURce<hw>]:BB:NR5G:UBWP:USER<us>:PUUCi:DSUNique) . \n
			:param userNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'User')
		...
		userNull_cmd_val = self._cmd_group.get_repcap_cmd_value(userNull, repcap.UserNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:NR5G:UBWP:USER{userNull_cmd_val}:PUUCi:DATA?')
