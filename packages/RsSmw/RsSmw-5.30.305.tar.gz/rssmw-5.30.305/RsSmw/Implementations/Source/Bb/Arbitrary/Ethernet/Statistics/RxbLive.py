...
class RxbLiveCls:
	...
	def get(self, channel=repcap.Channel.Default) -> int:
		...
		Snippet: value: int = driver.source.bb.arbitrary.ethernet.statistics.rxbLive.get(channel = repcap.Channel.Default) \n
		No command help available \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'RxbLive')
			:return: buffer_level_live: No help available"""
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ARBitrary:ETHernet:STATistics:RXBLive{channel_cmd_val}?')
