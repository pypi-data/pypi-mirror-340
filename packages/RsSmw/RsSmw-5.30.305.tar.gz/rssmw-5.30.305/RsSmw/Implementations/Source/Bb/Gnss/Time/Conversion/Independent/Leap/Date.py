...
class DateCls:
	...
	def set(self, year: int, month: int, day: int) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:TIME:CONVersion:INDependent:LEAP:DATE \n
		Snippet: driver.source.bb.gnss.time.conversion.independent.leap.date.set(year = 1, month = 1, day = 1) \n
		Sets the date of the leap second transition. For date format, use commas with no spacing to separate the year, month and
		day: <Year>,<Month>,<Day> \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:INDependent:LEAP:DATE {param}'.rstrip())
	...
	def get(self) -> DateStruct:
		"""SCPI: [SOURce<HW>]:BB:GNSS:TIME:CONVersion:INDependent:LEAP:DATE \n
		Snippet: value: DateStruct = driver.source.bb.gnss.time.conversion.independent.leap.date.get() \n
		Sets the date of the leap second transition. For date format, use commas with no spacing to separate the year, month and
		day: <Year>,<Month>,<Day> \n
			:return: structure: for return value, see the help for DateStruct structure arguments."""
		return self._core.io.query_struct(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:INDependent:LEAP:DATE?', self.__class__.DateStruct())
