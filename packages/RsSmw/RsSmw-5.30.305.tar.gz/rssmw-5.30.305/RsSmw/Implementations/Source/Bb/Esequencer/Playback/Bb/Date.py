...
class DateCls:
	...
	def get(self, baseband=repcap.Baseband.Default) -> str:
		"""SCPI: [SOURce<HW>]:BB:ESEQuencer:PLAYback:[BB<ST>]:DATE \n
		Snippet: value: str = driver.source.bb.esequencer.playback.bb.date.get(baseband = repcap.Baseband.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ESEQuencer:PLAYback:BB{baseband_cmd_val}:DATE?')
