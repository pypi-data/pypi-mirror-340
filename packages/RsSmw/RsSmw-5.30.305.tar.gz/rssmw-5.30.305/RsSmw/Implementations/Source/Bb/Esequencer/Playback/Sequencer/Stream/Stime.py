...
class StimeCls:
	...
	def get(self, sequencer=repcap.Sequencer.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:ESEQuencer:PLAYback:[SEQuencer<ST>]:STReam:STIMe \n
		Snippet: value: float = driver.source.bb.esequencer.playback.sequencer.stream.stime.get(sequencer = repcap.Sequencer.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ESEQuencer:PLAYback:SEQuencer{sequencer_cmd_val}:STReam:STIMe?')
