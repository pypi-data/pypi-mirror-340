...
class LlmSystem():
    ...
    llm_api_env_name: str = field(default=None)
    llm_api_url: str = field(default=None) 
    library: str = field(default=None)
    additional_hyperparams: dict = field(default_factory=dict)
    max_tokens: int = field(default=512)
    temperature: float = field(default=0.9)

    def __post_init__(self):
        assert (self.llm_api_env_name is None) != (self.llm_api_key is None), "You must provide either a llm_api_env_name or a llm_api_key."
        if self.llm_api_env_name:
            self.llm_api_key = os.getenv(self.llm_api_env_name)
            assert self.llm_api_key is not None, f"Environment variable {self.llm_api_env_name} is not set or is empty"
