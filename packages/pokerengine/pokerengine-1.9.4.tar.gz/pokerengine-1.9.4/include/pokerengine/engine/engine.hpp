...
class engine {
  ...
  auto join_player(
                  uint32_t stack,
                  const std::string &id,
                  std::optional< std::map< std::string, pybind11::object > > parameters = std::nullopt) -> player {
  ...
  auto left_player(const std::string &id) {
    ...
    auto iterable = get_players();
    std::vector<player> new_players;
    std::copy_if(iterable.cbegin(), iterable.cend(), std::back_inserter(new_players), [&](const auto&element) -> bool {
