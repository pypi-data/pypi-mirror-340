...
from tensorkube.constants import REGION, NODEPOOL, EC2NODECLASS
...
def install_nydus_snapshotter_helm(nodepool_name: str):
    chart_path = resource_filename("tensorkube",'configurations/helm-charts/nydus-snapshotter')
    namespace = 'nydus-snapshotter'
    try:
        subprocess.run(["helm", "upgrade", "--install", "nydus-snapshotter",  chart_path, "--create-namespace", "--namespace", namespace, "--set", f"nodeSelector.nodepool={nodepool_name}"], check=True)
    except subprocess.CalledProcessError as e:
        print(f"Failed to install nydus snapshotter helm chart in namespace '{namespace}' from {chart_path}: {e}")        
...
def create_nydus_ecr_service_account():
    ...
    attach_role_policy("aws", "AmazonEC2ContainerRegistryReadOnly", role_name, REGION)
