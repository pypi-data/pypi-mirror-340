...
def fft_blur_check(image, threshold=150):
    ...
    blur_metric = int(np.mean(magnitude_spectrum))

    # If it is less than the threshold, then it should return True.
...
def is_image_blur(image, laplace_threshold=70, canny_threshold=1500, check_threshold=150,
                  bright_reflection_threshold=240, country=''):
    ...
    conditions.append(fft_blur_check(image, check_threshold))
    conditions.append(has_bright_reflections(image, bright_reflection_threshold))
