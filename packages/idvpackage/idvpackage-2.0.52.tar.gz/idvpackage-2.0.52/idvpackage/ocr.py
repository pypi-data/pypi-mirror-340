    ...
    def extract_front_id_info(self, front_id, country, nationality):
        ...
        if country == 'SDN':
            ...
            try:
                ...
                width, height = processed_front_id.size
                if width <= 400 and height <= 400:
                    front_data['error'] = 'covered_photo'
                    front_data['error_details'] = 'Image Resolution Too Low'
                    print(f"Image Resolution is {width}x{height} which is too low.")
                    return front_data

                image = np.array(processed_front_id)
                if is_image_blur(image, laplace_threshold=80, canny_threshold=1500, check_threshold=145,
                                 bright_reflection_threshold=240):
                    print(f"Passport Document is blurry, marking as covered photo")
                    front_data['error'] = 'covered_photo'
                    front_data['error_details'] = 'Image is Blur or has Glare/Bright Reflection.'
                    return front_data
    ...
    def extract_back_id_info(self, back_id, country, nationality):
        ...
        if country == 'SDN':
            ...
            try:
                ...
                processed_back_id = self.image_conversion(back_id)
                logging.info(f'----------------Time taken for image conversion back: {time.time() - st} seconds\n')
                width, height = processed_back_id.size
                if width <= 400 and height <= 400:
                    back_data['error'] = 'covered_photo'
                    back_data['error_details'] = 'Image Resolution Too Low'
                    print(f"Image Resolution is {width}x{height} which is too low.")
                    return back_data
                ...
                if is_image_blur(image, laplace_threshold=80, canny_threshold=1500, check_threshold=145,
                    ...
                    print(f"Passport Document is blurry, marking as covered photo")
                    back_data['error'] = 'covered_photo'
                    back_data['error_details'] = 'Image is Blur or has Glare/Bright Reflection.'
                    return back_data
    ...
    def exract_passport_info(self, passport, country, nationality, step_data=None):
        ...
        if (nationality and nationality == 'SDN') or (not nationality and country == 'SDN'):
            ...
            try:
                ...
                processed_passport = self.image_conversion(passport)
                logging.info(f'----------------Time taken for image conversion passport: {time.time() - st} seconds\n')
                width, height = processed_passport.size
                if width <= 400 and height <= 400:
                    passport_data['error'] = 'covered_photo'
                    passport_data['error_details'] = 'Image Resolution Too Low'
                    print(f"Image Resolution is {width}x{height} which is too low.")
                    return passport_data
                ...
                if is_image_blur(image, laplace_threshold=80, canny_threshold=1500, check_threshold=145,
                    ...
                    print(f"Passport Document is blurry, marking as covered photo")
                    passport_data['error'] = 'covered_photo'
                    passport_data['error_details'] = 'Image is Blur or has Glare/Bright Reflection.'
                    return passport_data
