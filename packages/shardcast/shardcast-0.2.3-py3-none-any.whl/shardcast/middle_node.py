        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                if self.downloader.download_file(
                    DISTRIBUTION_FILE,
                    os.path.join(self.data_dir, DISTRIBUTION_FILE),
                ...
                if distribution_content:
                    ...
                    for version in Path(self.data_dir).glob(f"{VERSION_PREFIX}*"):
                        if version not in distribution:
                            shutil.rmtree(os.path.join(self.data_dir, version))
