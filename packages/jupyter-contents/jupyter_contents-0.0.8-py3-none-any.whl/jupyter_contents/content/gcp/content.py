...
class GoogleStorageCheckpoints(GenericCheckpointsMixin, Checkpoints):
    ...
    checkpoint_bucket = Unicode(
        "",
        config=True,
        help="The bucket name where to keep file checkpoints."
    ...
    def create_file_checkpoint(self, content, format, path):
        ...
        self.log.debug("creating checkpoint %s for %s as %s", checkpoint_id, path, cp)
    ...
    def get_file_checkpoint(self, checkpoint_id, path):
        ...
        if not exists:
            raise web.HTTPError(
                404, "No such checkpoint: %s for %s" % (checkpoint_id, path)
            )
        content, fmt = self.parent._read_file(blob, None)
        return {"type": "file", "content": content, "format": fmt}
    ...
    def list_checkpoints(self, path):
        ...
        try:
            ...
            it = bucket.list_blobs(
                prefix=bucket_path, delimiter="/", max_results=self.parent.max_list_size
            )
            checkpoints = [
                {
...
class GoogleStorageContentManager(ContentsManager):
    ...
    hide_dotted_blobs = Bool(
        True,
        config=True,
        help="Consider blobs which names start with dot " "as hidden.",
    )
    ...
    default_path = Unicode("", config=True, help="The default path to open.")
    ...
    def is_hidden(self, path):
        ...
        if bucket is None:
            return True
        if self.hide_dotted_blobs and self._get_blob_name(bucket_path).startswith("."):
            return True
        return False
    ...
    def get(self, path, content=True, type=None, format=None):
        ...
        if type == "directory":
            ...
            if not exists:
                raise web.HTTPError(404, "No such file: %s" % path)
    ...
    def save(self, model, path):
        ...
        if bucket_path == "" and model["type"] != "directory":
            raise web.HTTPError(
                403, "You may only create directories " "(buckets) at the root level."
            )
        if (
            bucket_path != ""
        ...
        try:
            if model["type"] == "notebook":
                ...
                raise web.HTTPError(00, "Unhandled contents type: %s" % model["type"])
    ...
    def delete_file(self, path):
        ...
        if bucket_path == "":
            ...
            return
        it = bucket.list_blobs(
            prefix=bucket_path, delimiter="/", max_results=self.max_list_size
        )
    ...
    def rename_file(self, old_path, new_path):
        ...
        if old_bucket_name == new_bucket_name:
            ...
            if not new_bucket_path.endswith("/"):
                new_bucket_path += "/"
            it = old_bucket.list_blobs(
                prefix=old_bucket_path, delimiter="/", max_results=self.max_list_size
            )
            old_blobs = list(islice(it, self.max_list_size))
            folders = it.prefixes
            for ob in old_blobs:
                old_bucket.rename_blob(ob, new_bucket_path + self._get_blob_name(ob))
            for f in folders:
                self.rename_file(
                    old_bucket_name + "/" + f,
                    new_bucket_name
        ...
        folders = it.prefixes
        for ob in old_blobs:
            old_bucket.copy_blob(
                ob, new_bucket, new_bucket_path + self._get_blob_name(ob)
            )
            ob.delete()
        for f in folders:
            self.rename_file(
                old_bucket_name + "/" + f,
                new_bucket_name + "/" + f.replace(old_bucket_path, new_bucket_path, 1),
            )
    ...
    def _resolve_storagetype(self, path, storagetype):
        ...
        if self.dir_exists(path):
            return "directory"
        raise web.HTTPError(404, "%s does not exist" % path, reason="bad type")
    ...
    def _fetch(self, path, content=True):
        ...
        if bucket_path == "" or bucket_path.endswith("/"):
            ...
            try:
                it = bucket.list_blobs(
                    prefix=bucket_path, delimiter="/", max_results=max_list_size
                )
                try:
            ...
            folders = it.prefixes
            return (
                bool(files or folders or bucket_path == ""),
        ...
        if format is None or format == "text":
            ...
            try:
                ...
                if format == "text":
                    raise web.HTTPError(
                        400,
                        "%s is not UTF-8 encoded" % self._get_blob_path(blob),
                        reason="bad format",
                    )
    ...
    def _dir_model(self, path, members, content=True):
        ...
        if content:
            ...
            for blob in blobs:
                if self._get_blob_path(blob) != path and self.should_list(
                    self._get_blob_name(blob)
                ):
                    contents.append(self.get(path=blob, content=False))
            if path != "":
            ...
            for folder in folders:
                if self.should_list(folder) and folder != this:
                    contents.append(self.get(path=tmpl % folder, content=False))
            model["format"] = "json"
        return model
