...
from jupyter_contentss.content.gcp.content import GoogleStorageContentManager
...
class TestGoogleStorageContentManager(TestCase):
 ...
 "nbformat_minor": 0
    ...
    def tearDownClass(cls):
        GoogleStorageContentManager().client.bucket(cls.BUCKET).delete(force=True)
    ...
    def test_file_exists(self):
        ...
        try:
            self.assertTrue(self.contents_manager.file_exists(self.path("test")))
            self.assertFalse(self.contents_manager.file_exists(self.path("test_")))
        finally:
            blob.delete()
        blob = bucket.blob("test/other")
        blob.upload_from_string(b"test")
        try:
            self.assertFalse(self.contents_manager.file_exists(self.path("test")))
            self.assertFalse(self.contents_manager.file_exists(self.path("test/")))
            self.assertTrue(self.contents_manager.file_exists(self.path("test/other")))
            self.assertFalse(
                self.contents_manager.file_exists(self.path("test/other/"))
            )
        finally:
            blob.delete()
    ...
    def test_dir_exists(self):
        ...
        self.assertTrue(self.contents_manager.dir_exists("/" + self.BUCKET + "/"))
        self.assertTrue(self.contents_manager.dir_exists(self.BUCKET + "/"))
        self.assertFalse(self.contents_manager.dir_exists(self.BUCKET + "/" + "test"))
        self.assertFalse(
            self.contents_manager.dir_exists("/" + self.BUCKET + "/" + "test")
        )
        self.assertFalse(self.contents_manager.dir_exists(self.BUCKET + "blahblah"))
        self.assertFalse(
            self.contents_manager.dir_exists("/" + self.BUCKET + "blahblah")
        )
        bucket = self.bucket
        blob = bucket.blob("test")
        blob.upload_from_string(b"test")
        try:
            self.assertFalse(self.contents_manager.dir_exists(self.path("wtf")))
            self.assertFalse(self.contents_manager.dir_exists(self.path("test")))
            self.assertFalse(self.contents_manager.dir_exists(self.path("test/")))
        finally:
            blob.delete()
        blob = bucket.blob("test/")
        ...
        try:
            self.assertTrue(self.contents_manager.dir_exists(self.path("test")))
            self.assertTrue(self.contents_manager.dir_exists(self.path("test/")))
        finally:
            blob.delete()
        blob = bucket.blob("test/other/")
        ...
        try:
            self.assertTrue(self.contents_manager.dir_exists(self.path("test")))
            self.assertTrue(self.contents_manager.dir_exists(self.path("test/")))
            self.assertTrue(self.contents_manager.dir_exists(self.path("test/other")))
            self.assertTrue(self.contents_manager.dir_exists(self.path("test/other/")))
        finally:
            blob.delete()
        blob = bucket.blob("test/other")
        ...
        try:
            self.assertTrue(self.contents_manager.dir_exists(self.path("test")))
            self.assertTrue(self.contents_manager.dir_exists(self.path("test/")))
            self.assertFalse(self.contents_manager.dir_exists(self.path("test/other")))
            self.assertFalse(self.contents_manager.dir_exists(self.path("test/other/")))
        finally:
            blob.delete()
    ...
    def test_is_hidden(self):
        ...
        self.assertFalse(self.contents_manager.is_hidden(self.BUCKET + "/"))
        self.assertFalse(self.contents_manager.is_hidden("/" + self.BUCKET + "/"))
        self.assertFalse(self.contents_manager.is_hidden(self.path("something")))
        self.assertTrue(self.contents_manager.is_hidden(self.BUCKET + "blahblah"))
        self.assertTrue(self.contents_manager.is_hidden(self.BUCKET + "blahblah/test"))

        self.contents_manager.hide_dotted_blobs = True
        self.assertTrue(self.contents_manager.is_hidden(self.path(".test")))
        self.assertTrue(self.contents_manager.is_hidden(self.path(".test/")))
        self.contents_manager.hide_dotted_blobs = False
        self.assertFalse(self.contents_manager.is_hidden(self.path(".test")))
        self.assertFalse(self.contents_manager.is_hidden(self.path(".test/")))
        self.contents_manager.hide_dotted_blobs = True
        self.assertFalse(self.contents_manager.is_hidden(self.path(".test/other")))
        self.contents_manager.hide_dotted_blobs = False
        self.assertFalse(self.contents_manager.is_hidden(self.path(".test/other")))
        self.contents_manager.hide_dotted_blobs = True
    ...
    def test_get(self):
        ...
        try:
            ...
            self.assertEqual(model["content"], "contents")
            self.assertEqual(model["format"], "text")
            self.assertEqual(model["writable"], True)

            model2 = self.contents_manager.get(self.path("test/other.txt"), type="file")
            self.assertEqual(model, model2)
            with self.assertRaises(web.HTTPError):
                self.contents_manager.get(self.path("test/other.txt"), type="directory")
            with self.assertRaises(nbformat.reader.NotJSONError):
                self.contents_manager.get(self.path("test/other.txt"), type="notebook")
        except:  # nopep8
            blob.delete()
            raise
    ...
    def test_get_base64(self):
        ...
        model = self.contents_manager.get(self.path("test.pickle"), format="base64")
        self.assertEqual(model["type"], "file")
    ...
    def test_get_notebook(self):
        ...
        try:
            model = self.contents_manager.get(self.path("test.ipynb"), type="notebook")
            self.assertEqual(model["type"], "notebook")
            self.assertEqual(model["mimetype"], "application/x-ipynb+json")
            self.assertEqual(model["format"], "json")
            self.assertIsInstance(model["content"], nbformat.notebooknode.NotebookNode)
        finally:
            blob.delete()
    ...
    def test_rename_file(self):
        ...
        try:
            self.contents_manager.rename_file(
                self.path("test/other.txt"), self.path("test1/other1.txt")
            )
            self.assertFalse(blob.exists())
        except:  # nopep8
            blob.delete()
        ...
        blob2.upload_from_string(b"contents")
        try:
            self.contents_manager.rename_file(self.path("test/"), self.path("test1/"))
            self.assertFalse(blob1.exists())
            self.assertFalse(blob2.exists())
        except:  # nopep8
            try:
        ...
        new_bucket = self.contents_manager.client.bucket("jgscm-%s-new" % uuid.uuid4())
        new_bucket.create()
        try:
            self.contents_manager.rename_file(
    ...
    def test_save_dir(self):
        self.contents_manager.save({"type": "directory"}, self.path("test/"))
        bucket = self.bucket
        blob = bucket.blob("test/")
        self.assertTrue(blob.exists())
        blob.delete()
    ...
    def test_save_file(self):
        self.contents_manager.save(
            {"type": "file", "content": "blah-blah-blah", "format": "text"},
            self.path("test.txt"),
        )
        bucket = self.bucket
    ...
    def test_save_notebook(self):
        ...
        self.contents_manager.save(
            {"type": "notebook", "content": nb}, self.path("test.ipynb")
        )
        bucket = self.bucket
        blob = bucket.blob("test.ipynb")
        self.assertTrue(blob.exists())
