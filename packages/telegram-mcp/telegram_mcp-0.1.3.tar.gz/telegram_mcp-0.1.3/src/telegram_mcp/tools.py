...
async def telegram_login_status_impl(
    ctx: Context,  # Context still passed but not used for client access
) -> LoginStatusOutput:
    # Use the global instance, or create a temporary one if global is None
    # This temporary creation might be problematic if lifespan failed, rely on global.
    checker = telegram_wrapper_instance
    if not checker:
        # If lifespan failed to set the global, return an error status
    ...
    try:
        ...
        if checker and checker.client:
            try:
                connected = await checker.client.is_connected()  # Await here
            except Exception:
