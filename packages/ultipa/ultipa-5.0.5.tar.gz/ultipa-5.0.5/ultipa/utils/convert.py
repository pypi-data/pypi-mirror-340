...
def convertToSchemas(res):
    ...
    for aliases in res.aliases:
        nodeschemas = res.alias("_nodeSchema").asSchemas() if aliases.alias == '_nodeSchema' else []
        schemas.extend(nodeschemas)
        edgeschemas = res.alias("_edgeSchema").asSchemas() if aliases.alias == '_edgeSchema' else []
