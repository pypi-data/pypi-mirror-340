...
def check_memory_limit(X: DataInputType, y: DataInputType) -> None:
    ...
    check_memory_limit_by_file_size(memory_per_df)


def check_memory_limit_by_file_size(total_memory_size: int) -> None:
    """
    Check the memory availability.

    :param total_memory_size: The memory size the dataset occupied.
    :raises: ResourceException
    """
    ...
    needed_memory = total_memory_size * 10
...
def _get_df_or_raise(X: DataInputType,
    ...
    if isinstance(X, pd.DataFrame):
        if not ignore_errors:
            _check_timeseries_input_column_names(X.columns)
        return X
