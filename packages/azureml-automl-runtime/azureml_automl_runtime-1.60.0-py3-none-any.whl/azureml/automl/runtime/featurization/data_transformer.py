...
class DataTransformer(AutoMLTransformer, FeaturizationInfoProvider):
    ...
    def _get_label_encoded_column_indexes(self):
        """
            Helper method that is used in distributed training. This finds indexes of all the label encoded columns.
            The output of this is intended to be fed to Trees and DNNs. This method requires that the transform
            is already called on the transformer prior to calling this method.
        """
        label_encoded_column_indexes = []
        index = 0
        for transformer_mapper in self.transformer_and_mapper_list:
            if self._get_is_label_encoder(transformer_mapper):
                label_encoded_column_indexes.append(index)

            index = index + len(transformer_mapper.mapper.transformed_names_)

        return label_encoded_column_indexes
