...
from azureml.automl.core.shared.constants import Metric, NumericalDtype, Tasks, SupportedModels
...
class MaterializedTabularDataValidator(AbstractTabularDataValidator):
    ...
    def __init__(
        ...
        :param blocked_models: List of blocked models.
        """
    ...
    def _check_nan_inf(self, tabular_data: MaterializedTabularData) -> None:
        ...
        # Check if any of the possible models allow nans.
        no_models_allow_nans = not self._check_models_allow_nans()
        ...
        common_data_validations.check_data_nan_inf(
            ...
            exception_for_nan=no_models_allow_nans
        )
        common_data_validations.check_data_nan_inf(
            y, input_data_name="y", check_nan=check_y_nan, check_inf=check_y_inf
        )

    def _check_models_allow_nans(self) -> bool:
        """Check if any of the possible models allow nans.
        :return: True if any of the possible models allow nans
        """
        if self._task_type == Tasks.CLASSIFICATION:
            return self._check_classification_models_allow_nans()
        elif self._task_type == Tasks.REGRESSION:
            return self._check_regression_models_allow_nans()
        else:
            return False

    def _check_classification_models_allow_nans(self) -> bool:
        """Check if any of the possible classification models allow nans.
        :return: True if any of the possible models allow nans
        """
        models_allowing_nans = [SupportedModels.Classification.LightGBMClassifier,
                                SupportedModels.Classification.XGBoostClassifier]

        return self._check_any_model_allows_nans(models_allowing_nans)

    def _check_regression_models_allow_nans(self) -> bool:
        """Check if any of the possible regression models allow nans.
        :return: True if any of the possible models allow nans
        """
        models_allowing_nans = [SupportedModels.Regression.LightGBMRegressor,
                                SupportedModels.Regression.XGBoostRegressor,
                                SupportedModels.Regression.FastLinearRegressor,
                                SupportedModels.Regression.OnlineGradientDescentRegressor]

        return self._check_any_model_allows_nans(models_allowing_nans)

    def _check_any_model_allows_nans(self, models_allowing_nans: List[str]) -> bool:
        """Check if any of the possible models allow nans.
        :return: True if any of the possible models allow nans
        """
        if self._allowed_models:
            return any(model in models_allowing_nans for model in self._allowed_models)
        elif self._blocked_models:
            return not all(model in self._blocked_models for model in models_allowing_nans)
        else:
            return True
