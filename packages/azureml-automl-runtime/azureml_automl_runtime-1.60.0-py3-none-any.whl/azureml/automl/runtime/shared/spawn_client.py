...
def serialize(
    ...
    srv_script_file = os.path.join(curdir, "spawn", "spawn_server.py")
    env = os.environ.copy()
...
def run_in_proc(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        python_executable = sys.executable
        curdir = os.path.dirname(__file__)
        # spawn_server.py needs to be in a subfolder so that the import of "dill"
        # does not indirectly import "types" from this folder.
        srv_script_file = os.path.join(curdir, "spawn", "spawn_server.py")
