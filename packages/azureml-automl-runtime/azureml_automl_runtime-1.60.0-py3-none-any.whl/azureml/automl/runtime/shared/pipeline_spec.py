...
class ObjectSpec:
    ...
    def from_dict(cls: Type['ObjectSpec'], d: Dict[str, Any]) -> 'ObjectSpec':
        ...
        ret._fix_quantile_range_tuple()
        ret._remove_normalize()
        ret._replace_criterion_mse()
        ret._replace_loss()
        ret._replace_penalty_none()
        return ret

    def _fix_quantile_range_tuple(self):
        if self.class_name == "RobustScaler" and "quantile_range" in self.param_kwargs:
            if isinstance(self.param_kwargs["quantile_range"], list):
                self.param_kwargs["quantile_range"] = tuple(self.param_kwargs["quantile_range"])

    def _remove_normalize(self):
        if self.class_name in ["ElasticNet", "LassoLars"] and "normalize" in self.param_kwargs:
            self.param_kwargs.pop("normalize")

    def _replace_criterion_mse(self):
        if "criterion" in self.param_kwargs:
            if self.param_kwargs["criterion"] == "mse":
                self.param_kwargs["criterion"] = "squared_error"
            if self.param_kwargs["criterion"] == "mae" and \
               self.class_name in ["ExtraTreesRegressor", "RandomForestRegressor", "DecisionTreeRegressor"]:
                self.param_kwargs["criterion"] = "absolute_error"

    def _replace_loss(self):
        if "loss" in self.param_kwargs:
            if self.class_name == "GradientBoostingRegressor":
                if self.param_kwargs["loss"] == "ls":
                    self.param_kwargs["loss"] = "squared_error"
                if self.param_kwargs["loss"] == "lad":
                    self.param_kwargs["loss"] = "absolute_error"
            if self.class_name == "SGDClassifierWrapper":
                if self.param_kwargs["loss"] == "log":
                    self.param_kwargs["loss"] = "log_loss"
            if self.class_name == "SGDRegressor":
                if self.param_kwargs["loss"] == "squared_loss":
                    self.param_kwargs["loss"] = "squared_error"

    def _replace_penalty_none(self):
        if "penalty" in self.param_kwargs:
            if self.class_name in ["SGDClassifierWrapper", "SGDRegressor"] :
                if self.param_kwargs["penalty"] == "none":
                    self.param_kwargs["penalty"] = None
