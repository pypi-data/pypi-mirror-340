...
def get_auto_validation_strategy(
    ...
    if number_of_cv != constants.RuleBasedValidation.DEFAULT_N_CROSS_VALIDATIONS and \
            automl_settings.task_type == constants.Tasks.CLASSIFICATION:
        # if CV was chosen, ensure that the number of samples for minority class is at least as much as the
        ...
        labels, counts = np.unique(np.delete(y, nan_y_index), return_counts=True)
        if min(counts) < number_of_cv:
            logger.error(f"Number of samples for the minority class label ({min(counts)}) is less than chosen "
                         f"n_cv_folds ({number_of_cv}). Resetting it to the default.")
            number_of_cv = constants.RuleBasedValidation.DEFAULT_N_CROSS_VALIDATIONS
