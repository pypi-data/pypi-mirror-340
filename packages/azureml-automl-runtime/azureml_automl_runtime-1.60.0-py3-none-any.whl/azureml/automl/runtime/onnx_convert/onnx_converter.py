...
if sys.version_info < OnnxConvertConstants.OnnxIncompatiblePythonVersion:
    ...
    from skl2onnx.common._registration import _shape_calculator_pool, _converter_pool  # noqa: E402
    ...
    from skl2onnx._supported_operators import sklearn_operator_name_map  # noqa: E402
...
# Metrics supported in the skl2onnx 1.10.0 for KNN learners
KNN_SUPPORTED_METRICS = ('sqeuclidean', 'euclidean', 'minkowski', 'manhattan', 'cityblock')
# Conversion table of unsupported metrics (see
...
class OnnxConverter:
        ...
        try:
            ...
            for i_init_tp, tpl in enumerate(stats_and_column_purposes):
                ...
                if self._if_model_can_use_batch_mode:
                    ...
                    onnx_tp = onnx_type(shape=[None, 1])
                else:
    ...
    def _convert_np_to_onnx_type(self, tp_str):
        # We interpret integer as float to avoid issues with SimpleImputer
        # where nan-s can not be used.
        # if tp_str == NumericalDtype.Floating or \
        #       tp_str == NumericalDtype.MixedIntegerFloat or\
        #       tp_str == NumericalDtype.Integer:
    ...
    def _if_estimator_contains_knn(self, model_obj):
        ...
        if ensemble_estimator is not None:
            ...
            if isinstance(ensemble_estimator, PreFittedSoftVotingClassifier):
    ...
    def _parse_model(self, model, target_opset, ignore_onnx_resource=False,
        ...
        # Init the model container.SklearnModelContainerNode
        ...
        for var_name, initial_type in self.conversion_phase_initial_types:
            var = scope.get(Topology._generate_unique_name(var_name, set()), None)
            inputs.append(var)
        ...
        for variable in outputs:
            # Set the is_leaf parameter to the output variable to finish the graph.
            variable.init_status(is_leaf=True)

        return topology

    def _handle_knn_metrics_maybe(self, step):
        """
        In the versions of skl2onnx >= 1.6.0 some KNN metrics are not supported. We will change their aliases here.

        :param step: The step tobe modified.
        :return: The same step if it is not KNN, or knn with metrics being replaced.
        """
        if isinstance(step, KNeighborsRegressor) or isinstance(step, KNeighborsClassifier):
            if step.metric not in KNN_SUPPORTED_METRICS:
                step.metric = KNN_METRICS_CONVERSION.get(step.metric, step.metric)
                if hasattr(step, 'effective_metric_'):
                    step.effective_metric_ = step.metric
        return step
        ...
        for step in model.steps:
            ...
            if isinstance(mdl_obj, DataTransformer):
                ...
                if model._final_estimator == mdl_obj and is_final_estimator_classifier and is_y_trans_pipe:
                    ...
                    mdl_obj = self._handle_knn_metrics_maybe(mdl_obj)
                    inputs = _sklearn_parse._parse_sklearn(scope, mdl_obj, inputs)
            else:
    ...
    def _parse_data_transformer(self, scope, dt, inputs, model_contains_xgboost_estimator):
        ...
        for columns, transformers, _ in self._get_built_features(dt):
            ...
            for j, trans_step in enumerate(transformers.steps):
                ...
                # calculator will change this later, unless it is a SimpleImputer,
                # which requires the same tensor type.
                if isinstance(transformer, SimpleImputer):
                    op_out = scope.declare_local_variable(var_out_name, input_var.type)
                else:
    ...
    def _parse_ensemble_estimator(self, scope, ensemble_estimator, inputs):
        ...
        if isinstance(ensemble_estimator, PreFittedSoftVotingClassifier):
            ...
            assert (isinstance(ensemble_estimator, PreFittedSoftVotingRegressor))
                ...
                for step in steps_to_parse:
                    if step is not None:
                        ...
                        if type(mdl_obj) in _sklearn_supported_operators.sklearn_operator_name_map:
                            ...
                            mdl_obj = self._handle_knn_metrics_maybe(mdl_obj)
                            pipe_step_inputs = _sklearn_parse._parse_sklearn(scope, mdl_obj, pipe_step_inputs)
                        elif is_identity_trans:
                            # The variables pass through the IdentityTransformer, no need to convert.
                ...
                if is_classification:
                    ...
                    if type(estimator) in _sklearn_supported_operators.sklearn_operator_name_map:
                        ...
                        if (
                            ...
                            inputs = self._add_missing_val_replacement_processor(scope, inputs)
                        estimator = self._handle_knn_metrics_maybe(estimator)
                        cur_est_outputs = _sklearn_parse._parse_sklearn(scope, estimator, inputs)
                    elif is_identity_trans:
