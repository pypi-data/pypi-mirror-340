...
def _save_artifacts_internal(
    ...
    with automl_run_context.get_run() as run:
        ...
        try:
            ...
            if is_timeseries:
                # Timeseries forecasting models need its own flavor to save and load the MLFlow model.
                # We don't use the signature since the customer can choose if to provide the target column or not,
                # during the inference call to do the forecasting.
                mlflow_options = {
                    constants.MLFlowLiterals.SCHEMA_SIGNATURE: None,
                    constants.MLFlowLiterals.FLAVOR_FORECASTING: True
                }
            else:
