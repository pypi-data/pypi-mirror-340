...
with importlib.resources.files("rf_humab.sequence_data").joinpath("sample_sequences.csv") as f:
    df = pd.read_pickle(f)
...
def load_models(forced):
    ...
    v_models = {}
    for i in range(1,8):
        with importlib.resources.files("rf_humab.saved_models").joinpath('v' + str(i) + '_model_' + kw + '.sav') as filename:
