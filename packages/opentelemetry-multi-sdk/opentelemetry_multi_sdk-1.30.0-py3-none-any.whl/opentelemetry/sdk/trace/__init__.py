...
# Default DATABASE value (can be overridden by the user)  
DATABASE = os.environ.get("DATABASE_URL", "sqlite:///:memory:")  
  
def configure_database(database_url: str):  
    """  
    Configure the database connection string for the library.  
  
    Args:  
        database_url (str): The database connection string (e.g., PostgreSQL, SQLite).  
    """  
    global DATABASE  
...
class APIKeyManager:  
    """Class to handle API key operations including decoding and fetching user ID."""  
      
    def __init__(self, api_key: str, database_url: str = None):  
        """  
        Initialize the APIKeyManager with an API key and optional database URL.  
  
        Args:  
            api_key (str): The API key for decoding.  
            database_url (str, optional): The database connection string. Defaults to the global DATABASE.  
        """  
        self.api_key = api_key  
        self.database_url = database_url or DATABASE  
        self.engine = create_engine(self.database_url)  
        self.Session = sessionmaker(bind=self.engine)  
  
    def decode_api_key(self):  
        """Decode the API key to extract API key ID."""  
        try:  
            decoded_token = jwt.decode(self.api_key, options={"verify_signature": False})  
            return decoded_token.get("jti").split(":")[-1]  # Extract API key ID  
...
# DATABASE_URL = "postgresql://postgres:arqum@localhost:5432/arqum"

# class APIKeyHandler:
#     def __init__(self, database_url: str = DATABASE_URL):
#         """Initialize the APIKeyHandler with a database session."""
...
#     def get_user_id_from_api_key(self, api_key: str, verbose: bool = True) -> Optional[str]:
#         """
#         Fetch the user ID from the API key stored in the database.

#         Args:
#             api_key (str): The encoded API key (JWT token).
#             verbose (bool): If True, logs details for debugging.
...
#             api_key_id = decoded_token.get("jti", "").split(":")[-1]

#             if not api_key_id:
#                 if verbose:
#                     print("Invalid API key format: Missing 'jti' claim.")
#                 return None

#             # Query the database for the API key
#             with self.get_database_session() as session:
#                 api_key_record = session.query(OrmApiKey).filter_by(id=api_key_id).first()

#                 if api_key_record:
#                     gql_api_key = to_gql_api_key(api_key_record)
#                     if verbose:
#                         print("GraphQL API Key:", gql_api_key)
#                         print("GraphQL API Key user:", gql_api_key.user_id)
#                     return str(gql_api_key.user_id)
