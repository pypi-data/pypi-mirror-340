...
- [1. Config File handling](#1-config-file-handling)
  - [1.1 Config Parser](#11-config-parser)
  - [1.2 Config Writer](#12-config-writer)
  - [1.3 Automatic Logger Configuration](#13-automatic-logger-configuration)
  - [1.4 Behind the Scenes (Automatic Initialization)](#14-behind-the-scenes-automatic-initialization)
- [2. Other utilities](#2-other-utilities)
  - [2.1 Logger utilities](#21-logger-utilities)
  - [2.2 Jira Utilities](#22-jira-utilities)
  - [2.3 Excel Utilities](#23-excel-utilities)
  - [2.4 AI Utilities](#24-ai-utilities)
  - [2.5 OS Utilities](#25-os-utilities)
  - [2.6 Config utilities](#26-config-utilities)
...
│   │   ├── output_writer.py        # Asynchronous context manager for writing file paths to text, CSV, or Excel with buffered writing
│   └── logger/     
...
## 1.3 Automatic Logger Configuration
...
## 1.4 Behind the Scenes (Automatic Initialization)
...
### Config Parser (`config_utils/__init__.py`)
...
### `open_buffered_output_file` (Function)

**Purpose**: Asynchronous context manager for writing file paths to text, CSV, or Excel with buffered writing.

**Usage**:
```python
from utilities import open_buffered_output_file

async with open_buffered_output_file('output.txt', buffer_size=100) as writer:
    await writer('C:/some/file1.txt')
    await writer('C:/some/file2.txt')
```

**Parameters**:
- `file_path` (str | Path): Required. Destination file.
- `buffer_size` (int): Optional. Default is 1000 lines.

**Supported formats**:
- `.txt`: written line-by-line.
- `.csv`, `.xls`, `.xlsx`: uses `pandas` to write buffered data in bulk.

## 2.6 Config utilities
