...
class Pamap2(ReaderInterface):
    ...
    def _fix_intermediate_version(data: DataArray, verbose) -> DataArray:
        ...
        train_or_test = ["train" if i in ts_id_train else "test" for i in data["ts_id"].values]
...
activity_names = {
    0: 'transient', 1: 'lying', 2: 'sitting', 3: 'standing', 4: 'walking', 5: 'running', 6: 'cycling',
...
def get_column_names():
    ...
    IMU = [
        "temperature",
        "acceleration_16g_0", "acceleration_16g_1", "acceleration_16g_2",
        "acceleration_6g_0", "acceleration_6g_1", "acceleration_6g_2",
        "gyroscope_0", "gyroscope_1", "gyroscope_2",
        "magnetometer_0", "magnetometer_1", "magnetometer_2",
...
def _read_pamap(filenames, verbose=True):
    count = -1
    last_values = None
    for file in tqdm(filenames, disable=not verbose, desc="reading PAMAP2 files", position=0, leave=False):
        subject_id = file.split("/")[-2][0] + file[:-4].split("subject")[-1]

        df_subject = pd.read_table(file, sep="\s+", header=None, names=get_column_names())
        df_subject["activity_name"] = df_subject.activity_id.astype(int).apply(lambda x: activity_names[x])
        df_subject["subject_id"] = subject_id

        ts_ids = []
        for row in tqdm(df_subject[["activity_id", "subject_id"]].itertuples(index=False), disable=not verbose,
                        total=len(df_subject), desc="partitioning the data", position=1, leave=False):
            if row != last_values:
                last_values = row
                count += 1
            ts_ids.append(count)
        ...
        melted_df_subject = df_subject.melt(
            id_vars=["ts_id", "timestamp_s", "activity_id", "activity_name", "subject_id"],
