...
class Neo4jConnectionInfo(BaseModel):
    ...
    database: str | None = "neo4j" # Default database
...
class neo4jDatabase:
    def __init__(self, connection_info: Neo4jConnectionInfo):
        ...
        logger.debug(f"Initializing database connection to {self.connection_info.uri} for user {self.connection_info.user} (database: {self.connection_info.database})")
        ...
        try:
            ...
            # Remove database kwarg if None (driver handles default)
            if driver_kwargs["database"] is None:
                 del driver_kwargs["database"]
