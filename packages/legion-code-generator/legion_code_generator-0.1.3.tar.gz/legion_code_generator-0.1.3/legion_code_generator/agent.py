...
"""
    ...
    def interactive_mode(self):
        ...
        display_message("Welcome to Legion Code Generator! Type 'help' for available commands or 'exit' to quit.", "info")
        ...
        while True:
            try:
                ...
                if user_input.lower() in ["exit", "quit"]:
                    display_message("Exiting Legion Code Generator. Goodbye!", "info")
                    break
                    
                elif user_input.lower() == "help":
                    display_message("Available commands:", "info")
                    display_message("  init <project_name> <project_type> - Initialize a new project", "info")
                    display_message("  run <command> - Run a shell command in the project directory", "info")
                    display_message("  explain - Generate an explanation of the current project", "info")
                    display_message("  api - Show information about the configured API", "info")
                    display_message("  exit - Exit the program", "info")
                    display_message("  <any other text> - Interpreted as a feature request to update the project", "info")
                
                elif user_input.lower().startswith("init "):
                    parts = user_input.split(maxsplit=2)
                    if len(parts) != 3:
                        display_message("Error: 'init' command requires a project name and type. Example: 'init my_project react'", "error")
                    else:
                    ...
                    display_message(f"API Type: {api_info['type']}", "info")
                    display_message(f"Model: {api_info['model']}", "info")
                    display_message(f"Key Status: {'Configured' if api_info['key_status'] else 'Not configured'}", "info")
                ...
                display_message("\nExiting Legion Code Generator. Goodbye!", "info")
                break
