...
class CommandExecutor:
    ...
    def run_command(self, command):
        ...
        # Flush message buffer before command execution
        ...
        try:
            # Run the command
            display_message(f"Running command: {command}", "command", flush=True)
            ...
            # Flush message buffer after command execution
            _flush_message_buffers()
            
            # Check if the command was successful
            if process.returncode != 0:
                display_message(f"Command failed with return code {process.returncode}", "warning", flush=True)
            ...
            display_message(error_message, "error", flush=True)
            self.last_result = {
                "stdout": "",
                "stderr": error_message,
                "return_code": -1
