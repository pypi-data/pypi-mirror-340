...
import numpy as np
from .layer import NaiveFourierKANLayer  # we'll split this out too (see below)


class FourierKAN(nn.Module):
    def __init__(self, layers=None, gridsizes=None):
        super(FourierKAN, self).__init__()
        self._layers = layers
        self._gridsizes = gridsizes
        self.layers = nn.ModuleList()

        if layers and gridsizes:
    ...
    def tune(self, dataset, input_shape, output_shape, device, trials=50,
             EPOCHS=100, MAX_DEPTH=10, MAX_NEURONS=50, MAX_GRID=10):
        import optuna
