...
class Region:
    def __init__(
        ...
        # Keep the namedtuple for __repr__ and __str__
...
class ExistingRegions:
    ...
    def get(self, **kwargs) -> Region:
        ...
        if all(value == None for value in kwargs.values()):
            return None

        for region in self.regions:
            if all(
                getattr(region, key) == value or value == None
    ...
    def get_ancestors(self, region: object) -> list:
        path = region.path
        return [
            self.get(name=n, parent=p) for n, p in zip(path, [None] + path[:-1])
        ][:-1]
