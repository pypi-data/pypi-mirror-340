...
_compound_parents = lambda parents: ["_".join(parents[: i + 1]).upper() for i in range(len(parents))]
_ing_to_labels = {i.name: [
        i.name, 
...
_ing_to_indices = {ing: [_label_to_idx[label] for label in labels] for ing, labels in _ing_to_labels.items()}


class Recipe:
    def __init__(self, phrases=None, ingredients=None, experimental=False):
        ...
        ing_is_charc = lambda x: any(
            [c == x.name.lower() or c in x.parents for c in charcs]
        )
    ...
    def _create_vector(self):
        ...
        indices = flatten([_ing_to_indices[ingredient.name] for ingredient in self.ingredients])

        # Initialize a numpy array of zeros of length n
        vector = np.zeros(len(_labels), dtype=int)

        
        # Set positions in 'indices' to 1
        vector[indices] = 1
...
def batch_recipes(phrases_list: list[list[str]]):
    ...
    for recipe in recipes:
        vector = {i: 0 for i in range(len(ings))}
