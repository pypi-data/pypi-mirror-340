...
/* methods */
static PyObject *
_gbagfx_main(PyObject *self, PyObject *py_args)
{
    ...
    */
    ...
    int res = gbagfx_main((int)argc, argv);
...
PyInit__gbagfx(void)
{
    PyObject *m;
    PyObject *threading;
    gbagfx_fprintf(stdout, "Gbagfx Module Init.");

    m = PyModule_Create(&_gbagfx_module);
    gbagfx_fprintf(stdout, "This module has been created properly: %s", !m ? "true" : "false");
    if (!m) return NULL;
