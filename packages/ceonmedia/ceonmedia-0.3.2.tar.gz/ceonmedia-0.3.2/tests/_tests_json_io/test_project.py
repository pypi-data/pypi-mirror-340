...
TEST_CLS = CstockProject
JSON_FILES_DIR = f"{Path(__file__).parent}/json_files/project"
...
# TODO move testing log intoa standalone module to be used in all test cases in case
...
def test_render_task_load_from_json_success() -> None:
    ...
    logger.warning(f"{sys.version=}")
    for test_file in FILES_TO_TEST_SUCCESS:
        logger.info(f"Loading file: {test_file}")
        test_object = json_io.cstock_from_json(test_file, TEST_CLS)
        # json_data = json_io.load(test_file)
        # test_object = cstock_from_dict(json_data, TEST_CLS)
...
def test_render_task_load_from_json_failure() -> None:
    ...
    for test_file in FILES_TO_TEST_FAILURE:
        with pytest.raises(
            (
                TypeError,
                errors.CstockUnknownTypeError,
                errors.CstockInstantiationError,
            )
        ):
            logger.info(f"Loading file: {test_file}")
            test_object = json_io.cstock_from_json(test_file, TEST_CLS)
