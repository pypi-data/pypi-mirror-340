...
async def create_from_json(elements, connection_code: str = None) -> str:
    ...
    JSON结构详细说明:
    1. 基本元素结构:
       - type: 元素类型，支持 "FRAME", "RECTANGLE", "TEXT" 等
       - name: 元素名称
       - x, y: 元素位置坐标 (相对于父元素或画布)
       - width, height: 元素尺寸
       - fills: 填充样式数组，如 [{"type": "SOLID", "color": {"r": 0.2, "g": 0.4, "b": 0.8, "a": 1}}]
       - children: 子元素数组 (仅FRAME类型或会自动转为FRAME的元素)
    
    2. 文本元素特殊属性:
       - characters: 文本内容 (不是text!)
       - fontSize: 字体大小
    ...
    3. 形状元素特殊属性:
       - cornerRadius: 圆角半径
       - effects: 效果数组，如阴影 [{"type": "DROP_SHADOW", "color": {...}, "offset": {...}, "radius": 4, "spread": 0}]
    ...
    Args:
        elements (list): 元素定义列表，按照上述JSON格式构造
        connection_code (str, optional): 连接代码。如果不提供则使用当前连接
    ...
    if isinstance(elements, str):
        try:
            ...
            print(f"MCP服务器: 从字符串解析elements成功，包含{len(elements)}个元素")
        except json.JSONDecodeError:
    ...
    # 打印处理后的elements内容用于调试
    print(f"MCP服务器: 处理后的elements内容: {json.dumps(elements)[:200]}...")
