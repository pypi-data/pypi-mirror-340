...
def plot_weekly_difference(weekly_prediction_future, models, delay, length):
    """
    ...
    # Initialize an empty dictionary to store model predictions
    model_diff = {model: [] for model in ['ARIMA'] + list(models.keys())}
    filtered_weeks = []
    ...
    for week in weeks:
        ...
        filtered_weeks.append(week)
        #print(week, filtered_weeks)
        #print(data_filtered['ARIMA'].iloc[0])
        # Extract observed values
        #model_predictions['Observed'].append(data_filtered['Observed'] if 'Observed' in data_filtered else None)

        # Extract model predictions
        model_diff['ARIMA'].append(data_filtered['diff_ARIMA'] if 'ARIMA' in data_filtered else None)
        #model_predictions['MLAMA'].append(data_filtered['diff_MLAMA'] if 'MLAMA' in data_filtered else None)

        for model in models.keys():
            if model == 'ARIMA':
                continue
            #print(model, data_filtered[model])
            model_diff[model].append(data_filtered['diff_'+model] if model in data_filtered else None)
    ...
    plt.xlabel("Week")
    plt.ylabel("Difference Value (prediction-observed)")
    plt.title(f"Weekly Differences for Shift (Responsiveness)={delay}, Length={length}")
    plt.legend()
    plt.xticks(rotation=45)
    plt.grid(True)
    plt.show()
...
def plot_model_weights(stan_optimal_weights, plot_type='bar', order="shift_pred"):
    """
    ...
    if order == "shift_pred":
        ...
        xlabel = "(Shift, Prediction Length)"
    else:
        x_labels = [f"({p}, {s})" for s, p in zip(shifts, pred_lengths)]
        xlabel = "(Prediction Length, Shift)"
