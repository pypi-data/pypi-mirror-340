...
class SuperDict(MutableMapping, dict):
    ...
    def __contains__(self, key):
        return any([
            self._contains_locally(key),
            (self._readable_from_ancestry(key)
             and self._parent is not None
             and key in self._parent),
            (self._readable_from_ancestry(key)
             and self._parent is not None
             and self._parent._parent is not None                               # NOQA
             and self._parent_key in self._parent._parent                       # NOQA
             and isinstance(self._parent._parent[self._parent_key], Mapping)    # NOQA
             and key in self._parent._parent[self._parent_key]),                # NOQA
        ])
    ...
    def __getitem__(self, key):
        if key in self:              # depends on __contains() to work correctly
            if self._contains_locally(key):
                value = self._store[_str_lower(key)][1] \
                ...
                value = self._parent._parent[self._parent_key][key]             # NOQA
        else:
