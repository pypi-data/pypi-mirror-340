...
class BaseAgent(ABC):
    ...
    async def set_context(
        ...
        Collects only local @tool methods (remote tools removed).
        """
        if not self.context: raise ValueError("Agent context not set")
        ...
        # Store only local tool schemas now
        self.tools_schemas = local_tool_schemas
        ...
        **kwargs
        ...
        while tool_iterations < max_tool_iterations:
            ...
            try:
                ...
                if stream:
                    logger.warning("Streaming not supported in RPyC create, using non-streaming.")
                response_dict = self.root.exposed_chat_completion(
                    ...
                    tools=self.tools_schemas or None, # Only local tools now
