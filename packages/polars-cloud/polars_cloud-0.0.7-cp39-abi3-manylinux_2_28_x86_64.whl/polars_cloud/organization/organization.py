...
class Organization:
    """A Polars Cloud organization.
     ...
     >>> organization = Organization.setup("my_organization")  # doctest: +SKIP
    ...
    def _update_from_api_schema(
        ...
        self._id = UUID(organization_schema.id)
    ...
    def _load_by_name(self) -> None:
        ...
        if len(matches) == 0:
            ...
            self._id = UUID(organization.id)

    def _load_by_id(self) -> None:
        """Load the workspace by id."""
        organization = pcr.get_organization(str(self._id))
        self._name = organization.name

    def delete(self) -> None:
        """Delete an organization."""
        pcr.delete_organization(str(self._id))
