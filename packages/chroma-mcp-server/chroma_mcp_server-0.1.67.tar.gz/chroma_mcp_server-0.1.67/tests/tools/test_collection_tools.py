...
class TestCollectionTools:
    ...
    async def test_create_collection_with_custom_metadata(self, mock_chroma_client_collections):
        ...
        # --- Act ---
        # Create Pydantic model instance - Use correct model
        input_model = CreateCollectionWithMetadataInput(collection_name=collection_name, metadata=custom_metadata_input)
        ...
        call_args = mock_client.create_collection.call_args
        assert call_args.kwargs["name"] == collection_name
        # Verify the exact metadata was passed to Chroma's create_collection
    ...
    @pytest.mark.skip(reason="Tool delisted due to API instability/client compatibility issues")
    @pytest.mark.asyncio
    ...
    async def test_create_collection_with_metadata_success(self, mock_chroma_client_collections):
        """Test successful creation using the _with_metadata variant."""
        (
            mock_client,
            _,
            mock_validate,
        ...
        custom_metadata_input = {"description": "My custom description", "hnsw:space": "ip"}
        mock_collection_id = str(uuid.uuid4())
        ...
        # Let's assume for the mock it stores what was passed
        created_collection_mock.metadata = custom_metadata_input
        created_collection_mock.count.return_value = 0
        mock_client.create_collection.return_value = created_collection_mock
        ...
        input_model = CreateCollectionWithMetadataInput(
            ...
            metadata=custom_metadata_input # Pass the required metadata
        )
        result_list = await _create_collection_with_metadata_impl(input_model)
        ...
        assert result_data.get("id") == mock_collection_id
        assert "metadata" in result_data
        # Check reconstructed metadata matches the input (assuming _reconstruct_metadata handles it)
        # This depends on how _reconstruct_metadata handles non-prefixed keys vs settings
        expected_reconstructed = {
            "description": "My custom description",
            "settings": {"hnsw:space": "ip"}
        }
        assert result_data["metadata"] == expected_reconstructed
        assert result_data.get("count") == 0
        assert result_data.get("status") == "success"

    # --- Tests for _list_collections_impl ---
    @pytest.mark.asyncio
    async def test_list_collections_success(self, mock_chroma_client_collections):
        """Test successful default collection listing."""
        mock_client, _, _ = mock_chroma_client_collections
