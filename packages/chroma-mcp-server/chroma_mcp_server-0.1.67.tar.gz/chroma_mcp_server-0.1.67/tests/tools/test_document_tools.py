...
class TestDocumentTools:
    ...
    async def test_add_documents_success(self, mock_chroma_client_document):
        ...
        mock_collection.add.assert_called_once_with(documents=documents_to_add, ids=None, metadatas=None)
        # Assert result format
        assert isinstance(result, list)
        assert len(result) == 1
        assert isinstance(result[0], types.TextContent)
        assert f"Successfully added {len(documents_to_add)} documents" in result[0].text
    ...
    async def test_add_documents_increment_index(self, mock_chroma_client_document):
        ...
        mock_validate.assert_called_once_with(collection_name)
        mock_client.get_collection.assert_called_once_with(name=collection_name)
        # Check add was called (we can't easily assert the index flag behavior with current mocks)
        mock_collection.add.assert_called_once_with(documents=documents_to_add, ids=None, metadatas=None)
