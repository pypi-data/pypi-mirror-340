...
class CreateCollectionWithMetadataInput(BaseModel):
    ...
    metadata: str = Field(..., description="Metadata and settings as a JSON string (e.g., '{\"description\": \"...\", \"settings\": {\"hnsw:space\": \"cosine\"}}' ).")
...
class SetCollectionDescriptionInput(BaseModel):
    """Input model for setting a collection's description."""
...
class SetCollectionSettingsInput(BaseModel):
    """Input model for setting a collection's settings."""
    collection_name: str = Field(description="The name of the collection to update.")
...
class UpdateCollectionMetadataInput(BaseModel):
    """Input model for updating a collection's metadata."""
    collection_name: str = Field(description="The name of the collection to update.")
...
async def _create_collection_with_metadata_impl(
    ...
    try:
        ...
        if f"Collection {collection_name} already exists." in str(e):
             logger.warning(f"Collection '{collection_name}' already exists.")
             raise McpError(ErrorData(code=INVALID_PARAMS, message=f"Tool Error: Collection '{collection_name}' already exists."))
