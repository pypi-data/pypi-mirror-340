...
async def call_tool(name: str, arguments: Dict[str, Any]) -> types.CallToolResult:
    ...
    logger = get_logger("call_tool")  # Get logger specific to this handler
    ...
    try:
        ...
        if name == TOOL_NAMES["GET_VERSION"]:
            try:
                ...
                return types.CallToolResult(
                    isError=True,
                    content=[types.TextContent(type="text", text="Tool Error: chroma-mcp-server package not found.")],
                )
            except Exception as e:
                ...
                return types.CallToolResult(
                    ...
                    content=[
                        types.TextContent(
                            type="text", text=f"Tool Error: Could not get server version. Details: {str(e)}"
                        )
                    ],
                )
        ...
        if not InputModel or not impl_function:
            ...
            return types.CallToolResult(
                isError=True, content=[types.TextContent(type="text", text=f"Tool Error: Unknown tool name '{name}'")]
            )
        ...
        try:
            ...
            return types.CallToolResult(
                isError=True,
                content=[
                    types.TextContent(type="text", text=f"Input Error: {str(e)}")
                ],  # Use Pydantic's default string representation
            )
        ...
        result: types.CallToolResult = await impl_function(validated_input)
        logger.debug(f"Implementation function for {name} returned result.")

        # Return the result from the _impl function (should already be CallToolResult)
        return result

    except Exception as error:
        # Catch unexpected errors during dispatch, validation, or execution
    ...
    try:
        ...
        if logger:
            try:
                ...
                version = "unknown"
            logger.debug(
                "Starting Chroma MCP server (version: %s) with stdio transport using standard Server instance", version
            )
