...
class AddDocumentsWithMetadataInput(BaseModel):
    ...
    collection_name: str = Field(..., description="Name of the collection to add documents to.")
    documents: List[str] = Field(..., description="List of document contents (strings).")
    metadatas: List[Dict[str, Any]] = Field(..., description="List of metadata dictionaries corresponding to the documents.")
    increment_index: Optional[bool] = Field(True, description="Whether to immediately index added documents.")
...
async def _add_documents_impl(input_data: AddDocumentsInput) -> List[types.TextContent]:
    ...
    logger.info(f"Adding {len(documents)} documents to '{collection_name}' (auto-IDs). Increment index: {increment_index}")
    try:
        ...
        return [types.TextContent(type="text", text=f"Successfully added {len(documents)} documents to '{collection_name}'.")]
...
async def _add_documents_with_metadata_impl(input_data: AddDocumentsWithMetadataInput) -> List[types.TextContent]:
    ...
    logger.info(f"Adding {len(documents)} documents with metadata to '{collection_name}' (auto-IDs). Increment index: {increment_index}")
    try:
        client = get_chroma_client()
        collection = client.get_collection(name=collection_name)

        await _validate_add_arguments(collection, documents, None, metadatas)
...
async def _add_documents_with_ids_and_metadata_impl(input_data: AddDocumentsWithIDsAndMetadataInput) -> List[types.TextContent]:
    ...
    logger.info(f"Adding {len(documents)} documents with {len(ids)} IDs and metadata to '{collection_name}'. Increment index: {increment_index}")
    try:
        client = get_chroma_client()
        collection = client.get_collection(name=collection_name)

        await _validate_add_arguments(collection, documents, ids, metadatas)
...
# Original _update_documents_impl commented out/removed
...
# Original _delete_documents_impl removed/commented out
