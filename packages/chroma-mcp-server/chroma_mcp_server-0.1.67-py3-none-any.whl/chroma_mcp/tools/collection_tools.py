...
class CreateCollectionInput(BaseModel):
    ...
    collection_name: str = Field(..., description="The name for the new collection. Must adhere to ChromaDB naming conventions.")
    # REMOVE the metadata field entirely for the base tool variant
    # metadata: Optional[Dict[str, Any]] = Field(
    #     None, description="Optional metadata and settings..."
    # )
...
class CreateCollectionWithMetadataInput(BaseModel):
    """Input model for creating a collection with specified metadata."""
    collection_name: str = Field(..., description="The name for the new collection. Must adhere to ChromaDB naming conventions.")
    metadata: Dict[str, Any] = Field(
        ..., description="Metadata and settings (e.g., {'description': '...', 'settings': {'hnsw:space': 'cosine'}}) to apply during creation."
    )
...
async def _create_collection_with_metadata_impl(
    ...
    """Core logic to create a collection with specified metadata."""
    logger = get_logger("create_collection_with_metadata")
    collection_name = input_data.collection_name
    # Metadata is now required in the input model
    ...
    try:
        ...
        logger.info(
            f"Attempting to create collection '{collection_name}' with provided metadata: {metadata}"
        )

        # Call ChromaDB
        collection = chroma_client.create_collection(
            name=collection_name,
            metadata=metadata, # Directly use the required metadata
