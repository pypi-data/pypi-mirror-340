...
class TestDocumentTools:
    ...
    async def test_add_document_increment_index(self, mock_chroma_client_document):
        ...
        input_model_false = AddDocumentInput(collection_name=collection_name, document=document_to_add, increment_index=False)
    ...
    async def test_add_document_with_id_success(self, mock_chroma_client_document):
        ...
        input_model = AddDocumentWithIDInput(
            collection_name=collection_name, document=document_to_add, id=id_to_add
        )
    ...
    async def test_add_document_validation_no_doc_or_id(self, mock_chroma_client_document):
        ...
        input_model_meta_meta = AddDocumentWithMetadataInput(collection_name=collection_name, document="doc", metadata="")
        ...
        input_full_doc = AddDocumentWithIDAndMetadataInput(collection_name=collection_name, document="", id="id", metadata='{}')
        with assert_raises_mcp_error("Document content cannot be empty."):
            await _add_document_with_id_and_metadata_impl(input_full_doc)

        input_full_id = AddDocumentWithIDAndMetadataInput(collection_name=collection_name, document="doc", id="", metadata='{}')
        with assert_raises_mcp_error("Document ID cannot be empty."):
            await _add_document_with_id_and_metadata_impl(input_full_id)

        input_full_meta = AddDocumentWithIDAndMetadataInput(collection_name=collection_name, document="doc", id="id", metadata="")
        with assert_raises_mcp_error("Metadata JSON string cannot be empty."):
    ...
    async def test_add_document_with_id_and_metadata_success(self, mock_chroma_client_document):
        ...
        mock_collection.add.assert_called_once_with(documents=[document_to_add], ids=[id_to_add], metadatas=[parsed_metadata])
        assert_successful_json_result(result, {"added_id": id_to_add})
    ...
    async def test_update_document_content_success(self, mock_chroma_client_document):
        ...
        input_model = UpdateDocumentContentInput(
            collection_name=collection_name, id=id_to_update, document=new_doc
        )
    ...
    async def test_update_document_validation_missing_args(self, mock_chroma_client_document):
        ...
        input_content = UpdateDocumentContentInput(
            collection_name=collection_name, id="", document="doc1"
        )
        ...
        input_meta_id = UpdateDocumentMetadataInput(
            collection_name=collection_name, id="", metadata={"k": "v"}
        )
        with assert_raises_mcp_error("ID cannot be empty for update."):
    ...
    async def test_delete_document_by_id_success(self, mock_chroma_client_document):
        ...
        mock_validate.assert_called_once_with(collection_name)
        mock_client.get_collection.assert_called_once_with(name=collection_name)
        # Assert delete called with list of size 1
        mock_collection.delete.assert_called_once_with(ids=[id_to_delete], where=None, where_document=None)
        assert_successful_json_result(result, {"deleted_id": id_to_delete})
    ...
    async def test_delete_document_by_id_not_found_silent(self, mock_chroma_client_document):
        ...
        mock_client.get_collection.assert_called_once_with(name=collection_name)
        mock_collection.delete.assert_called_once_with(ids=[id_to_delete], where=None, where_document=None)
        # Assert result is empty dict as per implementation logic
    ...
    async def test_generic_chroma_error_handling(self, mock_chroma_client_document):
        ...
        input_update_content = UpdateDocumentContentInput(collection_name=collection_name, id="id1", document="d") # Singular
        with assert_raises_mcp_error(f"ChromaDB Error: Failed to update document content. {generic_error_message}"): # Updated msg
        ...
        with assert_raises_mcp_error(f"ChromaDB Error: Failed to delete document. {generic_error_message}"): # Updated msg
