...
def stack_forecast(ds: xr.Dataset | str | list[str] | pd.DataFrame) -> xr.Dataset:
    ...
    if isinstance(ds, xr.Dataset) or isinstance(ds, xr.DataArray):
        ...
        return ds.assign_coords(time=(FCST_DIM, time_values)).expand_dims(
            dim={FCST_DIM: [forecast_date]}
        )
    elif isinstance(ds, str):
        # return stack_forecast(xr.open_dataset(ds)) # lazy-load w/ dask
        return stack_forecast(xr.load_dataset(ds))  # immediate load
    elif isinstance(ds, pd.DataFrame):
        return stack_forecast(ds.file_name)
    elif isinstance(ds, list) or isinstance(ds, pd.Series):
        return xr.open_mfdataset(
            ds, preprocess=stack_forecast, concat_dim=FCST_DIM, combine="nested"
        )
    else:
