...
class Scene:
    ...
    def update_def(self, oid, obj):
        """Update object associated to identifier.

        Raises: KeyError if oid not already existing
        
        Args:
            oid (str): id used to store the object
            obj (Shape):

        Returns:
            (None)
        """
        if oid not in self._defs:
            raise KeyError(f"{oid} not in store")
