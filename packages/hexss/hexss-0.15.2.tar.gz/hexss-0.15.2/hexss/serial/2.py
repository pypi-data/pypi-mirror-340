...
def get_comport(*args: str, verbose: bool = True) -> Optional[str]:
    ...
    Returns:
        Optional[str]: The device path of the first matching COM port, or None if no port is available.

    Raises:
        ValueError: If no matching COM port is found when specific descriptions are provided.
    """
...
class Arduino:
    # Arduino constants for pin modes and signal values.
    INPUT = 0
    OUTPUT = 1
    INPUT_PULLUP = 2
    LOW = 0
...
if __name__ == "__main__":
    try:
        with Arduino("USB-SERIAL CH340") as ar:
            ar.waiting_for_reply(max_wait=30)
            # Example: Read digital value from pin 13.
            val = ar.digitalRead(13)
            print(f"Initial digitalRead(13): {val}")

            # Set pin 13 as OUTPUT and toggle it 10 times.
            if ar.pinMode(13, ar.OUTPUT):
                for i in range(10):
                    if not ar.digitalWrite(13, ar.TOGGLE):
                        print(f"digitalWrite failed on iteration {i}")
                    time.sleep(0.5)
            else:
                print("Failed to set pinMode for pin 13.")
    except Exception as e:
        print(f"An error occurred during communication with the Arduino: {e}")
