...
async def authenticate_user(email, password):
    """
    Autentica um usuário via API OrNexus
    ...
    """
    try:
        # Usar a URL da API diretamente para login
        from ornexus.database import AUTH_SERVICE_URL
        import requests
        
        # Fazer requisição de login diretamente para a API
...
async def login_command():
    ...
    try:
        ...
        if user_response.status_code == 200:
            user_data = user_response.json()
            print(f"✅ Login bem-sucedido! Bem-vindo, {user_data['user']['name']}!")
        else:
        ...
        print(f"❌ Erro durante autenticação: {str(e)}")
        print("Por favor, verifique se o serviço API OrNexus está acessível.")
        return False
