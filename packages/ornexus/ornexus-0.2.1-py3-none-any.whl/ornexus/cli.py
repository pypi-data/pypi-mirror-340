...
async def authenticate_user(email, password):
    """
    Autentica um usuário verificando as credenciais no MongoDB
    
    Args:
        email: Email do usuário
        password: Senha do usuário (texto plano, será convertido para MD5)
    ...
    try:
        ...
        # Conectar ao MongoDB usando a conexão padrão
        mongo = MongoDBAsyncIntegration()
        
        # Buscar usuário pelo email
        user = await mongo.buscar_documento(
            colecao="users_cli",
            filtro={"email": email}
        )
        
        # Verificar se o usuário existe e a senha está correta
        if user and user.get("password") == password_md5:
            # Salvar credenciais do usuário localmente
            config_dir = Path.home() / ".ornexus"
            config_dir.mkdir(exist_ok=True)
            
            # Criar arquivo de config com as informações do usuário
            user_info = {
...
def check_login_status():
    ...
    """
    config_file = Path.home() / ".ornexus" / "config.json"
...
async def login_command():
    ...
    if user:
        print(f"✅ Login bem-sucedido! Bem-vindo, {user.get('name')}!")
        return True
    else:
...
def main():
    ...
    if args.command == "login":
        ...
        deploy_api_structure(args.path, force=args.force if hasattr(args, 'force') else False)
...
def deploy_api_structure(project_path=None, force=False):
    """
    ...
    Args:
        ...
        force: Se True, ignora a verificação de projeto válido
    """
    import subprocess
    import os
    ...
    # Verificar se é um projeto válido do OrNexus (pule se force=True)
    if not force and not (project_dir / "knowledge").exists():
        print(f"❌ Erro: O diretório não parece ser um projeto OrNexus válido.")
        print("  Execute 'ornexus create agno <nome_projeto>' primeiro para criar um projeto.")
        return False
