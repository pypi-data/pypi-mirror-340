...
@pytest.mark.parametrize(
    "endpoint, return_value, is_eks",
    [
        ("jobs/jobid/logs", {"projectId": "someid"}, False),
        ("v2/jobs/jobid/logs", {"projectId": "someid"}, True),
    ],
    ids=["default", "eks"],
)
...
@pytest.mark.parametrize(
    ...
    [
        ("jobs/jobid/logs", {"projectId": "someid"}),
    ],
    ids=["default"],
)
...
def test_deploy_watch_incremental_status(
    ...
    mock_requests_get = Mock(
        ...
        side_effect=[
            ...
            Mock(ok=True, json=Mock(return_value=job_details_response)),
            Mock(ok=True, json=Mock(return_value={"is_eks": False})),
            Mock(
                ok=True,
                json=Mock(
                    return_value={
...
def test_deploy_watch_incremental_status_eks(
    ...
    mock_requests_get = Mock(
        ...
        side_effect=[
            ...
            Mock(ok=True, json=Mock(return_value=job_details_response)),
            Mock(ok=True, json=Mock(return_value={"is_eks": True})),
            Mock(
                ok=True,
                json=Mock(
                    return_value={
