...
from qtpy.QtWidgets import QVBoxLayout, QFrame, QLabel
...
class FitEllipsoidWidget(Container):
    def __init__(self, napari_viewer):
        ...
        # vertical space to separate the plugin and the instructions
        self.native.layout().addSpacing(80)

        # Custom instructions frame
        instructions_frame = QFrame()
        instructions_frame.setFrameShape(QFrame.Box)
        instructions_frame.setFrameShadow(QFrame.Raised)

        instructions_layout = QVBoxLayout(instructions_frame)

        instructions_label = QLabel("INSTRUCTIONS:")
        instructions_label.setStyleSheet("font-size: 22px; font-family: Arial;")
        instructions_layout.addWidget(instructions_label)

        self.instructions_text = QLabel(
            "1. Open your 3D image stack in Napari.\n"
            "2. Select the point layer created by the plugin and begin \n clicking along the boundary of your object:\n"
                "\t 🖱️ Left-click to add a point\n"
                "\t 🖱️ Right-click to remove the last added point\n"
            "3. Once you've added at least 10 points, click on the 'Fit Ellipsoid' button.\n"
            "4. A blue ellipsoid will be fitted and displayed.\n"
                "\t ✅ If the shape fits well, you're done with that object.\n"
                "\t ❌ If it doesn't, return to the corresponding point layer \n \t and add or adjust points.\n"
            "5. Repeat the process for all objects you'd like to segment.\n The plugin automatically adds a new point layer after each fit.\n"
            "6. When you're finished, save the results as a .csv file \n and optionally export a segmentation mask."
        )
        self.instructions_text.setStyleSheet("font-size: 16px; font-family: Arial;")
        instructions_layout.addWidget(self.instructions_text)

        # Ajoute le QFrame au layout sous-jacent via native
        self.native.layout().addWidget(instructions_frame)
