...
# Dueling DQN 모델 정의
class Doubling_Dueling_DQN(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(Doubling_Dueling_DQN, self).__init__()
...
# DQN 학습 함수
def train_doubling_dueling_dqn(env, episodes=500, model_path="best_model.pth"):
    input_dim = 9
    output_dim = 3
    model = Doubling_Dueling_DQN(input_dim, output_dim).to(device)
    target_model = Doubling_Dueling_DQN(input_dim, output_dim).to(device)
