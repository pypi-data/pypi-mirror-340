...
from snake_dqn_core.train_rainbow_dqn import Rainbow_DQN
...
def play_rainbow_dqn_model(model_path="best_model.pth", episodes=5):
    env = SnakeGameEnv(render_mode=True)
    model = Rainbow_DQN(input_dim=9, output_dim=3).to(device)
    checkpoint = torch.load(model_path, map_location=device)
    model.load_state_dict(checkpoint["model_state_dict"])
    model.eval()
