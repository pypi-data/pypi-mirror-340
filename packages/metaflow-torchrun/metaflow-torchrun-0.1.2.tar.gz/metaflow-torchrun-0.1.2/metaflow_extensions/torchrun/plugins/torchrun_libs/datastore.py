...
from .constants import TORCHRUN_SUFFIX

# mimic a subset of the behavior of the Metaflow S3Object
TorchrunDatastoreBlob = namedtuple("TorchrunDatastoreBlob", "blob url text")
TorchrunListPathResult = namedtuple("TorchrunListPathResult", "url")


class TorchrunDatastore(object):
    """
    ...
    def get_storage_root(self):
        ...
        if self._backend.TYPE == "s3":
            return TORCHRUN_SUFFIX
        elif self._backend.TYPE == "azure":
            from metaflow.metaflow_config import DATASTORE_SYSROOT_AZURE

            return os.path.join(DATASTORE_SYSROOT_AZURE, TORCHRUN_SUFFIX)
        elif self._backend.TYPE == "gs":
            from metaflow.metaflow_config import DATASTORE_SYSROOT_GS

            return os.path.join(DATASTORE_SYSROOT_GS, TORCHRUN_SUFFIX)
        else:
            raise NotImplementedError(
                "Torchrun datastore does not support backend %s" % (self._backend.TYPE)
            )
    ...
    def list_paths(self, keys):
        ...
        if self._backend.TYPE == "gs":
            raise NotImplementedError(
                """Torchrun datastore does not support the list_paths API for Google Cloud storage. 
        ...
        list_path_results = [
            TorchrunListPathResult(url=list_content_result.path)
            for list_content_result in self._backend.list_content(keys)
        ]
        return list_path_results


def _best_effort_read_key(datastore: TorchrunDatastore, key: str):
    """
...
def _best_effort_get_keys(datastore: TorchrunDatastore, keys: List[str]):
    """
...
def wait_for_key_data(
    ...
    frequency=0.1,
) -> Dict[str, TorchrunDatastoreBlob]:
    """
