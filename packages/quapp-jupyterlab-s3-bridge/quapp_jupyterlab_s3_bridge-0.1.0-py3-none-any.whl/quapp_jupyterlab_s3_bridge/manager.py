...
class S3SelectiveContentsManager(AsyncContentsManager):
    ...
    def __init__(self, root_dir=None, backend_url=None, function_id=None, **kwargs):
        ...
        # Ngay khi khởi tạo, đăng ký gọi initialize_root() trong IOLoop
    ...
    async def _init_root_wrapper(self):
        ...
        """
        try:
            await self.initialize_root()
        except Exception as e:
            logger.exception("Error in initialize_root at startup: %s", e)

    async def initialize_root(self):
        """
        Pull object => 
          1) GET /api/v1/functions/{functionId}/versions
          2) Lấy version cuối
          3) GET /api/v1/functions/{functionId}/versions/{versionId} => presigned list
          4) Download về local
        """
        ...
        try:
            ...
            headers = {"x-project-id": str(project_id)}  # tuỳ
            ...
            logger.info("Latest version => id=%s", self.version_id)
            ...
            print("presigned_list", presigned_list)
    ...
    async def save(self, model: Dict[str, Any], path: str) -> Dict[str, Any]:
        """
        Ghi local => không gọi API. 
        User bấm “upload_to_backend” sau.
        """
        abs_path = self._abs_path(path)
    ...
    async def new_untitled(self, path: str, type: str = '', ext: str = '') -> Dict[str, Any]:
        ...
        if type == "notebook":
            # 1) Tạo notebook rỗng trên disk
            abs_path = self._abs_path(new_path)
            os.makedirs(os.path.dirname(abs_path), exist_ok=True)

            # Ghi notebook JSON rỗng
            nb_json = {
                "cells": [],
                "metadata": {},
                "nbformat": 4,
            ...
            return {
                ...
                "content": None,       # <--- Must be None
                "format": None         # <--- Must be None
            }
    ...
    # ------------------------------------------------------
    async def upload_to_backend(self, description: str = ""):
        """
        Quét file local => POST /api/v1/functions/{functionId}/versions
        templateFiles => [ {filename, language, content=[int,int,..]}, ...]
        """
    ...
    # hàm dưới đây để cập nhập lại các file hiện tại trong hàm initialize_root theo versionId
    def update_file_list(self, version: int):
        """
        Lấy danh sách version từ backend
        """
        try:
            ...
            # so sánh version với trường version trong data item
            # nếu có thì lấy versionId
            # nếu không có thì raise 404
            if not data:
                logger.warning("No versions found => skip pull.")
                return []
            ...
            for versionItem in data:
                if versionItem["id"] == self.version_id:
                    for fileItem in versionItem["files"]:
                        # remove prefix chỉ giữ lại tên file <prefix>/<tên file>
            ...
            # so sánh 2 list file hiện tại và file mới
            # nếu có file trong current_version_files mà không có trong new_version_files
            # thì xóa file đó
