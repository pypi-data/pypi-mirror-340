...
import pytest
from cmplib import And, Contains, DictFields, KeyEq, Not
...
def test_show_peer_overrides(getconf, run_mock, d):
    run_mock.on_call("/usr/bin/wg", "pubkey", _input="c1pk", retval="c1pub")
    run_mock.on_call("/usr/bin/wg", "pubkey", _input="c2pk", retval="c2pub")
    run_mock.on_call("/usr/bin/wg", "pubkey", _input="c3pk", retval="c3pub")
    ...
    d["clients"]["client1"] = {
        "PrivateKey": "c1pk",
        "AllowedIPs": ["192.168.1.1/24", "10.8.0.1/24"],
        "PersistentKeepalive": "25",
    }
    d["clients"]["client2"] = {
        "PrivateKey": "c2pk",
    ...
    assert conf["Peer"] == [
        DictFields(
            PublicKey="c1pub", AllowedIPs="18.8.0.255", PersistentKeepalive="10"
        ),
        DictFields(
            PublicKey="c2pub",
            AllowedIPs=["192.168.1.1/24", "10.8.0.1/24"],
        ...
        And(
            DictFields(PublicKey="c3pub", AllowedIPs=["192.168.1.1/24", "10.8.0.1/24"]),
            Not(Contains("PersistentKeepalive")),
        ),
    ]
