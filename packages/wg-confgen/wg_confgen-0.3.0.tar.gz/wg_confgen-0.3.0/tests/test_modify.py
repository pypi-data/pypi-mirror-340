...
def test_modify_init_client_from_scratch(getconf, run_mock, d, wg_param):
    run_mock.on_call("/usr/bin/wg", retval="key")

    conf = getconf("modify", "client", wg_param, "newval", input="{}")
    assert conf["clients"]["client"] == Contains("PrivateKey")
    assert conf["clients"]["client"] == KeyEq(wg_param, Or("newval", ["newval"]))


def test_modify_peer_overrides(getconf, run_mock, d, wg_param):
    run_mock.on_call("/usr/bin/wg", retval="key")
    ...
    conf = getconf("modify", "server", "--peer", "client1", wg_param, "value", input=d)
    assert conf["clients"]["server"]["overrides"] == {"client1": {wg_param: "value"}}


def test_modify_many_peer_overrides(getconf, run_mock, d, wg_param):
    run_mock.on_call("/usr/bin/wg", retval="key")
    ...
    assert conf["clients"]["server"]["overrides"] == {
        "client1": {wg_param: "value"},
        "client2": {wg_param: "value2"},
    }


def test_modify_peer_override_default(getconf, runsut, run_mock, d, wg_param):
    run_mock.on_call("/usr/bin/wg", retval="key")
    ...
    d["clients"]["server"] = {
        ...
        "Peers": ["client1", "client2"],
        "overrides": {
            "client1": {wg_param: "1.1.1.1"},
            "client2": {"AllowedIPs": "1.1.1.1"},
        },
    }
    d["clients"]["client1"] = {"PrivateKey": "c1pk"}
    ...
    assert conf["clients"]["server"] == Not(Contains("overrides"))


def test_modify_peer_default_single_override(getconf, run_mock, d, wg_param):
    run_mock.on_call("/usr/bin/wg", retval="key")
    ...
    conf = getconf(
        "modify", "server", "--peer", "client1", wg_param, "default", input=d
    )
    assert conf["clients"]["server"]["overrides"] == KeyEq(
        "client2", KeyEq("AllowedIPs", "1.1.1.1")
    )


def test_modify_peer_unset_override(getconf, run_mock, d, wg_param):
    run_mock.on_call("/usr/bin/wg", retval="key")

    d["clients"]["server"] = {
        "PrivateKey": "pk",
        "Peers": ["client1", "client2"],
        "overrides": {"client1": {wg_param: "1.1.1.1"}},
    }
    d["clients"]["client1"] = {"PrivateKey": "c1pk"}

    conf = getconf("modify", "server", "--peer", "client1", wg_param, "unset", input=d)
    assert conf["clients"]["server"]["overrides"]["client1"] == KeyEq(wg_param, None)


def test_modify_peer_add_override(getconf, run_mock, d, wg_param):
    run_mock.on_call("/usr/bin/wg", retval="key")

    d["clients"]["server"] = {
        "PrivateKey": "pk",
        "Peers": ["client1", "client2"],
        "overrides": {"client1": {wg_param: "orig"}},
    }

    conf = getconf(
        "modify", "server", "--peer", "client1", wg_param, "add", "value", input=d
    )
    assert conf["clients"]["server"]["overrides"]["client1"] == KeyEq(
        wg_param, ["orig", "value"]
    )


def test_modify_peer_remove_override(getconf, run_mock, d, wg_param):
    run_mock.on_call("/usr/bin/wg", retval="key")
    ...
    d["clients"]["server"] = {
        ...
        "overrides": {
            "client1": {wg_param: "value"},
            "client2": {wg_param: ["orig", "value"]},
            "client3": {wg_param: ["value"]},
        },
    }
    ...
    conf = getconf(
        "modify",
        "server",
        "--peer", "client1", wg_param, "remove", "value",
        "--peer", "client2", wg_param, "remove", "value",
        "--peer", "client3", wg_param, "remove", "value",
        input=d,
    )
    # fmt: on

    assert conf["clients"]["server"]["overrides"] == Not(Contains("client1"))
    assert conf["clients"]["server"]["overrides"]["client2"] == KeyEq(
        wg_param, ["orig"]
    )
    assert conf["clients"]["server"]["overrides"] == Not(Contains("client3"))
