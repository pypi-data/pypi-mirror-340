...
import pytest
from cmplib import Contains, KeyEq, Not, And
...
def test_remove_missing_clients(runsut, d):
    d["clients"] = {
        "server": {},
        "c1": {"Peers": ["server"], "overrides": {"server": {"AllowedIPs": "1.1.1.1"}}},
        "c2": {
            "Peers": ["c1", "server", "d1"],
            "overrides": {
                "server": {"AllowedIPs": "1.1.1.1"},
                "c1": {"AllowedIPs": "1.1.1.1"},
            },
        },
        "c3": {"Peers": ["c1", "server", "d1"]},
        "d1": {},
    ...
    assert conf["clients"] == {
        "c1": KeyEq("Peers", None) & Not(Contains("overrides")),
        "c2": And(
            KeyEq("Peers", ["c1", "d1"]),
            KeyEq("overrides", Contains("c1") & Not(Contains("server"))),
        ),
