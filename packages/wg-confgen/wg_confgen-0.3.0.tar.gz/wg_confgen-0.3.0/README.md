...
192.168.1.1/24 for all clients, except a "router" client, to which this is a
native subnet and it shouldn't route this traffic via VPN.
...
- `clients`: an ordered mapping of clients. Order of clients is important,
  ...
  always regenerates it.
    - under `clients`, there's a special `overrides` mapping, which contains
      parameter overrides in `[Peer]` sections when client is used as the
      interface.
