...
async def context_aware_handler(param, runtime_context=None):
    ...
    return ToolResult.from_success(f"Context-aware handler called with: {param}, context: {test_value}")
...
class TestContextAware:
    ...
    async def test_tool_manager_context_injection(self):
        ...
        manager.runtime_registry.register_tool("standard_tool", standard_handler, standard_def)
        manager.runtime_registry.register_tool("context_tool", context_aware_handler, context_def)
