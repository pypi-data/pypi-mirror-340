...
class TestTokenEfficientTools:
    ...
    def test_header_validation(self):
        ...
        with patch("llmproc.providers.anthropic_process_executor.logger") as mock_logger:
            ...
            if "anthropic-beta" in extra_headers and "token-efficient-tools" in extra_headers["anthropic-beta"] and not model_name.startswith("claude-3-7"):
                mock_logger.warning(f"Token-efficient tools header is only supported by Claude 3.7 models. Currently using {model_name}. The header will be ignored.")

            # Verify warning was logged
            mock_logger.warning.assert_called_with("Token-efficient tools header is only supported by Claude 3.7 models. Currently using claude-3-5-sonnet. The header will be ignored.")
    ...
    def test_extra_headers_passing(self):
        ...
        mock_client.messages.create(model="claude-3-7-sonnet-20250219", system="test", messages=[], tools=[], extra_headers=extra_headers, max_tokens=1000)
