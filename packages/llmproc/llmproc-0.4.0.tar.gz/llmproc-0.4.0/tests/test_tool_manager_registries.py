...
def test_load_builtin_tools():
    ...
    with patch("logging.Logger.info") as mock_info:
        ...
        assert not any("Loading builtin tools" in call[0][0] for call in mock_info.call_args_list)
...
def test_multiple_registries_isolation():
    ...
    manager.runtime_registry.register_tool("another_tool", another_handler, another_schema)
...
@pytest.mark.skip("This test is causing module attribute errors and needs to be completely rewritten")
@pytest.mark.asyncio
...
async def test_llmprocess_unified_mcp_initialization():
    """Test that LLMProcess delegates initialization to ToolManager with config."""
    # Import here to avoid circular imports in the test
    from llmproc.llm_process import LLMProcess
    
    # Create a mock LLMProcess instance for testing just the _initialize_tools method
    process = Mock(spec=LLMProcess)
    process._needs_async_init = True
    process.provider = "anthropic"
    process.mcp_enabled = True
    process.program = Mock()
    process.program.get_tool_configuration.return_value = {
        "mcp_enabled": True,
        "provider": "anthropic",
    ...
    # Mock the tool_manager's initialize_tools method
    with patch.object(tool_manager, 'initialize_tools') as mock_initialize:
        ...
        # Get the actual method implementation from LLMProcess
        original_method = LLMProcess._initialize_tools
        
        # Call the method directly with our mock instance
        await original_method(process)
        
        # Verify the tool manager's initialize_tools was called with the configuration
        # from the program, not the process itself
        mock_initialize.assert_called_once()
        # Get the first argument passed to initialize_tools
        args, _ = mock_initialize.call_args
        # Check that it's a dictionary, not the process object
        assert isinstance(args[0], dict)
    
    # Verify that create() no longer has a separate call to _initialize_mcp_tools
    # For this we just need to check that the method doesn't exist anymore
    from llmproc.llm_process import LLMProcess
    ...
    try:
        ...
        method = getattr(LLMProcess, '_initialize_mcp_tools')
        assert False, "The _initialize_mcp_tools method still exists but should be removed"
