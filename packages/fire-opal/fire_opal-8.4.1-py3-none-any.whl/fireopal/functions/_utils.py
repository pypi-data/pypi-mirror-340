...
def check_circuit_submission_validity(
    ...
    observables: Optional[
        dict[str, float] | list[tuple[str, float]] | list[str] | str
    ] = None,
    ...
    parameters : dict[str, float] or list[dict[str, float]] or None, optional
        ...
        Defaults to None.
    observables : dict[str, float] or list[tuple[str, float]] or list[str] or str, optional
        The observables to measure for the circuit, if this is an estimation task.
        Defaults to None.
    ...
    if observables is not None:
        if not isinstance(observables, (list, dict, str)):
            logging.error(
                "Q-CTRL: Invalid type received for observables input. "
                "The observables must be a list, dict, or a string."
            )
            raise QctrlArgumentsValueError(
        ...
        if isinstance(observables, list):
            if not all(
                isinstance(observable, (str, tuple)) for observable in observables
            ):
                logging.error(
                    "Q-CTRL: Invalid type received for observables input. "
                    "The observables must be a list of pauli strings or tuples of pauli string "
                    "and coefficient pairs."
                )
                raise QctrlArgumentsValueError(
                    "Invalid type received for observables input. "
                    "The observables must be a list of pauli strings or tuples of pauli string "
