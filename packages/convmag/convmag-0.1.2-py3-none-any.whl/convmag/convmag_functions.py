...
the various units use din magnetism.
"""
...
factors = {"M": "1e6", "k": "1e3", "m": "1e-3", "µ": "1e-6"}


units = {i for a in [x.split("_") for x in list(convmag.keys())] for i in a}
prefactors = [f"{k} ({v})" for k, v in factors.items()]
...
def convert_unit(number, startunit, endunit, verbose=False):
    ...
    try:
        ...
        try:
            key_alt = e_u + "_" + s_u
            conv = number * eval(pre) * (1.0 / eval(convmag[key_alt])) / eval(post)
        except KeyError:
...
def calculate_unitcell_volume(a, b, c, gamma=90):
    """
    Calculate the volume of an orthogonal or hexagonal unit cell.
    ...
    vol : FLOAT
        ...
        parameters. The unit is the cube of the unit used
        for a, b and c.

    Notes
    -----
    Only handles orthogonal or hexagonal unit cells,
    therefore gamma must be either 90 or 120 (degrees).
    """
    if gamma == 90:
        vol = a * b * c
    elif gamma == 120:
        ...
        msg = f"gamma must be 90 or 120 but {gamma} was entered!"
...
def muB_per_fu_to_Tesla(muB_per_fu, num_fu, vol):
    ...
    muB_per_fu : INT or FLOAT
        Moment in Bohr magnetons per formula unit (a.k.a. muB/fu)
    num_fu : INT
