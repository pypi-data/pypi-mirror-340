...
def process_configuration_context(
    ...
    #
    try:
        configurationVersion = int(read_configuration.get("version", 1))
    except Exception:
            ...
            for index, job in enumerate(read_configuration["tasks"]):
                ...
                retCode, job["sql"] = \
                    tailer_misc.apply_context(
                        payload=base64.b64decode(job["sql"]).decode("utf-8"),
                        context_data=context_data)

                # Encode SQL
                #
                read_configuration["tasks"][index]["sql"] = \
                    base64.b64encode(read_configuration["tasks"][index]["sql"].encode("utf-8")).decode("utf-8")

        else:

            # Apply context
            #
            retCode, read_configuration["sql"] = \
                tailer_misc.apply_context(
                    payload=base64.b64decode(read_configuration["sql"]).decode("utf-8"),
                    context_data=context_data)
            
            # Encode SQL 
            #
            read_configuration["sql"] = \
                base64.b64encode(read_configuration["sql"].encode("utf-8")).decode("utf-8")
