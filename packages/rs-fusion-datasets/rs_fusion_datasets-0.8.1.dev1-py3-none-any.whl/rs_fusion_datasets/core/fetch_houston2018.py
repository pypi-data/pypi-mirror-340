...
def fetch_houston2018(datahome: Optional[str] = None, download_if_missing=True) -> tuple[
    ...
    def fetch_houston2018zip(path: Path, download_if_missing: bool = True) -> Path:
        """Make sure `path` is the zip file of Houston2018, or raise FileNotFoundError
        """
        if not exists(path):
            if download_if_missing:
                url = "http://machinelearning.ee.uh.edu/QZ23es1aMPH/2018IEEE/phase2.zip"
                ...
                urllib.request.urlretrieve(url, path, reporthook=lambda blocknum, bs, size: print(f"Downloaded {blocknum * bs / size:.5%}", end="\r"))
    ...
    ZIP_PATH = DATA_HOME / 'Houston2018.zip'
    UNZIPED_PATH = DATA_HOME / 'Houston2018/'
    FILES_PATH = UNZIPED_PATH/'2018_DFTC'
    FILES_SHA256 = {
        "2018_IEEE_GRSS_DF_Contest_CASI.hdr": "869be3459978b535b873bca98b1cf05066c7acca9c160b486a86efd775005e8d",
        "2018_IEEE_GRSS_DF_Contest_CASI.tif": "1440f38594e8e82cc1944c084fc138ef55a70af54122828e999c4fb438574c14",
        "2018_IEEE_GRSS_DF_Contest_LiDAR.hdr": "053c083de1cb0d9ad51c56964b29669733ef2c7db05997d4f4e0779ab2f6aade",
        "2018_IEEE_GRSS_DF_Contest_LiDAR.tif": "9f4facce8876ee84642d9cb03536baf0389506de97ddc01b73366fe4521de981",
        "2018_IEEE_GRSS_DF_Contest_Samples_TR.roi": "feedf41f7064d8f80cf2d9bda72fcbcc98b64658d01e519ad0b90b1ca88f1375",
        "2018_IEEE_GRSS_DF_Contest_Samples_TR.txt": "16c69cf216535d7b4df2045b05d49c50a078609aa6d011a5e23e54f4cd08abda",
        "2018_IEEE_GRSS_DF_Contest_Samples_VA.zip": "aac7015c7a986063002a86eb7f7cc57ed6f14f5eaf3e9ca29c0cb1e63fd7e0d5",
        "copyright.txt": "63d908383566b1ff6fd259aa202e31dab9a629808919d87d94970df7ad25180d",
        "2018_IEEE_GRSS_DF_Contest_Samples_VA.txt": "768bb02193d04c8020b45f1f31a49926a5b914040f77f71a81df756d6e8b8dcb"
    }
    ...
    if exists(FILES_PATH) and len(os.listdir(FILES_PATH)) > 0:  # 已存在解压的文件夹且非空
        ...
        fetch_houston2018zip(ZIP_PATH, download_if_missing)
        # 解压 2018_DFTC 目录下的所有文件
    ...
    # 3. 数据加载
    with rasterio.open(FILES_PATH / '2018_IEEE_GRSS_DF_Contest_LiDAR.tif') as f:
        lidar = f.read()
    with rasterio.open(FILES_PATH / '2018_IEEE_GRSS_DF_Contest_CASI.tif') as f:
        casi = f.read()

    train_truth:coo_array= read_roi(FILES_PATH / '2018_IEEE_GRSS_DF_Contest_Samples_TR.txt', (349, 1905)) # (349 1905)
    test_truth :coo_array= read_roi(FILES_PATH / '2018_IEEE_GRSS_DF_Contest_Samples_VA.txt', (349, 1905)) # (349 1905)
