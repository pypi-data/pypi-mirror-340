...
real_api_key = os.getenv("DATURA_API_KEY", "dt_$Xjkz3efG-pOQTAw3fdEmIv8zQ0n1Oz8Ny-2uUSOSBbg")
print("Real API Key:", real_api_key)

# Skip the tests if no valid API key is found.
if not real_api_key:
    pytest.skip("Skipping real test because no valid DATURA_API_KEY provided.", allow_module_level=True)

# Import all tool classes from our package.
from langchain_datura.tools import (
    DesearchTool,
    BasicWebSearchTool,
...
def test_all_tools(tool_name, input_data):
    ...
    try:
        ...
        assert isinstance(result, (str, dict, list)), f"Result for {tool_name} is not a valid type: {type(result)}"
        if isinstance(result, str):
            assert len(result.strip()) > 0, f"Result for {tool_name} is empty or only whitespace."
            assert "error" not in result.lower(), f"Result for {tool_name} contains an error: {result}"
        else:
