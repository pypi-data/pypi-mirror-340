...
class DSLTranslator:
    ...
    def from_pattern(self, pattern: str, negation: bool) -> str:
        ...
        if obj.header:
            # If a value is provided, we ignore everything else and use it
            if obj.header_value:
                output.append(
                    f'{self.header_prefix}{obj.header} ~ "{self._escape(obj.header_value)}"'
                )
            # Header is set?
            elif obj.header_is_set:
                output.append(f"{self.header_prefix}{obj.header}")
            # Header with no value and not set means absence of the header
            else:
...
class HAProxyACLTranslator:
    ...
    def _translate_pattern(self, pattern: str) -> Dict[str, str]:
        ...
        if obj.header:
            ...
            # check existence of header only
            elif obj.header_is_set:
