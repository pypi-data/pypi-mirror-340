...
def read_s3_file(
    ...
    try:
        ...
        with s3_filesystem.open(url, mode="rb") as f:
            ...
            if CompressionHandler.is_compressed(url):
                file_stream, original_file_key = CompressionHandler.decompress(
                    url, file_content
                )
                file_source = file_stream  # Use decompressed BytesIO stream
                file_s3_opts = None  # No need for S3 options on decompressed data
