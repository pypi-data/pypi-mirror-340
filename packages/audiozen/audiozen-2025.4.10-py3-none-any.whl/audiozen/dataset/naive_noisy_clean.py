...
class NaiveNoisyCleanDatasetArgs(Serializable):
    """A simple dataset for speech enhancement where the noisy/clean samples are stored in two different folders.

    Note:
        The dataset must have the same number of noisy and clean audio files.
        The audio files must have the same filename.
...
class NaiveNoisyCleanDataset(Dataset):
