...
def _download_from_blob_metrics(datadriftdetector, local_temp_root, start_time, end_time, run_id,
    ...
    blob_list = data_store.blob_service.get_container_client(
        container=data_store.container_name
    ).list_blobs(name_starts_with=metrics_rel_base)
    # blobs can be iterated only once in new azure storage blob package v12
    blob_list, backup_blob_list = tee(blob_list)
    if len(data_store._filter_conflicting_blobs(backup_blob_list)) > 0:
        for f in blob_list:
...
def _get_metrics_path(target_date=None, drift_type=None, datadrift_id=None, datastore=None, logger=None):
    ...
    if datastore:
        blobs = datastore.blob_service.get_container_client(
            container=datastore.container_name
        ).list_blobs(name_starts_with=metrics_output_path)
