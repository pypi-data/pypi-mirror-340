...
public class CreateAnAllDayTaskTest {
    ...
    @Test
    public void createAnAllDayTaskTest() throws InterruptedException {
        Thread.sleep(5000);

        onView(withId(R.id.calendar_fab)).perform(click());
        onView(withId(R.id.fab_task_icon)).perform(click());

        // enter title
        onView(withId(R.id.task_title)).perform(replaceText("Holiday Shopping"));

        onView(allOf(withId(R.id.save), withContentDescription("Save"))).perform(click());
        Thread.sleep(1000);

        onView(allOf(withId(R.id.event_item_title), withText("Holiday Shopping"),
            withParent(hasDescendant(withId(R.id.event_item_task_image))))).check(matches(isDisplayed()));
