...
public class StartATestTest {
    ...
    @Test
    public void startATestTest() throws InterruptedException {

        onView(allOf(withText("Start Test"), withId(R.id.button_start_test))).perform(click());
        Thread.sleep(2000);
        onView(allOf(withId(R.id.text_deck_name), withText("Keto Fruit"))).perform(click());
        Thread.sleep(2000);
        onView(withId(R.id.button_ok)).perform(click());

        onView(allOf(withId(R.id.button_exit),
                withText(containsStringIgnoringCase("EXIT")))).check(matches(isClickable()));
