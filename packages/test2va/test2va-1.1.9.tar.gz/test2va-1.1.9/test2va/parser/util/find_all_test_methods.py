...
def find_all_test_methods(root: XMLElement) -> List[XMLElement]:
    ...
    test_methods = filter_list(res,
                               lambda e: e.find_descendants(
                                   lambda ele: ele.tag == f"{{{NSURL}}}name" and ele.text.find("Test") != -1))
    # These match annotation tags however, we need function tags.
    # So, we find the first ancestor which is a function tag.
    return map_list(test_methods, lambda e: e.find_first_ancestor(lambda ele: ele.tag == f"{{{NSURL}}}function"))
