...
public class UpdateThemeSettingsTest {
    ...
    @Test
    public void updateThemeSettingsTest() throws InterruptedException {

        onView(withClassName(containsStringIgnoringCase("ImageButton"))).perform(click());
        Thread.sleep(1000);
        onView(allOf(withId(R.id.menu_settings), withText("Settings"))).perform(click());
        Thread.sleep(1000);
        onView(allOf(withId(R.id.radio_light), withText("Light Mode"))).perform(click());
        Thread.sleep(1000);

        onView(allOf(withId(R.id.radio_light), withText("Light Mode"))).check(matches(isChecked()));
