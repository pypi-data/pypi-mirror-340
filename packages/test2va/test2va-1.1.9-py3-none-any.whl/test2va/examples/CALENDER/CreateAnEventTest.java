...
public class CreateAnEventTest {
    ...
    public void createAnEventTest() throws InterruptedException {
        ...
        onView(allOf(withId(R.id.calendar_fab))).perform(click());
        onView(allOf(withId(R.id.fab_event_label), withText("Event"))).perform(click());
        closeSoftKeyboard();

        // enter title
        onView(allOf(withId(R.id.event_title), withClassName(containsString("EditText"))))
            .perform(replaceText("Office Meeting"));

        // location
        onView(allOf(withId(R.id.event_location), withClassName(containsString("EditText"))))
            .perform(replaceText("Hall G89"));

        onView(allOf(withId(R.id.save), withContentDescription("Save"))).perform(click());
        Thread.sleep(1000);

        onView(allOf(withId(R.id.event_item_title), withText("Office Meeting")))
