...
from src.pyccup.core import _convert_tree
from src.pyccup.element import (css_include, image, javascipt_include,
                                javascript_with_content, link_to)

from tests.common import CommonTest
...
class ElementTest(CommonTest):
    ...
    def test_javascript_include(self):
        ...
        awaited = '<script src="jquery-1.11.1.min.js" type="text/javascript"></script>'
        conv = "".join(_convert_tree(data))
        self.assertEqual(awaited, self.normalize_result(conv))
    ...
    def test_javascript_include_remote(self):
        ...
        awaited = '<script src="http://an-url/jquery.mobile.min.css" type="text/javascript"></script>'
        conv = "".join(_convert_tree(data))
        self.assertEqual(awaited, self.normalize_result(conv))
    ...
    def test_javascript_include_mutlipe(self):
        ...
        awaited = (
            '<script src="http://an-url/jquery.mobile.min.css" type="text/javascript"></script>'
    ...
    def test_link_to(self):
        ...
        awaited = '<a href="http://github.com/~pyhiccup">pyhiccup</a>'
        conv = "".join(_convert_tree(data))
        self.assertEqual(awaited, self.normalize_result(conv))
    ...
    def test_css_include_multiple(self):
        ...
        awaited = (
            '<link href="http://homer.com/css" type="text/css"/>'
    ...
    def test_image_with_aprams(self):
        """Test image helper"""
        data = image(
            "http://homer.com/tho.jpg", alt="donut", height="20px", width="10px"
        )
        awaited = (
            '<img alt="donut" height="20px" '
