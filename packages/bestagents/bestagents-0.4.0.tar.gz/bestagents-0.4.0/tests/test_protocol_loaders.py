...
from bestagents.utils.protocol_loaders import load_protocol_adapters
...
class TestProtocolLoaders(unittest.TestCase):
    ...
    def test_load_protocol_adapters_with_manifest(self):
        ...
        protocol_names = ['bestagents.protocols.test.test_protocol']
        adapters = load_protocol_adapters(protocol_names)
        
        # Assertions
        self.assertEqual(len(adapters), 1)
        self.mock_find_spec.assert_called_with('bestagents.protocols.test.test_protocol')
        self.mock_import_module.assert_any_call('bestagents.protocols.test.test_protocol.adapter')
    ...
    def test_load_protocol_adapters_with_inheritance(self):
        ...
        with patch('bestagents.utils.protocol_loaders.issubclass') as mock_issubclass, \
    ...
    def test_load_protocol_adapters_import_error(self):
        ...
        def import_side_effect(name):
            if name == 'bestagents.protocols.nonexistent.protocol.adapter':
        ...
        protocol_names = ['bestagents.protocols.nonexistent.protocol']
        adapters = load_protocol_adapters(protocol_names)
        
        # Assertions
        self.assertEqual(len(adapters), 0)
        self.mock_import_module.assert_any_call('bestagents.protocols.nonexistent.protocol.adapter')
    ...
    def test_load_multiple_protocol_adapters(self):
        ...
        def side_effect(module_path):
            if module_path == 'bestagents.protocols.test.protocol1.adapter':
                return mock_module1
            elif module_path == 'bestagents.protocols.test.protocol2.adapter':
                return mock_module2
        ...
        protocol_names = [
            'bestagents.protocols.test.protocol1',
            'bestagents.protocols.test.protocol2'
        ]
        adapters = load_protocol_adapters(protocol_names)
        ...
        self.mock_import_module.assert_any_call('bestagents.protocols.test.protocol1.adapter')
        self.mock_import_module.assert_any_call('bestagents.protocols.test.protocol2.adapter')
