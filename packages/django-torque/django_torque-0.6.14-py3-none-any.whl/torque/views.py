    ...
    if qs:
        ...
        cache_documents = (
            cache_documents
                ...
                .annotate(rank=SearchRank(F("data_vector"), search_query))
                .order_by("-rank")
        )
...
def search_data(
    ...
    if qs:
        responses = search_filter.send(sender=None, cache_documents=cache_documents, qs=qs)

        if len(responses) == 1:
            receiver_identity, cache_documents = responses[0]
        else:
            # If there are multiple receivers, we want to use the last one that isn't the text filter.
            # This is until we have a better way to handle multiple search methods.
            for receiver_identity, docs in responses:
                if receiver_identity.__name__ != "text_filter":
                    cache_documents = docs

    results = (
        cache_documents.filter(
            document_limit_or,
