...
def r2_score(y_true, y_pred, axis=None, multioutput='raw_values'):
    ...
    if axis is not None:
        ...
        y_true = np.transpose(y_true, (*shape_collapse, *axis)).reshape(-1, np.prod(shape_final)) # Move axis to the end, and flatten the rest
        y_pred = np.transpose(y_pred, (*shape_collapse, *axis)).reshape(-1, np.prod(shape_final))
