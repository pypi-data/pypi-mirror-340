...
from .exceptions import BlobOperationError
...
class BlobOperations:
    ...
    def download_blob(
        ...
        Args:
            ...
            delete_after: Whether to delete the blob after downloading (default: None).

        Returns:
            BytesIO object containing the blob content.

        Raises:
            BlobOperationError: If the blob is not found or download fails.
        """
        validate_params(
        ...
        try:
            blob_client = self._get_blob_service_client(connection_string).get_blob_client(
            ...
            raise BlobOperationError(f"Blob not found: {blob_path}", 404)
    ...
    def upload_blob(
        ...
        Raises:
            BlobOperationError: If upload fails or data type is unsupported.
        """
        validate_params(
            connection_string=connection_string,
            container_name=container_name,
        ...
        if not content_type:
            content_type = self._infer_content_type(blob_path, data)
        ...
        try:
            if isinstance(data, (bytes, bytearray)):
                ...
                upload_data = data.read()
                if not content_type:
                    content_type = data.content_type or self._infer_content_type(blob_path, data)
                ...
                raise BlobOperationError(f"Unsupported data type: {type(data)}", 400)
            ...
            logger.info(f"Uploaded blob: {blob_path} to {container_name}")
        except Exception as e:
            logger.error(f"Failed to upload blob {blob_path}: {str(e)}")
            raise BlobOperationError(f"Upload failed: {blob_path}", 400)
    ...
    def move_blob(
        ...
        Raises:
            BlobOperationError: If move fails.
        """
        validate_params(
        ...
        try:
            source_service_client = self._get_blob_service_client(source_connection_string)
            dest_service_client = self._get_blob_service_client(dest_connection_string)
            ...
            self._wait_for_copy(dest_blob_client)
            source_blob_client.delete_blob()
            logger.info(f"Moved blob from {source_blob_path} to {dest_blob_path}")
        except Exception as e:
            logger.error(f"Failed to move blob {source_blob_path}: {str(e)}")
            raise BlobOperationError(f"Move failed: {source_blob_path}", 400)
    ...
    def delete_blob(
        ...
        Raises:
            BlobOperationError: If deletion fails.
        """
        validate_params(
            connection_string=connection_string,
            container_name=container_name,
        ...
        try:
            container_client = self._get_blob_service_client(connection_string).get_container_client(
                container_name
            )
            ...
            logger.error(f"Failed to delete blob {blob_path}: {str(e)}")
            raise BlobOperationError(f"Deletion failed: {blob_path}", 400)
    ...
    def _wait_for_copy(blob_client, max_retries: int = CONFIG.max_retries) -> None:
        ...
        if properties.copy.status != "success":
            raise BlobOperationError(f"Copy operation failed: {properties.copy.status}", 400)
