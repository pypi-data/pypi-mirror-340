...
public:\
    ...
    printf_ostream& operator<<(char* s) {\
        return *this << std::string(s);\
    }\
    printf_ostream& operator<<(const char* s) {\
        return *this << std::string(s);\
    }\
    printf_ostream& operator<<(std::string& s) {\
    ...
    printf_ostream& operator<<(std::ios_base& (*pf)(std::ios_base&)) {\
        pf(*this);\
        return *this;\
    }\
    printf_ostream& operator<<(std::basic_ostream<char>& (*pf)(std::basic_ostream<char>&)) {\
        pf(*this);\
        return *this;\
    }\
    template<typename T>\
    printf_ostream& operator<<(const T &val) {\
        static_cast<std::basic_ostream<char>&>(*this) << val;\
        return *this;\
    }\
    template<typename T>\
    printf_ostream& operator<<(T &val) {\
        static_cast<std::basic_ostream<char>&>(*this) << val;\
        return *this;\
    }\
};
...
def find_prog(prog):
    if os.path.isabs(prog) and os.path.exists(prog):
        return os.path.abspath(prog), False
    # file part of prog
    ...
    if os_dir is not None:
        ...
        if os.path.isfile(embedded_prog) and os.path.exists(embedded_prog):
            return os.path.abspath(embedded_prog), False
...
class Shell:
    ...
    def kill_and_run(self, restart=True):
        self.del_loop()
        self.process.kill()
        if restart:
            self.run()
    ...
    def _run(self, program, tool_found):
        ...
        self.program_with_args = " ".join(program_with_args)
        self.process = subprocess.Popen(
            self.program_with_args,
            # args=[],
            # text=True,
            # bufsize=1,
            shell=True,
...
class ClangReplKernel(Kernel):
    ...
    def __init__(self, **kwargs):
        ...
        for dy_libpath in ClangReplConfig.get_dynlib_dir():
            ...
            if dy_libpath not in self.my_shell.env[ClangReplConfig.DYLIB_PATH_ENV]:
    ...
    def do_shutdown(self, restart):
        ...
        kernel.
        """
        self.my_shell.kill_and_run(restart)
    ...
    def do_execute(self, code, silent, store_history=True, user_expressions=None, allow_stdin=False, *, cell_id=None,
        ...
        if ClangReplConfig.PLATFORM_NAME_ENUM == PlatformPath.Platform.Linux:
            code = code.replace("std::cout", "pcout")
            code = code.replace("std::endl", "'\\n'")
        # self.execution_count += 1
        self.my_shell.do_execute(code, send_response)

        if self.my_shell.process.returncode != None and self.my_shell.process.returncode != 0:
            return {
                'status': 'error',
