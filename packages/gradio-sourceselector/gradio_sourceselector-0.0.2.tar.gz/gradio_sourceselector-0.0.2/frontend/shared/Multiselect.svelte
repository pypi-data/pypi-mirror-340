<script lang="ts">
	...
	function handle_option_selected(e: any): void {
		...
		if (option_index === "all") {
			...
			add_or_remove_index(parseInt(option_index));
		}
	}
	...
	function handle_select_all(): void {
		selected_indices = choices.map((_, index) => index);
	}
...
<label class:container>
	...
	<div class="wrap">
		<div class="wrap-inner" class:show_options>
			{#if selected_indices.length === choices_names.length}
				Все источники
			{:else if selected_indices.length === 1}
				{choices_names[selected_indices[0]]}
			{:else if selected_indices.length === 2}
				{choices_names[selected_indices[0]]}, {choices_names[selected_indices[1]]}
			{:else if selected_indices.length > 2}
			{selected_indices.length} {selected_indices.length % 10 === 1 && selected_indices.length % 100 !== 11
				? 'источник'
				: (selected_indices.length % 10 >= 2 && selected_indices.length % 10 <= 4 && (selected_indices.length % 100 < 10 || selected_indices.length % 100 >= 20))
				? 'источника'
				: 'источников'}
			{/if}
		...
		<DropdownOptions
			...
			on:select_all={handle_select_all}
		/>
	</div>
</label>
