...
class Data(API):
    ...
    # --- PERFORMANCE & ALPHA ---
    def get_alpha_run(self, runsura:str, year:str) -> pd.DataFrame:
        """
        Retorna el método GET Alpha de la API.
        """
        api_url = f'{self.api_url}/Alpha_Run/{runsura}_{year}'
        data = self.engine(url=api_url)
        
        return data
    
    # --- PERFORMANCE & ALPHA ---
    def get_alpha(self, year:str) -> pd.DataFrame:
        """
        Retorna el método GET Alpha de la API.
        """
        api_url = f'{self.api_url}/Alpha/{year}'
        data = self.engine(url=api_url)
