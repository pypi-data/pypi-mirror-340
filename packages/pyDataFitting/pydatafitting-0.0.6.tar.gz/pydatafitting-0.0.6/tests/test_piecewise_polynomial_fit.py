...
from src.pyDataFitting.polynomial_regression import piecewise_polynomial_fit


class TestPPF(unittest.TestCase):

    def test_piecewise_poly_reg(self):
        ...
        control = np.array(
            [np.array([-0.64619257, 2.02020202]),
             np.array([-11.37979118, 6.96969697]),
             np.array([-53.66119103, 14.94949495])])

        for curr_seg, curr_par in zip(
                range(control.shape[0]), range(control.shape[1])):
            self.assertAlmostEqual(
                control[curr_seg, curr_par],
                np.array(coefs)[curr_seg, curr_par], 3)
