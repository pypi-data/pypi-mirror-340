...
def get_data(country, dataAdjustment):
    iso = get_iso(country)

    freq = 'Q'
    indic = INDICATORS[['NAME', dataAdjustment]]

    indic.dropna(how='any', inplace=True)
    indic.set_index(dataAdjustment, inplace=True)
...
app.layout = html.Div(children=[
    *make_top_banner('Filtering of historical time series data'),
    html.Div(children=[html.H4("Choose Series"),
                       ...
                       html.Table(id='dataAdjustment-selector', style={'width': '100%'},
...
@app.callback(
    [Output('dataAdjustment-selector', 'children')],
    [Input('country', 'value')])
def show_datatypes(country):
    ...
    return [app_choose_parameter('Adjusted', 'dataAdjustment', disponibles, disponible_inicial)]


@app.callback(
    [Output('select-indicator', 'children')],
    [Input('country', 'value'),
     Input('dataAdjustment', 'value'),],
)
def show_indicators(country, dataAdjustment):
    try:
        data = get_data(country, dataAdjustment)
