...
class LogsimDatasetInitStrategy(DatasetInitStrategy):
    ...
    def debug_task_dataset(self, task_id, data_id):
        """
        为debug模拟数据集
        """
        # 场景数据
        dig_dataset = db_util.get_dig_data_by_data_id(data_id)
        dataset_list = db_util.get_logsim_task_result_by_dataset_id(task_id, dig_dataset[0]["id"])
        if not dataset_list:
            raise Exception(f"回灌任务结果数据不存在 task_id:{task_id} data_id:{data_id}")
