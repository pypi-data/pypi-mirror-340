    ...
    if args.command == "auth":
        ...
        if success:
            ...
            success, message = WireGuardHelper.create_client_config(
                data, args.output, args.allowed_ips
            )
            print(message)
            ...
            if success:
                ...
                if "client_private_key" in data and data["client_private_key"]:
                    print("\nNOTE: Using server-provided private key")
                    if using_local_keypair:
                        print("      (Your generated keys were not used)")
                elif using_local_keypair:
                    print("\nNOTE: Using locally generated private key")
                else:
                    print("\nNOTE: Using user-provided public key")
        else:
            print(
                f"Failed to get WireGuard configuration: {data.get('error', 'Unknown error')}"
            )
            return 1
