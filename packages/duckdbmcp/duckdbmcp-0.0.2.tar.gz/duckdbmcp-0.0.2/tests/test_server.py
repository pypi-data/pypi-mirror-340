...
def test_create_database_in_nonexistent_directory():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        config = Config(db_path=db_path, readonly=False, db_in_memory=False)
        ...
        db.handler.run_query("CREATE TABLE test (id INTEGER)")
        db.handler.run_query("INSERT INTO test VALUES (1)")
        result = db.handler.run_query("SELECT * FROM test")
...
def test_readonly_mode_missing_directory():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        db_path = Path(tmpdir) / "subdir" / "test.db"
        config = Config(db_path=db_path, readonly=True, db_in_memory=False)
...
def test_normal_mode_create_and_write():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        db.handler.run_query("CREATE TABLE users (id INTEGER, name VARCHAR)")
        db.handler.run_query("INSERT INTO users VALUES (1, 'Alice'), (2, 'Bob')")

        result = db.handler.run_query("SELECT * FROM users ORDER BY id")
...
def test_readonly_mode_write_query_error():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        db_path = Path(tmpdir) / "test.db"
        config_writable = Config(db_path=db_path, readonly=False, db_in_memory=False)
        db_writable = DuckDBDatabase(config_writable)
        db_writable.handler.run_query("CREATE TABLE test (val INTEGER)")

        # Now open the same DB in readonly mode
        config_readonly = Config(db_path=db_path, readonly=True, db_in_memory=False)
        db_readonly = DuckDBDatabase(config_readonly)

        # Attempting to write should fail
        with pytest.raises(duckdb.Error):
            db_readonly.handler.run_query("INSERT INTO test VALUES (42)")
...
def test_readonly_flag_behavior():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        config_writable = Config(db_path=db_path_writable, readonly=False, db_in_memory=False)
        db_writable = DuckDBDatabase(config_writable)

        db_writable.handler.run_query("CREATE TABLE items (id INTEGER)")
        db_writable.handler.run_query("INSERT INTO items VALUES (100)")
        result = db_writable.handler.run_query("SELECT * FROM items")
        ...
        config_init = Config(db_path=db_path_readonly, readonly=False, db_in_memory=False)
        db = DuckDBDatabase(config_init)
        db.handler.run_query("CREATE TABLE ro_test (val INTEGER)")
        db.handler.run_query("INSERT INTO ro_test VALUES (100)")

        # Reopen in readonly mode
        config_ro = Config(db_path=db_path_readonly, readonly=True, db_in_memory=False)
        db_ro = DuckDBDatabase(config_ro)

        # Attempt to write should fail
        with pytest.raises(duckdb.Error):
            db_ro.handler.run_query("INSERT INTO ro_test VALUES (999)")

        # Check that we can still read
        result = db_ro.handler.run_query("SELECT * FROM ro_test;")
        assert result == [(100,)]

        result = db_ro.handler.run_query("PRAGMA table_info(ro_test);")
        assert result == [(0, "val", "INTEGER", False, None, False)]

        result = db_ro.handler.run_query("SHOW TABLES;")
