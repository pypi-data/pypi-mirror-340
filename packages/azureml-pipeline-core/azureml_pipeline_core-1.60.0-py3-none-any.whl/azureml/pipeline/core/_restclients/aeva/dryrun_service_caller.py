...
class DryRunServiceCaller(AE3PServiceCallerBase):
    ...
    def get_all_pipeline_endpoints_async(self, active_only=True, max_results=100):
        ...
        :raises:
         :class:`HttpOperationError<msrest.exceptions.HttpOperationError>`
        """
        pipeline_endpoints = list(self._pipeline_endpoint_entities.values())
        max_results_int = int(max_results)
        if max_results_int <= 0:
            return []
        if len(pipeline_endpoints) > max_results_int:
            return list(self._pipeline_endpoint_entities.values())[0:max_results_int]
