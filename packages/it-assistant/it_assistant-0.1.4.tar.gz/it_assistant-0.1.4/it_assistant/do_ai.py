...
if __name__ == '__main__':
    ...
    for i in data_qqq:
        if i['型号关键字词/asset_name'] and i['型号关键字词/asset_name'] != "NULL":
            asset_namelist = write_excletolist(i['型号类型/device_type'])
            i['匹配型号/AssetNamelist'] = get_SegSearchCandidates(i['型号关键字词/asset_name'],asset_namelist)
    aaaa = write_reslut(data_qqq, Testsuitelink, "测试")
    """

    asset_namelist = write_excletolist("asset")
