...
def detect_features_tool(
    ...
    try:
        ...
        return {
            "image_base64": encode_image(img_keypoints),
            "original_image_base64": encode_image(img) if draw else None,
...
def match_features_tool(
    ...
    Args:
        image1_base64: Base64 encoded first image string
        image2_base64: Base64 encoded second image string
        method: Feature detection method ('sift', 'orb', 'brisk', 'akaze')
    ...
    """
    try:
        img1 = decode_image(image1_base64)
        img2 = decode_image(image2_base64)
        ...
        return {
            "image_base64": encode_image(img_matches),
...
def detect_faces_tool(
    ...
    try:
        ...
        # When using in your own implementation, adjust this path as needed
        cascade_path = cv2.data.haarcascades
        haar_xml = os.path.join(cascade_path, 'haarcascade_frontalface_default.xml')
        
        # Check if cascade file exists
        if not os.path.exists(haar_xml):
            raise FileNotFoundError(f"Haar cascade file not found: {haar_xml}")
        ...
        if method.lower() == 'haar':
            ...
            # Check if model files exist - Skip this check for now
            # We assume these files might be in a different location in your environment
            ...
            try:
                ...
                return {
                    "error": "Failed to load DNN model. Please check file paths.",
                    "info": {
        ...
        return {
            "image_base64": encode_image(img_copy),
            "original_image_base64": encode_image(img) if draw else None,
...
def detect_objects_tool(
    ...
    try:
        ...
        # Note: You would need to adjust these paths for your environment
...
def register_tools(mcp):
    ...
    mcp.tool(detect_features_tool)
    mcp.tool(match_features_tool)
    mcp.tool(detect_faces_tool)
    mcp.tool(detect_objects_tool)