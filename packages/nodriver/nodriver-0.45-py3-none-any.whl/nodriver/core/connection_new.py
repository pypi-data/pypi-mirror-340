...
class Connection(metaclass=CantTouchThis):
        ...
        while True:
            try:
                if self._event.is_set():
                    print("listener:  waiting for event CLEAR")
                while self._event.is_set():
                    await asyncio.sleep(0.25)
                print("listener:  event CLEARED")
                raw = await asyncio.wait_for(self.websocket.recv(), 0.5)
                        ...
                        for callback in callbacks:
                            try:
                                if iscoroutinefunction(callback) or iscoroutine(
                                    ...
                                    try:
                                        await asyncio.to_thread(callback, event, self)
                                    except TypeError:
    ...
    async def send(
        ...
        self._event.clear()
        print("event CLEARED by send")
        await self.browser.wait(0.25)
