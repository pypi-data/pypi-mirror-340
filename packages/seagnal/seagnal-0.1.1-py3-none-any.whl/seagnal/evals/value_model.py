...
def main():
    ...
    output_dir = Path("../assets/eval_results/value_model/test")
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = output_dir / "value_results.json"
    with open(output_file, "w") as f:
        json.dump(results, f, indent=2)
    metrics_file = output_dir / "value_metrics.json"
    with open(metrics_file, "w") as f:
        json.dump(metrics, f, indent=2)
    print(f"\nSaved value model evaluation results to {output_file}")
    print(f"Saved value model metrics to {metrics_file}")
