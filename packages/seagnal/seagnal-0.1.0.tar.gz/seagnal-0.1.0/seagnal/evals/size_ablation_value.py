...
def prepare_inputs(task, action, state_image, processor):
    messages = [
        {
            "role": "system",
...
def create_summary(all_metrics):
    ...
    output_dir = Path("../assets/eval_results/value_model/size_ablation")
...
def main():
    ...
    for pixel_size in PIXELS:
        ...
        for example in tqdm(ds_val, desc="Evaluating test examples"):
            ...
            inputs = prepare_inputs(task, action, state_image, processor)
            outputs, generation_time, avg_memory, peak_memory = generate_answer(model, inputs)
        ...
        avg_memory_usage = sum_avg_memory / total_examples if total_examples > 0 else 0.0
        avg_peak_memory = sum_peak_memory / total_examples if total_examples > 0 else 0.0

        output_dir = Path("../assets/eval_results/value_model/size_ablation")
        output_dir.mkdir(parents=True, exist_ok=True)

        size_suffix = f"_size{pixel_size}"
        output_file = output_dir / f"value_results{size_suffix}.json"
        metrics_file = output_dir / f"value_metrics{size_suffix}.json"
        ...
        print(f"Classification Accuracy: {accuracy*100:.2f}% ({num_correct} / {total_examples})")
