...
class AgentTutorDataset(Dataset):
    def __init__(self, split: str):
        ...
        num_samples = int(len(self.data) * (TRAIN_SAMPLE_RATIO if split == "train" else EVAL_SAMPLE_RATIO))
        self.data = self.data.shuffle(seed=42).select(range(num_samples))
        logger.info(f"Loaded {(TRAIN_SAMPLE_RATIO if split == "train" else EVAL_SAMPLE_RATIO)*100}% ({num_samples} samples) of {split} split")
...
def load_model(model_name: str) -> Tuple[Qwen2_5_VLForConditionalGeneration, AutoProcessor]:
    # model
    if USE_LORA:
    ...
    model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
        model_name,
        torch_dtype=torch.bfloat16 if USE_QLORA else TORCH_DTYPE,
...
def prepare_inputs(batch: List[dict], processor, split: str) -> dict:
    ...
    texts = [processor.apply_chat_template(msg, tokenize=False, add_generation_prompt=True if split == "validation" else False) for msg in messages]
    image_inputs, _ = process_vision_info(messages)
    return processor(text=texts, images=image_inputs, padding=True, return_tensors="pt")


def train_collate_fn(batch: List[dict], processor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    inputs = prepare_inputs(batch, processor, split="train")
    labels = inputs["input_ids"].clone()
    labels[labels == processor.tokenizer.pad_token_id] = -100
    for image_token_id in [151652, 151653, 151655]:  # see tokenizer config https://huggingface.co/Qwen/Qwen2.5-VL-7B-Instruct/blob/main/tokenizer_config.json
        labels[labels == image_token_id] = -100
    return (inputs["input_ids"], inputs["attention_mask"], inputs["pixel_values"], inputs["image_grid_thw"], labels)


def val_collate_fn(batch: List[dict], processor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, List[str]]:
    suffixes = [f"<think>{example['reasoning']}</think>\n<answer>{'yes' if example['approved_label'] else 'no'}</answer>" for example in batch]
    inputs = prepare_inputs(batch, processor, split="validation")
    labels = inputs["input_ids"].clone()
    labels[labels == processor.tokenizer.pad_token_id] = -100
    for image_token_id in [151652, 151653, 151655]:
        labels[labels == image_token_id] = -100
    return (inputs["input_ids"], inputs["attention_mask"], inputs["pixel_values"], inputs["image_grid_thw"], labels, suffixes)
...
def init_dataloader(split: str, processor) -> DataLoader:
    ...
    return DataLoader(
        ...
        collate_fn=lambda batch: train_collate_fn(batch, processor) if split == "train" else val_collate_fn(batch, processor),
    )


def setup_wandb(project_name: str, run_name: str) -> None:
    os.environ["WANDB_PROJECT"] = project_name
    wandb.init(project=project_name, name=run_name, config={"EPOCHS": EPOCHS, "LEARNING_RATE": LEARNING_RATE, "BATCH_SIZE": BATCH_SIZE})


def init_optimizer(model: Qwen2_5_VLForConditionalGeneration) -> torch.optim.Optimizer:
    return AdamW(model.parameters(), lr=LEARNING_RATE, betas=(0.9, 0.999), eps=1e-8, weight_decay=WEIGHT_DECAY)


def compute_loss(batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor], model: Qwen2_5_VLForConditionalGeneration) -> torch.Tensor:
...
def calculate_metrics(predicted_labels: List[str], true_labels: List[str]) -> dict:
    true_positive_count = sum(1 for true, pred in zip(true_labels, predicted_labels) if true == "yes" and pred == "yes")
    false_positive_count = sum(1 for true, pred in zip(true_labels, predicted_labels) if true == "no" and pred == "yes")
    true_negative_count = sum(1 for true, pred in zip(true_labels, predicted_labels) if true == "no" and pred == "no")
    false_negative_count = sum(1 for true, pred in zip(true_labels, predicted_labels) if true == "yes" and pred == "no")
    ...
    f1 = 2 * true_positive_count / (2 * true_positive_count + false_positive_count + false_negative_count) if 2 * true_positive_count + false_positive_count + false_negative_count > 0 else 0
    return {
        "accuracy": accuracy,
        "precision": precision,
        "recall": recall,
...
def eval(model, val_dataloader, processor) -> dict:
    ...
    with torch.no_grad():
        for batch in tqdm(val_dataloader, desc="Evaluating"):
            num_batches += 1
            input_ids, attention_mask, pixel_values, image_grid_thw, labels, suffixes = batch  # unpack batch
            loss = compute_loss((input_ids, attention_mask, pixel_values, image_grid_thw, labels), model)  # compute loss
            ...
            trimmed_ids = [out_ids[len(in_ids) :] for in_ids, out_ids in zip(input_ids, outputs.sequences)]
            decoded_outputs = processor.tokenizer.batch_decode(trimmed_ids, skip_special_tokens=True)
            for _, (decoded_output, suffix) in enumerate(zip(decoded_outputs, suffixes)):
                example_count += 1
                try:
                    # true label
    ...
    if len(true_labels) > 0:
        ...
        metrics = {"error_rate": 1.0 if example_count > 0 else 0, "loss": avg_loss, "accuracy": 0, "precision": 0, "recall": 0, "f1": 0}
    logger.info(f"Evaluation completed. Average loss: {avg_loss:.4f}. Num batches: {num_batches}. Error rate: {metrics['error_rate']:.4f}")
    model.train()
...
def train(model: Qwen2_5_VLForConditionalGeneration, processor: AutoProcessor) -> None:
    ...
    for epoch in range(EPOCHS):
        progress_bar = tqdm(total=len(train_dataloader), desc=f"Epoch {epoch + 1}/{EPOCHS}", position=0, leave=True)
        for _, batch in enumerate(train_dataloader):
            ...
            if i % GRAD_ACCUM_STEPS == 0:
                ...
                progress_bar.update()
                if USE_WANDB:
                    wandb.log({"train/loss": loss.item(), "train/learning_rate": scheduler.get_last_lr()[0]})
            if i % EVAL_STEPS == 0:
                ...
                if eval_metrics["accuracy"] > best_val_accuracy and eval_metrics["error_rate"] == 0:
                    best_val_accuracy = eval_metrics["accuracy"]
                    logger.info(f"New best validation loss: {best_val_accuracy:.4f}")
                    os.makedirs(BEST_CHECKPOINT_DIR, exist_ok=True)
                    model.save_pretrained(BEST_CHECKPOINT_DIR)
                    model.push_to_hub("agentsea/Qwen2.5-VL-7B-SFT-adapter", private=True, commit_message=f"New best validation accuracy: {best_val_accuracy:.4f} at step {i}")
...
def main():
    ...
    if USE_WANDB:
        ...
        assert wandb_project_name is not None, "wandb_project_name is required for wandb usage"
    ...
    assert model_name in ["Qwen/Qwen2.5-VL-3B-Instruct", "Qwen/Qwen2.5-VL-7B-Instruct", "Qwen/Qwen2.5-VL-72B-Instruct"]
