...
API_KEY = os.environ.get("NVD_API_KEY", "98ea85ac-121b-40c3-a6fe-1d3085f33874")  # Fallback to hardcoded value

logging.basicConfig(level=logging.WARNING)

def search_cpe_from_user_input(product, version):
    query = f"{product} {version}"
    headers = {"apiKey": API_KEY} if API_KEY and API_KEY != "98ea85ac-121b-40c3-a6fe-1d3085f33874" else {}
    ...
    if response.status_code != 200:
        ...
        if response.status_code == 403 and not API_KEY:
            print("⚠️ NVD API key is required or invalid. Set NVD_API_KEY environment variable or update the script.")
        return []
...
def fetch_cves(cpe_uri, severity=None):
    headers = {"apiKey": API_KEY} if API_KEY and API_KEY != "your_api_key_here" else {}
