...
class AriadneTransformer(Transformer):
    ...
    def create(cls, client: yt.YtClient) -> "AriadneTransformer":
        ...
        return AriadneTransformer(
            token=token,
            ariadne_http=f"https://ariadne-http.{proxy}",
            ariadne_grpc=f"ariadne-grpc.{proxy}",
        )
...
{env} ray job submit --working-dir . -- python3 your_script.py --no-wait
{env} ray job logs raysubmit_hFndvmkyinHD9Fnk
        """
