...
@pytest.mark.webtest
def test_beer_history2(wardy_user: User) -> None:
    result = wardy_user.beer_history2()
...
def test_beer_brewery_history_buckettys(wardy_user: User) -> None:
    ...
    result = wardy_user.beer_brewery_history(brewery_id=484738, max_resorts=99)
...
def test_beer_brewery_history_4pines(wardy_user: User) -> None:
    ...
    result = wardy_user.beer_brewery_history(brewery_id=4254, max_resorts=3)

    assert result
    assert result.total_expected == 569  # NOTE fragile!
    assert result.total_found > UNTAPPD_BEER_HISTORY_SIZE * 4  # with style search
