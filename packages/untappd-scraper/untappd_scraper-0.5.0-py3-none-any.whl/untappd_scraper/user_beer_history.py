...
def load_user_beer_history_more(
    ...
    sort_keys = extract_sort_keys(soup, num_keys=max_resorts - 1)

    beers: set[WebUserHistoryBeer] = set(initial_beers)

    for sort_key in sort_keys:
        old_len = len(beers)
        if old_len < UNTAPPD_BEER_HISTORY_SIZE:
...
def extract_sort_keys(soup: BeautifulSoup, num_keys: int) -> list[str]:
