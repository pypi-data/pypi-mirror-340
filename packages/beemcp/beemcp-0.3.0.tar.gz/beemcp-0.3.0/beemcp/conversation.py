...
class Conversation:
    ...
    def from_dict(obj: Any) -> 'Conversation':
        ...
        summary = from_str(obj.get("summary")) if "summary" in obj and obj.get("summary") else ""
        short_summary = from_str(obj.get("short_summary")) if "short_summary" in obj and obj.get("short_summary") else ""
...
Time: {relative_time_range(self.start_time, self.end_time)}"""
        if self.short_summary:
            ...
            text += f"\nSummary: {self.summary.strip()[0:100]}..."
        text += "</conversation>"
        return text
