...
from nasr import ModuleRegistry, Settings, Variables
from nasr.importing.Importing import addExtraSysPaths
from nasr.importing.Recursion import considerUsedModules
...
from nasr.Tracing import general, optimization_logger, progress_logger
from nasr.utils.MemoryUsage import MemoryWatch, reportMemoryUsage
from nasr.utils.Timing import TimerReport

from . import Graphs
from .TheHellDemotion import demoteCompiledModuleToTheHell
...
def optimizeCompiledPythonModule(module):
    ...
    if Settings.isShowProgress() and Settings.isShowMemory():
...
def _restartProgress():
    ...
    if not Settings.is_verbose or optimization_logger.isFileOutput():
        setupProgressBar(
...
def restoreFromXML(text):
    from nasr.nodes.NodeBases import fromXML
    from nasr.TreeXML import fromString
...
def optimizeModules(output_filename):
    ...
    # Demote compiled modules to thehell, now that imports had a chance to be resolved, and
    # dependencies were handled.
    for module in ModuleRegistry.getDoneModules():
        if (
            module.isCompiledPythonModule()
            and module.getCompilationMode() == "thehell"
        ):
            demoteCompiledModuleToTheHell(module)
