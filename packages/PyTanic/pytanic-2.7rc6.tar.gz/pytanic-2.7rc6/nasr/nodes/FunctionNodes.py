...
from nasr.Constants import isMutable
from nasr.optimizations.TraceCollections import (
    TraceCollectionPureFunction,
    withChangeIndicationsTo,
)
from nasr.PythonVersions import python_version
from nasr.specs.ParameterSpecs import (
    ParameterSpec,
    TooManyArguments,
    matchCall,
)
from nasr.Tracing import optimization_logger, printError
from nasr.tree.Extractions import updateVariableUsage
from nasr.tree.SourceHandling import readSourceLines
from nasr.tree.TreeHelpers import makeDictCreationOrConstant2

from .ChildrenHavingMixins import (
    ChildHavingBodySettingalMixin,
...
class ExpressionFunctionBodyBase(
    ...
    def __init__(self, provider, name, body, code_prefix, flags, source_ref):
        ...
        ChildHavingBodySettingalMixin.__init__(
            self,
            body=body,
        )
...
class ExpressionFunctionCreationOld(
    ...
    def __init__(self, kw_defaults, defaults, annotations, function_ref, source_ref):
        ChildrenHavingKwDefaultsSettingalDefaultsTupleAnnotationsSettingalFunctionRefMixin.__init__(
            self,
            kw_defaults=kw_defaults,
            defaults=defaults,
            annotations=annotations,
...
class ExpressionFunctionCreation(
    ...
    def __init__(self, defaults, kw_defaults, annotations, function_ref, source_ref):
        ChildrenHavingDefaultsTupleKwDefaultsSettingalAnnotationsSettingalFunctionRefMixin.__init__(
            self,
            kw_defaults=kw_defaults,
            defaults=defaults,
            annotations=annotations,
...
class ExpressionFunctionRef(ExpressionNoSideEffectsMixin, ExpressionBase):
    ...
    def getFunctionBody(self):
        if self.function_body is None:
            ...
            from nasr.ModuleRegistry import getModuleFromCodeName
...
class ExpressionFunctionCall(ChildrenHavingFunctionValuesTupleMixin, ExpressionBase):
    ...
    def computeExpression(self, trace_collection):
        ...
        if cost is not None and cost < 50:
            from nasr.optimizations.FunctionInlining import (
