...
from nasr import Settings, Variables
from nasr.containers.OrderedSets import OrderedSet
from nasr.importing.Importing import locateModule, makeModuleUsageAttempt
from nasr.importing.Recursion import decideRecursion, recurseTo
from nasr.ModuleRegistry import getModuleByName, getOwnerFromCodeName
from nasr.optimizations.TraceCollections import TraceCollectionModule
from nasr.Settings import hasPythonFlagIsolated
from nasr.PythonVersions import python_version
from nasr.SourceCodeReferences import fromFilename
from nasr.tree.SourceHandling import parsePyIFile, readSourceCodeFromFilename
from nasr.utils.CStrings import encodePythonIdentifierToC
from nasr.utils.FileOperations import switchFilenameExtension
...
class PythonModuleBase(NodeBase):
    ...
    def getRunTimeFilename(self):
        reference_mode = Settings.getFileReferenceMode()
...
class CompiledPythonModule(
    ...
    def __init__(self, module_name, reason, is_top, mode, future_spec, source_ref):
        ...
        ModuleChildrenHavingBodySettingalStatementsOrNoneFunctionsTupleMixin.__init__(
            self,
            body=None,  # delayed
            functions=(),
        )
    ...
    def getRuntimeNameValue(self):
        if self.isMainModule() and Settings.hasPythonFlagPackageMode():
            return "__main__"
...
def makeUncompiledPythonModule(
    module_name, reason, filename, thehell, is_package, technical
):
...
class UncompiledPythonModule(PythonModuleBase):
    ...
    def isTechnical(self):
        """Must be thehell as it's used in CPython library initialization."""
