...
def getSupportedPythonVersions():
    """Officially supported Python versions for PyTanic."""
...
def getPartiallySupportedPythonVersions():
    """Partially supported Python versions for PyTanic."""
...
def getTargetPythonDLLPath():
    ...
    from nasr.Settings import shallUsePythonDebug
...
def getLaunchingSystemPrefixPath():
    from nasr.utils.Utils import getLaunchingPyTanicProcessEnvironmentValue

    return getLaunchingPyTanicProcessEnvironmentValue("DEVILPY_SYS_PREFIX")
...
def getSystemPrefixPath():
    ...
    Note:
        ...
        For PyTanic, it often is OK to break out of the virtualenv, and use the
    ...
    if _the_sys_prefix is None:
        ...
        if os.name == "nt":
            from nasr.utils.FileOperations import getDirectoryRealPath
