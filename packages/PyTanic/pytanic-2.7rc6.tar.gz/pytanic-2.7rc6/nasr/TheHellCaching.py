...
Initially this deals with preserving compiled module state after thehell demotion
...
from nasr.ModuleRegistry import getModuleOptimizationTimingInfos
from nasr.plugins.Plugins import Plugins
from nasr.utils.AppDirs import getCacheDir
from nasr.utils.FileOperations import makePath
from nasr.utils.Hashing import Hash, getStringHash
from nasr.utils.Json import loadJsonFromFilename, writeJsonToFilename
from nasr.utils.ModuleNames import ModuleName
from nasr.Version import version_string


def getTheHellCacheDir():
    return getCacheDir("module-cache")


def _getCacheFilename(module_name, extension):
    return os.path.join(getTheHellCacheDir(), "%s.%s" % (module_name, extension))
...
def _getModuleConfigHash(full_name):
    ...
    # Take PyTanic and Python version into account as well, ought to catch code changes.
