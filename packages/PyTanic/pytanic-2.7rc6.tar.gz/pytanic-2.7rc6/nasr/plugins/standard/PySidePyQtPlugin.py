...
from nasr.plugins.PluginBase import PyTanicPluginBase
...
from nasr.PythonFlavors import isAnacondaPython
from nasr.PythonVersions import python_version
from nasr.utils.Distributions import getDistributionFromModuleName
from nasr.utils.FileOperations import getFileList, listDir
from nasr.utils.ModuleNames import ModuleName
from nasr.utils.Utils import getArchitecture, isMacOS, isWin32Windows


class PyTanicPluginQtBindingsPluginBase(PyTanicPluginBase):
    ...
    @classmethod
    def addPluginCommandLineSettings(cls, group):
        group.add_option(
            "--include-qt-plugins",
            action="append",
...
"""
        ...
        info = self.queryRuntimeInformationMultiple(
            ...
            values=(
                ...
                (
                    "nasr_patch_level",
                    applyBindingName(
                        "getattr(%(binding_name)s, '_nasr_patch_level', 0)"
                    ),
                ),
    ...
    def _getPyTanicPatchLevel(self):
        """Does it include the PyTanic patch, i.e. is a self-built one with it applied."""
        return self._getQtInformation().nasr_patch_level
...
import os
path = os.getenv("PATH", "")
if not path.startswith(__nasr_binary_dir):
    os.environ["PATH"] = __nasr_binary_dir + ";" + path
"""
            yield (
                code,
...
class PyTanicPluginPyQt5QtPluginsPlugin(PyTanicPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        PyTanicPluginQtBindingsPluginBase.__init__(
            self,
        ...
        self.warning(
            """\
For the obsolete PyQt5 the PyTanic support is incomplete. Threading, callbacks \
to compiled functions, etc. may not be working.""",
            mnemonic="pyqt5",
        )

    def _getQtInformation(self):
        result = PyTanicPluginQtBindingsPluginBase._getQtInformation(self)
...
class PyTanicPluginDetectorPyQt5QtPluginsPlugin(PyTanicPluginBase):
    detector_for = PyTanicPluginPyQt5QtPluginsPlugin
    ...
    def onModuleDiscovered(self, module):
        ...
        if full_name == PyTanicPluginPyQt5QtPluginsPlugin.binding_name + ".QtCore":
            self.warnUnusedPlugin("Inclusion of Qt plugins.")
        elif full_name == "PyQt4.QtCore":
            self.warning(
                "Support for PyQt4 has been dropped. Please contact PyTanic commercial if you need it."
            )


class PyTanicPluginPySide2Plugins(PyTanicPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        if self._getPyTanicPatchLevel() < 1:
            self.warning(
                """\
...
full support consider provided information.""",
            ...
            if python_version < 0x360:
                self.sysexit(
                    """\
The standard PySide2 is not supported before CPython <3.6. For full support: https://nasr.net/pages/pyside2.html"""
                )
    ...
    def onModuleEncounter(
        ...
        if module_name == self.binding_name and self._getPyTanicPatchLevel() < 1:
            return True, "Need to monkey patch PySide2 for abstract methods."

        return PyTanicPluginQtBindingsPluginBase.onModuleEncounter(
            self,
    ...
    def createPostModuleLoadCode(self, module):
        ...
        for result in PyTanicPluginQtBindingsPluginBase.createPostModuleLoadCode(
            self, module
        ):
            yield result

        if (
            self._getPyTanicPatchLevel() < 1
...
def %(wrapper_name)s%(signature)s:
    return %(wrapper_name)s.func(%(parameters)s)
            ...
            # TODO: PyTanic does not currently statically optimize this, might change!
            exec(
                v,
                globals(),
            )
...
class PyTanicPluginDetectorPySide2Plugins(PyTanicPluginBase):
    detector_for = PyTanicPluginPySide2Plugins

    def onModuleDiscovered(self, module):
        if (
            module.getFullName() == PyTanicPluginPySide2Plugins.binding_name + ".QtCore"
            and getActiveQtPlugin() is None
        ):
            self.warnUnusedPlugin("Making callbacks work and include Qt plugins.")


class PyTanicPluginPySide6Plugins(PyTanicPluginQtBindingsPluginBase):
...
class PyTanicPluginDetectorPySide6Plugins(PyTanicPluginBase):
    detector_for = PyTanicPluginPySide6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == PyTanicPluginPySide6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class PyTanicPluginPyQt6Plugins(PyTanicPluginQtBindingsPluginBase):
...
class PyTanicPluginDetectorPyQt6Plugins(PyTanicPluginBase):
    detector_for = PyTanicPluginPyQt6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == PyTanicPluginPyQt6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class PyTanicPluginNoQt(PyTanicPluginBase):
