...
PyTanic can detect some things that "pkg_resources" may not even be able to during
...
from nasr.utils.Utils import withNoDeprecationWarning


class PyTanicPluginResources(PyTanicPluginBase):
...
class PyTanicProvider(EggProvider):
    def _has(self, path):
...
register_loader_type(__nasr_loader_type, PyTanicProvider)
"""

        yield (
            code,
            """Registering PyTanic loader with "pkg_resources".""",
        )
