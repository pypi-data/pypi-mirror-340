...
PyTanic can detect some things that PyLint and PyDev will complain about too,
and sometimes it's a false alarm, so people add disable markers into their
source code. PyTanic does it itself.

This tries to parse the code for these markers and uses hooks to prevent PyTanic
from warning about things, disabled to PyLint or Eclipse. The idea is that we
won't have another mechanism for PyTanic, but use existing ones instead.

The code for this is very incomplete, barely good enough to cover PyTanic's own
...
class PyTanicPluginPylintEclipseAnnotations(PyTanicPluginBase):
...
if False:
    ...
    class PyTanicPluginDetectorPylintEclipseAnnotations(PyTanicPluginBase):
        detector_for = PyTanicPluginPylintEclipseAnnotations
