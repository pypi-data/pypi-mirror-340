...
""" Standard plug-in to tell user about needed or useful settings for packages.
...
class PyTanicPluginSettingsNanny(PyTanicYamlPluginBase):
    plugin_name = "settings-nanny"
    plugin_desc = (
    ...
    def sysexitIllegalSettingValue(self, full_name, option, value):
        self.sysexit(
...
possible."""
            ...
            self.sysexitIllegalSettingValue(full_name, "macos_bundle", macos_bundle)
...
Error, package '%s' requires '--onefile' to be used on top of '--macos-create-app-bundle' or else it cannot work."""
            ...
            self.sysexitIllegalSettingValue(
    ...
    def getImplicitImports(self, module):
        ...
        for settings_config in self.config.get(full_name, section="settings"):
            for check in settings_config.get("checks", ()):
