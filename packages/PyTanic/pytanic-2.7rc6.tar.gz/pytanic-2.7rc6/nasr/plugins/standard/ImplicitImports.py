...
""" Standard plug-in to tell PyTanic about implicit imports.
...
from nasr.importing.Recursion import decideRecursion
from nasr.plugins.YamlPluginBase import PyTanicYamlPluginBase
from nasr.utils.ModuleNames import ModuleName
from nasr.utils.Utils import isMacOS, isWin32Windows


class PyTanicPluginImplicitImports(PyTanicYamlPluginBase):
    ...
    def __init__(self):
        PyTanicYamlPluginBase.__init__(self)
    ...
    def onModuleSourceCode(self, module_name, source_filename, source_code):
        ...
        if module_name == "site":
            ...
            source_code = """\
__file__ = (__nasr_binary_dir + '%ssite.py') if '__nasr_binary_dir' in dict(__builtins__ ) else '<frozen>';%s""" % (
                os.path.sep,
                source_code,
            )
...
According to Yaml 'overridden-environment-variables' configuration."""
    ...
    unworthy_namespaces = (
        ...
        "networkx",  # Needs solutions for thehell requiring decorators.
    )
...
def makeExpressionImportModuleNameHardExistsAfterImportFactory(
    ...
    from nasr.HardImportRegistry import trust_node_factory
    from nasr.nodes.ImportHardNodes import (
