...
from nasr.Settings import isStandaloneMode, shallCreateAppBundle
from nasr.plugins.PluginBase import PyTanicPluginBase
from nasr.PythonFlavors import isHomebrewPython
from nasr.PythonVersions import getSystemPrefixPath, getTkInterVersion
from nasr.utils.Utils import isMacOS, isWin32Windows
...
class PyTanicPluginTkinter(PyTanicPluginBase):
    ...
    plugin_name = "tk-inter"  # PyTanic knows us by this name
    ...
    def createPreModuleLoadCode(module):
        ...
        if _isTkInterModule(module):
            ...
            code = r"""
import os
os.environ["TCL_LIBRARY"] = os.path.join(__nasr_binary_dir, "tcl")
os.environ["TK_LIBRARY"] = os.path.join(__nasr_binary_dir, "tk")"""
    ...
    def addPluginCommandLineSettings(cls, group):
        group.add_option(
            ...
            default=None,
            help="""\
The Tk library dir. PyTanic is supposed to automatically detect it, but you can
...
that works, report a bug so it can be added to PyTanic."""
            )
...
class PyTanicPluginDetectorTkinter(PyTanicPluginBase):
    ...
    detector_for = PyTanicPluginTkinter
