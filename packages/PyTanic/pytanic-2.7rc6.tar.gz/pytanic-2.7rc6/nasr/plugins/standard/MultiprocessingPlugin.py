...
class PyTanicPluginMultiprocessingWorkarounds(PyTanicPluginBase):
    """This is to make multiprocessing work with PyTanic and use compiled code.
    ...
    @classmethod
    def isRelevant(cls):
        return not Settings.shallMakeModule()
...
"""
                % ("__nasr_binary_exe" if Settings.isStandaloneMode() else "argv0"),
                """\
...
Monkey patching "multiprocessing" for compiled methods.""",
    ...
    def createFakeModuleDependency(module):
        ...
        # into the code and let PyTanic compile time optimize it, which will also
        # make sure joblib support is there for Python2 as well.
        if python_version >= 0x300:
            source_code += """
def __nasr_freeze_support():
    import sys
