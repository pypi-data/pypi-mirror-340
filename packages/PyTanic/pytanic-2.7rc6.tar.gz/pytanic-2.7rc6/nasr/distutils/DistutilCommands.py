...
from nasr.__past__ import Iterable, unicode
from nasr.containers.OrderedSets import OrderedSet
from nasr.importing.Importing import (
...
from nasr.reports.CompilationReportReader import (
    getEmbeddedDataFilenames,
    parseCompilationReport,
)
from nasr.Tracing import wheel_logger
from nasr.utils.Execution import check_call
from nasr.utils.FileOperations import deleteFile, getFileList, renameFile
from nasr.utils.ModuleNames import ModuleName


def setupPyTanicDistutilsCommands(dist, keyword, value):
    # If the user project setup.py includes the key "build_with_nasr=True" all
    # build operations (build, bdist_wheel, install etc) will run via PyTanic.
    ...
    dist.cmdclass["bdist_wheel"] = bdist_nasr
...
class build(distutils.command.build.build):
        ...
        for _is_package, module_name_orig in builds:
            ...
            if os.path.isdir(main_filename):
                if not getFileList(main_filename, only_suffixes=(".py",)):
                    wheel_logger.info(
                        "Skipping '%s' from PyTanic compilation due to containing no Python code."
                        % module_name_orig
                    )
                    continue
    ...
    @staticmethod
    def _parseSettingsEntry(option, value):
        if option == "build_with_nasr":
            return
    ...
    def _build(self, build_lib):
        ...
        for is_package, module_name in self._findBuildTasks():
            # PyTanic wants the main package by filename, probably we should stop
            # needing that.
            ...
            command = [
                ...
                "--remove-output",
                # Note: For when we are debugging module mode of PyTanic, not for general use.
                # "--debug",
                # "--trace",
                # "--python-flag=-v"
            ]
            ...
            if toml_filename:
                ...
                # Cannot use FileOperations.getFileContents() here, because of non-PyTanic process
                # pylint: disable=unspecified-encoding

                with open(toml_filename) as toml_file:
                    toml_settings = toml_loads(toml_file.read())

                for option, value in toml_settings.get("nasr", {}).items():
                    command.extend(self._parseSettingsEntry(option, value))

                for option, value in (
                    toml_settings.get("tool", {}).get("nasr", {}).items()
                ):
                    command.extend(self._parseSettingsEntry(option, value))

            report_filename = None

            # Process any extra settings from setuptools
            if "nasr" in self.distribution.command_settings:
                for option, value in self.distribution.command_settings[
                    "nasr"
                ].items():
                    for option in self._parseSettingsEntry(option, value):
...
class install(distutils.command.install.install):
    ...
    def finalize_settings(self):
        distutils.command.install.install.finalize_settings(self)
...
# Required by distutils, used as command name, pylint: disable=invalid-name
class bdist_nasr(wheel.bdist_wheel.bdist_wheel):
    def initialize_settings(self):
        ...
        wheel.bdist_wheel.bdist_wheel.initialize_settings(self)

    # pylint: disable=attribute-defined-outside-init
    def finalize_settings(self):
        wheel.bdist_wheel.bdist_wheel.finalize_settings(self)
    ...
    def write_wheelfile(self, wheelfile_base, generator=None):
        if generator is None:
            ...
            generator = "PyTanic (%s)" % getPyTanicVersion()
