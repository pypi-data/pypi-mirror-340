...
""" PyTanic python -m build integration """
...
class PyTanicBuildMetaBackend(setuptools.build_meta._BuildMetaBackend):
    def build_wheel(
        self, wheel_directory, config_settings=None, metadata_directory=None
    ):
        # Allow falling back to setuptools when the `build_with_nasr` configuration setting is set to true.
        if config_settings:
            build_with_nasr = config_settings.pop("build_with_nasr", "true").lower()

            if build_with_nasr not in ("true", "false"):
                raise ValueError(
                    "When passing the 'build_with_nasr' setting, it must either be 'true' or 'false'."
                )

            if build_with_nasr == "false":
        ...
        with suppress_known_deprecation():
            return self._build_with_temp_dir(
                ["bdist_nasr"], ".whl", wheel_directory, config_settings
            )


_BACKEND = PyTanicBuildMetaBackend()
