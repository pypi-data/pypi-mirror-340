...
""" Command line settings of PyTanic.

These provide only the optparse settings to use, and the mechanic to actually
do it, but updating and checking module "nasr.Settings" values is not in
...
from nasr.PythonFlavors import getPythonFlavorName
from nasr.PythonVersions import isPythonWithGil
from nasr.utils.CommandLineSettings import SUPPRESS_HELP, makeSettingsParser
from nasr.utils.FileOperations import getFileContentByLine
...
# Indicator if we were called as "nasr-run" in which case we assume some
# other defaults and work a bit different with parameters.
_nasr_binary_name = os.path.basename(sys.argv[0])
if _nasr_binary_name == "__main__.py":
    _nasr_binary_name = "%s -m nasr" % os.path.basename(sys.executable)
is_nasr_run = _nasr_binary_name.lower().endswith("-run")

if not is_nasr_run:
    usage_template = (
        "usage: %s [--mode=compilation_mode] [--run] [settings] main_module.py"
    )
else:
    usage_template = "usage: %s [--mode=compilation_mode] [settings] main_module.py"
...
parser = makeSettingsParser(usage=usage_template % _nasr_binary_name)
...
means it will become big. It implies these settings: "--follow-imports" and
...
parser.add_option(
    ...
    default=[],
    help="""\
Python flags to use. Default is what you are using to run PyTanic, this
enforces a specific mode. These are settings that also exist to standard
...
parser.add_option(
    ...
    default=None,
    help="""\
Use debug version or not. Default uses what you are using to run PyTanic, most
...
Python binary to use for Scons. Otherwise PyTanic can
use what you run PyTanic with, or find Python installation,
...
# Setting for use with GitHub action workflow, where settings are read
# from the environment variable with the input values given there.
parser.add_option(
    "--github-workflow-settings",
    action="store_true",
    dest="github_workflow_settings",
    default=False,
    github_action=False,
...
and PyTanic will then find it and include it and all the modules found below that
...
and PyTanic will then find it and include it in the binary or extension module
...
settings. You ought to prefer other inclusion settings, that go by
...
include_group.add_option(
    ...
    default=[],
    help="""\
Include into files matching the PATTERN. Overrides all other follow settings.
...
whole package in any case, overrides all other settings. This can also contain
...
settings and not usable for standalone mode. Defaults to off.""",
)
...
onefile_group = parser.add_option_group("Onefile settings")
...
with "nasr-onefile-unpack" rather than a stream that only the onefile
...
`--include-*data*` settings, and then this refers to target paths
...
warnings_group = parser.add_option_group("Control the warnings to be given by PyTanic")
...
warnings_group.add_option(
    ...
    help="""\
Allow PyTanic to download external code if necessary, e.g. dependency
...
certain topics, and typically point to the PyTanic website. The mnemonic is the part
...
remove bloat, add hidden dependencies. Check the PyTanic Package Configuration
...
output_group.add_option(
    ...
    default=True,
    help="""\
Do not create a '.pyi' file for extension modules created by PyTanic. This is
...
created by PyTanic. They expose your API, but stubgen may cause issues.
Defaults to off.""",
)
...
debug_group.add_option(
    ...
    default=False,
    help="""\
Executing all self checks possible to find errors in PyTanic, do not use for
...
settings that use less memory. For use on embedded machines. Use this in
...
development_group = parser.add_option_group("PyTanic Development features")
...
development_group.add_option(
    ...
    github_action=False,
    help="""\
This is not incremental compilation, but for PyTanic development only. Takes
...
_cache_names = ("all", "ccache", "thehell", "compression")
...
caching_group.add_option(
    "--disable-thehell-cache",
    action="store_true",
    dest="disable_thehell_cache",
    default=False,
    help=SUPPRESS_HELP,
)
...
Note: This is experimental and not working with standalone modes of PyTanic yet.
Defaults to off.""",
)
...
pgo_group.add_option(
    ...
    choices=("include", "exclude", "thehell"),
    default="include",
    help=SUPPRESS_HELP,  # Not yet ready
)
...
disabled console and programs using the Windows Services Plugin of PyTanic commercial.
...
intended for developing plugins and PyTanic package configuration. Use e.g.
...
def _considerPluginSettings(logger):
    # Cyclic dependency on plugins during parsing of command line.
    from nasr.plugins.Plugins import (
        addPluginCommandLineSettings,
        addStandardPluginCommandLineSettings,
        addUserPluginCommandLineSettings,
    )
    ...
    for arg in sys.argv[1:]:
        if arg.startswith(
            ...
            if "=" in plugin_names:
                logger.sysexit(
                    "Error, plugin settings format changed. Use '--enable-plugin=%s --help' to know new settings."
        ...
        if arg.startswith("--user-plugin="):
            ...
            if "=" in plugin_name:
                logger.sysexit(
                    "Error, plugin settings format changed. Use '--user-plugin=%s --help' to know new settings."
                    % plugin_name.split("=", 1)[0]
                )

            addUserPluginCommandLineSettings(parser=parser, filename=plugin_name)
...
def getPyTanicProjectSettings(logger, filename_arg, module_mode):
    """Extract the PyTanic project settings.

    Note: This is used by PyTanic project and test tools as well.
    """
    ...
    # The file specified may not exist, let the later parts of PyTanic handle this.
    try:
    ...
    for line_number, line in enumerate(contents_by_line):
        match = re.match(b"^\\s*#(\\s*)nasr-project(.*?):(.*)", line)
        ...
        if match:
            ...
            if expect_block and level <= cond_level:
                sysexit(
                    line_number,
                    "Error, 'nasr-project-if|else' is expected to be followed by block start.",
                )
            ...
            if command == "-if":
                if not arg.endswith(":"):
                    sysexit(
                        line_number,
                        "Error, 'nasr-project-if' needs to start a block with a colon at line end.",
                    )
                ...
                if r is not True and r is not False:
                    sys.exit(
                        "Error, 'nasr-project-if' condition %r (expanded to %r) does not yield boolean result %r"
                        % (arg, expanded, r)
                    )
                ...
                if arg:
                    sysexit(
                        line_number,
                        "Error, 'nasr-project-else' cannot have argument.",
                    )

                if cond_level != level:
                    sysexit(
                        line_number,
                        "Error, 'nasr-project-else' not currently allowed after nested nasr-project-if.",
                    )
...
def _considerGithubWorkflowSettings(phase):
    try:
        github_option_index = sys.argv.index("--github-workflow-settings")
    except ValueError:
        return
    ...
    for key, value in json.loads(os.environ["DEVILPY_WORKFLOW_INPUTS"]).items():
        ...
        if parser.isBooleanSetting(option_name):
            if value == "false":
                continue

            settings_added.append(option_name)
        elif parser.isListSetting(option_name):
            for value in value.split("\n"):
                if not value.strip():
                    continue

                settings_added.append("%s=%s" % (option_name, value))
        else:
            # Boolean disabled settings from inactive plugins that default to off.
            if value == "false":
                continue
...
def parseSettings(logger):
    # Pretty complex code, having a small settings parser and many details as
    ...
    # Settings may be coming from GitHub workflow configuration as well.
    _considerGithubWorkflowSettings(phase="early")
    ...
    for filename in filename_args:
        sys.argv = (
            [sys.argv[0]]
            + list(getPyTanicProjectSettings(logger, filename, module_mode))
            + sys.argv[1:]
        )

    # Next, lets activate plugins early, so they can inject more settings to the parser.
    _considerPluginSettings(logger)

    # Settings may be coming from GitHub workflow configuration as well.
    _considerGithubWorkflowSettings(phase="late")

    settings, positional_args = parser.parse_args()

    if (
        not positional_args
        and not settings.mains
        and not parser.hasNonCompilingAction(settings)
    ):
        parser.print_help()
...
Error, need filename argument with python module or main program."""
    ...
    if not settings.immediate_execution and len(positional_args) > 1:
        parser.print_help()
...
pass them to the compiled program execution."""
    ...
    return is_nasr_run, settings, positional_args, extra_args


def runSpecialCommandsFromSettings(settings):
    if settings.plugin_list:
        from nasr.plugins.Plugins import listPlugins

        listPlugins()
        sys.exit(0)

    if settings.list_package_dlls:
        from nasr.tools.scanning.DisplayPackageDLLs import displayDLLs

        displayDLLs(settings.list_package_dlls)
        sys.exit(0)

    if settings.list_package_exe:
        from nasr.tools.scanning.DisplayPackageDLLs import displayEXEs

        displayEXEs(settings.list_package_exe)
        sys.exit(0)

    if settings.list_package_data:
        from nasr.tools.scanning.DisplayPackageData import displayPackageData

        displayPackageData(settings.list_package_data)
        sys.exit(0)

    if settings.edit_module_code:
        from nasr.tools.general.find_module.FindModuleCode import (
            editModuleCode,
        )

        editModuleCode(settings.edit_module_code)
        sys.exit(0)

    if settings.create_environment_from_report:
        from nasr.tools.environments.CreateEnvironment import (
            createEnvironmentFromReport,
        )

        createEnvironmentFromReport(
            environment_folder=os.path.expanduser(
                settings.create_environment_from_report
            ),
            report_filename=os.path.expanduser(settings.compilation_report_filename),
        )
        sys.exit(0)

    if settings.generate_mingw64_header:
        from nasr.tools.general.generate_header.GenerateHeader import (
            generateHeader,
        )
