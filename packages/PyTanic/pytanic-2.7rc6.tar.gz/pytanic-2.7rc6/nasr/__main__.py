...
"""
This is the main program of PyTanic, it checks the settings and then translates
...
def getLaunchingPyTanicProcessEnvironmentValue(environment_variable_name):
...
def main():
    ...
    if (
        ...
        ctypes.windll.user32.MessageBoxW(
            None,
            "You have to use the 'python.exe' and not a 'pythonw.exe' to run PyTanic",
            "Error",
    ...
    getLaunchingPyTanicProcessEnvironmentValue("DEVILPY_SITE_FILENAME")

    nasr_binary_name = getLaunchingPyTanicProcessEnvironmentValue("DEVILPY_BINARY_NAME")
    if nasr_binary_name is not None:
        sys.argv[0] = nasr_binary_name

    nasr_pythonpath_ast = getLaunchingPyTanicProcessEnvironmentValue(
        "DEVILPY_PYTHONPATH_AST"
    )

    if nasr_pythonpath_ast is not None:
        ...
        sys.path = [nasr_pythonpath_ast]
        import ast

        sys.path = ast.literal_eval(
            getLaunchingPyTanicProcessEnvironmentValue("DEVILPY_PYTHONPATH")
        )
    else:
        ...
        # from "nasr.distutils". Also ignore path elements that do not really
        # exist.
        sys.path = [
            path_element
    ...
    # changing it every single time PyTanic is run. This kills any attempt at
    ...
    if needs_re_execution:
        from nasr.utils.ReExecute import reExecutePyTanic  # isort:skip

        # Does not return
        reExecutePyTanic(pgo_filename=None)
    ...
    # module of nasr package and renamed to where we can get at easily for
    # other uses.
    __import__("nasr").getLaunchingPyTanicProcessEnvironmentValue = (
        getLaunchingPyTanicProcessEnvironmentValue
    )

    from nasr import Settings  # isort:skip

    Settings.parseArgs()

    Settings.commentArgs()

    # Load plugins after we know, we don't execute again.
    from nasr.plugins.Plugins import activatePlugins
    ...
    if "DEVILPY_NAMESPACES" in os.environ:
        ...
        from nasr.importing.PreloadedPackages import setPreloadedPackagePaths
    ...
    if "DEVILPY_PTH_IMPORTED" in os.environ:
        ...
        from nasr.importing.PreloadedPackages import setPthImportedPackages
    ...
    if "DEVILPY_USER_SITE" in os.environ:
        from nasr.utils.Distributions import setUserSiteDirectory

        setUserSiteDirectory(ast.literal_eval(os.environ["DEVILPY_USER_SITE"]))

        del os.environ["DEVILPY_USER_SITE"]

    # Now the real main program of PyTanic can take over.
    from nasr import MainControl  # isort:skip
...
if __name__ == "__main__":
    _nasr_package_home = getLaunchingPyTanicProcessEnvironmentValue(
        "DEVILPY_PACKAGE_HOME"
    )
    if _nasr_package_home is not None:
        sys.path.insert(0, _nasr_package_home)

        import nasr  # just to have it loaded from there, pylint: disable=unused-import

        assert sys.path[0] is _nasr_package_home
        del sys.path[0]
