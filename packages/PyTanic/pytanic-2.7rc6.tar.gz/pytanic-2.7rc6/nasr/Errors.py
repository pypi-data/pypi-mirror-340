...
class PyTanicErrorBase(Exception):
    pass


class PyTanicNodeError(PyTanicErrorBase):
    # Try to output more information about nodes passed.
    def __str__(self):
        try:
            from nasr.code_generation.Indentation import indented
            ...
            return "<PyTanicNodeError failed with %r>" % e


class PyTanicOptimizationError(PyTanicNodeError):
    pass


class PyTanicAssumptionError(AssertionError):
    pass


class PyTanicCodeDeficit(PyTanicErrorBase):
    pass


class PyTanicNodeDesignError(Exception):
    pass


class PyTanicForbiddenImportEncounter(Exception):
...
class CodeTooComplexCode(Exception):
    ...
    It cannot be compiled, with recursive code, and therefore the thehell
...
class PyTanicForbiddenDLLEncounter(Exception):
    """This DLL is not allowed to be included."""


class PyTanicSyntaxError(Exception):
