...
from nasr.__past__ import iterItems
...
from nasr.utils.Hashing import getStringHash
from nasr.utils.InstanceCounters import (
    counted_del,
    counted_init,
    isCountingInstances,
)
...
class TempMixin(object):
    ...
    def allocateExceptionKeeperVariables(self):
        ...
        debug = Settings.is_debug and python_version >= 0x300

        if debug:
            keeper_obj_init = "Empty_PyTanic_ExceptionPreservationItem"
        else:
    ...
    def addExceptionPreserverVariables(self, preserver_id):
        ...
        if preserver_id not in self.preserver_variable_declaration:
            needs_init = Settings.is_debug and python_version >= 0x300

            if needs_init:
                preserver_obj_init = "PyTanic_ExceptionStackItem_Empty"
            else:
    ...
    def addCleanupTempName(self, tmp_name):
        ...
        assert (
            tmp_name.c_type != "nasr_void" or tmp_name.code_name == "tmp_unused"
        ), tmp_name
...
if Settings.isExperimental("new-code-objects"):
    ...
    def pushFrameHandle(self, frame_code_name, is_light):
        ...
        if is_light:
            ...
            frame_identifier = self.variable_storage.addVariableDeclarationTop(
                "struct PyTanic_FrameObject *", frame_handle, None
            )
...
class PythonModuleContext(
    ...
    def getConstantCode(self, constant, deep_check=False):
        if deep_check and Settings.is_debug:
