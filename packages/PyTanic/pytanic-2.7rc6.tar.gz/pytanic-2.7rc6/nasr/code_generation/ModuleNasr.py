...
from nasr.code_generation import Emission
from nasr.PythonVersions import python_version
from nasr.utils.CStrings import encodePythonStringToC
from nasr.Version import getPyTanicVersion, getPyTanicVersionYear
...
from .CodeObjectNasr import getCodeObjectsDeclCode, getCodeObjectsInitCode
...
def getModuleCode(
    ...
    for _identifier, code in sorted(iterItems(context.getHelperNasr())):
    ...
    if is_top == 1 and Settings.shallMakeModule():
    ...
    module_init_codes = context.getModuleInitNasr()
    ...
    if Settings.isExperimental("new-code-objects"):
        ...
        # forever anyway.
        if Settings.getFileReferenceMode() == "frozen" or os.path.isabs(module_filename):
