...
from nasr.HardImportRegistry import isHardModule, isHardModuleDynamic
from nasr.nodes.LocalsScopes import GlobalsDictHandle
...
from .ErrorNasr import getErrorExitBoolCode, getErrorExitCode
from .LineNumberNasr import emitLineNumberUpdateCode
...
def generateConstantSysVersionInfoCode(to_name, expression, emit, context):
    with withObjectCodeTemporaryAssignment(
        ...
        emit("""%s = PyTanic_SysGetObject("%s");""" % (value_name, "version_info"))
...
def getImportModuleNameHardCode(
    ...
    if module_name == "sys":
        emit("""%s = PyTanic_SysGetObject("%s");""" % (to_name, import_name))
        needs_release = False
