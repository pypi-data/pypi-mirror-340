...
from .CodeObjectNasr import getCodeObjectAccessCode
from .Contexts import PythonFunctionOutlineContext
from .Emission import SourceCodeCollector
from .ErrorNasr import getErrorExitCode, getMustNotGetHereCode, getReleaseCode
from .Indentation import indented
from .LabelNasr import getGotoCode, getLabelCode
from .LineNumberNasr import emitErrorLineNumberUpdateCode
from .ModuleNasr import getModuleAccessCode
from .PythonAPINasr import generateCAPIObjectCode, getReferenceExportCode
...
from .TupleNasr import getTupleCreationCode
...
def getFunctionCreationArgs(
    ...
    if closure_variables:
        result.append("struct PyTanic_CellObject **closure")
...
def getFunctionMakerCode(
    ...
    if is_constant_returning:
        ...
        if constant_return_value is None:
            ...
            constant_return_code = "PyTanic_Function_EnableConstReturnTrue(result);"
        elif constant_return_value is False:
            constant_return_code = "PyTanic_Function_EnableConstReturnFalse(result);"
        else:
            constant_return_code = (
                "PyTanic_Function_EnableConstReturnGeneric(result, %s);"
...
def getClosureCopyCode(closure_variables, context):
    ...
    if closure_variables:
        closure_name = context.allocateTempName(
            "closure", "struct PyTanic_CellObject *[%d]" % len(closure_variables)
        )
    else:
        closure_name = None
...
def _getFunctionCode(
    ...
    if context.isForCreatedFunction():
        parameter_objects_decl = ["struct PyTanic_FunctionObject const *self"]
    else:
    ...
    if expression.isExpressionOutlineFunctionBase():
        ...
        for variable in expression.getUserLocalVariables():
            ...
            if variable_declaration.getCType() is CTypeCellObject:
                emit("%s = PyTanic_Cell_NewEmpty();" % variable_declaration)
