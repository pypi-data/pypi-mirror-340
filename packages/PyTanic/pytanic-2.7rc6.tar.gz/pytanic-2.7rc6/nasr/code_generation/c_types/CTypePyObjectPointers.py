...
""" CType classes for PyObject *, PyObject **, and struct PyTanic_CellObject *

"""

from nasr.__past__ import iterItems, xrange
from nasr.code_generation.ErrorNasr import (
    getErrorExitBoolCode,
    getReleaseCode,
)
from nasr.code_generation.templates.CodeTemplatesVariables import (
...
from nasr.Constants import getConstantValueGuide, isMutable
...
class CPythonPyObjectPtrBase(CTypeBase):
    ...
    @classmethod
    def emitAssignmentCodeToPyTanicIntOrLong(
...
"""
    ...
    @classmethod
    def emitAssignmentCodeToPyTanicBool(
        cls, to_name, value_name, needs_check, emit, context
    ):
...
class CTypePyObjectPtr(CPythonPyObjectPtrBase):
    ...
    def getCellObjectAssignmentCode(cls, target_cell_code, variable_code_name, emit):
        emit("%s = PyTanic_Cell_New0(%s);" % (target_cell_code, variable_code_name))
...
class CTypeCellObject(CTypeBase):
    c_type = "struct PyTanic_CellObject *"
    ...
    def getInitValue(cls, init_from):
        ...
        if init_from is not None:
            return "PyTanic_Cell_New1(%s)" % init_from
        else:
            return "PyTanic_Cell_NewEmpty()"
    ...
    def emitValueAccessCode(cls, value_name, emit, context):
        ...
        return VariableDeclaration(
            "PyObject *", "PyTanic_Cell_GET(%s)" % value_name, None, None
        )

    @classmethod
    def getVariableArgDeclarationCode(cls, variable_code_name):
        return "struct PyTanic_CellObject *%s" % variable_code_name
