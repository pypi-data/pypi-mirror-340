...
""" CType classes for nasr_bool, an enum to represent True, False, unassigned.

"""

from nasr.code_generation.ErrorNasr import getReleaseCode

from .CTypeBases import CTypeBase, CTypeNotReferenceCountedMixin


class CTypePyTanicBoolEnum(CTypeNotReferenceCountedMixin, CTypeBase):
    c_type = "nasr_bool"
    ...
    def emitAssignConversionCode(cls, to_name, value_name, needs_check, emit, context):
        if value_name.c_type == cls.c_type:
            ...
            value_name.getCType().emitAssignmentCodeToPyTanicBool(
                to_name=to_name,
