...
from .BuiltinNasr import getBuiltinCallViaSpecCode
from .CallNasr import (
    getCallCodeKwPairs,
    getCallCodeKwSplit,
    getCallCodeNoArgs,
...
from .ConstantNasr import addDistributionMetadataValue
from .DictNasr import getDictionaryCreationCode
from .ErrorNasr import getErrorExitCode
from .ImportNasr import getImportModuleNameHardCode
from .PythonAPINasr import generateCAPIObjectCode
...
def generatePkgResourcesDistributionValueCode(to_name, expression, emit, context):
    with withObjectCodeTemporaryAssignment(
        ...
        dict_value_names.append(
            'LOOKUP_BUILTIN_STR("__nasr_binary_dir")',
        )
...
def generateImportlibMetadataDistributionValueCode(to_name, expression, emit, context):
    ...
    with withObjectCodeTemporaryAssignment(
        ...
        emit(
            """%(to_name)s = PyTanic_Distribution_New(tstate, %(name)s);"""
            % {
                "to_name": value_name,
