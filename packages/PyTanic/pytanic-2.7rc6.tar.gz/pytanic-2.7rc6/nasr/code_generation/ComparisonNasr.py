...
from nasr.nodes.shapes.StandardShapes import tshape_unknown
from nasr.PythonOperators import (
    comparison_inversions,
    rich_comparison_arg_swaps,
)

from .c_types.CTypeBooleans import CTypeBool
from .c_types.CTypePyTanicBooleans import CTypePyTanicBoolEnum
from .c_types.CTypePyTanicVoids import CTypePyTanicVoidEnum
...
def getRichComparisonCode(
    ...
    if needs_check:
        ...
        # instead can use the CTypePyTanicBoolEnum that will easily convert to
        # it.
        if helper_type is CTypePyTanicVoidEnum:
            helper_type = CTypePyTanicBoolEnum
        ...
        # avoid the CTypePyTanicBoolEnum (NBOOL) helpers except for things that
    ...
    if helper_function is None and not report_missing:
        helper_type, helper_function = selectCodeHelper(
            ...
            specialized_helpers_set=specialized_helpers_set,
            non_specialized_helpers_set=non_specialized_helpers_set,
            result_type=CTypePyTanicBoolEnum,
            left_shape=left_shape,
            right_shape=right_shape,
            left_c_type=left_c_type,
            right_c_type=right_c_type,
...
def generateSubtypeCheckCode(to_name, expression, emit, context):
    ...
    emit(
        "%s = PyTanic_Type_IsSubtype((PyTypeObject *)%s, (PyTypeObject *)%s);"
        % (res_name, left_name, right_name)
    )
