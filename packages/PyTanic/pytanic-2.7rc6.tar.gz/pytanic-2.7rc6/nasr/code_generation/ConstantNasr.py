...
from nasr.ModuleRegistry import getRootTopModule
from nasr.PythonVersions import python_version
from nasr.Serialization import GlobalConstantAccessor
...
from nasr.Version import getPyTanicVersionTuple

from .CodeHelpers import withObjectCodeTemporaryAssignment
from .ErrorNasr import getAssertionCode
...
def getConstantsDefinitionCode():
    """Create the code code "__devils.c" and "__constants.h" files.
    ...
    constant_accessor = GlobalConstantAccessor(
        data_filename="__devils.const", top_level_name="global_constants"
    )
    ...
    if not Settings.shallMakeModule() and not Settings.isStandaloneMode():
    ...
    major, minor, micro, is_final, _rc_number = getPyTanicVersionTuple()
    ...
    body = template_constants_reading % {
        ...
        "nasr_version_major": major,
        "nasr_version_minor": minor,
        "nasr_version_micro": micro,
        "nasr_version_level": "release" if is_final else "candidate",
