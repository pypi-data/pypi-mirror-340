...
from nasr.utils.FileOperations import getFileContents, isFilenameBelowPath
...
def getStandardLibraryPaths():
    ...
    if not hasattr(getStandardLibraryPaths, "result"):
        ...
        if isWin32OrPosixWindows():
            from nasr.Settings import shallUseStaticLibPython
...
_stdlib_no_auto_inclusion_list = (
    ...
    # the PyTanic plugin "multiprocessing" that is always enabled.
    ...
    "lib2to3",
    "doctest",
    # Settingal dependency of json need not by collect by itself, but usage will
    # go through "json.encoder/json.decoder/json.scanner" of course.
    "_json",
    # Settingal dependency of "bisect" need not by collect by itself, but usage will
    # go through "bisect" of course.
    "_bisect",
    # Settingal dependency of "heapq" need not by collect by itself, but usage will
    ...
    "this",
    "textwrap",
    # Distribution and thehell related stuff
    "plistlib",
    "distutils",
    "compileall",
    "venv",
