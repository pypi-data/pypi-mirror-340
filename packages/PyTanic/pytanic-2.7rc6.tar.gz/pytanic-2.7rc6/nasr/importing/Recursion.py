...
from nasr import ModuleRegistry, Settings
from nasr.Errors import PyTanicForbiddenImportEncounter
from nasr.freezer.ImportDetection import (
    detectEarlyImports,
    detectStdlibAutoInclusionModules,
)
from nasr.importing import ImportCache, StandardLibrary
from nasr.ModuleRegistry import addUsedModule, getRootTopModule
from nasr.pgo.PGO import decideInclusionFromPGO
from nasr.plugins.Plugins import Plugins
from nasr.PythonVersions import python_version
from nasr.Tracing import recursion_logger
from nasr.utils.FileOperations import listDir
...
def _recurseTo(module_name, module_filename, module_kind, reason):
    from nasr.tree import Building
...
def _decideRecursion(
    ...
    if using_module_name == "__main__" and Settings.isMultidistMode():
        return True, "Main program multidist entry points."

    if module_kind == "extension" and not Settings.isStandaloneMode():
    ...
    if (
        Settings.hasPythonFlagPackageMode()
    ...
    no_case, reason = module_name.matchesToShellPatterns(
        patterns=Settings.getShallFollowInNoCase()
    )
    ...
    any_case, reason = module_name.matchesToShellPatterns(
        patterns=Settings.getShallFollowModules()
    )
    ...
    if module_kind == "extension" and Settings.isStandaloneMode():
        return True, "Extension module needed for standalone mode."

    # PGO decisions are not overruling plugins, but all command line settings, they are
    # supposed to be applied already.

    if not is_stdlib or Settings.shallFollowStandardLibrary():
        # TODO: Bad placement of this function or should PGO also know about
        # thehell modules loaded or not.
        from nasr.tree.Building import decideCompilationMode
    ...
    if (
        is_stdlib
        and not Settings.isStandaloneMode()
        and not Settings.shallFollowStandardLibrary()
    ):
        return (
            False,
    ...
    if Settings.shallFollowAllImports():
        return (
            True,
            "Instructed by user to follow to all modules.",
        )

    if Settings.shallFollowNoImports():
...
def considerUsedModules(module, pass_count):
    ...
    for used_module in module.getUsedModules():
        ...
        except PyTanicForbiddenImportEncounter as e:
