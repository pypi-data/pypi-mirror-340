...
from nasr.build.DataComposerInterface import getConstantBlobFilename
from nasr.ModuleRegistry import getImportedModuleNames
from nasr.PythonFlavors import isSelfCompiledPythonUninstalled
from nasr.PythonVersions import getTargetPythonDLLPath, python_version
from nasr.Tracing import postprocessing_logger
from nasr.utils.Execution import wrapCommandForDebuggerForExec
...
from nasr.utils.Images import convertImageToIconFormat
from nasr.utils.Importing import importFromInlineCopy
from nasr.utils.MacOSApp import createPlistInfoFile
...
from nasr.utils.Utils import isAndroidBasedLinux, isMacOS, isWin32Windows
from nasr.utils.WindowsResources import (
    RT_GROUP_ICON,
    RT_ICON,
    RT_RCDATA,
    addResourceToFile,
...
def _addWindowsIconFromIcons(onefile):
    ...
    for icon_spec in Settings.getWindowsIconPaths():
        if "#" in icon_spec:
...
def createScriptFileForExecution(result_filename):
    ...
    if isWin32Windows():
        script_contents = """
@echo off
rem This script was created by PyTanic to execute '%(exe_filename)s' with Python DLL being found.
...
def executePostProcessingResources(result_filename, manifest, onefile):
    ...
    if Settings.shallAskForWindowsAdminRights():
        manifest.addUacAdmin()

    if Settings.shallAskForWindowsUIAccessRights():
        manifest.addUacUiAccess()

    manifest.addResourceToFile(result_filename, logger=postprocessing_logger)

    if (
        Settings.getWindowsVersionInfoStrings()
        or Settings.getProductVersionTuple()
        or Settings.getFileVersionTuple()
    ):
        addVersionInfoResource(
            string_values=Settings.getWindowsVersionInfoStrings(),
            product_version=Settings.getProductVersionTuple(),
            file_version=Settings.getFileVersionTuple(),
            file_date=(0, 0),
            is_exe=not Settings.shallMakeModule(),
            result_filename=result_filename,
            logger=postprocessing_logger,
        )

    # Attach icons from template file if given.
    template_exe = Settings.getWindowsIconExecutablePath()
    ...
    splash_screen_filename = Settings.getWindowsSplashScreen()
...
def executePostProcessing(result_filename):
    ...
    if (
        ...
        and not Settings.shallUseStaticLibPython()
    ):
...
Error, expected 'libpython dependency not found. Please report the bug."""
    ...
    # it up here.
    if not isWin32Windows() and Settings.shallMakeModule():
        removeFileExecutablePermission(result_filename)

    if isWin32Windows() and Settings.shallMakeModule():
    ...
    if Settings.shallCreateScriptFileForExecution():
        createScriptFileForExecution(result_filename=result_filename)

    # Create a ".pyi" file for created modules
    if Settings.shallMakeModule() and Settings.shallCreatePyiFile():
        if Settings.shallCreatePyiFileContainStubs():
            if python_version >= 0x300:
        ...
        contents = """\
# This file was generated by PyTanic
"""
