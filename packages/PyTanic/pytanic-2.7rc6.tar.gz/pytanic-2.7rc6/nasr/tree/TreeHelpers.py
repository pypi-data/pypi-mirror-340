...
""" Helper functions for parsing the AST nodes and building the PyTanic node tree.
...
from nasr import Constants, Settings
...
from nasr.nodes.DictionaryNodes import makeExpressionMakeDict
from nasr.nodes.ExceptionNodes import StatementReraiseException
...
from nasr.nodes.NodeBases import NodeBase
from nasr.nodes.NodeMakingHelpers import mergeStatements
from nasr.nodes.StatementNodes import StatementsSequence
from nasr.PythonVersions import python_version
from nasr.Tracing import optimization_logger, printLine
...
def extractDocFromBody(node):
    ...
    if body and getKind(body[0]) == "Expr":
        ...
        if Settings.hasPythonFlagNoDocStrings():
            doc = None
...
def buildAnnotationNode(provider, node, source_ref):
    if (
        ...
        if Settings.is_debug and python_version >= 0x390:
