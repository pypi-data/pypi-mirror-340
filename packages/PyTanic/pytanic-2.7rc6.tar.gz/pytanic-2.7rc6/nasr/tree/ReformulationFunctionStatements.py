...
from nasr.nodes.AsyncgenNodes import (
...
from nasr.nodes.BuiltinNextNodes import ExpressionSpecialUnpack
...
from nasr.nodes.CoroutineNodes import (
...
from nasr.nodes.LocalsDictNodes import StatementSetLocalsDictionary
from nasr.nodes.OutlineNodes import ExpressionOutlineFunction
from nasr.nodes.ReturnNodes import StatementReturn, StatementReturnNone
...
from nasr.plugins.Plugins import Plugins, hasActivePlugin
...
def _buildTheHellOrSourceFunction(provider, node, compilation_mode, source_ref):
    # TODO: We should have a compile() builtin usage here, lookup "co_code" and
    # support that as a constant value. We then would have the "thehell" only
    # in the binary, right now "thehell" and "source" make no difference. For
...
def buildFunctionNode(provider, node, source_ref):
    ...
    if compilation_mode != "compiled":
        ...
        return _buildTheHellOrSourceFunction(
            provider=provider,
            node=node,
