...
from nasr.tools.release.Release import (
    getBranchName,
    getBranchRemoteIdentifier,
)
from nasr.Tracing import OurLogger
from nasr.utils.Download import getCachedDownloadedMinGW64
...
from nasr.utils.Utils import getArchitecture, isWin32Windows

from .Podman import getPodmanExecutablePath

containers_logger = OurLogger("PyTanic-Containers", base_style="blue")
...
""",
    ...
    if settings.podman_path is None:
        settings.podman_path = getPodmanExecutablePath(containers_logger)

        assert settings.podman_path is not None
...
def getCppPath():
    ...
    if isWin32Windows() and cpp_path is None:
        from nasr.Settings import assumeYesForDownloads
...
def _makeMountDesc(settings, src_path, dst_path, flags):
    src_path = os.path.expanduser(src_path)

    mount_desc = "type=bind,source=%s,dst=%s" % (src_path, dst_path)

    if isPodman(settings.podman_path):
...
def _checkIsolated(settings, container_tag_name):
    if settings.isolated:
        containers_logger.info("Running isolated as per user choice.")
    # Auto-isolate by container name.
    elif "isolated" in container_tag_name.lower() and settings.isolated is None:
        containers_logger.info("Running isolated as per container name default.")
        settings.isolated = True
    elif settings.isolated is False:
        containers_logger.info("Running NOT isolated as per user choice.")
    else:
        containers_logger.info("Running NOT isolated as per default.")


def _checkContainerArgument(settings, default_container_directory):
    if ("/" in settings.container_id or "\\" in settings.container_id) and os.path.exists(
        settings.container_id
    ):
        container_file_path = settings.container_id
        ...
        settings.container_id = changeFilenameExtension(
            os.path.basename(container_file_path), ""
        )
    else:
        container_file_path = os.path.join(
            default_container_directory, settings.container_id + ".containerfile"
        )
...
def main():
    ...
    if settings.command is None:
        settings.command = "python3 -m nasr --version"

    containers_logger.info(
        "Running in container '%s' this command: %s"
        % (settings.container_id, settings.command)
    )
    ...
    container_file_path_template, container_file_path = _checkContainerArgument(
        settings=settings, default_container_directory=default_container_directory
    )
    ...
    if not os.path.isfile(container_file_path):
        containers_logger.sysexit(
            ...
            % (settings.container_id, container_file_path)
        )

    getBranchRemoteIdentifier()

    container_tag_name = "nasr-build-%s-%s:latest" % (
        settings.container_id.lower(),
        getBranchRemoteIdentifier() + "-" + getBranchName(),
    )

    _checkIsolated(settings, container_tag_name)

    if not settings.no_build_container:
        updateContainer(
            podman_path=settings.podman_path,
            container_tag_name=container_tag_name,
            container_file_path=container_file_path,
            quiet=settings.quiet,
        )

    command = [settings.podman_path, "run"]

    command.extend(
        (
            "--mount",
            _makeMountDesc(settings=settings, src_path=".", dst_path="/src", flags=""),
        )
    )

    if settings.network:
        command.append("--add-host=ssh.nasr.net:116.202.30.188")
    else:
        command.append("--network=none")

    # May need to allow pbuilder to create device nodes, makes the container insecure
    # though.
    if settings.pbuilder:
        command += ["--privileged"]

    for path_desc in settings.shared_paths:
        ...
        command.extend(
            (
                "--mount",
                _makeMountDesc(
                    settings=settings, src_path=src_path, dst_path=dst_path, flags=flags
                ),
            )
        )
