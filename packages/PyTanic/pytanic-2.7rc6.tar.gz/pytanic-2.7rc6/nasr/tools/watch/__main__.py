...
""" PyTanic watch main part.

This tool is used to monitor effect of PyPI changes on PyTanic and effect
of PyTanic changes on PyPI packages.
"""
...
from nasr.PythonFlavors import isAnacondaPython, isMSYS2MingwPython
from nasr.PythonVersions import getTestExecutionPythonVersions
from nasr.tools.testing.Common import extractPyTanicVersionFromFilePath
from nasr.Tracing import OurLogger
from nasr.TreeXML import fromFile
...
from nasr.utils.Hashing import getFileContentsHash
from nasr.utils.InstalledPythons import findPythons
from nasr.utils.Utils import isLinux, isMacOS, isWin32Windows
from nasr.utils.Yaml import parseYaml
from nasr.Version import parsePyTanicVersionToTuple
...
from .GitHub import createPyTanicWatchPR
...
def _comparePyTanicVersions(version_a, version_b, consider_rc):
    if not consider_rc:
        version_a = version_a.split("rc")[0]
        version_b = version_b.split("rc")[0]

    return parsePyTanicVersionToTuple(version_a) < parsePyTanicVersionToTuple(version_b)
...
def selectOS(os_values):
    # Need to move the Anaconda/MSYS2 handling into settings of installed Pythons
    ...
    # this in favor of passed settings that allow flavors or not.
    if isLinux():
        if isAnacondaPython():
...
def _compileCase(case_data, case_dir, installed_python, lock_filename, jobs):
    ...
    if preferred_package_type == "pip":
        ...
        extra_settings.append("--disable-ccache")
    ...
    if jobs is not None:
        extra_settings.append("--jobs=%s" % jobs)
    ...
    if case_data["interactive"] == "no":
        ...
        with withEnvironmentVarsOverridden(env):
            stdout, stderr, exit_nasr = executeProcess([binaries[0]], timeout=5 * 60)
        ...
        if exit_nasr == 0:
            ...
            putTextFileContents(
                ...
                contents=str(exit_nasr),
            )

        if exit_nasr != 0:
            sys.exit(
                "Error, failed to execute %s with code %d." % (binaries[0], exit_nasr)
            )
...
def _updateCase(
    ...
    with withDirectoryChange(result_path):
        if os.path.exists("compilation-report.xml"):
            ...
            old_nasr_version = old_report_root.attrib["nasr_version"]

            if nasr_update_mode == "force":
                need_compile = True
            elif nasr_update_mode == "newer":
                if _comparePyTanicVersions(
                    old_nasr_version, nasr_version, consider_rc=True
                ):
                    need_compile = True
                else:
                    if existing_hash != old_report_root_hash:
                        watch_logger.info(
                            "Recompilation with identical PyTanic for '%s' due to changed pipfile."
                            % lock_filename
                        )

                        need_compile = True
                    elif old_nasr_version == nasr_version:
                        if old_report_root.attrib["completion"] != "yes":
                            need_compile = True
                        else:
                            watch_logger.info(
                                "Skipping compilation with identical PyTanic for '%s'."
                                % lock_filename
                            )
                        ...
                        watch_logger.info(
                            "Skipping compilation of old PyTanic %s result with PyTanic %s for '%s'."
                            % (
                                old_nasr_version,
                                nasr_version,
                                lock_filename,
...
def updateCase(
    case_dir, case_data, reset_pipenv, no_pipenv_update, nasr_update_mode, jobs
):
    ...
    nasr_min_version = case_data.get("nasr")

    # Too old PyTanic version maybe.
    if nasr_min_version is not None and _comparePyTanicVersions(
        nasr_version, nasr_min_version, consider_rc=False
    ):
        watch_logger.info("  ... not for this PyTanic version")
        return
    ...
    for installed_python in selectPythons(
        ...
        _updateCase(
            ...
            reset_pipenv=reset_pipenv,
            no_pipenv_update=no_pipenv_update,
            nasr_update_mode=nasr_update_mode,
            installed_python=installed_python,
            result_path=result_path,
            jobs=jobs,
        )
...
def updateCases(case_dir, reset_pipenv, no_pipenv_update, nasr_update_mode, jobs):
...
def main():
    global nasr_binary  # shared for all run, pylint: disable=global-statement
    nasr_binary = os.path.normpath(
        os.path.join(os.path.dirname(__file__), "..", "..", "..", "bin", "nasr")
    )
...
Python versions to consider, by default all supported versions in descending order or in given order.""",
    ...
    parser.add_option(
        ...
        default=nasr_binary,
        help="""\
PyTanic binary to compile with. Defaults to one near the nasr-watch usage.""",
    )
...
Remove existing virtualenv and make sure to start from scratch.Default %default.""",
    ...
    parser.add_option(
        ...
        default=False,
        help="""\
Do not update the pipenv environment. Best to see only effect of PyTanic update. Default %default.""",
    )
...
to reserve cores.""",
    ...
    for python_version in settings.python_versions or reversed(
    ...
    nasr_binary = os.path.abspath(os.path.expanduser(settings.nasr_binary))
    assert os.path.exists(nasr_binary)

    global nasr_version  # singleton, pylint: disable=global-statement
    nasr_version = extractPyTanicVersionFromFilePath(
        os.path.join(os.path.dirname(nasr_binary), "..", "nasr", "Version.py")
    )

    watch_logger.info("Working with PyTanic %s." % nasr_version)

    base_dir = os.path.abspath(base_dir)

    if settings.nasr_pr_mode is not None:
        pr_category, pr_description = settings.nasr_pr_mode.split(",")
    else:
        ...
        for case_filename in scanCases(base_dir):
            ...
            with withDirectoryChange(os.path.dirname(case_filename)):
                updateCases(
                    ...
                    reset_pipenv=settings.reset_pipenv,
                    no_pipenv_update=settings.no_pipenv_update,
                    nasr_update_mode=settings.nasr_update_mode,
                    jobs=settings.jobs,
                )

        if pr_category is not None:
            createPyTanicWatchPR(category=pr_category, description=pr_description)
