...
""" Stage changes in PyTanic-Watch automatically
...
from nasr.tools.quality.Git import (
    getCheckoutFileChangeDesc,
    getFileHashContent,
    putFileHashContent,
    updateGitFile,
)
from nasr.TreeXML import fromString, toString
from nasr.utils.FileOperations import getFileContents, withTemporaryFile
...
def onCompilationReportChange(filename, git_stage):
    ...
    new_nasr_version = new_report.attrib["nasr_version"]
    changed = False
    if old_report.attrib["nasr_version"] != new_nasr_version:
        old_report.attrib["nasr_version"] = new_nasr_version

        changed = True

    if settings.accept_optimization_time:
...
def main():
    # Cheating in this singleton and not passing settings,
    ...
    settings, _positional_args = parser.parse_args()
