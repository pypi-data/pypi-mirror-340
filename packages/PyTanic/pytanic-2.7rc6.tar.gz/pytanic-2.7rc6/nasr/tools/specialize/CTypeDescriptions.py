...
from nasr.__past__ import long
from nasr.code_generation.BinaryOperationHelperDefinitions import (
    isCommutativeType,
)
from nasr.code_generation.Namify import namifyConstant
...
class TypeDescBase(getMetaClassBase("Type", require_slots=False)):
    ...
    def getIndexCheckExpression(self, operand):
        if self.hasSlot("nb_index"):
            ...
            return "PyTanic_Index_Check(%s)" % operand
        else:
            return "0"
    ...
    def getTypeSubTypeCheckExpression(self, other, type2, type1):
        return "PyTanic_Type_IsSubtype(%s, %s)" % (
...
return %(return_value)s;""" % {
    ...
    def getAssignFromLongExpressionCode(cls, result, operand):
        if cls.type_name == "object":
            ...
            return "%s = PyTanic_PyInt_FromLong(%s);" % (result, operand)
        elif cls.type_name == "long":
            return "%s = PyTanic_LongFromCLong(%s);" % (result, operand)
        elif cls.type_name == "nbool":
            return "%s = %s;" % (
                result,
    ...
    def getAssignFromLongConstantCode(cls, result, value):
        if cls.type_name in ("object", "long"):
            ...
            const_name3 = "PyTanic_Long_GetSmallValue(%s)" % value
...
class LongDesc(ConcreteNonSequenceTypeBase):
    ...
    def getLongValueSizeExpression(operand):
        return "PyTanic_LongGetSignedDigitSize(%s_long_object)" % operand

    @staticmethod
    def getLongValueIsNegativeTestExpression(operand):
        return "PyTanic_LongIsNegative(%s_long_object)" % operand

    @staticmethod
    def getLongValueDigitCountExpression(operand):
        return "PyTanic_LongGetDigitSize(%s_long_object)" % operand

    @staticmethod
    def getLongValueDigitExpression(operand, index):
        return "PyTanic_LongGetDigitPointer(%s_long_object)[%s]" % (operand, index)

    @staticmethod
    def getLongValueDigitsPointerExpression(operand):
        return "PyTanic_LongGetDigitPointer(%s_long_object)" % operand
...
class CLongDesc(ConcreteCTypeBase):
    ...
    @staticmethod
    def getAsObjectValueExpression(operand):
        return "PyTanic_PyLong_FromLong(%s)" % operand
...
class NBoolDesc(ConcreteCTypeBase):
    type_name = "nbool"
    type_desc = "PyTanic C bool value"
    type_decl = "nasr_bool"
    ...
    def getToValueFromObjectExpression(cls, operand):
        # TODO: Seems wrong, int return values only happen to match nasr_bool here
...
class NILongDesc(DualTypeBase):
    type_name = "nilong"
    type_desc = "PyTanic int/long/C long value"
    type_decl = "nasr_ilong *"
...
class NVoidDesc(ConcreteCTypeBase):
    type_name = "nvoid"
    type_desc = "PyTanic C void value"
    type_decl = "nasr_void"
