...
import nasr.Settings

nasr.Settings.is_full_compat = False

# isort:start

import os

import nasr.specs.BuiltinBytesOperationSpecs
import nasr.specs.BuiltinDictOperationSpecs
import nasr.specs.BuiltinListOperationSpecs
import nasr.specs.BuiltinStrOperationSpecs
import nasr.specs.BuiltinUnicodeOperationSpecs
from nasr.code_generation.BinaryOperationHelperDefinitions import (
    getSpecializedBinaryOperations,
    isCommutativeOperation,
    parseTypesFromHelper,
)
from nasr.code_generation.c_types.CTypePyObjectPointers import (
    make_list_constant_direct_threshold,
    make_list_constant_hinted_threshold,
)
from nasr.code_generation.CallNasr import (
    getQuickCallCode,
...
from nasr.code_generation.ComparisonHelperDefinitions import (
    getSpecializedComparisonOperations,
)
from nasr.code_generation.ImportNasr import getImportModuleHardCodeName
from nasr.containers.OrderedSets import OrderedSet
from nasr.HardImportRegistry import (
    hard_modules,
    hard_modules_non_stdlib,
    hard_modules_version,
)
from nasr.nodes.shapes.BuiltinTypeShapes import (
    tshape_bool,
    tshape_bytes,
    tshape_dict,
    tshape_int,
...
from nasr.utils.Jinja2 import getTemplateC
...
def getDoExtensionUsingTemplateC(template_name):
    return getTemplateC(
        package_name="nasr.code_generation",
...
def makeHelpersComparisonOperation(operand, op_code):
    ...
    filename_c = "nasr/build/static_src/HelpersComparison%s.c" % op_code.capitalize()
    filename_h = "nasr/build/include/nasr/helper/comparisons_%s.h" % op_code.lower()
...
def makeHelpersComparisonDualOperation(operand, op_code):
    ...
    filename_c = (
        "nasr/build/static_src/HelpersComparisonDual%s.c" % op_code.capitalize()
    )
    filename_h = (
        "nasr/build/include/nasr/helper/comparisons_dual_%s.h" % op_code.lower()
    )
...
def makeHelpersBinaryOperation(operator, op_code):
    ...
    filename_c = (
        "nasr/build/static_src/HelpersOperationBinary%s.c" % op_code.capitalize()
    )
    filename_h = (
        "nasr/build/include/nasr/helper/operations_binary_%s.h" % op_code.lower()
    )
...
def makeHelpersInplaceOperation(operator, op_code):
    ...
    filename_c = (
        "nasr/build/static_src/HelpersOperationInplace%s.c" % op_code.capitalize()
    )
    filename_h = (
        "nasr/build/include/nasr/helper/operations_inplace_%s.h" % op_code.lower()
    )
...
def makeHelpersBinaryDualOperation(operand, op_code):
    ...
    filename_c = (
        "nasr/build/static_src/HelpersOperationBinaryDual%s.c" % op_code.capitalize()
    )
    filename_h = (
        "nasr/build/include/nasr/helper/operations_binary_dual_%s.h"
        % op_code.lower()
    )
...
def makeHelpersImportHard():
    filename_c = "nasr/build/static_src/HelpersImportHard.c"
    filename_h = "nasr/build/include/nasr/helper/import_hard.h"
...
def makeHelperCalls():
    filename_c = "nasr/build/static_src/HelpersCallingGenerated.c"
    filename_h = "nasr/build/include/nasr/helper/calling_generated.h"
    ...
    with withFileOpenedAndAutoFormatted(filename_c) as output_c:
        with withFileOpenedAndAutoFormatted(filename_h) as output_h:
            ...
            template = getTemplateC(
                "nasr.code_generation", "CodeTemplateCallsPositional.c.j2"
            )
            ...
            template = getTemplateC(
                "nasr.code_generation", "CodeTemplateCallsMixed.c.j2"
            )
...
def makeHelperLists():
    filename_c = "nasr/build/static_src/HelpersListsGenerated.c"
    filename_h = "nasr/build/include/nasr/helper/lists_generated.h"
    ...
    with withFileOpenedAndAutoFormatted(filename_c) as output_c:
        with withFileOpenedAndAutoFormatted(filename_h) as output_h:
            ...
            template = getTemplateC(
                "nasr.code_generation", "CodeTemplateMakeListSmall.c.j2"
            )
            ...
            template = getTemplateC(
                "nasr.code_generation", "CodeTemplateMakeListHinted.c.j2"
            )
...
generate_builtin_type_operations = [
    ...
    # PyTanic-Python eventually.
    (
        "tshape_dict",
        dict_desc,
        nasr.specs.BuiltinDictOperationSpecs,
        ("pop", "popitem", "setdefault"),
    ),
    (
        "tshape_list",
        list_desc,
        nasr.specs.BuiltinListOperationSpecs,
        (
            "pop",
    ...
    # that we do not have easy access to, but we might one day for PyTanic-Python
    # expose it for the static linking of it and then we could in fact call
    # these directly.
    (
        "tshape_str",
        str_desc,
        nasr.specs.BuiltinStrOperationSpecs,
        (
            "strip",
            "rstrip",
            "lstrip",
    ...
    (
        "tshape_unicode",
        unicode_desc,
        nasr.specs.BuiltinUnicodeOperationSpecs,
        (
            "strip",
            "rstrip",
            "lstrip",
    ...
    (
        "tshape_bytes",
        bytes_desc,
        nasr.specs.BuiltinBytesOperationSpecs,
        (
            "capitalize",
            "center",
            "count",
...
def makeDictCopyHelperNasr():
    filename_c = "nasr/build/static_src/HelpersDictionariesGenerated.c"
...
def _getCheckForShape(shape):
    ...
    if shape is tshape_str:
        return "PyTanic_String_CheckExact"
...
def makeHelperBuiltinTypeMethods():
    ...
    filename_c = "nasr/build/static_src/HelpersBuiltinTypeMethods.c"
    filename_h = "nasr/build/include/nasr/helper/operations_builtin_types.h"
    with withFileOpenedAndAutoFormatted(filename_c) as output_c:
        with withFileOpenedAndAutoFormatted(filename_h) as output_h:
