...
import nasr.code_generation.BinaryOperationHelperDefinitions
import nasr.code_generation.CodeGeneration
import nasr.code_generation.ComparisonNasr
import nasr.code_generation.Namify
import nasr.nodes.NetworkxNodes
import nasr.nodes.PackageMetadataNodes
import nasr.nodes.PackageResourceNodes
import nasr.nodes.SideEffectNodes
import nasr.nodes.TensorflowNodes
...
import nasr.specs.BuiltinTypeOperationSpecs
import nasr.specs.HardImportSpecs
import nasr.tree.Building
from nasr.containers.OrderedDicts import OrderedDict
from nasr.containers.OrderedSets import OrderedSet
from nasr.nodes.ImportNodes import hard_modules_non_stdlib
from nasr.nodes.NodeMetaClasses import NodeCheckMetaClass
...
from nasr.utils.Jinja2 import getTemplate
...
def _getMixinForShape(shape):
    ...
    if shape is tshape_str:
        return "nasr.nodes.ExpressionShapeMixins.ExpressionStrShapeExactMixin"
    elif shape is tshape_list:
        return "nasr.nodes.ExpressionShapeMixins.ExpressionListShapeExactMixin"
    elif shape is tshape_tuple:
        return "nasr.nodes.ExpressionShapeMixins.ExpressionTupleShapeExactMixin"
    elif shape is tshape_int:
        return "nasr.nodes.ExpressionShapeMixins.ExpressionIntShapeExactMixin"
    elif shape is tshape_bool:
        return "nasr.nodes.ExpressionShapeMixins.ExpressionBoolShapeExactMixin"
    elif shape is tshape_none:
        return "nasr.nodes.ExpressionShapeMixins.ExpressionNoneShapeExactMixin"
    elif shape is tshape_dict:
        return "nasr.nodes.ExpressionShapeMixins.ExpressionDictShapeExactMixin"
    elif shape is tshape_bytes:
        return "nasr.nodes.ExpressionShapeMixins.ExpressionBytesShapeExactMixin"
    else:
        assert False, shape
...
processTypeShapeAttribute(
    "tshape_type",
    nasr.specs.BuiltinTypeOperationSpecs,
    python2_type_methods,
    python3_type_methods,
)
...
def emitGenerationWarning(emit, doc_string, template_name):
    ...
    for spec_descriptions in getSpecVersions(nasr.specs.HardImportSpecs):
...
def makeAttributeNodes():
    filename_python = "nasr/nodes/AttributeNodesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit("from nasr.specs.BuiltinParameterSpecs import extractBuiltinArgs")

        emit("from nasr.nodes.ConstantRefNodes import makeConstantRefNode")
        emit(
            """\
from nasr.nodes.NodeMakingHelpers import (
    ...
    makeRaiseExceptionReplacementExpression
        ...
        emit(
            "from nasr.nodes.KeyValuePairNodes import makeKeyValuePairExpressionsFromKwArgs"
        )

        emit("from nasr.nodes.AttributeNodes import makeExpressionAttributeLookup")

        emit("from nasr.PythonVersions import getDictFromkeysNoArgErrorMessage")
...
def makeBuiltinOperationNodes():
    filename_python = "nasr/nodes/BuiltinOperationNodeBasesGenerated.py"
...
def makeChildrenHavingMixinNodes():
    ...
    filename_python = "nasr/nodes/ChildrenHavingMixins.py"
    filename_python2 = "nasr/nodes/ExpressionBasesGenerated.py"
    filename_python3 = "nasr/nodes/StatementBasesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit1(
            """
from nasr.nodes.Checkers import (
    checkStatementsSequenceOrNone,
...
def makeHardImportNodes():
    ...
    filename_python = "nasr/nodes/HardImportNodesGenerated.py"
