...
""" Main front-end to the tests of PyTanic.

Has many settings, read --help output.
"""
...
from nasr.PythonVersions import getTestExecutionPythonVersions
from nasr.tools.Basics import goHome
from nasr.tools.testing.Common import (
    getInstalledPythonVersion,
    getTempDir,
    my_print,
    test_logger,
    withExtendedExtraSettings,
)
from nasr.utils.Execution import check_call, check_output, getNullOutput
...
from nasr.utils.Timing import TimerReport
from nasr.utils.Utils import hasOnefileSupportedOS, hasStandaloneSupportedOS


def parseSettings():
    # There are freaking many settings to honor,
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The basic tests, execute these to check if PyTanic is healthy.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The syntax tests, execute these to check if PyTanic handles Syntax errors fine.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The programs tests, execute these to check if PyTanic handles programs, e.g.
import recursions, etc. fine. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The packages tests, execute these to check if PyTanic handles packages, e.g.
import recursions, etc. fine. Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The plugins tests, execute these to check if PyTanic handles its own plugin
interfaces, e.g. user plugins, etc. fine. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The optimization tests, execute these to check if PyTanic does optimize certain
constructs fully away. Default is %default.""",
    ...
    parser.add_option(
        ...
        help="""\
The standalone tests, execute these to check if PyTanic standalone mode, e.g.
...
Default is %default.""",
    ...
    parser.add_option(
        ...
        help="""\
The onefile tests, execute these to check if PyTanic works in onefile mode, e.g.
not referring to outside, important 3rd library packages like PyQt fine.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The reflection test compiles PyTanic with PyTanic, and then PyTanic with the
compile PyTanic and compares the outputs. Default is %default.""",
    )
...
Make a coverage analysis, that does not really check. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=False,
        help="""\
Allow PyTanic to download code if necessary, e.g. dependency walker on Windows. Default is %default.""",
    )
...
Enforce the use of MinGW64 on Windows. Defaults to off.""",
    ...
    parser.add_option(
        ...
        default=None,
        help="""\
The value of --jobs to use when calling PyTanic. Use negative values to
keep cores unused. PyTanic uses available cores by default. Defaults to
not being passed.""",
    )
    ...
    if settings.cpython_no_other:
        if sys.version_info[0:2] != (2, 6):
            settings.cpython26 = False
        if sys.version_info[0:2] != (2, 7):
            settings.cpython27 = False
        if sys.version_info[0:2] != (3, 2):
            settings.cpython32 = False
        if sys.version_info[0:2] != (3, 4):
            settings.cpython34 = False
        if sys.version_info[0:2] != (3, 5):
            settings.cpython35 = False
        if sys.version_info[0:2] != (3, 6):
            settings.cpython36 = False
        if sys.version_info[0:2] != (3, 7):
            settings.cpython37 = False
        if sys.version_info[0:2] != (3, 8):
            settings.cpython38 = False
        if sys.version_info[0:2] != (3, 9):
            settings.cpython39 = False
        if sys.version_info[0:2] != (3, 10):
            settings.cpython310 = False
        if sys.version_info[0:2] != (3, 11):
            settings.cpython311 = False
        if sys.version_info[0:2] != (3, 12):
            settings.cpython312 = False
        if sys.version_info[0:2] != (3, 13):
            settings.cpython313 = False

    if settings.cpython_none:
        settings.cpython26 = False
        settings.cpython27 = False
        settings.cpython32 = False
        settings.cpython34 = False
    ...
    if settings.coverage and os.path.exists(".coverage"):
        os.unlink(".coverage")
...
def publishCoverageData():
    ...
    with openTextFile("data.coverage", "w") as data_file:
        ...
        with withDirectoryChange(source_dir):
            nasr_id = check_output("git rev-parse HEAD".split())
...
def main():
    ...
    def checkExecutableCommand(command):
        ...
        # Do respect given settings to disable specific Python versions
        if command == "python2.6" and settings.no26:
            return False
        if command == "python2.7" and settings.no27:
            return False
        if command == "python3.4" and settings.no34:
            return False
        if command == "python3.5" and settings.no35:
            return False
        if command == "python3.6" and settings.no36:
            return False
        if command == "python3.7" and settings.no37:
            return False
        if command == "python3.8" and settings.no38:
            return False
        if command == "python3.9" and settings.no39:
            return False
        if command == "python3.10" and settings.no310:
            return False
        if command == "python3.11" and settings.no311:
            return False
        if command == "python3.12" and settings.no312:
            return False
        if command == "python3.13" and settings.no313:
            return False
    ...
    def getExtraFlags(where, name, flags):
        if settings.assume_yes_for_downloads and name in (
            "onefile",
            "standalone",
            "plugins",
        ):
        ...
        if os.name == "nt" and settings.mingw64:
            yield "--mingw64"

        if settings.jobs is not None:
            yield "--jobs=%s" % settings.jobs
    ...
    def _executeSubTest(command, hide_output):
        if settings.coverage and "search" in command:
    ...
    def execute_tests(where, use_python, flags):
        ...
        if settings.basic_tests:
            my_print(
                "Running the basic tests with settings '%s' with '%s':"
                % (flags, use_python)
            )
            with withExtendedExtraSettings(*getExtraFlags(where, "basics", flags)):
                executeSubTest("./tests/basics/run_all.py search")

        if settings.syntax_tests:
            my_print(
                "Running the syntax tests with settings '%s' with '%s':"
                % (flags, use_python)
            )
            with withExtendedExtraSettings(*getExtraFlags(where, "syntax", flags)):
                executeSubTest("./tests/syntax/run_all.py search")

        if settings.program_tests:
            my_print(
                "Running the program tests with settings '%s' with '%s':"
                % (flags, use_python)
            )
            with withExtendedExtraSettings(*getExtraFlags(where, "programs", flags)):
                executeSubTest("./tests/programs/run_all.py search")

        if settings.package_tests:
            my_print(
                "Running the package tests with settings '%s' with '%s':"
                % (flags, use_python)
            )
            with withExtendedExtraSettings(*getExtraFlags(where, "packages", flags)):
                executeSubTest("./tests/packages/run_all.py search")

        if settings.plugin_tests:
            my_print(
                "Running the plugin tests with settings '%s' with '%s':"
                % (flags, use_python)
            )
            with withExtendedExtraSettings(*getExtraFlags(where, "plugins", flags)):
        ...
        if use_python != "python2.6":
            if settings.optimization_tests:
                my_print(
                    "Running the optimizations tests with settings '%s' with '%s':"
                    % (flags, use_python)
                )
                with withExtendedExtraSettings(
                    *getExtraFlags(where, "optimizations", flags)
                ):
                    executeSubTest("./tests/optimizations/run_all.py search")

        if settings.standalone_tests and not settings.coverage:
            my_print(
                "Running the standalone tests with settings '%s' with '%s':"
                % (flags, use_python)
            )
            with withExtendedExtraSettings(*getExtraFlags(None, "standalone", flags)):
                executeSubTest("./tests/standalone/run_all.py search")

        if settings.onefile_tests and not settings.coverage:
            if hasOnefileSupportedOS():
                my_print(
                    "Running the onefile tests with settings '%s' with '%s':"
                    % (flags, use_python)
                )
                with withExtendedExtraSettings(*getExtraFlags(None, "onefile", flags)):
                    executeSubTest("./tests/onefile/run_all.py search")
            else:
                my_print("The onefile tests are not run due to missing requirements.")

        if settings.reflection_test and not settings.coverage:
            my_print(
                "Running the reflection test with settings '%s' with '%s':"
                % (flags, use_python)
            )
            with withExtendedExtraSettings(*getExtraFlags(None, "reflected", flags)):
        ...
        if not use_python.startswith("python3"):
            if os.path.exists("./tests/CPython26/run_all.py"):
                if settings.cpython26:
                    my_print(
                        "Running the CPython 2.6 tests with settings '%s' with '%s':"
                        % (flags, use_python)
                    )
            ...
            if use_python != "python2.6":
                if os.path.exists("./tests/CPython27/run_all.py"):
                    if settings.cpython27:
                        my_print(
                            "Running the CPython 2.7 tests with settings '%s' with '%s':"
                            % (flags, use_python)
                        )
                        with withExtendedExtraSettings(
        ...
        if "--debug" not in flags:
            ...
            if use_python not in ("python2.6", "python2.7") or not settings.coverage:
                if settings.cpython32:
