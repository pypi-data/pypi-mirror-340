...
""" Tool to compare reference counting behavior of CPython and PyTanic.
...
from nasr.PythonVersions import isDebugPython
from nasr.tools.testing.Common import checkReferenceCount, getTempDir
...
Try to explain the differences by comparing object counts.""",
    ...
    if positional_args and settings.checked_module is None:
        settings.checked_module = positional_args.pop()

    if settings.checked_module is None:
        sys.exit("\nNeed to provide checked module filename.")

    if positional_args and settings.checked_module:
        parser.print_help()

        sys.exit("\nError, no positional argument allowed.")

    # First with pure Python.
    checked_module = importFileAsModule(settings.checked_module)
    my_print("Using %s" % checked_module.main, style="blue")
    checkReferenceCount(checked_module.main, explain=settings.explain)
    ...
    module_name = os.path.basename(settings.checked_module).split(".")[0]
