...
""" Tool to compare output of CPython and PyTanic.
...
from nasr.SettingParsing import getPyTanicProjectSettings
...
from nasr.tools.testing.OutputComparison import compareOutput
from nasr.Tracing import my_print
...
from nasr.utils.FileOperations import deleteFile
from nasr.utils.Importing import getExtensionModuleSuffix
from nasr.utils.Timing import StopWatch
from nasr.utils.Utils import isMacOS
...
def main():
    ...
    project_settings = tuple(
        getPyTanicProjectSettings(
            logger=test_logger, filename_arg=filename, module_mode=module_mode
        )
    )

    if "--standalone" in project_settings or "--mode=standalone" in project_settings:
        standalone_mode = True
    if "--onefile" in project_settings or "--mode=onefile" in project_settings:
        standalone_mode = True
        onefile_mode = True
    if "--mode=app" in project_settings:
        if isMacOS():
            standalone_mode = True
    ...
    extra_settings = os.getenv("DEVILPY_EXTRA_OPTIONS", "").split()

    if os.path.normcase(os.environ["PYTHON"]).endswith(
        ("-dbg", "-debug", "d", "_d.exe")
    ):
        python_debug = True
    elif "--python-debug" in extra_settings or "--python-dbg" in extra_settings:
        python_debug = True
...
Taking coverage of '{filename}' using '{python}' with flags {args} ...""".format(
    ...
    if "DEVILPY" in os.environ:
        ...
        nasr_call = [os.environ["PYTHON"], os.environ["DEVILPY"]]
    else:
        if comparison_mode:
            nasr_call = [
                os.environ["PYTHON"],
                "-m",
                "nasr.__main__",  # Note: Needed for Python2.6
            ]
        else:
    ...
    if python_debug:
        extra_settings.append("--python-debug")

    if no_warnings:
        extra_settings.append("--python-flag=no_warnings")

    if remove_output:
        extra_settings.append("--remove-output")

    if original_file:
        extra_settings.append("--file-reference-choice=original")

    if runtime_file:
        extra_settings.append("--file-reference-choice=runtime")

    if full_compat:
        extra_settings.append("--full-compat")

    if no_prefer_source:
        extra_settings.append("--no-prefer-source")

    if python_flag_m:
        extra_settings.append("--python-flag=-m")

    if coverage_mode:
        # Coverage modules hates PyTanic to re-execute, and so we must avoid
        # that.
    ...
    if report:
        extra_settings.append("--report=%s" % report)
        deleteFile(report, must_exist=False)

    if nofollow_imports or (not follow_imports and not standalone_mode):
        extra_settings.append("--nofollow-imports")

    if follow_imports:
        extra_settings.append("--follow-imports")

    if nowarn_mnemonics:
        extra_settings.extend("--nowarn-mnemonic=" + v for v in nowarn_mnemonics)

    if coverage_mode:
        extra_settings.append("--must-not-re-execute")
        extra_settings.append("--generate-c-only")

    for plugin_enabled in plugins_enabled:
        extra_settings.append("--enable-plugin=" + plugin_enabled)

    for plugin_disabled in plugins_disabled:
        extra_settings.append("--disable-plugin=" + plugin_disabled)

    for user_plugin in user_plugins:
        extra_settings.append("--user-plugin=" + user_plugin)

    if not no_verbose_log:
        extra_settings.append("--verbose-output=%s.optimization.log" % filename)

    if not no_inclusion_log:
        extra_settings.append("--show-modules-output=%s.inclusion.log" % filename)

    if output_dir is not None:
        extra_settings.append("--output-dir=%s" % output_dir)
    else:
        # TODO: The run-tests uses DEVILPY_EXTRA_OPTIONS still.
        for extra_option in extra_settings:
    ...
    for include_package in include_packages:
        extra_settings.append("--include-package=%s" % include_package)

    for include_module in include_modules:
        extra_settings.append("--include-module=%s" % include_module)

    # Progress bar is not used.
    extra_settings.append("--no-progressbar")

    # Now build the command to run PyTanic.
    if not two_step_execution:
        if package_mode:
            extra_settings.append("--mode=package")
        elif module_mode:
            extra_settings.append("--mode=module")
        elif onefile_mode:
            extra_settings.append("--mode=onefile")
        elif app_bundle_mode:
            extra_settings.append("--mode=app")
        elif standalone_mode:
            extra_settings.append("--mode=standalone")

        nasr_cmd = nasr_call + extra_settings + ["--run", filename]

        if no_site:
            nasr_cmd.insert(len(nasr_cmd) - 1, "--python-flag=-S")
        ...
        if package_mode:
            ...
            nasr_cmd1 = nasr_call + extra_settings + ["--mode=onefile", filename]
        elif standalone_mode:
            nasr_cmd1 = nasr_call + extra_settings + ["--mode=standalone", filename]
        else:
            nasr_cmd1 = nasr_call + extra_settings + [filename]

        if no_site:
            nasr_cmd1.insert(len(nasr_cmd1) - 1, "--python-flag=-S")
    ...
    if module_mode:
        ...
        nasr_cmd2 = [os.environ["PYTHON"], "-W", "ignore", "-c", mini_script]
    else:
        ...
        nasr_cmd2 = [os.path.join(output_dir, exe_filename)]
    ...
    if not two_step_execution:
        if trace_command:
            traceExecutedCommand("PyTanic command", nasr_cmd)

        # Try a couple of times for permission denied, on Windows it can
        # be transient.
        for _i in range(5):
            with withPythonPathChange(nasr_package_dir):
                process = createProcess(command=nasr_cmd)

            stdout_nasr, stderr_nasr = process.communicate()
            exit_nasr = process.returncode

            if checkNoPermissionError(stdout_nasr) and checkNoPermissionError(
                stderr_nasr
            ):
                break

            my_print("Retrying nasr exe due to permission problems after delay.")
            time.sleep(2)

    else:
        if trace_command:
            my_print("PyTanic command 1:", nasr_cmd1)

        for _i in range(5):
            with withPythonPathChange(nasr_package_dir):
                stdout_nasr1, stderr_nasr1, exit_nasr1 = executeProcess(
                    nasr_cmd1
                )
...
%s
"""
                        % (nasr_cmd1, python_path_used, stderr_nasr1)
                    )

                exit_nasr = exit_nasr1
                stdout_nasr, stderr_nasr = stdout_nasr1, stderr_nasr1
                stdout_nasr2 = b"not run due to compilation error:\n" + stdout_nasr1
                stderr_nasr2 = stderr_nasr1
            else:
                # No execution second step for coverage mode.
                if comparison_mode:
                    if os.path.exists(nasr_cmd2[0][:-4] + ".cmd"):
                        nasr_cmd2[0] = nasr_cmd2[0][:-4] + ".cmd"

                    if trace_command:
                        my_print("PyTanic command 2:", nasr_cmd2)

                    # Need full manual control
                    process = createProcess(command=nasr_cmd2, new_group=send_kill)
                    ...
                    if send_kill:
                        ...
                        executeAfterTimePassed(
                            ...
                            func=lambda: killProcessGroup(
                                "PyTanic compiled program", process.pid
                            ),
                        )

                    stdout_nasr2, stderr_nasr2 = process.communicate()
                    stdout_nasr = stdout_nasr1 + stdout_nasr2
                    stderr_nasr = stderr_nasr1 + stderr_nasr2
                    exit_nasr = process.returncode

                    # In case of segfault or assertion triggered, run in debugger.
                    if exit_nasr in (-11, -6) and sys.platform != "nt":
                        nasr_cmd2 = wrapCommandForDebuggerForSubprocess(
                            command=nasr_cmd2
                        )

                        callProcess(nasr_cmd2, shell=False)
                else:
                    exit_nasr = exit_nasr1
                    stdout_nasr, stderr_nasr = stdout_nasr1, stderr_nasr1
    ...
    stop_watch.stop()
    nasr_time = stop_watch.getDelta()

    if "--encrypt-stderr" in project_settings:
        with withPythonPathChange(nasr_package_dir):
            stderr_nasr = decryptOutput(project_settings, stderr_nasr)

    if "--encrypt-stdout" in project_settings:
        with withPythonPathChange(nasr_package_dir):
            stdout_nasr = decryptOutput(project_settings, stdout_nasr)

    if not silent_mode:
        displayOutput(stdout_nasr, stderr_nasr)

        if coverage_mode:
            assert not stdout_nasr
            assert not stderr_nasr
    ...
    if comparison_mode:
        ...
        def makeComparisons(trace_result):
            exit_code_stdout = compareOutput(
                "stdout",
                stdout_cpython,
                stdout_nasr2 if two_step_execution else stdout_nasr,
                ignore_warnings,
                syntax_errors,
            )
            ...
            if ignore_stderr:
                ...
                exit_code_stderr = compareOutput(
                    "stderr",
                    stderr_cpython,
                    stderr_nasr2 if two_step_execution else stderr_nasr,
                    ignore_warnings,
                    syntax_errors,
                )

            exit_code_return = exit_cpython != exit_nasr

            if exit_code_return and trace_result:
                my_print(
                    """Exit codes {exit_cpython:d} (CPython) != {exit_nasr:d} (PyTanic)""".format(
                        exit_cpython=exit_cpython, exit_nasr=exit_nasr
                    )
                )
        ...
        if (
            exit_code_return
            and exit_nasr in (-11, -6)
            and sys.platform != "nt"
            and not module_mode
            and not two_step_execution
        ):
            nasr_cmd.insert(len(nasr_cmd) - 1, "--debugger")

            with withPythonPathChange(nasr_package_dir):
                callProcess(nasr_cmd, shell=False)
    ...
    if remove_output:
        if not module_mode:
            if os.path.exists(nasr_cmd2[0]) and remove_binary:
                if os.name == "nt":
                    ...
                    if os.path.exists(nasr_cmd2[0] + ".away"):
                        os.unlink(nasr_cmd2[0] + ".away")

                    for _i in range(10):
                        try:
                            os.rename(nasr_cmd2[0], nasr_cmd2[0] + ".away")
                        except OSError:
                            time.sleep(0.1)
                            continue
    ...
    if comparison_mode and timing:
        my_print("CPython took %.2fs vs %0.2fs PyTanic." % (cpython_time, nasr_time))

    if comparison_mode and not silent_mode:
        my_print("OK, same outputs.")


nasr_package_dir = os.path.normpath(
...
if __name__ == "__main__":
    ...
    sys.path.insert(0, nasr_package_dir)
