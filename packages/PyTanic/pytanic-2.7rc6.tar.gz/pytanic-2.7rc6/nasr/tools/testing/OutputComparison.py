...
logging_info_re = re.compile(r"^PyTanic(-\w+)?:([-\w]+:)? ")
logging_warning_re = re.compile(r"^PyTanic.*?:WARNING")

# Python3.11 style traceback carets are not done by PyTanic (yet?)
...
def makeDiffable(output, ignore_warnings, syntax_errors):
    ...
    for line in lines:
        ...
        if line.startswith("PyTanic-Inclusion:WARNING: Cannot follow import to module"):
            continue
        if line.startswith("PyTanic:WARNING: Cannot detect Linux distribution"):
            continue
        if line.startswith(
            "PyTanic-Settings:WARNING: You did not specify to follow or include"
        ):
            continue
        if line.startswith("PyTanic:WARNING: Using very slow fallback for ordered sets"):
            continue
        if line.startswith("PyTanic:WARNING: On Windows, support for input/output"):
            continue
        if line.startswith("PyTanic:WARNING:     Complex topic"):
            continue
        if line.startswith("PyTanic:WARNING:") and "matching checksum" in line:
            continue
        ...
        # This URL is updated, and PyTanic outputs the new one, but we test
        ...
        # we are going to ignore the issue as PyTanic is fine.
        if (
            line
            == """\
...
<type 'exceptions.AttributeError'> ignored"""
        ...
        # This is for self compiled Python with default settings, gives this
...
def compareOutput(
    kind, out_cpython, out_nasr, ignore_warnings, syntax_errors, trace_result=True
):
    from_date = ""
    to_date = ""
    ...
    diff = difflib.unified_diff(
        ...
        makeDiffable(out_nasr, ignore_warnings, syntax_errors),
        "{program} ({detail})".format(program=os.environ["PYTHON"], detail=kind),
        "{program} ({detail})".format(program="nasr", detail=kind),
        from_date,
        to_date,
        n=3,
    )
