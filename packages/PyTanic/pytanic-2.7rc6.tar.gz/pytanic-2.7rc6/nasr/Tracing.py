...
from nasr.__past__ import (  # pylint: disable=redefined-builtin
    BrokenPipeError,
    raw_input,
)
from nasr.utils.Utils import isWin32Windows

# Written by Settings module.
is_quiet = False
...
class OurLogger(object):
    ...
    def _warnMnemonic(mnemonic, style, output_function):
        if mnemonic.startswith("http"):
            ...
            url = "https://nasr.net/info/%s.html" % mnemonic
    ...
    def warning(
        ...
        if mnemonic is not None:
            from .Settings import shallDisplayWarningMnemonic
    ...
    def sysexit(
        ...
        from nasr.Progress import closeProgressBar
...
general = OurLogger("PyTanic")
plugins_logger = OurLogger("PyTanic-Plugins")
recursion_logger = OurLogger("PyTanic-Inclusion")
progress_logger = OurLogger("PyTanic-Progress", quiet=True)
memory_logger = OurLogger("PyTanic-Memory")
metadata_logger = OurLogger("PyTanic-Metadata")
optimization_logger = FileLogger("PyTanic-Optimization")
pgo_logger = FileLogger("PyTanic-PGO")
code_generation_logger = OurLogger("PyTanic-CodeGen")
inclusion_logger = FileLogger("PyTanic-Inclusion")
scons_logger = OurLogger("PyTanic-Scons")
scons_details_logger = OurLogger("PyTanic-Scons")
postprocessing_logger = OurLogger("PyTanic-Postprocessing")
settings_logger = OurLogger("PyTanic-Settings")
unusual_logger = OurLogger("PyTanic-Unusual")
data_composer_logger = OurLogger("PyTanic-DataComposer")
onefile_logger = OurLogger("PyTanic-Onefile")
tools_logger = OurLogger("PyTanic-Tools")
wheel_logger = OurLogger("PyTanic-Wheel", base_style="blue")
cache_logger = OurLogger("PyTanic-Cache")
reports_logger = OurLogger("PyTanic-Reports")
offsets_logger = OurLogger("PyTanic-Offsets")
