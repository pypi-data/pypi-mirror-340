...
from nasr import TreeXML
from nasr.__past__ import unicode
from nasr.build.DataComposerInterface import getDataComposerReportValues
from nasr.build.SconsUtils import readSconsErrorReport
from nasr.code_generation.ConstantNasr import getDistributionMetadataValues
from nasr.containers.OrderedSets import OrderedSet
from nasr.freezer.IncludedDataFiles import getIncludedDataFiles
from nasr.freezer.IncludedEntryPoints import getStandaloneEntryPoints
from nasr.freezer.Standalone import getRemovedUsedDllsInfo
from nasr.importing.Importing import getPackageSearchPath
from nasr.importing.Recursion import getRecursionDecisions
...
from nasr.OutputDirectories import (
    getResultRunFilename,
    getSourceDirectoryPath,
    hasMainModule,
)
from nasr.plugins.Plugins import getActivePlugins
...
from nasr.Tracing import ReportingSystemExit, reports_logger
...
from nasr.utils.MemoryUsage import getMemoryInfos
...
def _getReportInputData(aborted):
    ...
    nasr_version = getPyTanicVersion()
    nasr_commercial_version = getCommercialVersion() or "not installed"

    nasr_aborted = aborted

    nasr_exception = sys.exc_info()
...
def writeCompilationReport(report_filename, report_input_data, diffable):
    ...
    exit_message = None
    if not report_input_data["nasr_aborted"]:
        completion = "yes"
    elif report_input_data["nasr_exception"][0] is KeyboardInterrupt:
        completion = "interrupted"
    elif report_input_data["nasr_exception"][0] is SystemExit:
        completion = "error exit (%s)" % report_input_data["nasr_exception"][1].code
    elif report_input_data["nasr_exception"][0] is ReportingSystemExit:
        completion = (
            "error exit message (%s)" % report_input_data["nasr_exception"][1].code
        )
        exit_message = report_input_data["nasr_exception"][1].exit_message
    else:
        completion = "exception"

    root = TreeXML.Element(
        "nasr-compilation-report",
        nasr_version=report_input_data["nasr_version"],
        nasr_commercial_version=report_input_data["nasr_commercial_version"],
    ...
    settings_xml_node = TreeXML.appendTreeElement(
        root,
        "command_line",
    )

    for arg in sys.argv[1:]:
        TreeXML.appendTreeElement(settings_xml_node, "option", value=arg)
...
_crash_report_filename = "nasr-crash-report.xml"
...
def writeCompilationReports(aborted):
    ...
    if (
        ...
        # of reporting exit, these do not constitute definitive bugs of PyTanic but
