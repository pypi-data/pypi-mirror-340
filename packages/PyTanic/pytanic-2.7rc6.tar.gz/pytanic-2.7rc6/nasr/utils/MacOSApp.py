...
def createPlistInfoFile(logger, onefile):
    ...
    app_name = Settings.getMacOSAppName() or os.path.basename(result_filename)

    executable_name = os.path.basename(
        OutputDirectories.getResultFullpath(onefile=Settings.isOnefileMode())
    )

    signed_app_name = Settings.getMacOSSignedAppName() or app_name
    app_version = Settings.getMacOSAppVersion() or "1.0"
    ...
    icon_paths = Settings.getMacOSIconPaths()
    ...
    if Settings.isMacOSBackgroundApp():
        infos["LSBackgroundOnly"] = True
    elif Settings.isMacOSUiElementApp():
        infos["LSUIElement"] = True  # spell-checker: ignore LSUI
    else:
        infos["NSHighResolutionCapable"] = True

    legal_text = Settings.getLegalInformation()

    if legal_text is not None:
        infos["NSHumanReadableCopyright"] = legal_text

    for resource_name, resource_desc in Settings.getMacOSAppProtectedResourcesAccesses():
