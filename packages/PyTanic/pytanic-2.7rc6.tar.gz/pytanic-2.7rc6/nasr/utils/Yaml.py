...
""" PyTanic yaml utility functions.
...
from nasr.Settings import getUserProvidedYamlFiles
...
Error, empty (or malformed?) user package configuration '%s' used."""
    ...
    def get(self, name, section):
        ...
        # are not, and settings-nanny too.
...
def getYamlPackageConfiguration():
    """Get PyTanic package configuration. Merged from multiple sources."""
    ...
    if _package_config is None:
        _package_config = parsePackageYaml(
            ...
            "standard.nasr-package.config.yml",
        )
        _package_config.update(
            parsePackageYaml(
                "nasr.plugins.standard",
                "stdlib2.nasr-package.config.yml",
            )
        )
        _package_config.update(
            parsePackageYaml(
                "nasr.plugins.standard",
                "stdlib3.nasr-package.config.yml",
            )
        )
        ...
        try:
            _package_config.update(
                parsePackageYaml(
                    "nasr.plugins.commercial", "commercial.nasr-package.config.yml"
                )
            )
        except IOError:
...
def getYamlPackageConfigurationSchemaFilename():
    """Get the filename of the schema for PyTanic package configuration."""
    return os.path.join(
        os.path.dirname(__file__),
        "..",
        "..",
        "misc",
        "nasr-package-config-schema.json",
    )
