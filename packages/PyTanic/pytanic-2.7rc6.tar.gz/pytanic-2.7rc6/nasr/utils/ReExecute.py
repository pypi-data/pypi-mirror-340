...
"""Ability to restart PyTanic, needed for removing site module effects and using Python PGO after compile.
...
def setLaunchingPyTanicProcessEnvironmentValue(environment_variable_name, value):
    os.environ[environment_variable_name] = str(os.getpid()) + ":" + value


def reExecutePyTanic(pgo_filename):
    ...
    if "nasr.__main__" in sys.modules:
        our_filename = sys.modules["nasr.__main__"].__file__
    else:
        our_filename = sys.modules["__main__"].__file__

    args += ["-S", our_filename]

    setLaunchingPyTanicProcessEnvironmentValue(
        "DEVILPY_BINARY_NAME", sys.modules["__main__"].__file__
    )
    setLaunchingPyTanicProcessEnvironmentValue(
        "DEVILPY_PACKAGE_HOME",
        os.path.dirname(os.path.abspath(sys.modules["nasr"].__path__[0])),
    )

    if pgo_filename is not None:
        args.append("--pgo-python-input=%s" % pgo_filename)
    else:
        setLaunchingPyTanicProcessEnvironmentValue("DEVILPY_SYS_PREFIX", sys.prefix)

    # Same arguments as before.
    args += sys.argv[1:]

    from nasr.importing.PreloadedPackages import (
    ...
    if "site" in sys.modules:
        ...
        setLaunchingPyTanicProcessEnvironmentValue("DEVILPY_SITE_FILENAME", site_filename)
    ...
    setLaunchingPyTanicProcessEnvironmentValue("DEVILPY_PYTHONPATH", repr(sys.path))
    ...
    setLaunchingPyTanicProcessEnvironmentValue("DEVILPY_RE_EXECUTION", "1")
