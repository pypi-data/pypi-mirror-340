...
from nasr.Settings import getOnefileTempDirSpec, isOnefileTempDirMode
from nasr.OutputDirectories import getResultFullpath
from nasr.plugins.Plugins import Plugins
from nasr.PostProcessing import executePostProcessingResources
from nasr.PythonVersions import (
    getZstandardSupportingVersions,
    python_version,
)
from nasr.Tracing import onefile_logger, postprocessing_logger
from nasr.utils.Execution import withEnvironmentVarsOverridden
...
from nasr.utils.SharedLibraries import cleanupHeaderForAndroid
from nasr.utils.Signing import addMacOSCodeSignature
...
from nasr.utils.WindowsResources import RT_RCDATA, addResourceToFile
...
def _runOnefileScons(onefile_compression, onefile_archive):
    ...
    scons_settings["result_exe"] = OutputDirectories.getResultFullpath(onefile=True)
    scons_settings["source_dir"] = source_dir
    scons_settings["debug_mode"] = asBoolStr(Settings.is_debug)
    scons_settings["trace_mode"] = asBoolStr(Settings.shallTraceExecution())
    scons_settings["onefile_splash_screen"] = asBoolStr(
        Settings.getWindowsSplashScreen() is not None
    )
...
def getCompressorPython():
    # User may disable it.
    if Settings.shallNotCompressOnefile():
        return None
...
def runOnefileCompressor(
    ...
    if compressor_python is None or areSamePaths(
        ...
        from nasr.tools.onefile_compressor.OnefileCompressor import (
        ...
        attachOnefilePayload(
            ...
            as_archive=Settings.shallOnefileAsArchive(),
            use_compression_cache=not Settings.shallDisableCompressionCacheUsage(),
            file_checksums=file_checksums,
            win_path_sep=win_path_sep,
            low_memory=Settings.isLowMemory(),
        )
    else:
        ...
        mapping = {
            "DEVILPY_PACKAGE_HOME": os.path.dirname(
                os.path.abspath(sys.modules["nasr"].__path__[0])
            )
        }

        mapping["DEVILPY_PROGRESS_BAR"] = "1" if Settings.shallUseProgressBar() else "0"
        ...
        with withEnvironmentVarsOverridden(mapping):
            subprocess.check_call(
                [
                    ...
                    str(file_checksums),
                    str(win_path_sep),
                    str(Settings.isLowMemory()),
                    str(Settings.shallOnefileAsArchive()),
                    str(not Settings.shallDisableCompressionCacheUsage()),
                ],
                shell=False,
            )
...
def packDistFolderToOnefileBootstrap(onefile_output_filename, dist_dir):
    ...
    _runOnefileScons(
        ...
        onefile_archive=Settings.shallOnefileAsArchive(),
    )
