...
DLLs or extension modules, these need to be seen by PyTanic as entry points
...
from nasr.OutputDirectories import getStandaloneDirectoryPath
from nasr.Tracing import general, inclusion_logger, settings_logger
...
from nasr.utils.Importing import getExtensionModuleSuffixes
...
def makeIncludedDataFile(source_path, dest_path, reason, tracer, tags):
    ...
    if os.path.isdir(source_path):
        tracer.sysexit(
            "Error, cannot include directory '%s' as a data file. Data directories have their own settings."
            % source_path
        )
...
def _addIncludedDataFilesFromFileSettings():
    ...
    for pattern, source_path, dest_path, arg in getShallIncludeDataFiles():
        ...
        for filename in filenames:
            ...
            yield makeIncludedDataFile(
                filename, rel_path, file_reason, tracer=settings_logger, tags="user"
            )
    ...
    for source_path, dest_path in getShallIncludeDataDirs():
        ...
        if count == 0:
            settings_logger.warning("No data files in directory '%s.'" % source_path)
    ...
    for source_path, dest_path in getShallIncludeRawDirs():
        ...
        if count == 0:
            settings_logger.warning("No files in raw directory '%s.'" % source_path)


def addIncludedDataFilesFromFileSettings():
    """Early data files, from user settings that work with file system."""

    for included_datafile in _addIncludedDataFilesFromFileSettings():
...
def addIncludedDataFilesFromPlugins():
    # Cyclic dependency
    from nasr import ModuleRegistry
    from nasr.plugins.Plugins import Plugins
...
def addIncludedDataFilesFromPackageSettings():
    """Late data files, from plugins and user settings that work with packages"""
...
plugins '--embed-*' settings. Not done for '%s'."""
                    % included_datafile.dest_path
                )
