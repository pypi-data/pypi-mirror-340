...
from nasr.Errors import PyTanicForbiddenDLLEncounter
from nasr.plugins.Plugins import Plugins
from nasr.PythonFlavors import isAnacondaPython, isPyTanicPython
...
from nasr.utils.Json import loadJsonFromFilename
...
from nasr.utils.Utils import getArchitecture
...
def _resolveBinaryPathDLLsMacOS(
    ...
    for path in paths:
        if path.startswith("@rpath/"):
            ...
            continue
        elif isPyTanicPython() and not os.path.isabs(path) and not os.path.exists(path):
            # Although PyTanic Python statically links all packages, some of them
            ...
            # dependencies, so we need to scan the PyTanic Python library
        ...
        if not os.path.exists(resolved_path):
            ...
            if acceptable is True:
                raise PyTanicForbiddenDLLEncounter(binary_filename, plugin_name)
...
def fixupBinaryDLLPathsMacOS(
    ...
    except PyTanicForbiddenDLLEncounter:
