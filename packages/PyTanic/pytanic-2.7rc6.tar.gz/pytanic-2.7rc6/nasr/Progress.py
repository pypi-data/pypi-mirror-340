...
from nasr.utils.ThreadedExecutor import RLock
...
class PyTanicProgressBar(object):
...
def setupProgressBar(stage, unit, total, min_total=0):
    ...
    if use_progress_bar:
        Tracing.progress = PyTanicProgressBar(
            iterable=None,
            stage=stage,
            total=total,
            min_total=min_total,
...
@contextmanager
def withPyTanicDownloadProgressBar(*args, **kwargs):
    if not use_progress_bar or _getTqdmModule() is None:
        yield
    else:

        class PyTanicDownloadProgressBar(tqdm):
        ...
        with PyTanicDownloadProgressBar(*args, **kwargs) as progress_bar:
