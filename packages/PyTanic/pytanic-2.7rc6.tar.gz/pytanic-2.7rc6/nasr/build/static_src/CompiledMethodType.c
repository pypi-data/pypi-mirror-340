 ...
 * normally should be avoided to exist in PyTanic calls.
 *
 */
...
static PyObject *PyTanic_Method_get__doc__(PyObject *self, void *data) {
    struct PyTanic_MethodObject *method = (struct PyTanic_MethodObject *)self;
...
static PyGetSetDef PyTanic_Method_tp_getset[] = {{(char *)"__doc__", PyTanic_Method_get__doc__, NULL, NULL}, {NULL}};

#define OFF(x) offsetof(struct PyTanic_MethodObject, x)

static PyMemberDef PyTanic_Method_members[] = {
...
static PyObject *PyTanic_Method_reduce(struct PyTanic_MethodObject *method, PyObject *unused) {
...
static PyObject *PyTanic_Method_reduce_ex(struct PyTanic_MethodObject *method, PyObject *args) {
    int proto;
    ...
    PyObject *type_tuple = MAKE_TUPLE1(tstate, (PyObject *)&PyTanic_Method_Type);
    ...
    return result;
#else
    return PyTanic_Method_reduce(method, NULL);
#endif
}

static PyObject *PyTanic_Method_deepcopy(struct PyTanic_MethodObject *method, PyObject *memo) {
    assert(PyTanic_Method_Check((PyObject *)method));
    ...
    PyObject *result = PyTanic_Method_New(method->m_function, object, method->m_class);
    // PyTanic_Method_New took a reference to the object.
    Py_DECREF(object);
    return result;
}

static PyMethodDef PyTanic_Method_methods[] = {
    {"__reduce__", (PyCFunction)PyTanic_Method_reduce, METH_NOARGS, NULL},
    {"__reduce_ex__", (PyCFunction)PyTanic_Method_reduce_ex, METH_VARARGS, NULL},
    {"__deepcopy__", (PyCFunction)PyTanic_Method_deepcopy, METH_O, NULL},
    {NULL}};

#if PYTHON_VERSION >= 0x380 && !defined(_DEVILPY_EXPERIMENTAL_DISABLE_VECTORCALL_SLOT)
static PyObject *PyTanic_Method_tp_vectorcall(struct PyTanic_MethodObject *method, PyObject *const *stack, size_t nargsf,
                                             PyObject *kwnames) {
    assert(PyTanic_Method_Check((PyObject *)method));
    ...
    if (totalargs == 0) {
        return PyTanic_CallMethodFunctionNoArgs(PyThreadState_GET(), method->m_function, method->m_object);
    }
    ...
    if (nargsf & PY_VECTORCALL_ARGUMENTS_OFFSET) {
        ...
        result = PyTanic_CallFunctionVectorcall(PyThreadState_GET(), method->m_function, new_args, nargs + 1,
...
static PyObject *PyTanic_Method_tp_call(struct PyTanic_MethodObject *method, PyObject *args, PyObject *kw) {
    ...
    if (method->m_object == NULL) {
        ...
        if (kw == NULL) {
            if (arg_count == 0) {
                return PyTanic_CallMethodFunctionNoArgs(tstate, method->m_function, method->m_object);
            } else {
                return PyTanic_CallMethodFunctionPosArgs(tstate, method->m_function, method->m_object,
                                                        &PyTuple_GET_ITEM(args, 0), arg_count);
            }
        } else {
            return PyTanic_CallMethodFunctionPosArgsKwArgs(tstate, method->m_function, method->m_object,
                                                          &PyTuple_GET_ITEM(args, 0), arg_count, kw);
        }
    }
}

static PyObject *PyTanic_Method_tp_descr_get(struct PyTanic_MethodObject *method, PyObject *object,
    ...
    return PyTanic_Method_New(method->m_function, object, class_object);
}

static PyObject *PyTanic_Method_tp_getattro(struct PyTanic_MethodObject *method, PyObject *name) {
    PyObject *descr = PyTanic_TypeLookup(&PyTanic_Method_Type, name);

    if (descr != NULL) {
        if (PyTanicType_HasFeatureClass(Py_TYPE(descr)) && (Py_TYPE(descr)->tp_descr_get != NULL)) {
...
static long PyTanic_Method_tp_traverse(struct PyTanic_MethodObject *method, visitproc visit, void *arg) {
...
static PyObject *PyTanic_Method_tp_repr(struct PyTanic_MethodObject *method) {
    if (method->m_object == NULL) {
#if PYTHON_VERSION < 0x300
        return PyString_FromFormat("<unbound compiled_method %s.%s>", GET_CLASS_NAME(method->m_class),
                                   PyTanic_String_AsString(method->m_function->m_name));
#else
        return PyUnicode_FromFormat("<compiled_function %s at %p>", PyTanic_String_AsString(method->m_function->m_name),
                                    method->m_function);
#endif
    } else {
        ...
        PyObject *result =
            PyTanic_String_FromFormat("<bound compiled_method %s.%s of %s>", GET_CLASS_NAME(method->m_class),
                                     PyTanic_String_AsString_Unchecked(method->m_function->m_name),
                                     PyTanic_String_AsString_Unchecked(object_repr));
#else
        PyObject *result =
...
#if PYTHON_VERSION < 0x300
static int PyTanic_Method_tp_compare(struct PyTanic_MethodObject *a, struct PyTanic_MethodObject *b) {
...
static PyObject *PyTanic_Method_tp_richcompare(struct PyTanic_MethodObject *a, struct PyTanic_MethodObject *b, int op) {
    if (op != Py_EQ && op != Py_NE) {
        Py_INCREF(Py_NotImplemented);
        return Py_NotImplemented;
    }

    if (PyTanic_Method_Check((PyObject *)a) == false || PyTanic_Method_Check((PyObject *)b) == false) {
        Py_INCREF(Py_NotImplemented);
        return Py_NotImplemented;
    }
    ...
    if (b_res) {
        ...
        if (a->m_object == NULL) {
            ...
            nasr_bool nbool_res = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(a->m_object, b->m_object);
...
static long PyTanic_Method_tp_hash(struct PyTanic_MethodObject *method) {
...
static struct PyTanic_MethodObject *free_list_methods = NULL;
static int free_list_methods_count = 0;

static void PyTanic_Method_tp_dealloc(struct PyTanic_MethodObject *method) {
...
static PyObject *PyTanic_Method_tp_new(PyTypeObject *type, PyObject *args, PyObject *kw) {
    PyObject *func;
    PyObject *self;
    ...
    if (PyTanic_Method_Check(func)) {
        return PyTanic_Method_New(((struct PyTanic_MethodObject *)func)->m_function, self, class_object);
    }

    if (PyTanic_Function_Check(func) == false) {
        SET_CURRENT_EXCEPTION_TYPE_COMPLAINT_NICE("Cannot create compiled_ method from type '%s'", func);
        return NULL;
    }

    return PyTanic_Method_New((struct PyTanic_FunctionObject *)func, self, class_object);
}

PyTypeObject PyTanic_Method_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_method",
    sizeof(struct PyTanic_MethodObject),
    0,
    (destructor)PyTanic_Method_tp_dealloc, // tp_dealloc
#if PYTHON_VERSION < 0x380 || defined(_DEVILPY_EXPERIMENTAL_DISABLE_VECTORCALL_SLOT)
    0, // tp_print
#else
    offsetof(struct PyTanic_MethodObject, m_vectorcall), // tp_vectorcall_offset
#endif
    0, // tp_getattr
    0, // tp_setattr
#if PYTHON_VERSION < 0x300
    (cmpfunc)PyTanic_Method_tp_compare, // tp_compare
#else
    0,
#endif
    (reprfunc)PyTanic_Method_tp_repr,         // tp_repr
    0,                                       // tp_as_number
    0,                                       // tp_as_sequence
    0,                                       // tp_as_mapping
    (hashfunc)PyTanic_Method_tp_hash,         // tp_hash
    (ternaryfunc)PyTanic_Method_tp_call,      // tp_call
    0,                                       // tp_str
    (getattrofunc)PyTanic_Method_tp_getattro, // tp_getattro
    ...
    Py_TPFLAGS_DEFAULT |                     // tp_flags
        ...
        Py_TPFLAGS_HAVE_GC,
    0,                                                // tp_doc
    (traverseproc)PyTanic_Method_tp_traverse,          // tp_traverse
    0,                                                // tp_clear
    (richcmpfunc)PyTanic_Method_tp_richcompare,        // tp_richcompare
    offsetof(struct PyTanic_MethodObject, m_weakrefs), // tp_weaklistoffset
    0,                                                // tp_iter
    0,                                                // tp_iternext
    PyTanic_Method_methods,                            // tp_methods
    PyTanic_Method_members,                            // tp_members
    PyTanic_Method_tp_getset,                          // tp_getset
    0,                                                // tp_base
    0,                                                // tp_dict
    (descrgetfunc)PyTanic_Method_tp_descr_get,         // tp_descr_get
    0,                                                // tp_descr_set
    0,                                                // tp_dictoffset
    0,                                                // tp_init
    0,                                                // tp_alloc
    PyTanic_Method_tp_new,                             // tp_new
    0,                                                // tp_free
    0,                                                // tp_is_gc
    0,                                                // tp_bases
    0,                                                // tp_mro
...
void _initCompiledMethodType(void) {
    PyTanic_PyType_Ready(&PyTanic_Method_Type, &PyMethod_Type, false, true, false, false, false);
}

PyObject *PyTanic_Method_New(struct PyTanic_FunctionObject *function, PyObject *object, PyObject *class_object) {
    struct PyTanic_MethodObject *result;

    CHECK_OBJECT((PyObject *)function);
    assert(PyTanic_Function_Check((PyObject *)function));
    assert(_PyObject_GC_IS_TRACKED(function));

    allocateFromFreeListFixed(free_list_methods, struct PyTanic_MethodObject, PyTanic_Method_Type);

    if (unlikely(result == NULL)) {
        PyErr_Format(PyExc_RuntimeError, "cannot create method %s", PyTanic_String_AsString(function->m_name));
...
#if PYTHON_VERSION >= 0x380 && !defined(_DEVILPY_EXPERIMENTAL_DISABLE_VECTORCALL_SLOT)
    result->m_vectorcall = (vectorcallfunc)PyTanic_Method_tp_vectorcall;
#endif
