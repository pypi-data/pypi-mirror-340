...
Py_ssize_t PyTanic_PyObject_Size(PyObject *sequence) {
    CHECK_OBJECT(sequence);
    ...
    return PyTanic_PyMapping_Size(sequence);
}

PyObject *PyTanic_Number_Index(PyObject *item) {
    CHECK_OBJECT(item);
    ...
    if (unlikely(!PyTanic_Index_Check(item))) {
...
PyObject *PyTanic_Number_IndexAsLong(PyObject *item) {
    PyObject *result = PyTanic_Number_Index(item);
