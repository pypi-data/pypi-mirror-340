...
PyObject *CALL_METHOD_WITH_POS_ARGS(PyThreadState *tstate, PyObject *source, PyObject *attr_name,
    ...
    if (PyInstance_Check(source)) {
        ...
        if (called_object != NULL) {
            ...
            if (descr_get == PyTanic_Function_Type.tp_descr_get) {
                return PyTanic_CallMethodFunctionPosArgs(tstate, (struct PyTanic_FunctionObject const *)called_object,
...
#endif
    {
        ...
        if (type->tp_getattro != NULL) {
            ...
            called_object = (*type->tp_getattr)(source, (char *)PyTanic_String_AsString_Unchecked(attr_name));
        } else {
            SET_CURRENT_EXCEPTION_TYPE0_FORMAT2(PyExc_AttributeError, "'%s' object has no attribute '%s'",
                                                type->tp_name, PyTanic_String_AsString_Unchecked(attr_name));
...
char const *GET_CALLABLE_NAME(PyObject *object) {
    if (PyTanic_Function_Check(object)) {
        return PyTanic_String_AsString(PyTanic_Function_GetName(object));
    } else if (PyTanic_Generator_Check(object)) {
        return PyTanic_String_AsString(PyTanic_Generator_GetName(object));
    } else if (PyMethod_Check(object)) {
        return PyEval_GetFuncName(PyMethod_GET_FUNCTION(object));
    } else if (PyFunction_Check(object)) {
        return PyTanic_String_AsString(((PyFunctionObject *)object)->func_name);
    }
#if PYTHON_VERSION < 0x300
    else if (PyInstance_Check(object)) {
        return PyTanic_String_AsString(((PyInstanceObject *)object)->in_class->cl_name);
    } else if (PyClass_Check(object)) {
        return PyTanic_String_AsString(((PyClassObject *)object)->cl_name);
    }
#endif
...
char const *GET_CALLABLE_DESC(PyObject *object) {
    if (PyTanic_Function_Check(object) || PyTanic_Generator_Check(object) || PyMethod_Check(object) ||
...
char const *GET_CLASS_NAME(PyObject *class_object) {
    if (class_object == NULL) {
        ...
        if (PyClass_Check(class_object)) {
            return PyTanic_String_AsString(((PyClassObject *)class_object)->cl_name);
        }
#endif
...
void formatCannotInstantiateAbstractClass(PyThreadState *tstate, PyTypeObject *type) {
    ...
    PyObject *comma = PyTanic_String_FromString(", ");
    CHECK_OBJECT(comma);
#if PYTHON_VERSION < 0x300
    PyObject *joined = CALL_METHOD_WITH_SINGLE_ARG(tstate, comma, const_str_plain_join, sorted_methods);

    char const *joined_str = PyTanic_String_AsString(joined);
    ...
    SET_CURRENT_EXCEPTION_TYPE0_FORMAT3(PyExc_TypeError,
                                        ...
                                        method_count > 1 ? "s" : "", PyTanic_String_AsString(joined));
