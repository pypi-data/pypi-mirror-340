...
/**
 * This is responsible for updating parts of CPython to better work with PyTanic
...
static PyObject *_inspect_getgeneratorstate_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (PyTanic_Generator_Check(object)) {
        struct PyTanic_GeneratorObject *generator = (struct PyTanic_GeneratorObject *)object;
...
static PyObject *_inspect_getcoroutinestate_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (PyTanic_Coroutine_Check(object)) {
        struct PyTanic_CoroutineObject *coroutine = (struct PyTanic_CoroutineObject *)object;
...
static PyObject *_types_coroutine_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (PyTanic_Function_Check(func)) {
...
static bool PyTanic_FrameIsCompiled(_PyInterpreterFrame *frame) {
    return ((frame->frame_obj != NULL) && PyTanic_Frame_Check((PyObject *)frame->frame_obj));
}

static bool PyTanic_FrameIsIncomplete(_PyInterpreterFrame *frame) {
...
static PyObject *_sys_getframemodulename_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    PyObject *index_value = PyTanic_Number_IndexAsLong(depth_arg ? depth_arg : const_int_0);
    ...
    while ((frame != NULL) && ((PyTanic_FrameIsIncomplete(frame)) || depth_ssize-- > 0)) {
        frame = frame->previous;
    }

    if ((frame != NULL) && (PyTanic_FrameIsCompiled(frame))) {
...
"
...

    {
        DEVILPY_MAY_BE_UNUSED PyObject *module =
            PyImport_ExecCodeModule("nasr_types_patch", wrapper_enhancement_code_object);
        CHECK_OBJECT(module);

        DEVILPY_MAY_BE_UNUSED bool bool_res = PyTanic_DelModuleString(tstate, "nasr_types_patch");
...
#if PYTHON_VERSION >= 0x3c0
    orig_sys_getframemodulename = PyTanic_SysGetObject("_getframemodulename");
    ...
    PyTanic_SysSetObject("_getframemodulename", sys_getframemodulename_replacement);
#endif
...
static PyObject *PyTanic_type_tp_richcompare(PyObject *a, PyObject *b, int op) {
    if (likely(op == Py_EQ || op == Py_NE)) {
        if (a == (PyObject *)&PyTanic_Function_Type) {
            a = (PyObject *)&PyFunction_Type;
        } else if (a == (PyObject *)&PyTanic_Method_Type) {
            a = (PyObject *)&PyMethod_Type;
        } else if (a == (PyObject *)&PyTanic_Generator_Type) {
            a = (PyObject *)&PyGen_Type;
#if PYTHON_VERSION >= 0x350
        } else if (a == (PyObject *)&PyTanic_Coroutine_Type) {
            a = (PyObject *)&PyCoro_Type;
#endif
#if PYTHON_VERSION >= 0x360
        } else if (a == (PyObject *)&PyTanic_Asyncgen_Type) {
            a = (PyObject *)&PyAsyncGen_Type;
#endif
        }

        if (b == (PyObject *)&PyTanic_Function_Type) {
            b = (PyObject *)&PyFunction_Type;
        } else if (b == (PyObject *)&PyTanic_Method_Type) {
            b = (PyObject *)&PyMethod_Type;
        } else if (b == (PyObject *)&PyTanic_Generator_Type) {
            b = (PyObject *)&PyGen_Type;
#if PYTHON_VERSION >= 0x350
        } else if (b == (PyObject *)&PyTanic_Coroutine_Type) {
            b = (PyObject *)&PyCoro_Type;
#endif
#if PYTHON_VERSION >= 0x360
        } else if (b == (PyObject *)&PyTanic_Asyncgen_Type) {
...
void patchTypeComparison(void) {
    if (original_PyType_tp_richcompare == NULL) {
        ...
        PyType_Type.tp_richcompare = PyTanic_type_tp_richcompare;
    }
}
...
// existing type.
PyTracebackObject *MAKE_TRACEBACK(struct PyTanic_FrameObject *frame, int lineno) {
#if 0
...
static void PyTanic_tb_dealloc(PyTracebackObject *tb) {
    // Need to use official method as it checks for recursion.
    PyTanic_GC_UnTrack(tb);

#if 0
#if PYTHON_VERSION >= 0x380
    Py_TRASHCAN_BEGIN(tb, PyTanic_tb_dealloc);
#else
...
void patchTracebackDealloc(void) { PyTraceBack_Type.tp_dealloc = (destructor)PyTanic_tb_dealloc; }
