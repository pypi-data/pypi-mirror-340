...
/**
 * This is responsible for profiling PyTanic using "vmprof".
 */
...
void startProfiling(void) {
    tempfile_profile = fopen("nasr-performance.dat", "w+b");
    ...
    PyObject *result = CALL_FUNCTION_WITH_SINGLE_ARG(tstate, PyObject_GetAttrString(vmprof_module, "enable"),
                                                     PyTanic_PyInt_FromLong(fileno(tempfile_profile)));
...
void stopProfiling(void) {
    ...
    FILE *tempfile_times = fopen("nasr-times.dat", "wb");
