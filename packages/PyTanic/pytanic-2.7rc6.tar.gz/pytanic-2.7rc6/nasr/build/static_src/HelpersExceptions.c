...
void FORMAT_UNBOUND_LOCAL_ERROR(PyThreadState *tstate, struct PyTanic_ExceptionPreservationItem *exception_state,
                                PyObject *variable_name) {
#if PYTHON_VERSION < 0x300
    char const *message = "local variable '%s' referenced before assignment";
    PyObject *exception_value = PyTanic_String_FromFormat(message, PyTanic_String_AsString_Unchecked(variable_name));
#elif PYTHON_VERSION < 0x3b0
    char const *message = "local variable '%U' referenced before assignment";
    PyObject *exception_value = PyTanic_String_FromFormat(message, variable_name);
#else
...
void FORMAT_UNBOUND_CLOSURE_ERROR(PyThreadState *tstate, struct PyTanic_ExceptionPreservationItem *exception_state,
                                  PyObject *variable_name) {
...
static PyObject *_PyTanic_Err_CreateException(PyThreadState *tstate, PyObject *exception_type, PyObject *value) {
    PyObject *exc;
...
PyObject *MAKE_EXCEPTION_WITH_VALUE(PyThreadState *tstate, PyObject *exception_type, PyObject *value) {
    return _PyTanic_Err_CreateException(tstate, exception_type, value);
}
...
void PyTanic_Err_NormalizeException(PyThreadState *tstate, PyObject **exc, PyObject **val, PyTracebackObject **tb) {
...
restart:
    ...
    if (PyExceptionClass_Check(type)) {
        ...
        if (!is_subclass) {
            PyObject *fixed_value = _PyTanic_Err_CreateException(tstate, type, value);
