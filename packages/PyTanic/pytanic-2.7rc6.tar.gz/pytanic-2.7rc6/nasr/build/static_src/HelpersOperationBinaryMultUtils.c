...
static Py_ssize_t CONVERT_LONG_TO_REPEAT_FACTOR(PyObject *value) {
    ...
    Py_ssize_t digits_count = PyTanic_LongGetDigitSize(value);

    if (digits_count == 0) {
        return 0;
    }

    bool is_negative = PyTanic_LongIsNegative(value);

    PyLongObject *long_value = (PyLongObject *)value;

    digit *digits = PyTanic_LongGetDigitPointer(long_value);
