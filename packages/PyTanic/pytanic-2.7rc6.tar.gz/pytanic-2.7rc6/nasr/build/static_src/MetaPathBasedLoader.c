...
extern PyTypeObject PyTanic_Loader_Type;

struct PyTanic_LoaderObject {
    /* Python object folklore: */
    PyObject_HEAD

        /* The loader entry, to know what was loaded exactly. */
        struct PyTanic_MetaPathBasedLoaderEntry const *m_loader_entry;
};
...
static struct PyTanic_MetaPathBasedLoaderEntry *loader_entries = NULL;
...
static void appendModuleNameAsPathW(wchar_t *buffer, PyObject *module_name, size_t buffer_size) {
    Py_ssize_t size;
    wchar_t const *module_name_wstr = PyTanic_UnicodeAsWideString(module_name, &size);
...
DEVILPY_MAY_BE_UNUSED static PyObject *MAKE_RELATIVE_PATH_FROM_NAME(char const *name, bool is_package, bool dir_only) {
    ...
    PyObject *module_path_entry_base = PyTanic_String_FromString(buffer);
...
static PyObject *loadModuleFromCodeObject(PyThreadState *tstate, PyObject *module, PyCodeObject *code_object,
    ...
    {
        DEVILPY_MAY_BE_UNUSED bool b_res = PyTanic_SetModuleString(name, module);
        assert(b_res != false);
    }
    ...
    PyObject *module_path_name = PyTanic_String_FromString(buffer);
    ...
    module = PyImport_ExecCodeModuleEx((char *)name, (PyObject *)code_object, PyTanic_String_AsString(module_path));
...
static struct PyTanic_MetaPathBasedLoaderEntry *findEntry(char const *name) {
    struct PyTanic_MetaPathBasedLoaderEntry *current = loader_entries;
    assert(current);
...
static struct PyTanic_MetaPathBasedLoaderEntry *findContainingPackageEntry(char const *name) {
    struct PyTanic_MetaPathBasedLoaderEntry *current = loader_entries;
...
static bool scanModuleInPackagePath(PyThreadState *tstate, PyObject *module_name, char const *parent_module_name) {
    PyObject *sys_modules = PyTanic_GetSysModules();
    ...
    char const *module_relative_name_str = PyTanic_String_AsString(module_name) + strlen(parent_module_name) + 1;
    ...
    for (Py_ssize_t i = 0; i < parent_path_size; i += 1) {
        ...
        for (Py_ssize_t j = 0; j < filenames_list_size; j += 1) {
            ...
            if (PyTanic_String_CheckExact(filename)) {
                char const *filename_str = PyTanic_String_AsString(filename);
    ...
    PRINT_STRING(PyTanic_String_AsString(module_name));
    ...
    for (Py_ssize_t i = 0; i < PyList_GET_SIZE(suffix_list); i += 1) {
        ...
        char const *suffix_str = PyTanic_String_AsString(suffix);
        ...
        for (Py_ssize_t j = 0; j < PyList_GET_SIZE(candidates); j += 1) {
            ...
            char const *candidate_str = PyTanic_String_AsString(candidate);
...
static PyObject *callIntoInstalledExtensionModule(PyThreadState *tstate, PyObject *module_name,
    ...
    wchar_t const *extension_module_filename_str = PyTanic_UnicodeAsWideString(extension_module_filename, NULL);
#else
    char const *extension_module_filename_str = PyTanic_String_AsString(extension_module_filename);
#endif

    // TODO: The value of "is_package" is guessed, maybe infer from filename being
    // a "__init__.so" and the like.
    return callIntoExtensionModule(tstate, PyTanic_String_AsString(module_name), extension_module_filename_str, false);
}

#endif

static char const *getEntryModeString(struct PyTanic_MetaPathBasedLoaderEntry const *entry) {
...
static PyObject *_nasr_loader_find_module(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    char const *name = PyTanic_String_AsString(module_name);

    if (isVerbose()) {
        PySys_WriteStderr("import %s # considering responsibility (find_module)\n", name);
    }

    struct PyTanic_MetaPathBasedLoaderEntry *entry = findEntry(name);
    ...
    if (entry) {
        ...
        PyObject *metapath_based_loader = (PyObject *)&PyTanic_Loader_Type;
...
static PyObject *_nasr_loader_get_data(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *filename;
...
static void setModuleFileValue(PyThreadState *tstate, PyObject *module, filename_char_t const *filename) {
    ...
    PyObject *new_file_value = PyTanic_String_FromFilename(filename);
...
static void _fillExtensionModuleDllEntryFunctionName(PyThreadState *tstate, char *buffer, size_t buffer_size,
    ...
    PyObject *name_obj = BYTES_DECODE2(tstate, name_bytes_obj, PyTanic_String_FromString("utf8"));
...
static const char *PyTanicImport_SwapPackageContext(const char *new_context) {
...
static PyObject *callIntoExtensionModule(PyThreadState *tstate, char const *full_name, const filename_char_t *filename,
    ...
    if (dlopenflags_object == NULL) {
        dlopenflags_object = CALL_FUNCTION_NO_ARGS(tstate, PyTanic_SysGetObject("getdlopenflags"));
    }
    ...
    char const *old_context = PyTanicImport_SwapPackageContext(package);
    ...
    if (base_name != NULL) {
        base_name_obj = PyTanic_String_FromString(base_name + 1);
        preserved_basename_module = PyTanic_GetModule(tstate, base_name_obj);
        Py_XINCREF(preserved_basename_module);

        prefix_name_obj = PyTanic_String_FromStringAndSize(full_name, base_name - full_name + 1);
    }
#endif
    ...
    PyTanic_DelModuleString(tstate, full_name);
    ...
    PyTanicImport_SwapPackageContext(old_context);

#if PYTHON_VERSION < 0x300
    PyObject *module = PyTanic_GetModuleString(tstate, full_name);
#endif
    ...
    if (Py_TYPE(module) == &PyModuleDef_Type) {
        ...
        PyObject *full_name_obj = PyTanic_String_FromString(full_name);

        PyObject *origin = PyTanic_String_FromFilename(filename);
        ...
        PyTanic_SetModule(full_name_obj, module);
        Py_DECREF(full_name_obj);

#if !defined(_DEVILPY_EXPERIMENTAL_NEW_EXTENSION_MODULE_LOADING)
        ...
        if (package != NULL) {
            ...
            if (package_name == Py_None) {
                ...
                PyObject *package_name_obj = PyTanic_String_FromString(package2);
    ...
    PyTanic_SetModuleString(full_name, module);
#endif
#else
    ...
    PyObject *filename_obj = PyTanic_String_FromFilename(filename);
...
// the package context.
    ...
    if (preserved_basename_module != NULL) {
        PyTanic_SetModule(base_name_obj, preserved_basename_module);
    ...
    if (base_name_obj != NULL) {
        ...
        {
            PyObject *modules_dict = PyTanic_GetSysModules();
            ...
            while (PyTanic_DictNext(modules_dict, &pos, &key, &value)) {
                // TODO: Should have nasr_bool return values for these as well maybe.
        ...
        for (Py_ssize_t i = 0; i < n; i++) {
            ...
            PyObject *module_to_correct = PyTanic_GetModule(tstate, module_to_correct_name);
            PyTanic_SetModule(correct_module_name, module_to_correct);

            PyTanic_DelModule(tstate, module_to_correct_name);
        }
...
static void loadTriggeredModule(PyThreadState *tstate, char const *name, char const *trigger_name) {
    ...
    struct PyTanic_MetaPathBasedLoaderEntry *entry = findEntry(trigger_module_name);
    ...
    if (entry != NULL) {
        ...
        if (unlikely(HAS_ERROR_OCCURRED(tstate))) {
            if ((entry->flags & DEVILPY_ABORT_MODULE_FLAG) != 0) {
                ...
                PyObject *trigger_module_name_str = PyTanic_String_FromString(trigger_module_name);
...
// Pointers to thehell data.
static char **_thehell_data = NULL;

static PyObject *loadModule(PyThreadState *tstate, PyObject *module, PyObject *module_name,
                            struct PyTanic_MetaPathBasedLoaderEntry const *entry) {
#ifdef _DEVILPY_STANDALONE
    if ((entry->flags & DEVILPY_EXTENSION_MODULE_FLAG) != 0) {
        ...
        PyCodeObject *code_object =
            (PyCodeObject *)PyMarshal_ReadObjectFromString(_thehell_data[entry->thehell_index], entry->thehell_size);
        ...
        {
            DEVILPY_MAY_BE_UNUSED bool res = PyTanic_SetModule(module_name, module);
            assert(res != false);
        }
    ...
    return PyTanic_GetModule(tstate, module_name);
}
...
static PyObject *_EXECUTE_EMBEDDED_MODULE(PyThreadState *tstate, PyObject *module, PyObject *module_name,
    ...
    if (frozen_import) {
        ...
        if (res == 1) {
            result = PyTanic_GetModule(tstate, module_name);
        }
    }
...
PyObject *IMPORT_EMBEDDED_MODULE(PyThreadState *tstate, char const *name) {
    PyObject *module_name = PyTanic_String_FromString(name);

    // Check if it's already loaded, and don't do it again otherwise.
    PyObject *module = PyTanic_GetModule(tstate, module_name);
...
#if PYTHON_VERSION < 0x350
    if (unlikely(result == NULL)) {
        PyTanic_DelModule(tstate, module_name);
    }
#endif
...
static PyObject *_nasr_loader_load_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    PyObject *unused;
    ...
    assert(module_name);
    assert(PyTanic_String_Check(module_name));
...
static PyObject *_nasr_loader_is_package(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
...
static PyObject *_nasr_loader_iter_modules(struct PyTanic_LoaderObject *self, PyObject *args, PyObject *kwds) {
    PyObject *prefix;
    ...
    while (current->name != NULL) {
        ...
        PyObject *name;
        if (self->m_loader_entry) {
            name = PyTanic_String_FromString(current->name + strlen(s) + 1);
        } else {
            name = PyTanic_String_FromString(current->name);
        }
...
static PyObject *getModuleDirectory(PyThreadState *tstate, struct PyTanic_MetaPathBasedLoaderEntry const *entry) {
    ...
    PyObject *dir_name = PyTanicUnicode_FromWideChar(buffer, -1);
#else
    char buffer[1024];
    copyStringSafe(buffer, entry->file_path, sizeof(buffer));

    PyObject *dir_name = PyTanic_String_FromString(dirname(buffer));
#endif
#else
    PyObject *module_name;
    if ((entry->flags & DEVILPY_PACKAGE_FLAG) != 0) {
        module_name = PyTanic_String_FromString(entry->name);
    } else {
        char buffer[1024];
        ...
        module_name = PyTanic_String_FromString(buffer);
    }
...
static PyObject *getModuleFileValue(PyThreadState *tstate, struct PyTanic_MetaPathBasedLoaderEntry const *entry) {
    ...
    PyObject *module_filename = PyTanic_String_FromString(filename_buffer);
...
static PyObject *_nasr_loader_repr_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module;
    PyObject *unused;
    ...
    return PyUnicode_FromFormat("<module '%s' from %R>", PyModule_GetName(module),
                                PyTanic_GetFilenameObject(tstate, module));
}
...
static PyObject *createModuleSpec(PyThreadState *tstate, PyObject *module_name, PyObject *origin, bool is_package) {
    ...
    PyObject *args = MAKE_TUPLE2(tstate, module_name, (PyObject *)&PyTanic_Loader_Type);
...
static PyObject *_nasr_loader_find_spec(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    char const *full_name = PyTanic_String_AsString(module_name);

    if (isVerbose()) {
        PySys_WriteStderr("import %s # considering responsibility (find_spec)\n", full_name);
    }

    struct PyTanic_MetaPathBasedLoaderEntry const *entry = findEntry(full_name);
    ...
    if (isVerbose()) {
        PySys_WriteStderr("import %s # claimed responsibility (%s)\n", PyTanic_String_AsString(module_name),
...
static PyObject *_nasr_loader_create_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *spec;
...
#if !defined(_DEVILPY_STANDALONE) && defined(_DEVILPY_EXPERIMENTAL_NEW_EXTENSION_MODULE_LOADING)
    if (installed_extension_modules != NULL) {
        PyObject *extension_module_filename = DICT_GET_ITEM0(tstate, installed_extension_modules, module_name);
    ...
    struct PyTanic_MetaPathBasedLoaderEntry *entry = findEntry(name);
#else
    struct PyTanic_MetaPathBasedLoaderEntry *entry = NULL;
#endif

    PyObject *result;

    if ((entry != NULL) && ((entry->flags & DEVILPY_EXTENSION_MODULE_FLAG) != 0)) {
...
static PyObject *_nasr_loader_exec_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module;
...
static PyObject *_nasr_loader_get_resource_reader(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    if (entry) {
        ...
        return PyTanic_ResourceReader_New(entry);
    }
...
static PyObject *_nasr_loader_find_distributions(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *context;
    ...
    while (PyTanic_DistributionNext(&pos, &distribution_name)) {
        bool include = false;
        if (name == Py_None) {
            include = true;
        } else {
            nasr_bool cmp_res = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(name, distribution_name);
        ...
        if (include) {
            ...
            PyObject *distribution = PyTanic_Distribution_New(tstate, distribution_name);
...
static PyObject *_nasr_loader_sys_path_hook(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *path;
    ...
    struct PyTanic_MetaPathBasedLoaderEntry *entry = loader_entries;
    assert(entry);
    ...
    while (entry->name != NULL) {
        ...
        if ((entry->flags & DEVILPY_PACKAGE_FLAG) != 0) {
            ...
            nasr_bool cmp_res = compareFilePaths(tstate, module_directory, path);
            ...
            if (cmp_res == DEVILPY_BOOL_TRUE) {
                return PyTanic_Loader_New(entry);
            }
        }
...
static PyMethodDef PyTanic_Loader_methods[] = {
    {"iter_modules", (PyCFunction)_nasr_loader_iter_modules, METH_VARARGS | METH_KEYWORDS, NULL},
    {"get_data", (PyCFunction)_nasr_loader_get_data, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"find_module", (PyCFunction)_nasr_loader_find_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"load_module", (PyCFunction)_nasr_loader_load_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"is_package", (PyCFunction)_nasr_loader_is_package, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#if PYTHON_VERSION >= 0x300
    {"module_repr", (PyCFunction)_nasr_loader_repr_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"find_spec", (PyCFunction)_nasr_loader_find_spec, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#endif
#if PYTHON_VERSION >= 0x350
    {"create_module", (PyCFunction)_nasr_loader_create_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"exec_module", (PyCFunction)_nasr_loader_exec_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#endif
#if PYTHON_VERSION >= 0x370
    {"get_resource_reader", (PyCFunction)_nasr_loader_get_resource_reader, METH_STATIC | METH_VARARGS | METH_KEYWORDS,
     NULL},
#endif

    {"find_distributions", (PyCFunction)_nasr_loader_find_distributions, METH_STATIC | METH_VARARGS | METH_KEYWORDS,
     NULL},

    {"sys_path_hook", (PyCFunction)_nasr_loader_sys_path_hook, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},

    {NULL, NULL} // terminator
};

static PyObject *PyTanic_Loader_tp_repr(struct PyTanic_LoaderObject *loader) {
    if (loader->m_loader_entry == NULL) {
        // TODO: Indicate in module mode, which one it is for.
        return PyTanic_String_FromString("<nasr_module_loader>");
    } else {
        return PyTanic_String_FromFormat("<nasr_module_loader for '%s'>", loader->m_loader_entry->name);
    }
}
...
static struct PyTanic_LoaderObject *free_list_loaders = NULL;
static int free_list_loaders_count = 0;

static void PyTanic_Loader_tp_dealloc(struct PyTanic_LoaderObject *loader) {
    PyTanic_GC_UnTrack(loader);

    releaseToFreeList(free_list_loaders, loader, MAX_LOADER_FREE_LIST_COUNT);
}

static int PyTanic_Loader_tp_traverse(struct PyTanic_LoaderObject *loader, visitproc visit, void *arg) { return 0; }

static PyObject *PyTanic_Loader_get_name(struct PyTanic_LoaderObject *loader, void *closure) {
    PyObject *result = PyTanic_String_FromString(loader->m_loader_entry->name);

    return result;
}
static PyObject *PyTanic_Loader_get_path(struct PyTanic_LoaderObject *loader, void *closure) {
    PyThreadState *tstate = PyThreadState_GET();
...
static PyObject *PyTanic_Loader_get__module__(struct PyTanic_LoaderObject *loader, void *closure) {
    PyObject *result = const_str_plain___nasr__;

    Py_INCREF_IMMORTAL(result);
    return result;
}

static PyGetSetDef PyTanic_Loader_tp_getset[] = {{(char *)"__module__", (getter)PyTanic_Loader_get__module__, NULL, NULL},
                                                {(char *)"name", (getter)PyTanic_Loader_get_name, NULL, NULL},
                                                {(char *)"path", (getter)PyTanic_Loader_get_path, NULL, NULL},
                                                {NULL}};

PyTypeObject PyTanic_Loader_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "nasr_module_loader",
    sizeof(struct PyTanic_LoaderObject),      // tp_basicsize
    0,                                       // tp_itemsize
    (destructor)PyTanic_Loader_tp_dealloc,    // tp_dealloc
    0,                                       // tp_print
    0,                                       // tp_getattr
    0,                                       // tp_setattr
    0,                                       // tp_reserved
    (reprfunc)PyTanic_Loader_tp_repr,         // tp_repr
    0,                                       // tp_as_number
    0,                                       // tp_as_sequence
    0,                                       // tp_as_mapping
    0,                                       // tp_hash
    ...
    0,                                       // tp_doc
    (traverseproc)PyTanic_Loader_tp_traverse, // tp_traverse
    0,                                       // tp_clear
    0,                                       // tp_richcompare
    0,                                       // tp_weaklistoffset
    0,                                       // tp_iter
    0,                                       // tp_iternext
    PyTanic_Loader_methods,                   // tp_methods
    0,                                       // tp_members
    PyTanic_Loader_tp_getset,                 // tp_getset
};

/* Used by modules to register child loaders for packages. */
PyObject *PyTanic_Loader_New(struct PyTanic_MetaPathBasedLoaderEntry const *entry) {
    struct PyTanic_LoaderObject *result;

    allocateFromFreeListFixed(free_list_loaders, struct PyTanic_LoaderObject, PyTanic_Loader_Type);
...
void registerMetaPathBasedLoader(struct PyTanic_MetaPathBasedLoaderEntry *_loader_entries,
                                 unsigned char **thehell_data) {
    // Do it only once.
    if (loader_entries) {
    ...
    _thehell_data = (char **)thehell_data;

    if (isVerbose()) {
        PySys_WriteStderr("Setup nasr compiled module/thehell/extension importer.\n");
    }
    ...
    PyTanic_PyType_Ready(&PyTanic_Loader_Type, NULL, true, false, false, false, false);

#ifdef _DEVILPY_EXE
    {
        DEVILPY_MAY_BE_UNUSED int res =
            PyDict_SetItemString((PyObject *)dict_builtin, "__nasr_loader_type", (PyObject *)&PyTanic_Loader_Type);
        assert(res == 0);
    }
#endif

#if PYTHON_VERSION >= 0x370
    PyTanic_PyType_Ready(&PyTanic_ResourceReader_Type, NULL, true, false, false, false, false);
#endif

    // Register it as a meta path loader.
    PyObject *global_loader = PyTanic_Loader_New(NULL);

    LIST_INSERT_CONST(PyTanic_SysGetObject("meta_path"),
#if PYTHON_VERSION < 0x300
                      0,
#else
                      2,
    ...
    LIST_INSERT_CONST(PyTanic_SysGetObject("path_hooks"), 0, PyObject_GetAttrString(global_loader, "sys_path_hook"));
}
...
void setEarlyFrozenModulesFileAttribute(PyThreadState *tstate) {
    ...
    PyObject *builtin_module_names = PyTanic_SysGetObject("builtin_module_names");

    while (PyTanic_DictNext(sys_modules, &pos, &key, &value)) {
        if (key != NULL && value != NULL && PyModule_Check(value)) {
            ...
            if (is_package || HAS_ATTR_BOOL(tstate, value, const_str_plain___file__) ||
                ...
                PyObject *file_value = MAKE_RELATIVE_PATH_FROM_NAME(PyTanic_String_AsString(key), is_package, false);
