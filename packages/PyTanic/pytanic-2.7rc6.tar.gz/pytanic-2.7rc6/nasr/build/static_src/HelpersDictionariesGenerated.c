...
static inline PyDictValues *_PyTanic_PyDict_new_values(Py_ssize_t size) {
    Py_ssize_t values_size = sizeof(PyObject *) * size;

#if PYTHON_VERSION < 0x3b0
    return (PyDictValues *)PyTanicMem_Malloc(values_size);
#else
    ...
    uint8_t *mem = (uint8_t *)PyTanicMem_Malloc(n);
...
PyObject *DICT_COPY(PyThreadState *tstate, PyObject *dict_value) {
    ...
    if (((PyDictObject *)dict_value)->ma_used == 0) {
        ...
        if (_PyDict_HasSplitTable(dict_mp)) {
            ...
            PyDictValues *new_values = _PyTanic_PyDict_new_values(size);
            ...
            if (dict_mp->ma_values == NULL && IS_COMPACT(dict_mp)) {
                ...
                Py_ssize_t keys_size = _PyTanic_Py_PyDict_KeysSize(dict_mp->ma_keys);
                result_mp->ma_keys = _PyTanic_AllocatePyDictKeysObject(tstate, keys_size);
            ...
            while (PyTanic_DictNext((PyObject *)dict_mp, &pos, &key, &value)) {
                CHECK_OBJECT(key);
                CHECK_OBJECT(value);
...
PyObject *DEEP_COPY_DICT(PyThreadState *tstate, PyObject *dict_value) {
    ...
    while (PyTanic_DictNext(dict_value, &pos, &key, &value)) {
...
static PyObject *COPY_DICT_KW(PyThreadState *tstate, PyObject *dict_value) {
    ...
    if (((PyDictObject *)dict_value)->ma_used == 0) {
        ...
        if (_PyDict_HasSplitTable(dict_mp)) {
            ...
            while (PyTanic_DictNext((PyObject *)dict_mp, &pos, &key, &_value)) {
                CHECK_OBJECT(key);
                CHECK_OBJECT(_value);
