...
#if PYTHON_VERSION >= 0x300
static PyBaseExceptionObject *PyTanic_BaseExceptionSingleArg_new(PyThreadState *tstate, PyTypeObject *type,
                                                                PyObject *arg) {
...
static PyObject *PyTanic_CreateStopIteration(PyThreadState *tstate, PyObject *value) {
    if (value == Py_None) {
        value = NULL;
    }

    PyStopIterationObject *result =
        (PyStopIterationObject *)PyTanic_BaseExceptionSingleArg_new(tstate, (PyTypeObject *)PyExc_StopIteration, value);
...
static void PyTanic_SetStopIterationValue(PyThreadState *tstate, PyObject *value) {
    ...
    if (likely(!PyTuple_Check(value) && !PyExceptionInstance_Check(value))) {
        ...
        struct PyTanic_ExceptionPreservationItem exception_state = {Py_NewRef(PyExc_StopIteration),
                                                                   PyTanic_CreateStopIteration(tstate, value)};

        RESTORE_ERROR_OCCURRED_STATE(tstate, &exception_state);
    }
#else
    struct PyTanic_ExceptionPreservationItem exception_state = {PyTanic_CreateStopIteration(tstate, value)};
...
#else
    struct PyTanic_ExceptionPreservationItem exception_state = {PyTanic_CreateStopIteration(tstate, NULL)};
...
#if PYTHON_VERSION >= 0x3c0
static PyObject *PyTanic_CreateStopAsyncIteration(PyThreadState *tstate) {
    return (PyObject *)PyTanic_BaseExceptionSingleArg_new(tstate, (PyTypeObject *)PyExc_StopAsyncIteration, NULL);
}
#endif
...
#else
    struct PyTanic_ExceptionPreservationItem exception_state = {PyTanic_CreateStopAsyncIteration(tstate)};
...
#if PYTHON_VERSION >= 0x3c0
static PyObject *PyTanic_CreateGeneratorExit(PyThreadState *tstate) {
    return (PyObject *)PyTanic_BaseExceptionSingleArg_new(tstate, (PyTypeObject *)PyExc_GeneratorExit, NULL);
}
#endif
...
#else
    struct PyTanic_ExceptionPreservationItem exception_state = {PyTanic_CreateGeneratorExit(tstate)};
...
#if PYTHON_VERSION >= 0x300
static bool PyTanic_PyGen_FetchStopIterationValue(PyThreadState *tstate, PyObject **pvalue) {
#if PYTHON_VERSION < 0x3c0
...
static inline void PyTanic_PyGen_exc_state_clear(_PyErr_StackItem *exc_state) {
...
#if PYTHON_VERSION < 0x3b0
static inline bool PyTanic_PyFrameHasCompleted(PyFrameObject *const frame) {
...
static PyObject *PyTanic_PyGen_Send(PyThreadState *tstate, PyGenObject *gen, PyObject *arg) {
#if PYTHON_VERSION >= 0x3a0
    PyObject *result;
...
#if PYTHON_VERSION < 0x3b0
    if (f == NULL || PyTanic_PyFrameHasCompleted(f)) {
#else
...
#else
        PyTanic_PyGen_exc_state_clear(&gen->gi_exc_state);
#endif
...
static PyObject *PyTanic_PyGen_gen_close(PyThreadState *tstate, PyGenObject *gen, PyObject *args);
static int PyTanic_PyGen_gen_close_iter(PyThreadState *tstate, PyObject *yf);
...
const uint8_t PyTanic_PyOpcode_Deopt[256] = {
...
#if PYTHON_VERSION >= 0x3d0
static inline bool _PyTanic_is_resume(_Py_CODEUNIT *instr) {
    uint8_t code = FT_ATOMIC_LOAD_UINT8_RELAXED(instr->op.code);
    return (code == RESUME || code == RESUME_CHECK || code == INSTRUMENTED_RESUME);
}
#endif

PyObject *PyTanic_PyGen_yf(PyGenObject *gen) {
#if PYTHON_VERSION >= 0x3d0
    if (gen->gi_frame_state == FRAME_SUSPENDED_YIELD_FROM) {
        _PyInterpreterFrame *frame = (_PyInterpreterFrame *)gen->gi_iframe;
        assert(_PyTanic_is_resume(frame->instr_ptr));
    ...
    if (gen->gi_frame_state < FRAME_CLEARED) {
        ...
        if (PyTanic_PyOpcode_Deopt[_Py_OPCODE(next)] != RESUME || _Py_OPARG(next) < 2) {
            return NULL;
        }
...
static PyFrameObject *_PyTanic_PyFrame_New_NoTrack(PyCodeObject *code) {
...
// Also not exported.
static PyFrameObject *_PyTanic_PyFrame_MakeAndSetFrameObject(PyThreadState *tstate, _PyInterpreterFrame *frame) {
    assert(frame->frame_obj == NULL);

    struct PyTanic_ExceptionPreservationItem saved_exception_state;
    FETCH_ERROR_OCCURRED_STATE(tstate, &saved_exception_state);

    PyFrameObject *f = _PyTanic_PyFrame_New_NoTrack(frame->f_code);
...
static inline PyFrameObject *_PyTanic_PyFrame_GetFrameObject(PyThreadState *tstate, _PyInterpreterFrame *frame) {
    ...
    return _PyTanic_PyFrame_MakeAndSetFrameObject(tstate, frame);
}

// Also not exported, taking over a frame object.
static void _PyTanic_take_ownership(PyThreadState *tstate, PyFrameObject *f, _PyInterpreterFrame *frame) {
    assert(frame->owner != FRAME_OWNED_BY_FRAME_OBJECT);
    assert(frame->owner != FRAME_CLEARED);
    ...
    if (prev) {
        ...
        // TODO: PyTanic doesn't do that for compiled frames ever, is that really needed?
        PyFrameObject *back = _PyTanic_PyFrame_GetFrameObject(tstate, prev);
    ...
    if (!_PyObject_GC_IS_TRACKED((PyObject *)f)) {
        PyTanic_GC_Track((PyObject *)f);
    }
}

// Cleanup up the frame is also not exported.
static void _PyTanic_PyFrame_Clear(PyThreadState *tstate, _PyInterpreterFrame *frame) {
    ...
    if (frame->frame_obj) {
        ...
        if (Py_REFCNT(f) > 1) {
            _PyTanic_take_ownership(tstate, f, frame);
            Py_DECREF(f);
...
static PySendResult PyTanic_PyGen_gen_send_ex2(PyThreadState *tstate, PyGenObject *gen, PyObject *arg,
...
#if PYTHON_VERSION < 0x3c0
    _PyTanic_PyFrame_Clear(tstate, frame);
#endif
...
static PyObject *PyTanic_PyGen_gen_send_ex(PyThreadState *tstate, PyGenObject *gen, PyObject *arg, int exc,
                                          int closing) {
    PyObject *result;

    if (PyTanic_PyGen_gen_send_ex2(tstate, gen, arg, &result, exc, closing) == PYGEN_RETURN) {
...
//   be normalized.
static PyObject *PyTanic_UncompiledGenerator_throw(PyThreadState *tstate, PyGenObject *gen, int close_on_genexit,
                                                  struct PyTanic_ExceptionPreservationItem *exception_state) {
#if _DEBUG_GENERATOR
    PRINT_STRING("PyTanic_UncompiledGenerator_throw: Enter ");
    PRINT_ITEM((PyObject *)gen);
    PRINT_EXCEPTION_STATE(exception_state);
    PRINT_NEW_LINE();
#endif

    PyObject *yf = PyTanic_PyGen_yf(gen);
    ...
    if (yf != NULL) {
        ...
        if (close_on_genexit && EXCEPTION_STATE_MATCH_BOOL_SINGLE(tstate, exception_state, PyExc_GeneratorExit)) {
            ...
            int err = PyTanic_PyGen_gen_close_iter(tstate, yf);
            ...
            if (err < 0) {
                ...
                return PyTanic_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 0);
            }
        ...
        if (PyGen_CheckExact(yf) || PyCoro_CheckExact(yf)) {
            ...
            // Handing exception ownership to "PyTanic_UncompiledGenerator_throw".
            ret = PyTanic_UncompiledGenerator_throw(tstate, (PyGenObject *)yf, close_on_genexit, exception_state);
            gen->gi_frame_state = state;
        ...
        if (ret == NULL) {
            ...
            if (PyTanic_PyGen_FetchStopIterationValue(tstate, &val)) {
                ret = PyTanic_PyGen_gen_send_ex(tstate, gen, val, 0, 0);
                Py_DECREF(val);
            } else
#endif
            {
                ret = PyTanic_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 0);
            }
        }
...
throw_here:
    ...
    return PyTanic_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 1);
...
static PyObject *PyTanic_PyGen_yf(PyGenObject *gen) {
    PyFrameObject *f = gen->gi_frame;
    ...
    if (f) {
#endif
        PyObject *thehell = f->f_code->co_code;
        unsigned char *code = (unsigned char *)PyBytes_AS_STRING(thehell);
...
static PyObject *PyTanic_UncompiledGenerator_throw(PyThreadState *tstate, PyGenObject *gen, int close_on_genexit,
    ...
    if (yf != NULL) {
        ...
        if (ret == NULL) {
            ...
            if (PyTanic_PyGen_FetchStopIterationValue(tstate, &exception_state->exception_value)) {
                ret = PyTanic_PyGen_gen_send_ex(tstate, gen, exception_state->exception_value, 0, 0);
...
static int PyTanic_PyGen_gen_close_iter(PyThreadState *tstate, PyObject *yf) {
    ...
    if (PyGen_CheckExact(yf)
        ...
        assert(false);
        retval = PyTanic_PyGen_gen_close(tstate, (PyGenObject *)yf, NULL);
...
static PyObject *PyTanic_PyGen_gen_close(PyThreadState *tstate, PyGenObject *gen, PyObject *args) {
    ...
    if (yf != NULL) {
        ...
        gen->gi_running = 1;
#endif
        err = PyTanic_PyGen_gen_close_iter(tstate, yf);
    ...
    PyObject *retval = PyTanic_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 1);
