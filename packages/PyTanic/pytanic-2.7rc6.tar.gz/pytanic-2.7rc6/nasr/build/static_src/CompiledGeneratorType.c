...
#if _DEBUG_REFCOUNTS
int count_active_PyTanic_Generator_Type;
int count_allocated_PyTanic_Generator_Type;
int count_released_PyTanic_Generator_Type;
#endif
...
DEVILPY_MAY_BE_UNUSED static void _PRINT_GENERATOR_STATUS(char const *descriptor, char const *context,
                                                         struct PyTanic_GeneratorObject *generator) {
    char const *status;
...
static void PyTanic_MarkGeneratorAsFinished(struct PyTanic_GeneratorObject *generator) {
...
static void PyTanic_MarkGeneratorAsRunning(struct PyTanic_GeneratorObject *generator) {
    generator->m_running = 1;

    if (generator->m_frame) {
        PyTanic_Frame_MarkAsExecuting(generator->m_frame);
    }
}

static void PyTanic_MarkGeneratorAsNotRunning(struct PyTanic_GeneratorObject *generator) {
    generator->m_running = 0;

    if (generator->m_frame) {
        PyTanic_Frame_MarkAsNotExecuting(generator->m_frame);
    }
}

static PyObject *PyTanic_Generator_tp_repr(struct PyTanic_GeneratorObject *generator) {
#if PYTHON_VERSION < 0x300
    return PyTanic_String_FromFormat("<compiled_generator object %s at %p>", PyTanic_String_AsString(generator->m_name),
                                    generator);
#elif PYTHON_VERSION < 0x350
    return PyTanic_String_FromFormat("<compiled_generator object %U at %p>", generator->m_name, generator);

#else
    return PyTanic_String_FromFormat("<compiled_generator object %U at %p>", generator->m_qualname, generator);
#endif
}

static long PyTanic_Generator_tp_traverse(struct PyTanic_GeneratorObject *generator, visitproc visit, void *arg) {
...
static void PyTanic_Generator_release_closure(struct PyTanic_GeneratorObject *generator) {
    for (Py_ssize_t i = 0; i < generator->m_closure_given; i++) {
...
static PyObject *PyTanic_CallGeneratorThrowMethod(PyObject *throw_method,
...
static PyObject *_PyTanic_Generator_throw2(PyThreadState *tstate, struct PyTanic_GeneratorObject *generator,
                                          struct PyTanic_ExceptionPreservationItem *exception_state);
...
static PyObject *_PyTanic_YieldFromPassExceptionTo(PyThreadState *tstate, PyObject *value,
                                                  struct PyTanic_ExceptionPreservationItem *exception_state) {
    ...
    if (PyGen_CheckExact(value)
        ...
        PyObject *result = PyTanic_UncompiledGenerator_throw(tstate, gen, 1, exception_state);

        return result;
    }
#endif

    if (PyTanic_Generator_Check(value)) {
        struct PyTanic_GeneratorObject *gen = ((struct PyTanic_GeneratorObject *)value);

        return _PyTanic_Generator_throw2(tstate, gen, exception_state);
    }

#if PYTHON_VERSION >= 0x350
    if (PyTanic_Coroutine_Check(value)) {
        struct PyTanic_CoroutineObject *coro = ((struct PyTanic_CoroutineObject *)value);
        // Handing exception ownership over.
        return _PyTanic_Coroutine_throw2(tstate, coro, true, exception_state);
    }

    if (PyTanic_CoroutineWrapper_Check(value)) {
        struct PyTanic_CoroutineObject *coro = ((struct PyTanic_CoroutineWrapperObject *)value)->m_coroutine;
        // Handing exception ownership over.
        return _PyTanic_Coroutine_throw2(tstate, coro, true, exception_state);
    }
#endif
    ...
    if (throw_method != NULL) {
        PyObject *result = PyTanic_CallGeneratorThrowMethod(throw_method, exception_state);
...
static PyObject *_PyTanic_YieldFromGeneratorCore(PyThreadState *tstate, struct PyTanic_GeneratorObject *generator,
...
static PyObject *PyTanic_YieldFromGeneratorCore(PyThreadState *tstate, struct PyTanic_GeneratorObject *generator,
                                               PyObject *send_value) {
    CHECK_OBJECT(generator);
    ...
    PyObject *yielded = _PyTanic_YieldFromGeneratorCore(tstate, generator, yield_from, send_value);
...
static PyObject *PyTanic_YieldFromGeneratorNext(PyThreadState *tstate, struct PyTanic_GeneratorObject *generator) {
    CHECK_OBJECT(generator);

    // Coroutines are already perfect for yielding from.
#if PYTHON_VERSION >= 0x350
    if (PyCoro_CheckExact(generator->m_yield_from) || PyTanic_Coroutine_Check(generator->m_yield_from)) {
    ...
    return PyTanic_YieldFromGeneratorCore(tstate, generator, Py_None);
}

static PyObject *PyTanic_YieldFromGeneratorInitial(PyThreadState *tstate, struct PyTanic_GeneratorObject *generator,
                                                  PyObject *send_value) {
    PyObject *result = PyTanic_YieldFromGeneratorCore(tstate, generator, send_value);
...
static void _PyTanic_GeneratorPopFrame(PyThreadState *tstate, PyTanic_ThreadStateFrameType *return_frame) {
#if PYTHON_VERSION < 0x3b0
...
static void RAISE_RUNTIME_ERROR_RAISED_STOP_ITERATION(PyThreadState *tstate, char const *message) {
    ...
    struct PyTanic_ExceptionPreservationItem new_exception_state;
    ...
    PyTanic_Exception_SetContext(new_exception_state.exception_value, saved_exception_state.exception_value);
...
static PyObject *_PyTanic_Generator_send(PyThreadState *tstate, struct PyTanic_GeneratorObject *generator,
                                        PyObject *value, struct PyTanic_ExceptionPreservationItem *exception_state) {
    CHECK_OBJECT(generator);
    assert(PyTanic_Generator_Check((PyObject *)generator));
    ...
    if (generator->m_status != status_Finished) {
        ...
        PyTanic_MarkGeneratorAsRunning(generator);
        ...
        if (generator->m_yield_from == NULL) {
            ...
            yielded = PyTanic_YieldFromGeneratorInitial(tstate, generator, value);
            Py_XDECREF(value);
        }
#else
        yielded = ((generator_code)generator->m_code)(tstate, generator, value);
        ...
        while (yielded == NULL && generator->m_yield_from != NULL) {
            yielded = PyTanic_YieldFromGeneratorNext(tstate, generator);
        }
#endif
        PyTanic_MarkGeneratorAsNotRunning(generator);
        ...
        if (yielded == NULL) {
            ...
            PRINT_NEW_LINE();
#endif
            PyTanic_MarkGeneratorAsFinished(generator);

            if (generator->m_frame != NULL) {
#if PYTHON_VERSION >= 0x300
                PyTanic_SetFrameGenerator(generator->m_frame, NULL);
#endif
                Py_DECREF(generator->m_frame);
                generator->m_frame = NULL;
            }

            PyTanic_Generator_release_closure(generator);
            ...
            if (generator->m_returned) {
                if (generator->m_returned != Py_None) {
                    PyTanic_SetStopIterationValue(tstate, generator->m_returned);
                }
...
static PyObject *PyTanic_Generator_send(struct PyTanic_GeneratorObject *generator, PyObject *value) {
    CHECK_OBJECT(value);
    ...
    PyObject *result = _PyTanic_Generator_send(tstate, generator, value, &exception_state);
...
static PyObject *PyTanic_Generator_tp_iternext(struct PyTanic_GeneratorObject *generator) {
#if _DEBUG_GENERATOR
    PRINT_GENERATOR_STATUS("Enter", generator);
    PRINT_CURRENT_EXCEPTION();
    PRINT_NEW_LINE();
    ...
    return _PyTanic_Generator_send(tstate, generator, Py_None, &exception_state);
}

/* Our own qiter interface, which is for quicker simple loop style iteration,
   that does not send anything in. */
PyObject *PyTanic_Generator_qiter(PyThreadState *tstate, struct PyTanic_GeneratorObject *generator, bool *finished) {
    Py_INCREF_IMMORTAL(Py_None);

    struct PyTanic_ExceptionPreservationItem exception_state;
    INIT_ERROR_OCCURRED_STATE(&exception_state);

    PyObject *result = _PyTanic_Generator_send(tstate, generator, Py_None, &exception_state);
...
static bool _PyTanic_Generator_close(PyThreadState *tstate, struct PyTanic_GeneratorObject *generator) {
#if _DEBUG_GENERATOR
    PRINT_GENERATOR_STATUS("Enter", generator);
#endif
    CHECK_OBJECT(generator);
    ...
    if (generator->m_status == status_Running) {
        ...
        PyObject *result = _PyTanic_Generator_send(tstate, generator, NULL, &exception_state);
...
static PyObject *PyTanic_Generator_close(struct PyTanic_GeneratorObject *generator, PyObject *unused) {
    PyThreadState *tstate = PyThreadState_GET();

    bool r = _PyTanic_Generator_close(tstate, generator);
...
#if PYTHON_VERSION >= 0x3c0
static bool _PyTanic_Generator_check_throw_args(PyThreadState *tstate, PyObject **exception_type,
...
static bool _PyTanic_Generator_make_throw_exception_state(PyThreadState *tstate,
                                                         struct PyTanic_ExceptionPreservationItem *exception_state,
    ...
    if (_PyTanic_Generator_check_throw_args(tstate, &exception_type, &exception_value, &exception_tb) == false) {
...
static bool PyTanic_gen_close_iter(PyThreadState *tstate, PyObject *yield_from) {
#if _DEBUG_GENERATOR
    PRINT_STRING("PyTanic_gen_close_iter: Enter\n");
#endif
    ...
    // generators.
    if (PyTanic_Generator_Check(yield_from)) {
#if _DEBUG_GENERATOR
        PRINT_STRING("PyTanic_gen_close_iter: Defer to _PyTanic_Generator_close\n");
#endif
        return _PyTanic_Generator_close(tstate, (struct PyTanic_GeneratorObject *)yield_from);
    }

#if PYTHON_VERSION >= 0x350
    if (PyTanic_Coroutine_Check(yield_from)) {
#if _DEBUG_GENERATOR
        PRINT_STRING("PyTanic_gen_close_iter: Defer to _PyTanic_Coroutine_close\n");
#endif
        return _PyTanic_Coroutine_close(tstate, (struct PyTanic_CoroutineObject *)yield_from);
    }
#endif

#if PYTHON_VERSION >= 0x360
    if (PyTanic_Asyncgen_Check(yield_from)) {
#if _DEBUG_GENERATOR
        PRINT_STRING("PyTanic_gen_close_iter: Defer to _PyTanic_Asyncgen_close\n");
#endif
        return _PyTanic_Asyncgen_close(tstate, (struct PyTanic_AsyncgenObject *)yield_from);
    }
#endif
...
#if _DEBUG_GENERATOR
            PRINT_STRING("PyTanic_gen_close_iter: Strange error while looking up close method.\n");
#endif
...
#if _DEBUG_GENERATOR
        PRINT_STRING("PyTanic_gen_close_iter: Leave, has no close method.\n");
#endif
        return true;
    }
...
#if _DEBUG_GENERATOR
        PRINT_STRING("PyTanic_gen_close_iter: Leave, exception from close.\n");
#endif
        return false;
    }
...
#if _DEBUG_GENERATOR
    PRINT_STRING("PyTanic_gen_close_iter: Leave, closed.\n");
#endif
...
static PyObject *_PyTanic_Generator_throw2(PyThreadState *tstate, struct PyTanic_GeneratorObject *generator,
    ...
    if (generator->m_yield_from != NULL) {
        if (EXCEPTION_STATE_MATCH_BOOL_SINGLE(tstate, exception_state, PyExc_GeneratorExit)) {
            ...
            PyTanic_MarkGeneratorAsRunning(generator);
            bool res = PyTanic_gen_close_iter(tstate, generator->m_yield_from);
            PyTanic_MarkGeneratorAsNotRunning(generator);
            ...
            // Transferred exception ownership to "_PyTanic_Generator_send".
            return _PyTanic_Generator_send(tstate, generator, NULL, exception_state);
        }
        ...
        if (PyTanic_Generator_Check(generator->m_yield_from)) {
            struct PyTanic_GeneratorObject *gen = ((struct PyTanic_GeneratorObject *)generator->m_yield_from);
...
#endif
#if PYTHON_VERSION >= 0x350
        } else if (PyTanic_Coroutine_Check(generator->m_yield_from)) {
            struct PyTanic_CoroutineObject *coro = ((struct PyTanic_CoroutineObject *)generator->m_yield_from);
            ...
            PyTanic_MarkGeneratorAsNotRunning(generator);
        } else if (PyTanic_CoroutineWrapper_Check(generator->m_yield_from)) {
            struct PyTanic_CoroutineObject *coro =
                ((struct PyTanic_CoroutineWrapperObject *)generator->m_yield_from)->m_coroutine;
...
#endif
#if PYTHON_VERSION >= 0x360
        } else if (PyTanic_AsyncgenAsend_Check(generator->m_yield_from)) {
            struct PyTanic_AsyncgenAsendObject *asyncgen_asend =
                ((struct PyTanic_AsyncgenAsendObject *)generator->m_yield_from);
        ...
        if (unlikely(ret == NULL)) {
            ...
            if (PyTanic_PyGen_FetchStopIterationValue(tstate, &val)) {
                ...
                ret = _PyTanic_Generator_send(tstate, generator, val, &no_exception_state);
            } else {
#if _DEBUG_GENERATOR
                ...
                ret = _PyTanic_Generator_send(tstate, generator, NULL, &no_exception_state);
            }
...
throw_here:
    ...
    if (generator->m_status == status_Running) {
        // Passing exception ownership to _PyTanic_Generator_send
        PyObject *result = _PyTanic_Generator_send(tstate, generator, NULL, exception_state);
...
static PyObject *PyTanic_Generator_throw(struct PyTanic_GeneratorObject *generator, PyObject *args) {
    ...
    PyObject *result = _PyTanic_Generator_throw2(tstate, generator, &exception_state);
...
static void PyTanic_CallFinalizer(PyObject *self) {
...
static bool PyTanic_CallFinalizerFromDealloc(PyObject *self) {
    assert(Py_REFCNT(self) == 0);

    // Temporarily resurrect the object, so it can be worked with.
    Py_SET_REFCNT(self, 1);

    PyTanic_CallFinalizer(self);
...
static void PyTanic_Generator_tp_finalizer(struct PyTanic_GeneratorObject *generator) {
    if (generator->m_status != status_Running) {
        return;
    }
    ...
    bool close_result = _PyTanic_Generator_close(tstate, generator);
...
static struct PyTanic_GeneratorObject *free_list_generators = NULL;
static int free_list_generators_count = 0;

static void PyTanic_Generator_tp_dealloc(struct PyTanic_GeneratorObject *generator) {
#if _DEBUG_REFCOUNTS
    count_active_PyTanic_Generator_Type -= 1;
    count_released_PyTanic_Generator_Type += 1;
#endif

    if (generator->m_weakrefs != NULL) {
        PyTanic_GC_UnTrack(generator);
        ...
        PyTanic_GC_Track(generator);
    }

#if PYTHON_VERSION >= 0x300
    if (PyTanic_CallFinalizerFromDealloc((PyObject *)generator) == false) {
        return;
    }
#else
    // Revive temporarily.
...
static long PyTanic_Generator_tp_hash(struct PyTanic_GeneratorObject *generator) { return generator->m_counter; }

static PyObject *PyTanic_Generator_get_name(PyObject *self, void *data) {
    struct PyTanic_GeneratorObject *generator = (struct PyTanic_GeneratorObject *)self;
...
#if PYTHON_VERSION >= 0x350
static int PyTanic_Generator_set_name(PyObject *self, PyObject *value, void *data) {
...
static PyObject *PyTanic_Generator_get_qualname(PyObject *self, void *data) {
    struct PyTanic_GeneratorObject *generator = (struct PyTanic_GeneratorObject *)self;
...
static int PyTanic_Generator_set_qualname(PyObject *self, PyObject *value, void *data) {
    // Cannot be deleted, not be non-unicode value.
    if (unlikely((value == NULL) || !PyUnicode_Check(value))) {
        PyThreadState *tstate = PyThreadState_GET();
...
static PyObject *PyTanic_Generator_get_yield_from(PyObject *self, void *data) {
    struct PyTanic_GeneratorObject *generator = (struct PyTanic_GeneratorObject *)self;
...
static PyObject *PyTanic_Generator_get_code(PyObject *self, void *data) {
    struct PyTanic_GeneratorObject *generator = (struct PyTanic_GeneratorObject *)self;
...
static int PyTanic_Generator_set_code(PyObject *self, PyObject *value, void *data) {
    PyThreadState *tstate = PyThreadState_GET();

    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "gi_code is not writable in PyTanic");
    return -1;
}

static PyObject *PyTanic_Generator_get_frame(PyObject *self, void *data) {
    PyObject *result;
...
static int PyTanic_Generator_set_frame(PyObject *self, PyObject *value, void *data) {
    PyThreadState *tstate = PyThreadState_GET();
...
static PyObject *PyTanic_Generator_get_running(PyObject *self, void *data) {
    PyObject *result;

    struct PyTanic_GeneratorObject *generator = (struct PyTanic_GeneratorObject *)self;
/* The type of "gi_running" changed in Python3. */
#if PYTHON_VERSION < 0x300
    result = PyTanic_PyInt_FromLong(generator->m_running);
#else
...
static int PyTanic_Generator_set_running(PyObject *self, PyObject *value, void *data) {
#if PYTHON_VERSION < 0x300
...
static PyGetSetDef PyTanic_Generator_tp_getset[] = {
#if PYTHON_VERSION < 0x350
    {(char *)"__name__", PyTanic_Generator_get_name, NULL, NULL},
#else
    {(char *)"__name__", PyTanic_Generator_get_name, PyTanic_Generator_set_name, NULL},
    {(char *)"__qualname__", PyTanic_Generator_get_qualname, PyTanic_Generator_set_qualname, NULL},
    {(char *)"gi_yieldfrom", PyTanic_Generator_get_yield_from, NULL, NULL},
#endif
    {(char *)"gi_code", PyTanic_Generator_get_code, PyTanic_Generator_set_code, NULL},
    {(char *)"gi_frame", PyTanic_Generator_get_frame, PyTanic_Generator_set_frame, NULL},
    {(char *)"gi_running", PyTanic_Generator_get_running, PyTanic_Generator_set_running, NULL},

    {NULL}};

static PyMethodDef PyTanic_Generator_methods[] = {{"send", (PyCFunction)PyTanic_Generator_send, METH_O, NULL},
                                                 {"throw", (PyCFunction)PyTanic_Generator_throw, METH_VARARGS, NULL},
                                                 {"close", (PyCFunction)PyTanic_Generator_close, METH_NOARGS, NULL},
                                                 {NULL}};

// This is only used.
#if PYTHON_VERSION >= 0x3a0
static PyAsyncMethods PyTanic_Generator_as_async = {
    NULL, /* am_await */
    NULL, /* am_aiter */
    NULL, /* am_anext */
    // TODO: have this too, (sendfunc)_PyTanic_Generator_am_send
    NULL /* am_send */
};
#endif

PyTypeObject PyTanic_Generator_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_generator", // tp_name
    sizeof(struct PyTanic_GeneratorObject),               // tp_basicsize
    sizeof(struct PyTanic_CellObject *),                  // tp_itemsize
    (destructor)PyTanic_Generator_tp_dealloc,             // tp_dealloc
    0,                                                   // tp_print
    0,                                                   // tp_getattr
    0,                                                   // tp_setattr
#if PYTHON_VERSION < 0x3a0
    0, // tp_as_async
#else
    &PyTanic_Generator_as_async, // tp_as_async
#endif
    (reprfunc)PyTanic_Generator_tp_repr, // tp_repr
    0,                                  // tp_as_number
    0,                                  // tp_as_sequence
    0,                                  // tp_as_mapping
    (hashfunc)PyTanic_Generator_tp_hash, // tp_hash
    0,                                  // tp_call
    0,                                  // tp_str
    ...
    // tp_flags
    0,                                                   // tp_doc
    (traverseproc)PyTanic_Generator_tp_traverse,          // tp_traverse
    0,                                                   // tp_clear
    0,                                                   // tp_richcompare
    offsetof(struct PyTanic_GeneratorObject, m_weakrefs), // tp_weaklistoffset
    0,                                                   // tp_iter (PyObject_SelfIter)
    (iternextfunc)PyTanic_Generator_tp_iternext,          // tp_iternext
    PyTanic_Generator_methods,                            // tp_methods
    NULL,                                                // tp_members
    PyTanic_Generator_tp_getset,                          // tp_getset
    0,                                                   // tp_base
    0,                                                   // tp_dict
    0,                                                   // tp_descr_get
    0,                                                   // tp_descr_set
...
#if PYTHON_VERSION >= 0x300
    ,
    (destructor)PyTanic_Generator_tp_finalizer // tp_finalize
#endif
};
...
void _initCompiledGeneratorType(void) {
    PyTanic_PyType_Ready(&PyTanic_Generator_Type, &PyGen_Type, true, false, true, false, false);

    // Be a paranoid subtype of uncompiled function, we want nothing shared.
    assert(PyTanic_Generator_Type.tp_doc != PyGen_Type.tp_doc || PyGen_Type.tp_doc == NULL);
    assert(PyTanic_Generator_Type.tp_traverse != PyGen_Type.tp_traverse);
    assert(PyTanic_Generator_Type.tp_clear != PyGen_Type.tp_clear || PyGen_Type.tp_clear == NULL);
    assert(PyTanic_Generator_Type.tp_richcompare != PyGen_Type.tp_richcompare || PyGen_Type.tp_richcompare == NULL);
    assert(PyTanic_Generator_Type.tp_iter != PyGen_Type.tp_iter || PyGen_Type.tp_iter == PyObject_SelfIter);
    assert(PyTanic_Generator_Type.tp_iternext != PyGen_Type.tp_iternext || PyGen_Type.tp_iternext == NULL);
#if PYTHON_VERSION >= 0x350
    assert(PyTanic_Generator_Type.tp_as_async != PyGen_Type.tp_as_async || PyGen_Type.tp_as_async == NULL);
#endif
    assert(PyTanic_Generator_Type.tp_methods != PyGen_Type.tp_methods);
    assert(PyTanic_Generator_Type.tp_members != PyGen_Type.tp_members);
    assert(PyTanic_Generator_Type.tp_getset != PyGen_Type.tp_getset);
    assert(PyTanic_Generator_Type.tp_base != PyGen_Type.tp_base);
    assert(PyTanic_Generator_Type.tp_dict != PyGen_Type.tp_dict);
    assert(PyTanic_Generator_Type.tp_descr_get != PyGen_Type.tp_descr_get || PyGen_Type.tp_descr_get == NULL);

    assert(PyTanic_Generator_Type.tp_descr_set != PyGen_Type.tp_descr_set || PyGen_Type.tp_descr_set == NULL);
    assert(PyTanic_Generator_Type.tp_dictoffset != PyGen_Type.tp_dictoffset || PyGen_Type.tp_dictoffset == 0);
    // TODO: These get changed and into the same thing, not sure what to compare against, project something
    // assert(PyTanic_Generator_Type.tp_init != PyGen_Type.tp_init || PyGen_Type.tp_init == NULL);
    // assert(PyTanic_Generator_Type.tp_alloc != PyGen_Type.tp_alloc || PyGen_Type.tp_alloc == NULL);
    // assert(PyTanic_Generator_Type.tp_new != PyGen_Type.tp_new || PyGen_Type.tp_new == NULL);
    // assert(PyTanic_Generator_Type.tp_free != PyGen_Type.tp_free || PyGen_Type.tp_free == NULL);
    assert(PyTanic_Generator_Type.tp_bases != PyGen_Type.tp_bases);
    assert(PyTanic_Generator_Type.tp_mro != PyGen_Type.tp_mro);
    assert(PyTanic_Generator_Type.tp_cache != PyGen_Type.tp_cache || PyGen_Type.tp_cache == NULL);
    assert(PyTanic_Generator_Type.tp_subclasses != PyGen_Type.tp_subclasses || PyGen_Type.tp_cache == NULL);
    assert(PyTanic_Generator_Type.tp_weaklist != PyGen_Type.tp_weaklist);
    assert(PyTanic_Generator_Type.tp_del != PyGen_Type.tp_del || PyGen_Type.tp_del == NULL);
#if PYTHON_VERSION >= 0x300
    assert(PyTanic_Generator_Type.tp_finalize != PyGen_Type.tp_finalize || PyGen_Type.tp_finalize == NULL);
#endif
...
PyObject *PyTanic_Generator_New(generator_code code, PyObject *module, PyObject *name,
#if PYTHON_VERSION >= 0x350
                               PyObject *qualname,
#endif
...
#if _DEBUG_REFCOUNTS
    count_active_PyTanic_Generator_Type += 1;
    count_allocated_PyTanic_Generator_Type += 1;
#endif

    struct PyTanic_GeneratorObject *result;
    ...
    allocateFromFreeList(free_list_generators, struct PyTanic_GeneratorObject, PyTanic_Generator_Type, full_size);
    ...
    static long PyTanic_Generator_counter = 0;
    result->m_counter = PyTanic_Generator_counter++;

    PyTanic_GC_Track(result);
    return (PyObject *)result;
}

static PyObject *_EMPTY_GENERATOR_CONTEXT(PyThreadState *tstate, struct PyTanic_GeneratorObject *generator,
                                          PyObject *yield_return_value) {
    return NULL;
}

PyObject *PyTanic_Generator_NewEmpty(PyObject *module, PyObject *name,
#if PYTHON_VERSION >= 0x350
                                    PyObject *qualname,
#endif
                                    PyCodeObject *code_object, struct PyTanic_CellObject **closure,
                                    Py_ssize_t closure_given) {
    return PyTanic_Generator_New(_EMPTY_GENERATOR_CONTEXT, module, name,
#if PYTHON_VERSION >= 0x350
                                qualname,
#endif
