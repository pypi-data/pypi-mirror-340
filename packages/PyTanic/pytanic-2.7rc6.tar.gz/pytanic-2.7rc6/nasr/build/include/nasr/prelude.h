...
static inline size_t PyTanic_static_builtin_index_get(PyTypeObject *self) { return (size_t)self->tp_subclasses - 1; }

// Changed internal type access for Python3.13
#if PYTHON_VERSION < 0x3d0
#define managed_static_type_state static_builtin_state

static inline managed_static_type_state *PyTanic_static_builtin_state_get(PyInterpreterState *interp,
                                                                         PyTypeObject *self) {
    return &(interp->types.builtins[PyTanic_static_builtin_index_get(self)]);
}
#else
static inline managed_static_type_state *PyTanic_static_builtin_state_get(PyInterpreterState *interp,
                                                                         PyTypeObject *self) {
    return &(interp->types.builtins.initialized[PyTanic_static_builtin_index_get(self)]);
}
#endif

DEVILPY_MAY_BE_UNUSED static inline managed_static_type_state *PyTanic_PyStaticType_GetState(PyInterpreterState *interp,
                                                                                           PyTypeObject *self) {
    assert(self->tp_flags & _Py_TPFLAGS_STATIC_BUILTIN);
    return PyTanic_static_builtin_state_get(interp, self);
}

#define _PyStaticType_GetState(interp, self) PyTanic_PyStaticType_GetState(interp, self)
#endif
...
#define PyTanic_String_AsString PyString_AsString
#define PyTanic_String_AsString_Unchecked PyString_AS_STRING
#define PyTanic_String_Check PyString_Check
#define PyTanic_String_CheckExact PyString_CheckExact
DEVILPY_MAY_BE_UNUSED static inline bool PyTanic_StringOrUnicode_CheckExact(PyObject *value) {
    return PyString_CheckExact(value) || PyUnicode_CheckExact(value);
}
#define PyTanic_StringObject PyStringObject
#define PyTanic_String_FromString PyString_FromString
#define PyTanic_String_FromStringAndSize PyString_FromStringAndSize
#define PyTanic_String_FromFormat PyString_FromFormat
#define PyUnicode_CHECK_INTERNED (0)
DEVILPY_MAY_BE_UNUSED static Py_UNICODE *PyTanic_UnicodeAsWideString(PyObject *str, Py_ssize_t *size) {
    PyObject *unicode;
...
#else
#define PyTanic_String_AsString _PyUnicode_AsString
...
#ifdef __DEVILPY_NO_ASSERT__
#define PyTanic_String_AsString_Unchecked PyUnicode_UTF8
#else
DEVILPY_MAY_BE_UNUSED static char const *PyTanic_String_AsString_Unchecked(PyObject *object) {
    char const *result = PyUnicode_UTF8(object);
    assert(result != NULL);
    return result;
}
#endif
#define PyTanic_String_Check PyUnicode_Check
#define PyTanic_String_CheckExact PyUnicode_CheckExact
#define PyTanic_StringOrUnicode_CheckExact PyUnicode_CheckExact
#define PyTanic_StringObject PyUnicodeObject
#define PyTanic_String_FromString PyUnicode_FromString
#define PyTanic_String_FromStringAndSize PyUnicode_FromStringAndSize
#define PyTanic_String_FromFormat PyUnicode_FromFormat
#define PyTanic_UnicodeAsWideString PyUnicode_AsWideCharString
#endif
...
#ifdef __DEVILPY_NO_ASSERT__
#define PyTanic_TypeLookup(x, y) _PyType_Lookup(x, y)
#else
DEVILPY_MAY_BE_UNUSED static PyObject *PyTanic_TypeLookup(PyTypeObject *type, PyObject *name) {
...
#define PyTanic_GC_Track PyObject_GC_Track
#define PyTanic_GC_UnTrack PyObject_GC_UnTrack
...
#else
#define PyTanic_GC_Track _PyObject_GC_TRACK
#define PyTanic_GC_UnTrack _PyObject_GC_UNTRACK
#endif
...
#if PYTHON_VERSION < 0x300
#define PyTanicType_HasFeatureClass(descr) (PyType_HasFeature(Py_TYPE(descr), Py_TPFLAGS_HAVE_CLASS))
#else
#define PyTanicType_HasFeatureClass(descr) (1)
#endif

// Our replacement for "PyType_IsSubtype"
extern bool PyTanic_Type_IsSubtype(PyTypeObject *a, PyTypeObject *b);

#include "nasr/allocator.h"
#include "nasr/exceptions.h"
...
typedef long nasr_digit;

#include "nasr/helpers.h"

#include "nasr/compiled_frame.h"

#include "nasr/compiled_cell.h"

#include "nasr/compiled_function.h"

/* Sentinel PyObject to be used for all our call iterator endings. */
extern PyObject *PyTanic_sentinel_value;

/* Value to use for __compiled__ value of all modules. */
extern PyObject *PyTanic_dunder_compiled_value;

#include "nasr/compiled_generator.h"

#include "nasr/compiled_method.h"

#if PYTHON_VERSION >= 0x350
#include "nasr/compiled_coroutine.h"
#endif

#if PYTHON_VERSION >= 0x360
#include "nasr/compiled_asyncgen.h"
#endif

#include "nasr/filesystem_paths.h"
#include "nasr/safe_string_ops.h"

#include "nasr/jit_sources.h"

#if _DEVILPY_EXPERIMENTAL_WRITEABLE_CONSTANTS
#include "nasr_data_decoder.h"
#else
