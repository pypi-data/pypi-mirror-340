...
// PyTanic frames.
#if PYTHON_VERSION < 0x3b0
typedef PyFrameObject PyTanic_ThreadStateFrameType;
#else
typedef _PyInterpreterFrame PyTanic_ThreadStateFrameType;
#endif
...
extern void PRINT_COMPILED_FRAME(char const *prefix, struct PyTanic_FrameObject *frame);
extern void PRINT_INTERPRETER_FRAME(char const *prefix, PyTanic_ThreadStateFrameType *frame);
#else
...
extern struct PyTanic_FrameObject *MAKE_MODULE_FRAME(PyCodeObject *code, PyObject *module);
extern struct PyTanic_FrameObject *MAKE_FUNCTION_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
                                                      Py_ssize_t locals_size);
extern struct PyTanic_FrameObject *MAKE_CLASS_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
...
#else
    // Starting for Python3.11, we just proper thehell that raises
...
extern PyTypeObject PyTanic_Frame_Type;

static inline bool PyTanic_Frame_CheckExact(PyObject *object) {
    CHECK_OBJECT(object);
    return Py_TYPE(object) == &PyTanic_Frame_Type;
}

static inline bool PyTanic_Frame_Check(PyObject *object) {
    assert(object);
    ...
    if (PyTanic_Frame_CheckExact(object)) {
        return true;
    }
...
DEVILPY_MAY_BE_UNUSED static inline bool isFrameUnusable(struct PyTanic_FrameObject *frame_object) {
...
#if PYTHON_VERSION >= 0x3b0
inline static PyCodeObject *PyTanic_InterpreterFrame_GetCodeObject(_PyInterpreterFrame *frame) {
...
inline static PyCodeObject *PyTanic_Frame_GetCodeObject(PyFrameObject *frame) {
#if PYTHON_VERSION >= 0x3b0
    assert(frame->f_frame);
    return PyTanic_InterpreterFrame_GetCodeObject(frame->f_frame);
#else
    return frame->f_code;
#endif
}
...
inline static void assertPythonFrameObject(PyFrameObject *frame_object) {
    ...
    CHECK_CODE_OBJECT(PyTanic_Frame_GetCodeObject(frame_object));
}

inline static void assertFrameObject(struct PyTanic_FrameObject *frame_object) {
    CHECK_OBJECT(frame_object);
...
inline static void assertThreadFrameObject(PyTanic_ThreadStateFrameType *frame) {
#if PYTHON_VERSION < 0x3b0
...
static inline void PyTanic_PythonFrame_MarkAsExecuting(PyFrameObject *frame) {
...
static inline void PyTanic_Frame_MarkAsExecuting(struct PyTanic_FrameObject *frame) {
    CHECK_OBJECT(frame);
#if PYTHON_VERSION >= 0x3b0
...
#else
#define PyTanic_Frame_MarkAsExecuting(frame) ;
#endif

#if PYTHON_VERSION >= 0x300
static inline void PyTanic_Frame_MarkAsNotExecuting(struct PyTanic_FrameObject *frame) {
    CHECK_OBJECT(frame);
#if PYTHON_VERSION >= 0x3b0
...
#else
#define PyTanic_Frame_MarkAsNotExecuting(frame) ;
#define PyTanic_PythonFrame_MarkAsExecuting(frame) ;
#endif

#if PYTHON_VERSION >= 0x300
static inline bool PyTanic_Frame_IsExecuting(struct PyTanic_FrameObject *frame) {
    CHECK_OBJECT(frame);
#if PYTHON_VERSION >= 0x3b0
...
DEVILPY_MAY_BE_UNUSED inline static void pushFrameStackPythonFrame(PyThreadState *tstate, PyFrameObject *frame_object) {
    ...
    if (old) {
        ...
        CHECK_CODE_OBJECT(PyTanic_Frame_GetCodeObject(old));
    }
    ...
    PyTanic_PythonFrame_MarkAsExecuting(frame_object);
...
DEVILPY_MAY_BE_UNUSED inline static void pushFrameStackCompiledFrame(PyThreadState *tstate,
    ...
    PyTanic_Frame_MarkAsExecuting(frame_object);
    Py_INCREF(frame_object);
#endif
}
...
DEVILPY_MAY_BE_UNUSED inline static void popFrameStack(PyThreadState *tstate) {
...
#if PYTHON_VERSION < 0x3b0
    struct PyTanic_FrameObject *frame_object = (struct PyTanic_FrameObject *)(tstate->frame);
    CHECK_OBJECT(frame_object);

#if _DEBUG_FRAME
    printf("Taking off frame %s %s\n", PyTanic_String_AsString(PyObject_Str((PyObject *)frame_object)),
    ...
    PyTanic_Frame_MarkAsNotExecuting(frame_object);
    Py_DECREF(frame_object);
#else
    assert(CURRENT_TSTATE_INTERPRETER_FRAME(tstate));

    struct PyTanic_FrameObject *frame_object =
        (struct PyTanic_FrameObject *)CURRENT_TSTATE_INTERPRETER_FRAME(tstate)->frame_obj;
    CHECK_OBJECT(frame_object);
...
#if PYTHON_VERSION >= 0x300
DEVILPY_MAY_BE_UNUSED static void PyTanic_SetFrameGenerator(struct PyTanic_FrameObject *nasr_frame,
                                                          PyObject *generator) {
#if PYTHON_VERSION < 0x3b0
    nasr_frame->m_frame.f_gen = generator;
#else
    nasr_frame->m_generator = generator;
#endif

    // Mark the frame as executing
    if (generator) {
        PyTanic_Frame_MarkAsExecuting(nasr_frame);
    }
}
#endif

DEVILPY_MAY_BE_UNUSED static PyCodeObject *PyTanic_GetFrameCodeObject(struct PyTanic_FrameObject *nasr_frame) {
#if PYTHON_VERSION < 0x3b0
    return nasr_frame->m_frame.f_code;
#else
    return PyTanic_InterpreterFrame_GetCodeObject(&nasr_frame->m_interpreter_frame);
#endif
}

DEVILPY_MAY_BE_UNUSED static int PyTanic_GetFrameLineNumber(struct PyTanic_FrameObject *nasr_frame) {
    return nasr_frame->m_frame.f_lineno;
}

DEVILPY_MAY_BE_UNUSED static PyObject **PyTanic_GetCodeVarNames(PyCodeObject *code_object) {
#if PYTHON_VERSION < 0x3b0
...
extern void PyTanic_Frame_AttachLocals(struct PyTanic_FrameObject *frame, char const *type_description, ...);

DEVILPY_MAY_BE_UNUSED static PyTanic_ThreadStateFrameType *_PyTanic_GetThreadStateFrame(PyThreadState *tstate) {
#if PYTHON_VERSION < 0x3b0
    return tstate->frame;
#else
...
DEVILPY_MAY_BE_UNUSED inline static void pushFrameStackGenerator(PyThreadState *tstate,
                                                                PyTanic_ThreadStateFrameType *frame_object) {
#if PYTHON_VERSION < 0x3b0
...
// Nasr used for type_description.
...
#if _DEBUG_REFCOUNTS
extern int count_active_PyTanic_Frame_Type;
extern int count_allocated_PyTanic_Frame_Type;
extern int count_released_PyTanic_Frame_Type;
#endif
