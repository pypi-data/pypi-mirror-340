    ...
    if (free_list != NULL) {                                                                                           \
        ...
        if (Py_SIZE(result) < size) {                                                                                  \
            ...
            assert(result != NULL);                                                                                    \
                                                                                                                       ...
                                                                                                                       \
        PyTanic_Py_NewReference((PyObject *)result);                                                                    \
    } else {                                                                                                           \
        result = (object_type *)PyTanic_GC_NewVar(&type_type, size);                                                    \
    }                                                                                                                  \
    CHECK_OBJECT(result);
...
#define allocateFromFreeListFixed(free_list, object_type, type_type)                                                   \
    if (free_list != NULL) {                                                                                           \
        ...
        result = (object_type *)PyTanic_GC_New(&type_type);                                                             \
    }                                                                                                                  \
    CHECK_OBJECT(result);
...
DEVILPY_MAY_BE_UNUSED static inline struct _Py_object_freelists *_PyTanic_object_freelists_GET(PyThreadState *tstate) {
