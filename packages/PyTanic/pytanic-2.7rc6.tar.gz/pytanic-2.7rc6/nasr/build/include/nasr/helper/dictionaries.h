...
typedef PyDictEntry *PyTanic_DictEntryHandle;

static PyDictEntry *GET_STRING_DICT_ENTRY(PyDictObject *dict, PyTanic_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(PyTanic_String_CheckExact(key));
...
DEVILPY_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(PyTanic_DictEntryHandle handle) { return handle->me_value; }

DEVILPY_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(PyTanic_DictEntryHandle handle, PyObject *value) {
    handle->me_value = value;
}

static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, PyTanic_StringObject *key) {
...
typedef PyObject **PyTanic_DictEntryHandle;

#if PYTHON_VERSION >= 0x3b0
extern Py_ssize_t PyTanic_Py_unicodekeys_lookup_unicode(PyDictKeysObject *dk, PyObject *key, Py_hash_t hash);

extern Py_ssize_t PyTanic_PyDictLookup(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
extern Py_ssize_t PyTanic_PyDictLookupStr(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
#endif

static PyTanic_DictEntryHandle GET_STRING_DICT_ENTRY(PyDictObject *dict, PyTanic_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(PyTanic_String_CheckExact(key));
    ...
    // We cannot use PyTanic_PyDictLookupStr in all cases, some modules misbehave
    // and put non-strings in module dictionaries.
    if (unlikely(dict->ma_keys->dk_kind == DICT_KEYS_GENERAL)) {
        found = PyTanic_PyDictLookup(dict, (PyObject *)key, hash, &value);
    } else {
        found = PyTanic_PyDictLookupStr(dict, (PyObject *)key, hash, &value);
    }
...
DEVILPY_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(PyTanic_DictEntryHandle handle) { return *handle; }

DEVILPY_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(PyTanic_DictEntryHandle handle, PyObject *value) {
    *handle = value;
}

DEVILPY_MAY_BE_UNUSED static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, PyTanic_StringObject *key) {
    PyTanic_DictEntryHandle handle = GET_STRING_DICT_ENTRY(dict, key);
...
extern uint64_t nasr_dict_version_tag_counter;
#endif

DEVILPY_MAY_BE_UNUSED static void UPDATE_STRING_DICT0(PyDictObject *dict, PyTanic_StringObject *key, PyObject *value) {
    CHECK_OBJECT(value);

    PyTanic_DictEntryHandle entry = GET_STRING_DICT_ENTRY(dict, key);
...
#if _DEVILPY_MAINTAIN_DICT_VERSION_TAG
            dict->ma_version_tag = nasr_dict_version_tag_counter++;
#endif
            CHECK_OBJECT(old);
...
DEVILPY_MAY_BE_UNUSED static void UPDATE_STRING_DICT_INPLACE(PyDictObject *dict, PyTanic_StringObject *key,
                                                            PyObject *value) {
    CHECK_OBJECT(value);
...
DEVILPY_MAY_BE_UNUSED static void UPDATE_STRING_DICT1(PyDictObject *dict, PyTanic_StringObject *key, PyObject *value) {
    CHECK_OBJECT(key);
    CHECK_OBJECT(value);
...
extern bool PyTanic_DictNext(PyObject *dict, Py_ssize_t *pos, PyObject **key_ptr, PyObject **value_ptr);
