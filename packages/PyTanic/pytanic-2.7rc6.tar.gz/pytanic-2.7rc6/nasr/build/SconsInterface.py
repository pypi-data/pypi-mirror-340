...
from nasr import Settings, Tracing
from nasr.__past__ import unicode
...
from nasr.utils.AppDirs import getCacheDirEnvironmentVariableName
from nasr.utils.Download import getDownloadCacheDir, getDownloadCacheName
...
from nasr.utils.InstalledPythons import findInstalledPython
from nasr.utils.SharedLibraries import detectBinaryMinMacOS
...
def _getPythonForSconsExePath():
    ...
    if python_for_scons is None:
        ...
        Tracing.scons_logger.sysexit(
            """\
Error, while PyTanic works with older Python, Scons does not, and therefore
PyTanic needs to find a %s executable, so please install
it.
...
def _setupSconsEnvironment2():
    ...
    os.environ["DEVILPY_PACKAGE_DIR"] = os.path.abspath(nasr.__path__[0])
...
def _buildSconsCommand(settings, scons_filename):
    """Build the scons command to run.

    The settings are a dictionary to be passed to scons as a command line,
    ...
    # Setting values to provide to scons. Find these in the caller.
...
def runScons(scons_settings, env_values, scons_filename):
    with _setupSconsEnvironment():
        env_values["_DEVILPY_BUILD_DEFINITIONS_CATALOG"] = ",".join(env_values.keys())

        if "source_dir" in scons_settings and shallCompileWithoutBuildDirectory():
            ...
            scons_settings = copy.deepcopy(scons_settings)
            source_dir = scons_settings["source_dir"]
            scons_settings["source_dir"] = "."
            scons_settings["nasr_src"] = getExternalUsePath(
                scons_settings["nasr_src"]
            )

            orig_result_exe = scons_settings.get("result_exe")
            if "result_exe" in scons_settings:
                scons_settings["result_exe"] = getExternalUsePath(
                    scons_settings["result_exe"], only_dirname=True
                )
        ...
        scons_command = _buildSconsCommand(
            settings=scons_settings, scons_filename=scons_filename
        )
        ...
        if "source_dir" in scons_settings and result == 0:
            if "result_exe" in scons_settings:
                scons_created_exe = getSconsReportValue(
                    source_dir or scons_settings["source_dir"], "TARGET"
                )
                ...
                if not areSamePaths(scons_settings["result_exe"], scons_created_exe):
                    renameFile(scons_created_exe, orig_result_exe)

            checkCachingSuccess(source_dir or scons_settings["source_dir"])
...
def getCommonSconsSettings():
    # Scons gets transported many details, that we express as variables, and
    # have checks for them, leading to many branches and statements.
    # pylint: disable=too-many-branches,too-many-statements

    scons_settings = OrderedDict()

    scons_settings["nasr_src"] = getSconsDataPath()

    scons_settings["python_version"] = python_version_str

    scons_settings["python_prefix"] = getDirectoryRealPath(getSystemPrefixPath())

    scons_settings["experimental"] = ",".join(getExperimentalIndications())

    scons_settings["debug_modes"] = ",".join(getDebugModeIndications())

    scons_settings["deployment"] = asBoolStr(isDeploymentMode())

    scons_settings["no_deployment"] = ",".join(Settings.getNoDeploymentIndications())

    if Settings.shallRunInDebugger():
        scons_settings["full_names"] = asBoolStr(True)

    if Settings.assumeYesForDownloads():
        scons_settings["assume_yes_for_downloads"] = asBoolStr(True)

    if not Settings.shallUseProgressBar():
        scons_settings["progress_bar"] = asBoolStr(False)

    if Settings.isClang():
        scons_settings["clang_mode"] = asBoolStr(True)

    if Settings.isShowScons():
        scons_settings["show_scons"] = asBoolStr(True)

    if Settings.isMingw64():
        scons_settings["mingw_mode"] = asBoolStr(True)

    if Settings.getMsvcVersion():
        scons_settings["msvc_version"] = Settings.getMsvcVersion()

    if Settings.shallDisableCCacheUsage():
        scons_settings["disable_ccache"] = asBoolStr(True)

    if isWin32Windows() and Settings.getWindowsConsoleMode() != "attach":
        scons_settings["console_mode"] = Settings.getWindowsConsoleMode()

    if Settings.getLtoMode() != "auto":
        scons_settings["lto_mode"] = Settings.getLtoMode()

    if isWin32OrPosixWindows() or isMacOS():
        scons_settings["noelf_mode"] = asBoolStr(True)

    if Settings.isUnstripped():
        scons_settings["unstripped_mode"] = asBoolStr(True)

    if isAnacondaPython():
        scons_settings["anaconda_python"] = asBoolStr(True)

    if isMSYS2MingwPython():
        scons_settings["msys2_mingw_python"] = asBoolStr(True)

    if isSelfCompiledPythonUninstalled():
        scons_settings["self_compiled_python_uninstalled"] = asBoolStr(True)

    cpp_defines = Plugins.getPreprocessorSymbols()
    if cpp_defines:
        scons_settings["cpp_defines"] = ",".join(
    ...
    if cpp_include_dirs:
        scons_settings["cpp_include_dirs"] = ",".join(cpp_include_dirs)

    link_dirs = Plugins.getExtraLinkDirectories()
    if link_dirs:
        scons_settings["link_dirs"] = ",".join(link_dirs)

    link_libraries = Plugins.getExtraLinkLibraries()
    if link_libraries:
        scons_settings["link_libraries"] = ",".join(link_libraries)
    ...
    if isMacOS():
        ...
        scons_settings["macos_min_version"] = macos_min_version

        scons_settings["macos_target_arch"] = Settings.getMacOSTargetArch()

    scons_settings["target_arch"] = getArchitecture()

    if Settings.getFcfProtectionMode() != "auto":
        scons_settings["cf_protection"] = Settings.getFcfProtectionMode()

    env_values = OrderedDict()

    string_values = Settings.getWindowsVersionInfoStrings()
    ...
    # in file system.
    product_version = Settings.getProductVersion()
    file_version = Settings.getFileVersion()
    ...
    if isPyTanicPython() and not isWin32OrPosixWindows():
    ...
    return scons_settings, env_values


def setPythonTargetSettings(scons_settings):
    scons_settings["gil_mode"] = asBoolStr(isPythonWithGil())
