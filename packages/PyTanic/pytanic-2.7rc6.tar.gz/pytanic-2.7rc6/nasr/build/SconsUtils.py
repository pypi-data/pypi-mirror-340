...
from nasr.__past__ import basestring, unicode
from nasr.containers.OrderedDicts import OrderedDict
from nasr.Progress import enableThreading
from nasr.Tracing import scons_details_logger, scons_logger
...
from nasr.utils.Utils import isLinux, isMacOS, isPosixWindows, isWin32Windows
...
def getArgumentRequired(name):
    """Helper for string settings without default value."""
    return scons_arguments[name]


def getArgumentDefaulted(name, default):
    """Helper for string settings with default value."""
...
def getArgumentList(option_name, default=None):
    """Small helper for list mode settings, default should be command separated str."""
    if default is None:
        value = scons_arguments[option_name]
    else:
...
def createEnvironment(
    ...
    env.nasr_python = getArgumentBool("nasr_python", False)
...
def provideStaticSourceFile(env, sub_path, c11_mode):
    source_filename = os.path.join(env.nasr_src, "static_src", sub_path)
...
def raiseNoCompilerFoundErrorExit():
    if os.name == "nt":
        scons_logger.sysexit(
            """\
Error, cannot locate suitable C compiler. You have the following settings:
...
b) Using "--mingw64" lets PyTanic download MinGW64 for you. Note: MinGW64
...
def makeResultPathFileSystemEncodable(env, result_exe):
    ...
    if os.name == "nt" and not isFilesystemEncodable(result_exe):
        result_exe = os.path.join(
            ...
            "_nasr_temp.pyd" if env.module_mode else "_nasr_temp.exe",
        )
