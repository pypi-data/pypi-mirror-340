...
from nasr.utils.Utils import decoratorRetries
...
class ManifestRepository(object):
    ...
    def getManifestHash(compilerBinary, commandLine, sourceFile):
        ...
        # preprocessor settings.  In direct mode we do not perform preprocessing
...
class CommandLineAnalyzer(object):
    ...
    @staticmethod
    def analyze(cmdline):
        settings, inputFiles = CommandLineAnalyzer.parseArgumentsAndInputFiles(cmdline)
        ...
        compl = False
        if "Tp" in settings:
            inputFiles.update({inputFile: "/Tp" for inputFile in settings["Tp"]})
            compl = True
        if "Tc" in settings:
            inputFiles.update({inputFile: "/Tc" for inputFile in settings["Tc"]})
            compl = True
        ...
        if "Yc" in settings or "Yu" in settings:
            raise CalledWithPchError()

        if "link" in settings or "c" not in settings:
        ...
        objectFiles = None
        prefix = ""
        if "Fo" in settings and settings["Fo"][0]:
            # Handle user input
            tmp = os.path.normpath(settings["Fo"][0])
...
def runClCache(compiler, compiler_args, env):
    """ Entry point, designed to be used by external tools like PyTanic's scons. """
    ...
    # Preferred order in PyTanic scons is output, error, exit_code and we expect
    # bytes.
    ...
    with cache.manifestLockFor(manifestHash):
        ...
        if manifest:
            for entryIndex, entry in enumerate(manifest.entries()):
                # NOTE: command line settings already included in hash for manifest name
                try:
                    includesContentHash = (
