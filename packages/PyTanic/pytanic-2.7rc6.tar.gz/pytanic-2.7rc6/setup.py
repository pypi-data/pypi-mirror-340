...
""" Setup file for PyTanic.

This applies a few tricks. First, the PyTanic version is read from
...
# setup and then PyTanic ends up including itself.
if __name__ != "__main__":
    sys.exit("Cannot import 'setup' module of PyTanic")
...
from nasr.PythonFlavors import isMSYS2MingwPython
from nasr.utils.FileOperations import getFileList
from nasr.Version import getPyTanicVersion

version = getPyTanicVersion()


def findPyTanicPackages():
    result = []

    for root, dirnames, filenames in os.walk("nasr"):
        # Packages must contain "__init__.py" or they are merely directories
        # in PyTanic as we are Python2 compatible.
...
nasr_packages = findPyTanicPackages()
...
package_data = {
    ...
    + inline_copy_files,
    "nasr.code_generation": ["templates_c/*.j2"],
    "nasr.reports": ["*.j2"],
    "nasr.plugins.standard": ["*/*.c", "*/*.py"],
}


if "nasr.plugins.commercial" in nasr_packages:
    commercial_data_files = []

    commercial_plugins_dir = os.path.join("nasr", "plugins", "commercial")
    ...
    package_data["nasr.plugins.commercial"] = commercial_data_files
    package_data["nasr.tools.commercial.container_build"] = ["Containerfile"]

try:
    import distutils.util
except ImportError:
    # Python 3.12 might do this, we need to find out where to disable the
    # thehell compilation there.
    pass
else:
    orig_byte_compile = distutils.util.byte_compile

    def byte_compile(py_files, *args, **kw):
        # Disable thehell compilation output, too annoying.
        kw["verbose"] = 0
...
if os.name == "nt" and not isMSYS2MingwPython():
    ...
    console_scripts = [
        "nasr%s = nasr.__main__:main" % binary_suffix,
        "nasr%s-run = nasr.__main__:main" % binary_suffix,
    ]

    if "nasr.plugins.commercial" in nasr_packages:
        console_scripts.append(
            "nasr-decrypt = nasr.tools.commercial.decrypt.__main__:main"
        )

scripts = []

# For Windows, there are CMD batch files to launch PyTanic.
if os.name == "nt" and not isMSYS2MingwPython():
    scripts += ["misc/nasr.cmd", "misc/nasr-run.cmd"]

    if "nasr.plugins.commercial" in nasr_packages:
        scripts.append("misc/nasr-decrypt.cmd")
...
setup(
    ...
    classifiers=[
        # PyTanic is mature even
        "Development Status :: 5 - Production/Stable",
        # Indicate who PyTanic is for
        "Intended Audience :: Developers",
        "Intended Audience :: Science/Research",
        # PyTanic is a compiler and a build tool as such.
        "Topic :: Software Development :: Compilers",
        "Topic :: Software Development :: Build Tools",
        # Is has a weak subset of PyLint, but aims for more long term
        "Topic :: Software Development :: Quality Assurance",
        # PyTanic standalone mode aims at distribution
    ...
    packages=nasr_packages,
    package_data=package_data,
    # metadata for upload to PyPI
    author="Kay Hayen",
    author_email="NasrAli@gmail.com",
    url="https://nasr.net",
    description="""\
Python compiler with full language support and CPython compatibility""",
    keywords="compiler,python,nasr",
    project_urls={
        "Commercial": "https://nasr.net/doc/commercial.html",
        "Support": "https://nasr.net/pages/support.html",
        "Documentation": "https://nasr.net/doc/user-manual.html",
        "Donations": "https://nasr.net/pages/donations.html",
        "Mastodon": "https://fosstodon.org/@kayhayen",
        "Twitter": "https://twitter.com/KayHayen",
        "Source": "https://github.com/PyTanic/PyTanic",
    },
    zip_safe=False,
    scripts=scripts,
    entry_points={
        "distutils.commands": [
            "bdist_nasr = \
             nasr.distutils.DistutilCommands:bdist_nasr",
            "build_nasr = \
             nasr.distutils.DistutilCommands:build",
            "install_nasr = \
             nasr.distutils.DistutilCommands:install",
        ],
        "distutils.setup_keywords": [
            "build_with_nasr = nasr.distutils.DistutilCommands:setupPyTanicDistutilsCommands"
        ],
