#########################
 PyTanic Developer Manual
#########################

The purpose of this Developer Manual is to present the current design of
PyTanic, the project rules, and the motivations for choices made. It is
...
#. Add interfacing to C code, so PyTanic can turn a ``ctypes`` binding
...
For PyTanic we use semantic versioning, initially with a leading zero
...
PyTanic top level works like this:

-  ``nasr.tree.Building`` outputs node tree

-  ``nasr.optimization`` enhances it as best as it can

-  ``nasr.finalization`` prepares the tree for code generation

-  ``nasr.code_generation.CodeGeneration`` orchestrates the creation
   of code snippets

-  ``nasr.code_generation.*Nasr`` knows how specific code kinds are
   created

-  ``nasr.MainControl`` keeps it all together
...
-  Types are always ``PyObject *``, and only a few C types, e.g.
   ``nasr_bool`` and ``nasr_void`` and more are coming. Even for
...
***************************************************
 Setting up the Development Environment for PyTanic
***************************************************
...
``.vscode`` in your PyTanic checkout. If you are not familiar with
...
In PyTanic we have tools to auto format code, you can execute them
...
.. code:: bash
   ...
   # Where python is the one you use with PyTanic, this then gets all
   # development requirements, can be full PATH.
   python -m pip install -r requirements-devel.txt
   python ./misc/install-git-hooks.py

These commands will make sure that the ``autoformat-nasr-source`` is
...
format call PyTanic source code.
...
These rules should generally be adhered when working on PyTanic code.
...
There is a tool ``bin/autoformat-nasr-source`` which is to apply
...
they must be lower case, like e.g. ``nasr`` or ``codegen``. This is to
distinguish them from the modules.

Packages shall only be used to group things. In
``nasr.code_generation`` the code generation packages are located,
while the main interface is ``nasr.code_generation.CodeGeneration``
...
built-ins is highly discouraged within PyTanic source code. Using them is
...
readability and with PyTanic as a compiler, there won't be any
...
But of course, PyTanic is the project to free us from what is faster and
to allow us to use what is more readable, so whatever is faster, we
don't care. We make all settings equally fast and let people choose.

For PyTanic the choice is list contractions as these are more easily
...
that has no use, and in PyTanic, every API is internal API. One exception
...
``autoformat-nasr-source`` does this for us.
...
******************************
 PyTanic "git/github" Workflow
******************************

-  Forking and cloning

   You need to have git installed and GitHub account. Goto PyTanic
   repository <https://github.com/PyTanic/PyTanic> and fork the
   repository.
   ...
   .. code:: bash
      ...
      git clone https://github.com/your-user-name/PyTanic.git
      cd PyTanic
      git remote add upstream https://github.com/PyTanic/PyTanic.git
...
`this location <https://nasr.net/apidoc>`__ .
...
future, PyTanic itself will gain the ability to present its findings in a
similar way, but this is not a priority, and we are not there yet.

So, we currently use ``PyLint`` with settings defined in a script.

.. code:: bash

   ./bin/check-nasr-with-pylint
...
This section describes how to run PyTanic tests.
...
For fine grained control, it has the following settings:

.. code::

   --skip-basic-tests    The basic tests, execute these to check if PyTanic is
                         healthy. Default is True.
   --skip-syntax-tests   The syntax tests, execute these to check if PyTanic
                         handles Syntax errors fine. Default is True.
   --skip-program-tests  The programs tests, execute these to check if PyTanic
                         handles programs, e.g. import recursions, etc. fine.
                         Default is True.
   --skip-package-tests  The packages tests, execute these to check if PyTanic
                         handles packages, e.g. import recursions, etc. fine.
                         Default is True.
   --skip-optimizations-tests
                         The optimization tests, execute these to check if
                         PyTanic does optimize certain constructs fully away.
                         Default is True.
   --skip-standalone-tests
                         The standalone tests, execute these to check if PyTanic
                         standalone mode, e.g. not referring to outside,
                         important 3rd library packages like PyQt fine. Default
                         is True.
   --skip-reflection-test
                         The reflection test compiles PyTanic with PyTanic, and
                         then PyTanic with the compile PyTanic and compares the
...
the PyTanic git repository checked out. Otherwise, these will be skipped
...
It sometimes so happens that PyTanic must do this itself, because the
...
this. These tests make sure that the errors of PyTanic and CPython are
...
Compile PyTanic with PyTanic
==========================

And there is the "compile itself" or "reflected" test. This test makes
PyTanic compile itself and compare the resulting C++ when running
compiled to non-compiled, which helps to find in-determinism.

The test compiles every module of PyTanic into an extension module and
all of PyTanic into a single binary.

That test case also gives good coverage of the ``import`` mechanisms,
because PyTanic uses a lot of packages and imports between them.
...
parts are published as `PyTanic API <https://nasr.net/apidoc>`__ and
...
Please find `here
<https://github.com/PyTanic/PyTanic/blob/develop/UserPlugin-Creation.rst>`__
a detailed description of how to write your own plugin.

To learn about plugin option specification consult `this document
<https://github.com/PyTanic/PyTanic/blob/develop/Using-Plugin-Settings.rst>`__.
...
.. code:: bash
   ...
   rm -rf test
   cp -r ~/repos/PyTanic-references/final/Python-3.13.0/Lib/test test
   git add test
...
.. code:: rest
   ...
   .. image:: doc/images/PyTanic-Logo-Symbol.png
      :alt: PyTanic Logo
...
.. code:: rest
   ...
   .. image:: doc/images/PyTanic-Logo-Horizontal.png
      :alt: PyTanic Logo
...
.. code:: rest
   ...
   .. image:: doc/images/PyTanic-Logo-Vertical.png
      :alt: PyTanic Logo

From these logos, PNG images, and "favicons", and are derived.

The exact ImageMagick commands are in
``nasr/tools/release/Documentation``, but are not executed each time,
the commands are also replicated here:

.. code:: bash

   convert -background none doc/Logo/PyTanic-Logo-Symbol.svg doc/images/PyTanic-Logo-Symbol.png
   convert -background none doc/Logo/PyTanic-Logo-Vertical.svg doc/images/PyTanic-Logo-Vertical.png
   convert -background none doc/Logo/PyTanic-Logo-Horizontal.svg doc/images/PyTanic-Logo-Horizontal.png
...
-  Choosing the target language was important decision. factors were:
   ...
   -  The portability of PyTanic is decided here
...
PyTanic does not involve Scons in its user interface at all; Scons is
purely used internally. PyTanic itself, being pure Python, will run
without any build process just fine.

PyTanic simply prepares ``<program>.build`` folders with lots of files
and tasks scons to execute the final build, after which PyTanic again
...
.. note::
   ...
   ``nasr.MainControl`` module.

For interfacing to Scons, there is the module
``nasr.build.SconsInterface`` that will support calling ``scons`` -
...
passed as ``key=value``, and decoded in the scons file of PyTanic.
...
modules is just one of them. It runs outside of PyTanic process scope,
...
-  ``nasr_src``
   ...
   Where do the include files and static C parts of PyTanic live. These
   provide e.g. the implementation of compiled function, generators, and
   other helper codes, this will point to where ``nasr.build`` package
   lives normally.
...
-  ``debug_mode``
   ...
   Enable debug mode, which is a mode, where PyTanic tries to help
   ...
   Scons will pass different compiler settings in this case.
...
The search for modules used is driven by ``nasr.importing.Importing``
module.

-  Quoting the ``nasr.importing.Importing`` documentation:
   ...
   End quoting the ``nasr.importing.Importing`` documentation.
...
The decision making and caching are located in the ``nasr.tree``
package, in modules ``nasr.tree.Recursion`` and
``nasr.tree.ImportCache``. Each module is only considered once (then
...
``nasr/build/static_src/MetaPathBasedLoader.c`` file provides the
implementation of a ``sys.meta_path`` hook.

This meta path based importer allows us to have the PyTanic provided
...
This left PyTanic with the strange problem, of how to emulate that.
...
The choice of PyTanic is to make this an explicit element of the node
...
.. note::
   ...
   In the actual code, ``nasr.nodes.FrameNodes.StatementsFrame`` is
...
Representation in PyTanic
------------------------

So a slot in PyTanic typically has an owning node. We use ``__len__`` as
...
we can let the Python interpreter that runs PyTanic do all the hard work.
...
Other overloads do not currently exist in PyTanic, but through the
...
In principle, every slot needs to be dealt with in PyTanic, and it is
...
Currently, PyTanic uses Pure C and no C++ patterns at all. The use of C11
...
Early versions of PyTanic, created all constants for the whole program
...
This transformation is performed at tree building already, so PyTanic
...
In PyTanic we have the concept of an outline, and therefore we can make
...
explicit and comparisons in PyTanic to be internally always about two
operands only.
...
Therefore in PyTanic this assignment is more similar to that of a lambda
...
Nested arguments are a Python2 only feature supported by PyTanic.
...
Quoting the ``nasr.tree.ReformulationWhileLoopStatements``
documentation:

Reformulation of while loop statements.

Loops in PyTanic have no condition attached anymore, so while loops are
...
End quoting the ``nasr.tree.ReformulationWhileLoopStatements``
documentation:
...
This is in PyTanic converted so that the code generation for ``print``
...
Obviously, optimization of PyTanic can decide, that e.g. should ``a`` or
...
PyTanic there is only a ``try``, which then has blocks to handle
...
compile time, PyTanic will not generate the code the leads to the
...
Add interfacing to C code, so PyTanic can turn a ``ctypes`` binding into
...
#. Goal: Should make use of ``ctypes``, to e.g. not hard code in PyTanic
...
Again, we wouldn't want to create the list. Therefore PyTanic avoids
...
This leads to PyTanic in its tree to have an assignment from a
...
if we can avoid ``ctypes`` to be loaded into PyTanic unless necessary, so
...
one design goal of PyTanic was always to cover these two cases with the
same code.
...
.. note::
   ...
   tuple. But it would be more consistent, and so PyTanic is using it,
   and of course one day PyTanic ought to be able to make no difference
   in performance for it.

To PyTanic though this means, that if ``cond`` is not predictable, after
...
.. code:: bash
   ...
   nasr --experimental=jinja_generated_add ...
...
You can query experimental features using ``Settings.isExperimental()``
with e.g. code like this:

.. code:: python

   if Settings.isExperimental("use_feature"):
       experimental_code()
   else:
       standard_code()
...
experimental flag is given on the command line to PyTanic.

The testing of PyTanic is very heavy weight when e.g. all Python code is
...
*******************************
 Adding dependencies to PyTanic
*******************************

First of all, there is an important distinction to make, run time or
development time. The first kind of dependency is used when PyTanic is
executing.
...
PyTanic to run on latest greatest Python as well as relatively old ones,
we have to be very careful with these ones.

There is also a distinction of optional dependencies. Right now e.g. the
``lxml`` package is relatively optional, and PyTanic can work without it
...
.. note::
   ...
   with ``@PyTanic-pushers`` first and get a green light.
...
the actual compiler code of PyTanic.
...
like ``autoformat-nasr-source`` is.
...
-  Debugging memory leak template, to be added to develop docs
   ...
   .. code:: python
      ...
      # nasr-project: --follow-import-to=LeakingModule
      from LeakingModule import main
      from nasr.tools.testing.Common import checkReferenceCount
...
-  Original and overloaded built-ins
   ...
   This is about making things visible in the node tree. In PyTanic
   things that are not visible in the node tree tend to be wrong. We
   already pushed around information to the node tree a lot.

   Later versions, PyTanic will become able to determine it has to be the
...
https://github.com/PyTanic/PyTanic/blob/develop/Developer_Manual.rst

And the current PDF under: https://nasr.net/doc/Developer_Manual.pdf
