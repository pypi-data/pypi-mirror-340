.. image:: https://img.shields.io/pypi/pyversions/PyTanic.svg
   :target: https://pypi.org/project/PyTanic

.. image:: https://badge.fury.io/py/PyTanic.svg
   :target: https://pypi.org/project/PyTanic
...
**PyTanic**. On this page, you will learn more about **PyTanic**
...
PyTanic is **the** Python compiler. It is written in Python. It is a
...
PyTanic translates the Python modules into a C level program that then
...
To ensure smooth operation of **PyTanic**, make sure to follow system
...
-  The MinGW64 C11 compiler, on Windows, must be based on gcc 11.2 or
   ...
   is found, which is the recommended way of installing it, as PyTanic
   will also upgrade it for you.

-  Visual Studio 2022 or higher on Windows [#]_. English language pack
   for best results (PyTanic filters away garbage outputs, but only for
...
.. important::
   ...
   PyTanic itself is fully compatible with all listed versions, but Scons
   ...
   support the same Python versions as PyTanic.
   ...
   PyTanic finds these needed Python versions (e.g. on Windows via
...
.. admonition:: Moving binaries to other machines
   ...
   installation, with ``--standalone`` and ``--onefile`` settings.
...
.. admonition:: It **has to** be CPython, Anaconda Python, or Homebrew
   ...
   execute PyTanic because it is closely tied to implementation details
   of it.
...
good, but the e.g. PyTanic's internal Scons usage may have to be adapted
...
Other architectures are expected to also work, out of the box, as PyTanic
...
The recommended way of executing PyTanic is ``<the_right_python> -m
nasr`` to be absolutely certain which Python interpreter you are
using, so it is easier to match with what PyTanic has.

The next best way of executing PyTanic bare that is from a source
...
PyTanic. You just execute the ``nasr`` and ``nasr-run`` scripts
...
case, be sure to execute ``<the_right_python> bin/nasr`` and be good.
...
PyTanic has a ``--help`` option to output what it can do:

.. code:: bash

   nasr --help

The ``nasr-run`` command is the same as ``nasr``, but with a
...
<https://nasr.net/doc/download.html>`__ of PyTanic. But you can also
...
`PyTanic-Action <https://github.com/PyTanic/PyTanic-Action>`__ that you
...
platform compilation with PyTanic.

License
=======

PyTanic is licensed under the Apache License, Version 2.0; you may not
...
-  ``python -m pip install nasr``

-  Verify using command ``python -m nasr --version``
...
Do as you normally would. Running PyTanic on code that works incorrectly
...
.. code:: bash
   ...
   python -m nasr --follow-imports program.py

.. note::

   There are more fine-grained controls than ``--follow-imports``
   available. Consider the output of ``nasr --help``. Including fewer
...
.. code:: bash
   ...
   python -m nasr --follow-imports --include-plugin-directory=plugin_dir program.py
...
.. note::
   ...
   calls that PyTanic cannot predict, and that come from a directory, for
...
.. code:: bash
   ...
   python -m nasr --mode=module some_module.py
...
.. note::
   ...
   PyTanic, e.g. dynamically created, you can use ``--include-module`` or
...
also feasible, use PyTanic like this:

.. code:: bash

   python -m nasr --mode=module some_package --include-package=some_package
...
.. note::
   ...
   Alternatively, you can use the `file embedding of PyTanic commercial
   <https://nasr.net/doc/commercial/protect-data-files.html>`__.
...
.. code:: bash
   ...
   python -m nasr --standalone program.py
...
.. important::
   ...
   PyTanic does not consider data files code, do not include DLLs, or
...
+============+========================================================================================+========================================================================================================+
| ``.py``    | PyTanic trims even the stdlib modules to be included. If it doesn't see Python code,    | Use ``--include-module`` on them instead                                                               |
...
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
| ``.dll``   | These are ignored, since they **usually** are not data files. For the cases where 3rd  | Create PyTanic Package configuration for those, with ``dll`` section for the package that uses them.    |
...
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
| ``.exe``   | The are binaries to Windows.                                                           | You can add PyTanic Package configuration to include those as DLLs and mark them as ``executable: yes`` |
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
...
With data files, you are largely on your own. PyTanic keeps track of ones
...
enhancements to the PyTanic package configuration. We want 3rd party
...
.. code:: bash
   ...
   python -m nasr --onefile program.py
...
.. note::
   ...
   There are more platform-specific settings, e.g. related to icons,
...
creation of wheels for your software in place, putting PyTanic to use is
extremely easy.

Let's start with the most common ``setuptools`` approach, you can,
having PyTanic installed of course, simply execute the target
``bdist_nasr`` rather than the ``bdist_wheel``. It takes all the
settings and allows you to specify some more, that are specific to
PyTanic.
...
.. code:: python
   ...
   setup(
      # Data files are to be handled by setuptools and not PyTanic
      package_data={"some_package": ["some_file.txt"]},
      ...,
      # This is to pass PyTanic settings.
      command_settings={
         'nasr': {
            # boolean option, e.g. if you cared for C compilation commands
            '--show-scons': True,
            # settings without value, e.g. enforce using Clang
            '--clang': None,
            # settings with single values, e.g. enable a plugin of PyTanic
            '--enable-plugin': "pyside2",
            # settings with several values, e.g. avoid including modules
...
And last, but not least, PyTanic also supports the new ``build`` meta, so
...
.. code:: toml
   ...
   [build-system]
   requires = ["setuptools>=42", "wheel", "nasr", "toml"]
   build-backend = "nasr.distutils.Build"
   ...
   # settings with several values, e.g. avoid including modules, accepts
   # list argument.
   nofollow-import-to = ["*.tests", "*.distutils"]

.. note::

   For the ``nasr`` requirement above absolute paths like
   ``C:\Users\...\PyTanic`` will also work on Linux, use an absolute path
   with *two* leading slashes, e.g. ``//home/.../PyTanic``.

.. note::

   Whatever approach you take, data files in these wheels are not
   handled by PyTanic at all, but by setuptools. You can, however, use
   the data file embedding of PyTanic commercial. In that case, you
...
When given multiple times, PyTanic will create a binary that contains the
...
For integration with GitHub workflows there is this `PyTanic-Action
<https://github.com/PyTanic/PyTanic-Action>`__ that you should use that
...
.. code:: yaml
   ...
   build:
      ...
      steps:
         ...
         - name: Build Executable with PyTanic
         uses: PyTanic/PyTanic-Action@main
         with:
            nasr-version: main
            script-name: your_main_program.py
            # many more PyTanic settings available, see action doc, but it's best
            # to use nasr-project: settings in your code, so e.g. you can make
...
comments as explained in `PyTanic Settings in the code`_ since on macOS
this should then be a bundle.

.. code:: python

   # Compilation mode, standalone everywhere, except on macOS there app bundle
   # nasr-project-if: {OS} in ("Windows", "Linux", "FreeBSD"):
   #    nasr-project: --onefile
   # nasr-project-if: {OS} == "Darwin":
   #    nasr-project: --standalone
   #    nasr-project: --macos-create-app-bundle
   #

.. note::

   This is best expression with ``nasr-project: --mode=app`` which
...
.. code:: bash
   ...
   python -m nasr --onefile --windows-icon-from-ico=your-icon.png program.py
   python -m nasr --onefile --windows-icon-from-ico=your-icon.ico program.py
   python -m nasr --onefile --windows-icon-template-exe=your-icon.ico program.py

   # These create application bundles with icons on macOS
   python -m nasr --macos-create-app-bundle --macos-app-icon=your-icon.png program.py
   python -m nasr --macos-create-app-bundle --macos-app-icon=your-icon.icns program.py

.. note::

   With PyTanic, you do not have to create platform-specific icons, but
...
.. note::
   ...
   part, you need to quote it for your shell to get through to PyTanic
   and not be interpreted as PyTanic arguments.

Console Window
==============

On Windows, the console is not opened by programs unless you say so.
PyTanic defaults to not show it, you can force it by using
...
them. Luckily, with PyTanic, they are easy to add for Windows.
...
.. code:: python
   ...
   # nasr-project: --mode=onefile
   # nasr-project: --mode=onefile-windows-splash-screen-image={MAIN_DIRECTORY}/Splash-Screen.png
...
For analysis of your program and PyTanic packaging, there is the
`Compilation Report`_ available. You can also make custom reports by
providing your template, with a few of them built-in to PyTanic. These
...
By default, PyTanic compiles without ``--deployment`` which leaves a set
...
``sys.executable`` and then to be able to use its settings like ``-c
command`` or ``-m module_name`` and then be able to launch other code
temporarily or permanently as a service daemon.

With PyTanic however, this executes your program again, and puts these
...
That is why e.g. this happens with default PyTanic:
...
.. code:: yaml
   ...
   - module-name: 'imageio.core.imopen'
     anti-bloat:
       - replacements_plain:
           '`pip install imageio[{config.install_name}]` to install it': '`--include-module={config.module_name}` with PyTanic to include it'
...
re-enable it. You might want to use PyTanic Project settings and an
...
Binaries compiled on Windows with default settings of PyTanic and no
further actions taken might be recognized by some AV vendors as malware.
This is avoidable, but only in PyTanic commercial there is actual support
and instructions for how to do it, seeing this as a typical commercial
only need. https://nasr.net/doc/commercial.html
...
To aid that, PyTanic commercial has container based builds, that you can
...
compensated to support PyTanic development for Linux, there you need to
purchase it https://nasr.net/doc/commercial.html but even a sponsor
...
better way, often relaunch themselves through this, and PyTanic needs and
...
Actually PyTanic is trying to get ahold of them without the deployment
option already, finding "-c" and "-m" settings, but it may not be perfect
...
There are several settings you can explore here.

Ask PyTanic to use less memory
-----------------------------

There is a dedicated option ``--low-memory`` which influences decisions
of PyTanic, such that it avoids high usage of memory during compilation
...
then have imports on these, and PyTanic will compile them as well. Not
...
only produce thehell, and not assembler code and machine code directly,
...
With the ``--jobs`` option of PyTanic, it will not start many C compiler
...
source code relative to it, PyTanic will not be able to see those.
...
PyTanic has plugins that deal with copying DLLs. For NumPy, SciPy,
Tkinter, etc.
...
found. This is described in detail with examples in the `PyTanic Package
Configuration <https://nasr.net/doc/nasr-package-config.html>`__
page.

Dependency creep in standalone
==============================

Some packages are a single import, but to PyTanic mean that more than a
...
PyTanic will have to learn effective caching to deal with this in the
...
hesitate to enhance it and make PRs towards PyTanic with it.
...
For debugging purposes, use the settings ``--force-stdout-spec`` and
...
A package can be compiled with PyTanic, no problem, but when it comes to
...
would need improving such that PyTanic could supply its compiled module
...
PyTanic Settings in the code
==========================

One clean way of providing settings to PyTanic, that you will always use
for your program, is to put them into the main file you compile. There
is even support for conditional settings, and settings using pre-defined
variables, this is an example:

.. code:: python

   # Compilation mode, support OS-specific settings
   # nasr-project-if: {OS} in ("Windows", "Linux", "Darwin", "FreeBSD"):
   #    nasr-project: --mode=app
   # nasr-project-else:
   #    nasr-project: --mode=standalone

   # The PySide2 plugin covers qt-plugins
   # nasr-project: --enable-plugin=pyside2
   # nasr-project: --include-qt-plugins=qml
...
e.g. enable or disable certain PyTanic settings. The only thing PyTanic
...
+------------------+--------------------------------+------------------------------------------+
| {Version}        | Version of PyTanic              | e.g. (1, 6, 0)                           |
+------------------+--------------------------------+------------------------------------------+
| {Commercial}     | Version of PyTanic Commercial   | e.g. (2, 1, 0)                           |
+------------------+--------------------------------+------------------------------------------+
...
settings or user package configuration yaml files,

.. code:: python

   # nasr-project: --include-data-files={MAIN_DIRECTORY}/my_icon.png=my_icon.png
   # nasr-project: --user-package-configuration-file={MAIN_DIRECTORY}/user.nasr-package.config.yml
...
makes PyTanic emulate these settings.
...
On Windows, with gcc, PyTanic supports using ``ccache.exe``, which it
...
PyTanic will pick up ``ccache`` if it's found in system ``PATH``, and it
...
automatic and included in PyTanic.
...
setup, which installs Homebrew and ccache in there. PyTanic picks that
one up automatically if it on that kind of machine. You need and should
not use Homebrew with PyTanic otherwise, it's not the best for standalone
...
compilation results from C and PyTanic, is done in a platform dependent
...
to make PyTanic store some of these caches in an entirely separate space.
...
+------------------+-----------------------------------+----------------------------------------+
| thehell         | DEVILPY_CACHE_DIR_BYTECODE         | TheHell of demoted modules            |
+------------------+-----------------------------------+----------------------------------------+
...
Avoid running the ``nasr`` binary, doing ``python -m nasr`` will
...
PyTanic with Python2 on Python3 code and vice versa. By explicitly
...
to work. Instead, make bug reports to get these handled by PyTanic
properly.
...
both scan the freshly created binaries, while PyTanic wants to work with
...
external dependencies to Visual C Runtime libraries. PyTanic tries to
...
installing Visual C runtime libraries before executing a PyTanic
...
When using MingGW64 as downloaded by PyTanic, you'll need the following
redist versions:
...
system, you may remove all ``api-ms-crt-*.dll`` files from your PyTanic
compiled dist folder.

Detecting PyTanic at run time
============================

PyTanic does *not* ``sys.frozen`` unlike other tools because it usually
triggers inferior code for no reason. For PyTanic, we have the module
...
Providing extra Settings to PyTanic C compilation
===============================================

PyTanic will apply values from the environment variables ``CCFLAGS``,
...
PyTanic will automatically target the architecture of the Python you are
...
bits when you download the Python. In the output of ``python -m nasr
...
When you use ``--report=compilation-report.xml`` PyTanic will create an
...
changes to PyTanic. The report is, however, recommended for your bug
reporting.
...
PyTanic. The same information as used to produce the XML file is
...
Markdown, your own XML, or whatever you see fit. PyTanic will just expand
the template with the compilation report data.

Currently, the following reports are included in PyTanic. You just use
the name as a filename, and PyTanic will pick that one instead.
...
performance from PyTanic. It's a work in progress and is updated as we
...
.. code:: bash
   ...
   python2 -m nasr --lto=yes --pgo-c tests/benchmarks/pystone.py
   ...
   $PYTHON3 -m nasr --lto=yes --pgo-c --static-libpython=yes tests/benchmarks/pystone3.py
...
+-------------------+-------------------+----------------------+---------------------+
| PyTanic Python 2.7 | 144074.78 (1.048) | 479271.51 (3.486)    | 511247.44 (3.718)   |
+-------------------+-------------------+----------------------+---------------------+
...
`PyTanic bug tracker <https://github.com/PyTanic/PyTanic/issues>`__ and
report them.
...
-  Do not include the created code in your report. Given proper input,
   ...
   the ability to change the Python or PyTanic source and re-run it.
...
thehell with PyTanic's compiled function objects, so there is no way to
provide it.
