...
def resolve_payload(node, **kwargs):
    ...
    scan_nested_usd(node, path_list)
    ...
    try:
        ...
        if node_type in node_list:
            ...
            if usd_file:
                ...
                # scan for nested USD files
                scan_nested_single_usd(node, usd_file, path_list)
    except Exception as e:
...
def scan_nested_single_usd(node, file_path, result):
...
def scan_nested_usd(node, result):
