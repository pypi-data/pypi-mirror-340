...
from inception_audittrail_logger.audittrail_elastic import index_document
import asyncio
import sys
import json
...
async def log(
    ...
    try:
        ...
        results = await asyncio.gather(
            ...
            index_document(document_id=formatted_data.get("_id"), body= json.dumps(es_doc)),
        )
...
async def log_sys_error(
    ...
    try:
        ...
        results = await index_document(
            document_id=formatted_data.get("_id"), body=json.dumps(formatted_data)
        )
        return results
    except Exception as e:
