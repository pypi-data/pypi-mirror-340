...
async def format_audittrail_data(data: dict, user: dict, correlation_id: str, user_agent_str: str, ip_address: str):
    """
    Format audit trail data for MongoDB and Elasticsearch.
    """
    id = f"{user['owner_id']}-{user['account_no']}-{str(uuid.uuid4())}"
    document = {
        "_id": id,
        "id": id,
        ...
        "user_account_no": user.get("account_no"),
        "user_email": user.get("email"),
        "user_photo_url": user.get("photo_url"),
        "owner_id": user.get("owner_id"),
        "owner_fullname": user.get("owner_fullname"),
        "owner_account_no": user.get("owner_account_no"),
        "owner_email": user.get("owner_email"),
        "owner_photo_url": user.get("owner_photo_url"),
...
async def format_sys_error_data(data: dict, user: dict, correlation_id: str, user_agent_str: str, ip_address: str):
    """
...
async def get_geolocation(ip: str):
    try:
        ...
        response = await loop.run_in_executor(None, lambda: requests.get(f"http://ip-api.com/json/{ip}"))
