...
def create_strike_image(xlma_params: XLMAParams,
    ...
    if xlma_params.cartopy_paths:
        for path in xlma_params.cartopy_paths:
            ...
            counties_in_box.boundary.plot(ax=ax3, edgecolor=marker_color, linewidth=1, zorder=2, alpha=0.05)

            # Add county name labels
            for idx, row in counties_in_box.iterrows():
                centroid = row.geometry.centroid
                ax3.text(centroid.x, centroid.y, row['NAME'], ha='center', color='lime')
