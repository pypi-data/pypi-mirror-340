...
```python
from openmcp.core.network import AgentNetworkServer
...
```python
from openmcp.core.agent import Agent
...
Visit our project website at [https://openmcp.org](https://openmcp.org) for more information, documentation, and resources.

## Overview

OpenMCP provides an engine for running a network with a set of protocols. The framework is designed to be modular, allowing developers to:
...
OpenMCP includes several built-in protocols:
...
We welcome contributions to the OpenMCP framework! Whether you want to fix bugs, add new features, or create new protocols, your help is appreciated.
...
pytest --cov=src/openmcp --cov-report=xml
```
...
[![Python Tests](https://github.com/openmcp/openmcp/actions/workflows/pytest.yml/badge.svg)](https://github.com/openmcp/openmcp/actions/workflows/pytest.yml)
...
OpenMCP provides a command-line interface for basic operations:

```bash
# Launch a network with a configuration file
openmcp network launch config.json --runtime 3600

# Get help on available commands
openmcp --help

# Set logging level
openmcp --log-level DEBUG network launch config.json
```
