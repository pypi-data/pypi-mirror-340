...
class GestorArchivos:
    ...
    def write(self, data, level=9):
        ...
        if not isinstance(data, (str, bytes)):
            print("❌ El contenido a escribir debe ser tipo str o bytes.")
            return
        if len(data) == 0:
            print("⚠️ No se puede guardar contenido vacío.")
            return
        """Escribe nuevos datos como una nueva versión, usando barra de progreso real (tqdm)."""
    ...
    def listar_versiones(self):
        """
        Lista todas las versiones disponibles del archivo.
        """
        ...
        for i, version in enumerate(self.inodo["versiones"]):
            ...
            tamano = sum(b["offset_fin"] - b["offset_inicio"] for b in version.get("bloques", []))
            print(f"🔹 {i+1}. ID: {vid} | Tamaño: {tamano} bytes | Bloques: {bloques} | Creado: {timestamp}")
