...
class Task:
    ...
    def __init__(self, total: int, process_name: str,
                 iswaiting: bool = False, fill_char="-", empty_char="-", wait_char="=", fill_color: str = "blue", empty_color: str = "black", wait_color: str = "blue"):
        self.total = total
        self.current = 0
    ...
    def _render(self, final: bool = False):
        ...
        line = (
            f"{self.process_name} - "
            f"({self._color('reset')}{bar}{self._color('reset')}) "
    ...
    def _build_bar(self, progress: float) -> str:
        ...
        if self.iswaiting:
            ...
            self.wait_step += 1
            before = self.fill_char * busy_pos
            after = self.empty_char * (length - busy_pos - 1)
            return f"{self._color(self.fill_color)}{before}{self._color(self.wait_color)}{self.wait_char}{self._color(self.empty_color)}{after}"
        else:
            filled = int(progress * length)
            empty = length - filled
            return (
                f"{self._color(self.fill_color)}{self.fill_char * filled}"
                f"{self._color(self.empty_color)}{self.empty_char * empty}"
            )
