...
class Task:
    ...
    def log(self, text: str):
        """
        プログレスバーの下にログを出力し、再描画します。
        """
        down = len(Modern._tasks) - self.index
    ...
    def _render(self, final: bool = False):
        ...
        if final:
            ...
            status = f"[WAIT] [{self.current}/{self.total}]"
        else:
    ...
    def _build_bar(self, progress: float) -> str:
        length = 20
        if self.iswaiting:
            busy_pos = self.busy_step % (length + 1)
            self.busy_step += 1
            before = "-" * busy_pos
            after = "-" * (length - busy_pos - 1)
            return f"{self._color('black')}{before}{self._color('blue')}-{self._color('black')}{after}"
        else:
            filled = int(progress * length)
            empty = length - filled
            return (
                f"{self._color('blue')}{'-' * filled}"
