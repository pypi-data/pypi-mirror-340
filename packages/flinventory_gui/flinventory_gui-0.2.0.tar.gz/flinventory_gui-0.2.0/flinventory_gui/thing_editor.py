...
import flinventory
from flinventory import BoxedThing, Sign
...
def update_sign_element(
    sign_ui_element: ui.element, sign: Sign, options: flinventory.Options
):
    ...
    Args:
        ...
        sign: the sign that should be displayed and is changable by the new input fields
    ...
    with sign_ui_element:
        for sign_member, lang, tooltip in (
            ...
            input_fields[f"sign.{member}"] = (
                ...
                .on_value_change(SlowSignChanger(sign, member))
            )
            if tooltip:
...
class SlowLocationSaver(SlowInputSaver[flinventory.Location]):
    ...
    def __init__(
        ...
        super().__init__(data, level.levelname, wait_time=0.3)
        self.level = level
    ...
    def setter(self, value: Any):
        ...
        try:
            ...
            for _ in new_thing_options:
                ui.notify(
                    f"There are different things with the name {value}. "
                    "Use the id which you can copy from the display view of the thing."
                )
                new_thing = None
                break
...
def update_list_input(
    ...
    with ui_element:
        ...
        for id in thing.get_undefaulted(member, backup=tuple()):
            try:
                other_thing = inventory.get_by_id(id)
            except KeyError:
                # ignore
                continue
            with ui.label():
                ...
                ui.button("X").on_click(
                    ListRemover(inventory, thing, member, id, ui_element, description)
                )
        input_element = ui.input(label="new").props("autogrow dense")
...
def show_thing_changer(
    ...
    """
    # includes not all input fields. In particular sign and location fields are missing.
    input_fields: dict[tuple[str, str] | str, ui.element] = {}
    ...
    with ui_element:
        with ui.row():
            with ui.column():
                ...
                with ui.column() as super_class_row:
                    update_list_input(
                        super_class_row,
                        inventory,
                        thing.thing,
                ...
                with ui.column() as super_row:
                    update_list_input(
                        super_row, inventory, thing.thing, "part_of", "Teil von"
                    )
            with ui.column() as location_column:
                update_location_element(location_column, thing.location)
            with ui.column() as sign_column:
                update_sign_element(sign_column, thing.sign, options)
