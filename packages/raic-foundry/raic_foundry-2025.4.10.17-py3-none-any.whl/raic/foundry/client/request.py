...
def _build_endpoint(request, query_params: Optional[dict] = None) -> str:
    ...
    endpoint = '{}/{}'.format(raic.foundry.client.context.get_raic_configuration('ApiRoot'), safe_request.lstrip('/'))
    if query_params is not None:
        params_string = '&'.join([f'{k}={urllib.parse.quote(str(v))}'.format() for k, v in query_params.items()])
        endpoint += f'?{params_string}'
