...
class ScoringExplainer(ChainedIdentity):
    ...
    :param raw_features: Optional list of feature names for the raw features that can be specified
        if the original explainer computes the explanation on the engineered features.
    :type raw_features: list[str]
    :param engineered_features: Optional list of feature names for the engineered features that can
        be specified if the original explainer has transformations passed in and only computes the
        importances on the raw features.
    :type engineered_features: list[str]
    """

    __metaclass__ = ABCMeta

    def __init__(self, original_explainer, feature_maps=None, raw_features=None,
        ...
        raw_features = self._convert_features_to_list(raw_features)
        engineered_features = self._convert_features_to_list(engineered_features)

        if original_explainer.transformations is not None:
            self._data_mapper = DataMapper(original_explainer.transformations)
            if raw_features is None:
                raw_features = self._convert_features_to_list(original_explainer.features)
            self._features = raw_features
            self._raw_features = raw_features
            self._engineered_features = engineered_features
        else:
            self._data_mapper = None
            if feature_maps is None:
                self._features = self._convert_features_to_list(original_explainer.features)
                self._raw_features = None
                self._engineered_features = None
            else:
                if engineered_features is None:
                    engineered_features = self._convert_features_to_list(original_explainer.features)
                self._features = engineered_features
                self._raw_features = raw_features
                self._engineered_features = engineered_features
    ...
    def raw_features(self):
        ...
        If the original explainer did not have transformations passed to it, and feature_maps
        have been passed to the scoring explainer, the raw feature names need to be passed into
        the scoring explainer constructor using the raw_features parameter.  Otherwise the raw
        features will be the same as the features.

        :return: The raw feature names, or None if none were given by the user.
        :rtype: list[str] or None
        """
    ...
    def engineered_features(self):
        ...
        If the original explainer had transformations passed to it, the engineered features
        will need to be passed to the scoring explainer constructor using the engineered_features
        parameter. Otherwise, if feature maps were passed to the scoring explainer, the engineered
        features will be the same as the features.

        :return: The engineered feature names, or None if none were given by the user.
        :rtype: list[str] or None
        """
        return self._engineered_features

    def _convert_features_to_list(self, features):
        """Convert the features to a list if it is not already a list.

        :param features: The features to convert.
        :type features: list[str] or None
        :return: The features as a list, or None if it is not specified.
        :rtype: list[str] or None
        """
...
class KernelScoringExplainer(ScoringExplainer):
    ...
    def explain(self, evaluation_examples, get_raw=None):
        """Use the KernelExplainer and model for scoring to get the feature importance values of data.
...
class TreeScoringExplainer(ScoringExplainer):
    ...
    def __init__(self, original_explainer, **kwargs):
        ...
        if has_explainer and type(original_explainer.explainer) == shap.TreeExplainer:
            ...
            has_model_explainer = (hasattr(original_explainer, SURROGATE_MODEL)
                                   and hasattr(original_explainer.surrogate_model, MODEL_TREE_EXPLAINER))
