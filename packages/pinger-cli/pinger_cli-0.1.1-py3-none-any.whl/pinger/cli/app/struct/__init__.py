...
app_sub = typer.Typer(
    help="Subcommands to manage local dev and ECS deployments via the App class."
)


@app_sub.command("poetry-install")
def poetry_install():
    """
    Install project dependencies with Poetry non-interactively.
    """
    app().poetry_install()
...
def ci(
    ...
    ecr_repo_name: str = typer.Argument(..., help="Name of the ECR repository."),
):
    """
    Build and push the Docker image to ECR for the given AWS profile/environment.
    """
    app().ci(env, ecr_repo_name)
