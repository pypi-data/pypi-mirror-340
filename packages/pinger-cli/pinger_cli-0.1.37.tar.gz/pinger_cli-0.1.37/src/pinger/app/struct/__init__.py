...
class App:
    ...
    def edit(cls, env: str):
        """
        Edit secrets file for the given environment.
        If it doesn't exist, create and encrypt it with SOPS.
        If it exists but is not encrypted, encrypt it first.
        """
        secrets_path = Path(f"envs/{env}/secrets.yml")
        secrets_path.parent.mkdir(
            parents=True, exist_ok=True
        )  # Ensure envs/<env>/ exists

        if not secrets_path.exists():
            typer.secho(
                f"No secrets file found for '{env}'. Creating one with SOPS...",
                fg=typer.colors.YELLOW,
            )
            try:
                # Create a basic placeholder YAML and encrypt it
                temp_file = secrets_path.with_suffix(".tmp.yml")
                temp_file.write_text("example_key: value\n")
                cls.sh(f"sops --encrypt {temp_file} > {secrets_path}")
                temp_file.unlink()
                typer.secho(
                    "Encrypted secrets file created successfully.",
                    fg=typer.colors.GREEN,
                )
            except Exception as e:
                typer.secho(
                    f"✘ Failed to create secrets file: {e}", fg=typer.colors.RED
                )
                raise typer.Exit(code=1)

        else:
            # Check if file exists but is not encrypted
            try:
                content = secrets_path.read_text()
                if "sops:" not in content:
                    typer.secho(
                        f"⚠️  Secrets file exists but is not encrypted. Encrypting with SOPS...",
                        fg=typer.colors.YELLOW,
                    )
                    temp_file = secrets_path.with_suffix(".tmp.yml")
                    ...
                    temp_file.unlink()
                    typer.secho(
                        "Secrets file encrypted successfully.", fg=typer.colors.GREEN
                    )
            except Exception as e:
                typer.secho(
                    f"✘ Failed to inspect or encrypt secrets file: {e}",
                    fg=typer.colors.RED,
                )
                raise typer.Exit(code=1)
        ...
        try:
            ...
            typer.secho(f"✘ Failed to open secrets file: {e}", fg=typer.colors.RED)
            raise typer.Exit(code=1)
