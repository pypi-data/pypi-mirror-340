...
self.set_betas()
X= self.betas_all
y = self.idat_handler.annotation_df["Methylation class"]
...
# Example pipeline
pipeline = Pipeline([
    ('drop_constant', ConstantFeatureRemover()),
    ('select_k_best', SelectKBest(k=100)),
])

# Example pipeline
pipeline_old = Pipeline([
    ('drop_constant', VarianceThreshold(threshold=0)),
    ('select_k_best', SelectKBest(k=100)),
])
