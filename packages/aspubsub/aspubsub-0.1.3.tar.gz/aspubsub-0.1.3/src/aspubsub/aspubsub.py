...
class GeneralPurposePubSubManager(ABC):
    ...
    def __init__(
        ...
        self._channel_dispatch_locks: Dict[str, asyncio.Lock] = defaultdict(asyncio.Lock)
    ...
    async def _run_central_listener(self):
        """
        Listens for messages from the backend queue, creates tasks
        to dispatch them, ensuring per-channel order via locks.
        """
            ...
            while not self._stop_event.is_set():
                try:
                    event: Optional[Event] = await asyncio.wait_for(
                        self.psb.next_published(), timeout=1.0 # Adjust timeout as needed
                    )
                    # in the future Backend might signal closure or timeout yielded None
                    if event is None: 
                        continue
                    ...
                    dispatch_task = asyncio.create_task(
                        self._dispatch_message_wrapper(event.channel, event.data),
                        name=f"dispatch-{event.channel}-{id(event.data)}" # Optional: name task for debugging
                    )
                    ...
                    logger.exception(f"Error in central listener fetching loop: {e}")
                    await asyncio.sleep(0.1) # Prevent fast error loops
        except asyncio.CancelledError:
            logger.info("Central listener task finally cancelled.")
        finally:
            logger.info("Central listener stopping. Waiting for outstanding dispatch tasks...")
            if running_dispatch_tasks:
                await asyncio.wait(running_dispatch_tasks, timeout=5.0)
            logger.info("Central listener stopped.")
            
    async def _dispatch_message_wrapper(self, concrete_channel: str, message: str):
        """
        A wrapper function that acquires the channel-specific lock
        before calling the actual dispatch_message.
        """
        channel_lock = self._channel_dispatch_locks[concrete_channel]
        async with channel_lock:
            try:
                await self.dispatch_message(concrete_channel, message)
            except Exception as e:
                logger.exception(f"Error during dispatch_message for channel {concrete_channel}: {e}")
