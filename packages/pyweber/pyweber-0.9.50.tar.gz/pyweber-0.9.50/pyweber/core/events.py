...
class EventHandler:
    ...
    def update(self):
        if inspect.iscoroutinefunction(self.__update):
            Thread(target=asyncio.run, args=(self.__update(self.__send, self.session_id),), daemon=True).start()
        
        else:
            self.__update(self.__send, self.session_id)
    
    def reload(self):
        if inspect.iscoroutinefunction(self.__update):
            Thread(target=asyncio.run, args=(self.__reload(self.__send, self.session_id),), daemon=True).start()
        else:
            self.__reload(self.__send, self.session_id)
        return self.update()
...
class EventConstrutor:
    def __init__(self, ws_message: 'wsMessage', ws_update: Callable, ws_reload: Callable, send=None):
