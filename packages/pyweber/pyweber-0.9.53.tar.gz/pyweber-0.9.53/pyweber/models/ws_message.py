...
class wsMessage:
    def __init__(self, raw_message: dict[str, (str, float)], app: Pyweber, protocol: str = 'pyweber'):
    ...
    def get_window(self):
        ...
        window.session_storage = json.loads(self.get_window_values(key='sessionStorage')) #json
        window.local_storage = json.loads(self.get_window_values(key='localStorage')) #json

        return window
    
    def get_form_values(self) -> dict[str, (str, int, float)]:
        return json.loads(self.get_value(key='values')) or {} #json
    
    def get_raw_window(self):
        return json.loads(self.get_value(key='window_data')) #json
