...
class LoadStaticFiles:
    ...
    def __init__(self, path: str):
        ...
        if sys.platform == 'win32':
            ...
            self.path = os.path.join(self.script_dir, self.path)
            self.path_2 = str(Path(self.script_dir) / path.removeprefix('/'))
    ...
    def load(self) -> str | bytes:
        ...
        if os.path.exists(self.path):
            return self.__read_file(path=self.path, mode=mode, encoding=encoding)
        
        if sys.platform != 'win32':
            if os.path.exists(self.path_2):
                return self.__read_file(path=self.path_2, mode=mode, encoding=encoding)
