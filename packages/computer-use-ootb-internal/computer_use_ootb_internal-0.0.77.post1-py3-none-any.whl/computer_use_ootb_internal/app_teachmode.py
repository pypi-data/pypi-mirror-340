...
async def stop_processing():
    if shared_state.is_processing:
        # Set both flags to ensure stopping
        ...
        # Give the thread a chance to terminate naturally
        ...
        if shared_state.is_processing and shared_state.processing_thread and shared_state.processing_thread.is_alive():
            shared_state.is_processing = False
        
        return JSONResponse(
            content={"status": "success", "message": "Processing has been stopped"},
            status_code=200
        )
    else:
        return JSONResponse(
...
def process_input():
    ...
    try:
        ...
        # shared_state.chatbot_messages.append(error_message)
