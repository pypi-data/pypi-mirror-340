...
from xync_schema.models import Ex, TestEx, Ad, Coin, Cur, Pmcur, Cred, CredEx, Actor
...
class TestAgent(BaseTest):
    ...
    async def test_get_orders(self, clients: list[tuple[BaseAgentClient, BaseAgentClient]]):
        for maker, taker in clients:
            ...
            t, _ = await TestEx.update_or_create({"ok": ok}, ex=taker.ex.name, action=ExAction.get_orders)
    ...
    async def test_order_request(self, clients: list[tuple[BaseAgentClient, BaseAgentClient]]):
        for maker, taker in clients:
            ads = await maker.ex_client.ads("USDT", "RUB", False)
            for ad in ads:
                # req = BaseOrderReq(
                #     ad_id=ad.exid,
                #     is_sell=ad.,
            ...
            t, _ = await TestEx.update_or_create({"ok": ok}, ex=taker.ex.name, action=ExAction.order_request)
            assert t.ok, "No get orders"
    ...
    async def test_ad_del(self, clients: list[tuple[BaseAgentClient, BaseAgentClient]]):
        for maker, taker in clients:
            ...
            t, _ = await TestEx.update_or_create({"ok": ad_del}, ex=taker.ex.name, action=ExAction.ad_del)
            assert t.ok, "No add new ad"
    ...
    async def test_ad_switch(self, clients: list[tuple[BaseAgentClient, BaseAgentClient]]):
        for maker, taker in clients:
            ...
            t, _ = await TestEx.update_or_create({"ok": ad_switch}, ex=taker.ex.name, action=ExAction.ad_switch)
    ...
    async def test_ads_switch(self, clients: list[tuple[BaseAgentClient, BaseAgentClient]]):
        for maker, taker in clients:
            ...
            t, _ = await TestEx.update_or_create({"ok": ads_switch}, ex=taker.ex.name, action=ExAction.ads_switch)
    ...
    async def test_get_user(self, clients: list[tuple[BaseAgentClient, BaseAgentClient]]):
        for maker, taker in clients:
            ...
            t, _ = await TestEx.update_or_create({"ok": ok}, ex=taker.ex.name, action=ExAction.get_user)
