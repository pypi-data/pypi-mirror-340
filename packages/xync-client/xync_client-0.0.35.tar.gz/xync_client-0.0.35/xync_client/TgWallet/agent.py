...
class AgentClient(BaseAgentClient, AuthClient):
    ...
    async def order_request(self, base_req: BaseOrderReq) -> OrderEpyd | dict:
        common = OrderReqTrait(
            offerId=base_req.ad_id, type="SALE" if base_req.is_sell else "BUY", paymentDetailsId=base_req.cred_id
        )
        req = (
            OrderAmountReq(**common.model_dump(), amount=AvailableAmountVolume)
            if base_req.fiat_amount
            else OrderVolumeReq(**common.model_dump(), volume=AvailableAmountVolume)
        )
    ...
    async def ad_epyd2pydin(self, ad_: OneAdTakerMakerSale | OneAdMakerBuy | OneAdTakerBuy) -> AdBuyIn | AdSaleIn:
        ...
        return AdSaleIn(**ad_in.model_dump(), creds_=[cx.cred for cx in credsexs])
