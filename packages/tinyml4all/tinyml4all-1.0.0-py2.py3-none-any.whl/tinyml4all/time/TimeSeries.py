...
class TimeSeries(HasSources, IsPickleable):
    ...
    @classmethod
    def merge(cls, table: "TimeSeries", update: dict) -> Self:
        """
        ...
        :return:
        """
        assert len(table.sources) == 1, 'TimeSeries is not collapsed'

        table.sources = [table.sources[0].update(source_name=':memory:', index=0, **update)]
