...
def debug_package_files():
    """Debug function to check if necessary files are present and their content."""
    package_dir = os.path.dirname(__file__)
    print(f"\nDebugging package files in: {package_dir}")
    ...
    if os.path.exists(entrypoint_path):
        print(f"✓ entrypoint.sh exists at {entrypoint_path}")
        with open(entrypoint_path, 'r') as f:
            content = f.read()
        print("entrypoint.sh content:")
        print("-" * 40)
        print(content)
        print("-" * 40)
    else:
...
def build_and_run_mysql_container():
    ...
    if not ensure_docker_files_exist():
        print("\n⚠️ Required package files are missing. Falling back to official MySQL image.")
    ...
    try:
        ...
        try:
            ...
            # Check specifically for entrypoint.sh error
            if process.returncode != 0:
                stderr = process.stderr.lower()
                if "exec /entrypoint.sh: no such file or directory" in stderr:
                    print("Detected missing entrypoint.sh in the Docker image.")
                    print("Falling back to official MySQL image...")
                    ...
                    print("Please try running with sudo if you have permission issues.")
                    return 1
...
if __name__ == "__main__":
    # Add this call at the start of build_and_run_mysql_container function
