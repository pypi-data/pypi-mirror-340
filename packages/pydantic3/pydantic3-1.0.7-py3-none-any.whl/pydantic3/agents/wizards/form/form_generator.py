...
class SelectOption(BaseModel):
    ...
    label: str = Field(description="Display label for the option in the specified language")
    value: str = Field(description="Unique identifier for the option (English, camel_case)")

class FormField(BaseModel):
    """Definition of a form field."""
    name: str = Field(description="Unique identifier for the field (snake_case)")
    title: str = Field(description="Display title for the field (human-readable)")
    description: str = Field(description="Helpful description text")
...
class FormGenerator:
    ...
    def __init__(self, form_intent: FormIntentRequest, model_name: str = "openai/gpt-4o-2024-11-20", api_key: Optional[str] = None, language: str = "en"):
        self.form_intent = form_intent
    ...
    async def generate_complete_structure(self) -> CompleteFormStructure:
        ...
        # Convert wizard data to YAML for easier reading by the LLM
        wizard_data = self.form_intent.data.model_dump()
        flat_wizard_data = yaml.dump(wizard_data)
        schema = CompleteFormStructure.model_json_schema()

        # Get language-specific instructions
        language_instructions = self._get_language_specific_instructions()
...
## LANGUAGE SETTINGS
Language for form titles, labels, and options: {self.language}
...
2. A "relation" object containing:
   ...
   - "is_multiple": Boolean indicating if multiple instances can be referenced (optional, default false)
...
"""
    ...
    def _get_language_specific_instructions(self) -> str:
        """Get language-specific instructions based on the language setting."""
        if self.language == "ru":
            return """
            Russian (ru) language instructions:
            - All titles, descriptions, and labels should be in Russian
            - Field names should still be in English snake_case for technical compatibility
            - For select/multiselect options, use Russian for labels and English snake_case for values
            - Form explanations should be written in Russian
            """
        elif self.language == "es":
            return """
            Spanish (es) language instructions:
            - All titles, descriptions, and labels should be in Spanish
            - Field names should still be in English snake_case for technical compatibility
            - For select/multiselect options, use Spanish for labels and English snake_case for values
            - Form explanations should be written in Spanish
            """
        elif self.language == "fr":
            return """
            French (fr) language instructions:
            - All titles, descriptions, and labels should be in French
            - Field names should still be in English snake_case for technical compatibility
            - For select/multiselect options, use French for labels and English snake_case for values
            - Form explanations should be written in French
            """
        # Default English instructions
        return """
        English (en) language instructions:
        - All titles, descriptions, and labels should be in English
        - Field names should be in English snake_case
        - For select/multiselect options, use English for both labels and values (in snake_case)
        - Form explanations should be written in English
        """
