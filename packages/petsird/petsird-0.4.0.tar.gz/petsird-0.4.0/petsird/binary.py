...
class DetectionBinSerializer(_binary.RecordSerializer[DetectionBin]):
    def __init__(self) -> None:
        super().__init__([("det_el_idx", _binary.uint32_serializer), ("energy_idx", _binary.uint32_serializer)])

    def write(self, stream: _binary.CodedOutputStream, value: DetectionBin) -> None:
        if isinstance(value, np.void):
            self.write_numpy(stream, value)
            return
        self._write(stream, value.det_el_idx, value.energy_idx)

    def write_numpy(self, stream: _binary.CodedOutputStream, value: np.void) -> None:
        self._write(stream, value['det_el_idx'], value['energy_idx'])

    def read(self, stream: _binary.CodedInputStream) -> DetectionBin:
        field_values = self._read(stream)
        return DetectionBin(det_el_idx=field_values[0], energy_idx=field_values[1])


class CoincidenceEventSerializer(_binary.RecordSerializer[CoincidenceEvent]):
    def __init__(self) -> None:
        super().__init__([("detection_bins", _binary.FixedVectorSerializer(DetectionBinSerializer(), 2)), ("tof_idx", _binary.uint32_serializer)])
...
class DetectionEfficienciesSerializer(_binary.RecordSerializer[DetectionEfficiencies]):
    ...
    def read(self, stream: _binary.CodedInputStream) -> DetectionEfficiencies:
        field_values = self._read(stream)
        return DetectionEfficiencies(detection_bin_efficiencies=field_values[0], module_pair_sgidlut=field_values[1], module_pair_efficiencies_vector=field_values[2])
...
class TripleEventSerializer(_binary.RecordSerializer[TripleEvent]):
    def __init__(self) -> None:
        super().__init__([("detection_bins", _binary.FixedVectorSerializer(DetectionBinSerializer(), 3)), ("tof_indices", _binary.FixedVectorSerializer(_binary.uint32_serializer, 2))])
