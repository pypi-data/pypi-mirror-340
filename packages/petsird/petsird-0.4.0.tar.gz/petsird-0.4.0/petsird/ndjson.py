...
class DetectionBinConverter(_ndjson.JsonConverter[DetectionBin, np.void]):
    def __init__(self) -> None:
        self._det_el_idx_converter = _ndjson.uint32_converter
        self._energy_idx_converter = _ndjson.uint32_converter
        super().__init__(np.dtype([
            ("det_el_idx", self._det_el_idx_converter.overall_dtype()),
            ("energy_idx", self._energy_idx_converter.overall_dtype()),
        ]))

    def to_json(self, value: DetectionBin) -> object:
        if not isinstance(value, DetectionBin): # pyright: ignore [reportUnnecessaryIsInstance]
            raise TypeError("Expected 'DetectionBin' instance")
        json_object = {}

        json_object["detElIdx"] = self._det_el_idx_converter.to_json(value.det_el_idx)
        json_object["energyIdx"] = self._energy_idx_converter.to_json(value.energy_idx)
        return json_object
    ...
    def numpy_to_json(self, value: np.void) -> object:
        ...
        json_object["detElIdx"] = self._det_el_idx_converter.numpy_to_json(value["det_el_idx"])
        json_object["energyIdx"] = self._energy_idx_converter.numpy_to_json(value["energy_idx"])
        return json_object

    def from_json(self, json_object: object) -> DetectionBin:
        if not isinstance(json_object, dict):
            raise TypeError("Expected 'dict' instance")
        return DetectionBin(
            det_el_idx=self._det_el_idx_converter.from_json(json_object["detElIdx"],),
            energy_idx=self._energy_idx_converter.from_json(json_object["energyIdx"],),
        )
    ...
    def from_json_to_numpy(self, json_object: object) -> np.void:
        if not isinstance(json_object, dict):
            raise TypeError("Expected 'dict' instance")
        return (
            self._det_el_idx_converter.from_json_to_numpy(json_object["detElIdx"]),
            self._energy_idx_converter.from_json_to_numpy(json_object["energyIdx"]),
        ) # type:ignore 


class CoincidenceEventConverter(_ndjson.JsonConverter[CoincidenceEvent, np.void]):
    def __init__(self) -> None:
        self._detection_bins_converter = _ndjson.FixedVectorConverter(DetectionBinConverter(), 2)
...
class DetectionEfficienciesConverter(_ndjson.JsonConverter[DetectionEfficiencies, np.void]):
    def __init__(self) -> None:
        self._detection_bin_efficiencies_converter = _ndjson.OptionalConverter(_ndjson.NDArrayConverter(_ndjson.float32_converter, 2))
...
class TripleEventConverter(_ndjson.JsonConverter[TripleEvent, np.void]):
    def __init__(self) -> None:
        self._detection_bins_converter = _ndjson.FixedVectorConverter(DetectionBinConverter(), 3)
