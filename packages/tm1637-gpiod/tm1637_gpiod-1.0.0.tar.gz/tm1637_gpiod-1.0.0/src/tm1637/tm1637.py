...
class TM1637:
    ...
    def _write_byte(self, byte):
        for i in range(8):
            bit = Value.ACTIVE if (byte >> i) & 1 else Value.INACTIVE
            self._lines.set_value(self.dio, bit)
    ...
    def show(self, string, sep=False):
        ...
        string = "{:4s}".format(string)[:4] # truncate to length 4
    ...
    def scroll(self, string, delay=0.25):
        ...
        for i in range(len(data) - (4 - 1)): # exclude right padding from traversal
    ...
    def temperature(self, num):
        ...
        Values outside of this range show 'lo' or 'hi'."""
        if num < -9:
            self.show("lo")  # low
        elif num > 99:
            self.show("hi")  # high
