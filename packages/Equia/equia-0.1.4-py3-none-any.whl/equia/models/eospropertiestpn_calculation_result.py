...
class EosPropertiesTPnCalculationResult:
    ...
    def __str__(self):
        """
        Returns a string representation of the EosPropertiesTPnCalculationResult instance.
        """
        parts = ["EosPropertiesTPnCalculationResult:"]
        parts.append(f"  Success: {self.success if not isinstance(self.success, Unset) else 'N/A'}")
        parts.append(f"  Calculation ID: {self.calculation_id if not isinstance(self.calculation_id, Unset) else 'N/A'}")

        if not isinstance(self.exception_info, Unset) and self.exception_info:
            parts.append("  Exception Info:")
            parts.append("    " + str(self.exception_info).replace("\n", "\n    "))
        else:
            parts.append("  Exception Info: None")

        if not isinstance(self.profiling, Unset) and self.profiling:
            parts.append("  Profiling:")
            parts.append("    " + str(self.profiling).replace("\n", "\n    "))
        else:
            parts.append("  Profiling: None")

        if not isinstance(self.point, Unset) and self.point:
            parts.append("  Point:")
            parts.append("    " + str(self.point).replace("\n", "\n    "))
        else:
            parts.append("  Point: None")
