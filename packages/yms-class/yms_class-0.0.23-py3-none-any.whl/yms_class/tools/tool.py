...
def get_all_artifacts_from_project(project_path, max_runs=None):
    """获取指定WandB项目的所有Artifact（包括所有Run中记录的Artifact）

    Args:
        project_path (str): 项目路径，格式为 "entity/project"
        max_runs (int, optional): 最大获取Run数量（用于调试）
    ...
    try:
        ...
        # 添加进度条（需要先安装tqdm: pip install tqdm）
        ...
        print(f"Error accessing project {project_path}: {str(project_error)}")
        return []
