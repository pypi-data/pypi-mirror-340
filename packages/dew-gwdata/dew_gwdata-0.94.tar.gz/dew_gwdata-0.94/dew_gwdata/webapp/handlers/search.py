...
def well_find(
    ...
    kwargs = {k: v for k, v in request.query_params.items() if not k in [
        "query", "env", "unit_no", "obs_no", "dh_no", "singular_search_only", "redirect_to"
    ]}
    types = []
    if unit_no:
    ...
    if len(wells) == 0:
        ...
        return RedirectResponse(
            f"/app/{redirect_to}?dh_no={wells.iloc[0].dh_no:.0f}&redirect_to={redirect_to}&env={env}" + extra
        )
    else:
        return RedirectResponse(
...
def wells_find_post(
    request: Request, payload_bytes: bytes = Depends(get_body)
):
    logger.info(f"wells_find_post Called")
    ...
    if dh_no:
        types.append("dh_no")
    logger.info(f"looking for types: {types}")
    wells = db.find_wells(query, types=types)
    ...
    if len(wells) == 0:
        ...
        url_str = webapp_utils.dhnos_to_urlstr(wells.dh_no)
        logger.info(f"{len(wells)} wells identified, redirecting via url_str length {len(url_str)}")
        return RedirectResponse(f"/app/wells_summary?url_str={url_str}&env={env}", status_code=status.HTTP_303_SEE_OTHER)
