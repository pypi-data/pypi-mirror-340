...
def fetch_wl_data(
    ...
    **kwargs,
    ...
    logger.debug(f"Fetching SA Geodata water levels for all {len(potential_aq_sites)} wells")
    ...
    if len(aq_sites) > 0:
        ...
        for aq_locid in aq_sites:
            ...
            logger.debug(f"Timeseries found for {aq_locid}:\n{aq_md2[['param', 'label']]}")
            if len(aq_md2) == 0:
                ts_uid = None
...
class ProductionZoneData:
    ...
    def __init__(self, wells, db=None, index_id="obs_no"):
        ...
        open_pzone_intervals.loc[
            pd.isnull(open_pzone_intervals.obs_no), "obs_no"
        ] = "NA"
        pzone = (
...
def add_elev_cols(df, dh_no, ignore_empty=True):
    orig_cols = [str(c) for c in df.columns]
    df["dh_no"] = dh_no
    df2 = depth_to_elev(df)
    df = df[orig_cols]
...
def current_ref_height(df):
    ...
    """    
    df.loc[(~pd.isnull(df.ground_elev)) & (~pd.isnull(df.ref_elev)), "ref_height_calc"] = df.ref_elev - df.ground_elev
    ...
    if len(df_calc):
        ...
        if len(df_manual):
            applied = df_manual.dropna(subset=["applied_date"]).sort_values("applied_date")
            if len(applied):
