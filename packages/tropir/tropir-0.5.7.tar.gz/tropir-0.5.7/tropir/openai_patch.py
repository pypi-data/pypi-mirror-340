...
def log_openai_call(provider, request_args, response, duration, success):
    ...
    # Try to run asynchronously if possible, otherwise fall back to threaded execution
    try:
        # Check if we're in an async context with a running event loop
        loop = asyncio.get_running_loop()
        # If we are, create a task
        asyncio.create_task(_async_log_openai_call(provider, request_args, response, duration, success))
    except RuntimeError:
        # No running event loop, use a thread instead
        def run_async_in_thread():
            ...
            try:
                ...
                loop.run_until_complete(_async_log_openai_call(provider, request_args, response, duration, success))
            finally:
                loop.close()
...
async def _async_log_openai_call(provider, request_args, response, duration, success):
    """Async implementation of OpenAI API call logging."""
    try:
        ...
        # Write to log file - run in thread pool to avoid blocking
        loop = asyncio.get_event_loop()
        await loop.run_in_executor(_thread_pool, send_log, log_entry)
