...
def log_anthropic_call(provider, request_args, response, duration, success):
    ...
    try:
        ...
        asyncio.create_task(_async_log_anthropic_call(provider, request_args, response, duration, success))
    except RuntimeError:
        ...
        def run_async_in_thread():
            ...
            try:
                ...
                loop.run_until_complete(_async_log_anthropic_call(provider, request_args, response, duration, success))
            finally:
                loop.close()
...
async def _async_log_anthropic_call(provider, request_args, response, duration, success):
    """Async implementation of Anthropic API call logging."""
    try:
...
def setup_anthropic_patching():
    ...
    logger.info("Setting up Anthropic tracking...")
    ...
    try:
        ...
        if hasattr(Messages, "create") and not getattr(Messages.create, '_llm_tracker_patched', False):
            logger.debug("Attempting to patch Anthropic Messages.create")
            ...
            logger.info("Successfully patched anthropic.resources.messages.Messages.create")
        ...
        if hasattr(AsyncMessages, "create") and not getattr(AsyncMessages.create, '_llm_tracker_patched', False):
            logger.debug("Attempting to patch Anthropic AsyncMessages.create")
            ...
            logger.info("Successfully patched anthropic.resources.messages.AsyncMessages.create")
    ...
    logger.info("Anthropic tracking setup complete.") 