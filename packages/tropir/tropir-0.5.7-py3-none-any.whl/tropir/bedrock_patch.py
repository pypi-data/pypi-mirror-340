...
def log_bedrock_call(provider, request_args, response, duration, success):
    ...
    try:
        ...
        asyncio.create_task(_async_log_bedrock_call(provider, request_args, response, duration, success))
    except RuntimeError:
        ...
        def run_async_in_thread():
            ...
            try:
                ...
                loop.run_until_complete(_async_log_bedrock_call(provider, request_args, response, duration, success))
            finally:
                loop.close()
...
async def _async_log_bedrock_call(provider, request_args, response, duration, success):
    """Async implementation of Bedrock API call logging."""
    try:
        ...
        log_entry = process_template_matching(processed_body.get("messages", []), user_id, stack_info, log_entry)
...
def setup_bedrock_patching():
    ...
    logger.info("Setting up Amazon Bedrock tracking...")
    ...
    try:
        ...
        if not getattr(BaseClient, "_bedrock_patch_applied", False):
            ...
            logger.info("Successfully patched BaseClient._make_api_call for Bedrock Runtime")
    ...
    logger.info("Amazon Bedrock tracking setup complete.")
