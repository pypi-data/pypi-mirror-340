...
def log_openai_call(provider, request_args, response, duration, success):
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Sending request to template matching API: {api_url}api/match_template")
            api_response = requests.post(
                api_url + "api/match_template", 
                json={"prompt": response_text}, 
                timeout=5
            )
            
            if api_response.status_code == 200:
                api_data = api_response.json()
                logger.debug(f"Received API response: {api_response.status_code}, data: {api_data}")
                ...
                logger.debug(f"Received API response with error status: {api_response.status_code}")
                log_entry["stack_trace"] = stack_info
        except Exception as e:
