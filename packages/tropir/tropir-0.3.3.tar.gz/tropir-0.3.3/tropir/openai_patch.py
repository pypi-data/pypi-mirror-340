...
def log_openai_call(provider, request_args, response, duration, success):
    ...
    try:
        ...
        # Send response to template matching API before adding stack_trace
        try:
            api_url = os.getenv("TROPIR_API_URL", "http://localhost:8080/")
            logger.info(f"⭐ SENDING REQUEST to template matching API: {api_url}api/match-template")
            
            # Extract user message from processed_messages
            user_message = next((msg.get("content", "") for msg in processed_messages if msg.get("role") == "user"), "")
            ...
            if api_response.status_code == 200:
                ...
                logger.info(f"⭐ RECEIVED API response: {api_response.status_code}, data: {api_data}")
                if api_data.get("match", False):
                    # If template match is found, add substitutions instead of stack_trace
                    log_entry["template_substitutions"] = api_data.get("template_substitutions", {})
                else:
                    # No match found, add stack_trace as usual
                    log_entry["stack_trace"] = stack_info
            else:
                # API request failed, add stack_trace as usual
                logger.info(f"⭐ RECEIVED API response with error status: {api_response.status_code}")
                log_entry["stack_trace"] = stack_info
        except Exception as e:
...
def setup_openai_patching():
    ...
    logger.info("Setting up OpenAI tracking...")
    ...
    try:
        ...
        if hasattr(OpenAICompletions, "create") and not getattr(OpenAICompletions.create, '_llm_tracker_patched', False):
            logger.debug("Attempting to patch OpenAI Completions.create")
            ...
            logger.info("Successfully patched openai.resources.chat.completions.Completions.create")

        elif hasattr(OpenAICompletions, "create") and getattr(OpenAICompletions.create, '_llm_tracker_patched', False):
            logger.debug("OpenAI Completions.create already patched.")
        else:
            # This case means the class exists but doesn't have 'create'
            logger.warning("Could not find 'create' method on OpenAI Completions class for patching.")
    ...
    logger.info("OpenAI tracking setup complete.") 