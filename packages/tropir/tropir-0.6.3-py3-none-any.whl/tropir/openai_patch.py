        ...
        try:
            ...
            for method_name in methods_to_patch:
                if hasattr(OpenAIResponses, method_name) and not getattr(getattr(OpenAIResponses, method_name), '_llm_tracker_patched', False):
                    ...
                    logger.debug(f"Successfully patched OpenAI Responses.{method_name} method")
                elif hasattr(OpenAIResponses, method_name) and getattr(getattr(OpenAIResponses, method_name), '_llm_tracker_patched', False):
                    logger.debug(f"OpenAI Responses.{method_name} already patched.")
                else:
                    logger.warning(f"Could not find '{method_name}' method on OpenAI Responses class for patching.")
            ...
            for method_name in async_methods_to_patch:
                if hasattr(AsyncOpenAIResponses, method_name) and not getattr(getattr(AsyncOpenAIResponses, method_name), '_llm_tracker_patched', False):
                    ...
                    logger.debug(f"Successfully patched AsyncOpenAI Responses.{method_name} method")
                elif hasattr(AsyncOpenAIResponses, method_name) and getattr(getattr(AsyncOpenAIResponses, method_name), '_llm_tracker_patched', False):
                    logger.debug(f"AsyncOpenAI Responses.{method_name} already patched.")
                else:
                    logger.warning(f"Could not find '{method_name}' method on AsyncOpenAI Responses class for patching.")
                
        except ImportError:
            logger.warning("Could not import 'openai.resources.responses.Responses'. OpenAI responses tracking may not work.")
        except Exception as e:
