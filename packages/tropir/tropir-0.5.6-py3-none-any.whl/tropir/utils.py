...
def get_user_id():
    """
    Gets the user_id associated with the API key.
    ...
    try:
        ...
        base_url = os.getenv("TROPIR_API_URL", "http://localhost:8080")
        ...
        try:
            ...
            if result.get("status") == "ok" and "user_id" in result:
                ...
                logger.debug(f"API key validated successfully for user: {user_id}")
                return user_id
            else:
            ...
            # Return None if response parsing fails
            return None
...
def process_template_matching(processed_messages, user_id, stack_info, log_entry):
    ...
    # Always add stack_trace to the log entry first to ensure it's available
    # even if the API call doesn't complete
    log_entry["stack_trace"] = stack_info
    
    # If there are no messages to process, return early
    if not processed_messages:
        return log_entry
    
    # Define the API call function that will run in a separate thread
    def call_template_api():
        try:
            ...
            if api_response.status_code == 200:
                ...
                # If all messages have matches, we can remove the stack_trace
                if not need_stack_trace:
                    log_entry.pop("stack_trace", None)
        except Exception as e:
            logger.error(f"Error calling template matching API: {str(e)}")
            # Stack trace is already added, so no need to do anything here
    
    # Start the API call in a separate thread and don't wait for it
    thread = threading.Thread(target=call_template_api)
    thread.daemon = True
    thread.start()
    
    # Return the log entry immediately with the stack trace
    # The API call will update it asynchronously if successful
    return log_entry