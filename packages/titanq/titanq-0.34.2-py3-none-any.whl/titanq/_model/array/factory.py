...
class ArrayLikeFactory:
    ...
    def create_scipy_coo_array(self, array: ArrayLikeFactoryInputs) -> ScipyCooArray:
        """Creates a `ScipyCooArray` array like object."""
        array_builder = self._as_array_builder(array)

        return self._array_like_from_array_builder(array_builder, ScipyCooArray)
    ...
    def _as_array_builder(self, array: ArrayLikeFactoryInputs, data_type: np.dtype = _TITANQ_DTYPE) -> ArrayBuilder:
        ...
        if isinstance(array, np.ndarray): # simply create the NumpyArray
            return NumpyArray(array.astype(data_type))

        elif isinstance(array, coo_array): # simply create the ScipyCooArray
