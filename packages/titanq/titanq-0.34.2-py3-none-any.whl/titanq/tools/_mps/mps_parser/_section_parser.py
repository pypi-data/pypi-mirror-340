...
SINGLE_VALUE_PATTERN = re.compile(r"^(\S+)\s+(\S+)\s+([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)\s*$")
# Matches "C1 R1 2.0 R2 3.0" | "C1 R1 -2.0E+10 R2 3.0E+10"
DOUBLE_VALUE_PATTERN = re.compile(r"^(\S+)\s+(\S+)\s+([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)\s+(\S+)\s+([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)\s*$")
...
class BoundParser(SectionParser):
    ...
    def __init__(self, start_index, lines):
        ...
        self._bounds_pattern = re.compile(r"^(LO|LI|UP|UI|FX|FR|MI|PL|BV|SC)\s+(\S+)\s+(\S+)(\s+[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)?\s*$")
