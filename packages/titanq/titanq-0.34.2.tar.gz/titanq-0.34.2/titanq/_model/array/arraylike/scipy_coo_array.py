...
class ScipyCooArray(ArrayLike):
    ...
    def __iter__(self) -> Iterator['ScipyCooArray']:
        if self.ndim() == 1:
            raise NotImplementedError(f"{self.__class__.__name__}: 1D arrays are not iterable")

        for row_index in range(self._array.shape[0]):
            row_data = self._array.data[self._array.row == row_index]
            row_cols = self._array.col[self._array.row == row_index]

            new_array = np.zeros(self._array.shape[1])
            for data, col in zip(row_data, row_cols):
                new_array[col] = data

            yield ScipyCooArray(coo_array(new_array))
