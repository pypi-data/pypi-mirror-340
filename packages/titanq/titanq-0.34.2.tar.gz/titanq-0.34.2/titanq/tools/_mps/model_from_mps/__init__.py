...
def _get_constraint_weights(
    ...
    return coo_array((data, (rows, cols)), shape=(num_constraints, variables_length))
...
def _create_constraints(model: Model, variables_map: Dict[str, int], mps: MPSObject) -> None:
    ...
    constraint_weights_array_like = ScipyCooArray(constraint_weights)
    constraint_bounds_array_like = NumpyArray(np.array(constraint_bounds))
