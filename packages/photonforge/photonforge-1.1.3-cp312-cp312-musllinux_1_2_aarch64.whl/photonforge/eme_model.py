...
class EMEModel(Model):
    ...
    def as_bytes(self) -> bytes:
        ...
        if self.medium is not None:
            obj["medium"] = tidy3d_model._convert_arrays(self.medium.dict())
        if len(self.monitors) > 0:
            obj["monitors"] = [tidy3d_model._convert_arrays(m.dict()) for m in self.monitors]
        if len(self.structures) > 0:
            obj["structures"] = [tidy3d_model._convert_arrays(s.dict()) for s in self.structures]
        if self.grid_spec is not None:
            obj["grid_spec"] = (
                tidy3d_model._convert_arrays(self.grid_spec.dict())
                if isinstance(self.grid_spec, tidy3d.GridSpec)
                else self.grid_spec
            )
