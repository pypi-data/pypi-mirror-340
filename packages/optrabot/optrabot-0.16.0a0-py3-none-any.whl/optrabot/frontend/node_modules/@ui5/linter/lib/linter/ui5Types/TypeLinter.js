...
export default class TypeChecker {
    ...
    #workspace;
    #filePathsWorkspace;
    constructor({ workspace, filePathsWorkspace, context }) {
    ...
    async lint() {
        ...
        const files = new Map();
        const sourceMaps = new Map(); // Maps a source path to source map content
        ...
        const program = ts.createProgram(allResources.map((resource) => resource.getPath()), this.#compilerOptions, host);
        createProgramDone();
        ...
        for (const sourceFile of program.getSourceFiles()) {
            if (!sourceFile.isDeclarationFile && pathsToLint.includes(sourceFile.fileName)) {
                ...
                const linter = new SourceFileLinter(this.#context, sourceFile.fileName, sourceFile, sourceMaps, checker, reportCoverage, messageDetails, apiExtract, manifestContent);
                await linter.lint();
                linterDone();
            }
        }
