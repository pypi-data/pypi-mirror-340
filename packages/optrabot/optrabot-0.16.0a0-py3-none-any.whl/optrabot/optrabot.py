...
class OptraBot():
	...
	async def startup(self):
		...
		environment = os.environ.get('OPTRABOT_ENV', 'DEV')
		if environment == 'DEV':
			logger.warning('Run the UI by command: npm start')
		else:
			webPort = conf.get('general.port')
			url = f"http://localhost:{webPort}/optrabot.html"
			webbrowser.open(url)
		...
		for item in conf.getTemplates():
			...
			if trigger.type == TriggerType.Time:	
				...
				if trigger._trigger_time.time() < now.time():
					...
					continue
				logger.debug(f'Scheduling time trigger for template {template.name} at {trigger._trigger_time}')
