...
import { getPropertyNameText, getSymbolForPropertyInConstructSignatures, getPropertyAssignmentInObjectLiteralExpression, getPropertyAssignmentsInObjectLiteralExpression, findClassMember, isClassMethod, } from "./utils.js";
...
function isSourceFileOfPseudoModuleType(sourceFile) {
    return sourceFile.fileName.startsWith("/types/@ui5/linter/overrides/pseudo-modules/");
}
function isSourceFileOfTypeScriptLib(sourceFile) {
...
export default class SourceFileLinter {
    ...
    #metadata;
    constructor(context, resourcePath, sourceFile, sourceMaps, checker, reportCoverage = false, messageDetails = false, apiExtract, manifestContent) {
        ...
        this.#reporter = new SourceFileReporter(context, resourcePath, sourceFile, sourceMaps?.get(sourceFile.fileName));
    ...
    analyzeControlRendererInternals(node) {
        ...
        if (node && (ts.isObjectLiteralExpression(node) || ts.isVariableDeclaration(node))) {
            ...
            if (nodeToHighlight) {
                ...
                this.context.addLintingMessage(nodeSourceFile.fileName, MESSAGE.NO_DEPRECATED_RENDERER, undefined, getPositionsForNode({
    ...
    analyzeIconCallInRenderMethod(node) {
        ...
        const findIconCallExpression = (childNode) => {
            if (ts.isCallExpression(childNode) &&
                ...
                const nodeSourceMap = this.sourceMaps?.get(nodeSourceFile.fileName);
                this.context.addLintingMessage(nodeSourceFile.fileName, MESSAGE.NO_ICON_POOL_RENDERER, undefined, getPositionsForNode({
                    node: childNode,
                    sourceFile: nodeSourceFile,
                    resourcePath: nodeSourceFile.fileName,
                    traceMap: nodeSourceMap ? new TraceMap(nodeSourceMap) : undefined,
    ...
    analyzeNewExpression(node) {
        ...
        else if (nodeType.symbol.declarations?.some((declaration) => this.isUi5ClassDeclaration(declaration, "sap/ui/core/Control"))) {
    ...
    analyzeCallExpression(node) {
        ...
        if (exprType.symbol && moduleDeclaration) {
            ...
            if (symbolName === "init" && moduleName === "sap/ui/core/Lib") {
                ...
                nodeType.symbol?.declarations?.some((declaration) => this.isUi5ClassDeclaration(declaration, "sap/ui/core/Control"))) {
                this.#analyzeNewAndApplySettings(node);
            }
    ...
    getSymbolModuleDeclaration(symbol) {
    ...
    #analyzeOdataModelV2CreateEntry(node) {
        ...
        if (properties && ts.isArrayLiteralExpression(properties.initializer)) {
            this.#reporter.addMessage(MESSAGE.PARTIALLY_DEPRECATED_ODATA_MODEL_V2_CREATE_ENTRY_PROPERTIES_ARRAY, properties);
        }
    }
    ...
    #analyzeNewOdataModelV4(node) {
        if (!node.arguments || node.arguments.length < 1 || !ts.isObjectLiteralExpression(node.arguments[0])) {
            return;
        }
        const synchronizationModeProb = getPropertyAssignmentInObjectLiteralExpression("synchronizationMode", node.arguments[0]);
    ...
    */
    #isPropertyBinding(node, propNames) {
        const controlAmbientModule = this.getSymbolModuleDeclaration(this.checker.getTypeAtLocation(node).symbol);
        let classArg;
    ...
    analyzeExportedValuesByLib(node) {
        ...
        const libAmbientModule = ambientModules.find((module) => module.name === `"${potentialLibImport}"`);
        const isRegisteredAsUi5Module = ambientModules.some((module) => module.name === `"${moduleName}"`);
    ...
    analyzeImportDeclaration(importDeclarationNode) {
        ...
        if (this.isSymbolOfPseudoModuleType(symbol)) {
            ...
            if (isEnum) {
                this.#reporter.addMessage(MESSAGE.NO_DIRECT_ENUM_ACCESS, { moduleName: moduleSpecifierNode.text }, moduleSpecifierNode);
            }
            else { // Data Type
                this.#reporter.addMessage(MESSAGE.NO_DIRECT_DATATYPE_ACCESS, { moduleName: moduleSpecifierNode.text }, moduleSpecifierNode);
            }
        }
    }
