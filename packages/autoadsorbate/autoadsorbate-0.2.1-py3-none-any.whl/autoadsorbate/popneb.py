...
class popNEB:
    ...
    def run_few_steps(self, initial, final):
        ...
        """
        dyn_index = 'iniNEB'
        few_step_trj = self.run_neb(initial, final, self.ini_chain_steps, self.n_ini_chain, dyn_index=dyn_index, neb_type='iniNEB')
        return few_step_trj
    ...
    def run_neb(self, initial, final, max_steps, n_chain, dyn_index, neb_type='NEB'):
        ...
        neb = NEB(images)
        qn = BFGS(neb)
        qn.attach(neb_observer, dyn=qn, dyn_index=dyn_index, uid=self._id, traj_file=self.tmp_file.name, interval=1)
    ...
    def relax_images(self, images):
        ...
        for i, atoms in enumerate(relax_chain):
            ...
            dyn.attach(relax_observer, dyn=dyn, dyn_index=i, uid=self._id, traj_file=self.tmp_file.name)
    ...
    def run_neb_from_minima(self):
        ...
        for a1, a2 in zip(self.minima_allong_chain, self.minima_allong_chain[1:]):
            self.neb_path += self.run_neb(a1, a2, self.max_steps, self.n_chain, dyn_index)
            dyn_index+=1
    ...
    def update_result(self):
        ...
        result_df['converged'] = [max_f < self.f_max for max_f in result_df.max_f.values]
    ...
    def show_neb_report(self, x_scale=1, y_scale=1.5, converged=[True]):
        ...
        type = 'NEB'
        pdf = self.result_df[self.result_df.type.isin([type]) & self.result_df.converged.isin(converged)]
        ...
        gs = GridSpec(len(pdf.dyn_index.unique())+1, len(pdf.image_index.unique()), figure=fig)
...
def get_default_soap(traj):
    ...
    soap = SOAP(species=species,r_cut=4.5,n_max=8,l_max=4,sigma=0.5,periodic=False,rbf='gto',crossover=True)
    return soap
...
def get_best_clustering(X):
    ...
    best_clustering_ind = silhouette_scores.index(max(silhouette_scores)) #index shift due to cut list
...
def compare_cartesian(self, images, compare_threshold):
    ...
    for i, atoms in enumerate(images):
        deltas = [np.sum(np.abs(atoms.positions - images[j].positions)) for j in unique_index]
