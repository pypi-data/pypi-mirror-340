...
def get_shrinkwrap_site_n_vector(atoms: Atoms, site: np.ndarray, grid: Atoms, touch_sphere_size: float) -> np.ndarray:
    ...
    if len(grid_site_indices) == 0:
        raise ValueError(f"No grid points found within the specified radius. Closest grid point at {np.min(grid_distances)}, cutoff {touch_sphere_size * 1.2}")
...
def get_shrinkwrap_site_h_vector(ads_site_atoms: Atoms, n_vector: np.ndarray) -> np.ndarray:
    ...
    if len(ads_site_atoms) == 1:
        ...
        h_vector = ads_site_atoms.positions[closest_indices[0]] - ads_site_atoms.positions[closest_indices[1]]
        h_vector /= np.linalg.norm(h_vector)

    return h_vector

def get_shrinkwrap_ads_sites(atoms: Atoms, precision: float = 0.5, touch_sphere_size: float = 3, return_trj: bool = False):
    ...
    grid = get_shrinkwrap_grid(atoms, precision=precision, touch_sphere_size=touch_sphere_size)
    surf_ind = shrinkwrap_surface(atoms, precision=precision, touch_sphere_size=touch_sphere_size)
    ...
    for target in targets:
        ...
        if len(target) == 1:
            ...
            for c in itertools.combinations([atom.index for atom in extended_atoms if atom.symbol == 'X'], len(target)):
                c = list(c)
        ...
        n_vec =  get_shrinkwrap_site_n_vector(extended_atoms, site_coord, extended_grid, touch_sphere_size)
    ...
    if return_trj:  # demo mode
        extended_atoms = extended_atoms[[atom.index for atom in extended_atoms if np.linalg.norm(atom.position - site_coord) < 7]]
        for m in range(20):
...
def get_list_of_touching(atoms: Atoms, grid: Atoms, surf_ind: np.ndarray, touch_sphere_size: float = 3, epsilon: float = 0.3, precision: float = 0.5) -> list:
    ...
    surf_ind_extended_plusone = surf_ind_extended + 1  # Avoid 0 index and account for PBC
    ...
    for g in grid:
        ...
        mask = (distances > touch_sphere_size - epsilon) & (distances < touch_sphere_size + epsilon)
...
def get_shrinkwrap_grid(slab: Atoms, precision: float, drop_increment: float = 0.1, touch_sphere_size: float = 3, marker: str = 'He', raster_speed_boost: bool = False) -> Atoms:
    ...
    if raster_speed_boost:
        raster_surf_index = get_surface_from_rasterized_top_view(slab, pixel_per_angstrom=10)
    ...
    while (distances_to_grid > touch_sphere_size).any():
        grid_positions -= drop_vectors * (distances_to_grid > touch_sphere_size)[:, np.newaxis]
        distances_to_grid = cdist(grid_positions, slab_positions).min(axis=1)
        
        if (distances_to_grid > touch_sphere_size).all() and (grid_positions[:, 2] <= 0).all():
            break
    
    grid = Atoms([marker for _ in grid_positions], grid_positions, pbc=[True, True, True], cell=slab.cell)
    grid = grid[[atom.index for atom in grid if atom.position[2] > 0]]
    
    return grid


def shrinkwrap_surface(slab: Atoms, precision: float = 0.5, touch_sphere_size: float = 3.1) -> np.ndarray:
    ...
    """
    grid_positions = get_shrinkwrap_grid(slab, precision, touch_sphere_size=touch_sphere_size-0.2).positions
...
def shrinkwrap_particle(particle: Atoms, precision: float, touch_sphere_size: float, angular_resolution: float, return_grid: bool = False):
    ...
    for ax, ay in zip(angle_x, angle_y):
        ...
        print(f'At angle_x: {ax}, angle_y: {ay}. Surface atoms found: {len(surf_indices)}')
        ...
        if return_grid:
            ...
            grid += get_shrinkwrap_grid(ratoms, precision=precision, touch_sphere_size=touch_sphere_size)

        surf_indices.update(shrinkwrap_surface(ratoms, precision=precision, touch_sphere_size=touch_sphere_size))
...
def _place_adsorbate(slab: Atoms, adsorbate: Atoms, surf_nvector: np.ndarray, site: np.ndarray, surf_hvector: np.ndarray = np.array([1, 0, 0])) -> Atoms:
...
def get_adsorbate_formula(atoms: Atoms) -> tuple:
    ...
    fragment_indices = fragment_indices[fragment_indices != 0]  # Exclude the first fragment

    adsorbate_atoms = atoms[[i for i, atom in enumerate(atoms) if atoms.arrays['fragments'][i] in fragment_indices]]
...
def NEW_attach_intermediate(atoms=None,
                            smiles_list=[],
                            sites_dict={
                                'coordinates': None,
                                'connectivity': None,
    ...
    if not check_sites_dict(sd):
        raise ValueError('Site dictionary has inconsistent length. All values in sites_dict shall be lists of equal length.')
    ...
    for i, coord in enumerate(coordinates):
        for smile in smiles_list:
            ...
            if smile == 'H':
                new_atoms = handle_hydrogen(atoms, coord, i, smile, coordinates, connectivity, topology, n_vector, h_vector, site_formula)
                out_trj.append(new_atoms)
            else:
                handle_other_adsorbates(atoms, smile, i, coord, fragment_overlap_thr, configs_per_smiles, try_perturb, perturb_scale, sample_rotation, verbose, coordinates, connectivity, topology, n_vector, h_vector, site_formula, out_trj)

    return out_trj


def handle_hydrogen(atoms: Atoms, coord, i: int, smile: str, coordinates, connectivity, topology,
                    n_vector, h_vector, site_formula) -> Atoms:
    """
...
def handle_other_adsorbates(atoms: Atoms, smile: str, i: int, coord, fragment_overlap_thr: float,
                            configs_per_smiles: int, try_perturb: int, perturb_scale: float,
                            sample_rotation: bool, verbose: bool, coordinates, connectivity, topology,
                            n_vector, h_vector, site_formula, out_trj: list) -> None:
    """
    ...
    adsorbates = Smile.NEW_create_adsorbates([smile], conformations_per_smiles=configs_per_smiles)

    for adsorbate in adsorbates:
        if (adsorbate[0].symbol == 'Cl' and connectivity[i] == 1) or (adsorbate[0].symbol == 'S' and connectivity[i] > 1):
            if verbose:
                print(f'Trying smile: {smile}')

            new_slab = atoms.copy()
            new_slab, adsorbate = assign_fragment(new_slab, adsorbate)
            new_slab = _place_adsorbate(new_slab, adsorbate, n_vector[i], coord, surf_hvector=h_vector[i])

            mfd = minimum_fragment_distance(new_slab)

            if mfd > fragment_overlap_thr:
                _update_atoms_info(new_slab, smile, coordinates[i], connectivity[i], topology[i], n_vector[i], h_vector[i], site_formula[i])
                ...
                _try_perturb_adsorbate(atoms, adsorbate, fragment_overlap_thr, try_perturb, perturb_scale, sample_rotation, verbose, n_vector[i], coord, h_vector[i], smile, coordinates[i], connectivity[i], topology[i], site_formula[i], out_trj)


def _try_perturb_adsorbate(atoms, adsorbate, fragment_overlap_thr: float, try_perturb: int,
                           perturb_scale: float, sample_rotation: bool, verbose: bool,
                           n_vector, coord, h_vector, smile: str, site_coord, site_connectivity,
                           site_topology, site_formula, out_trj: list) -> None:
    """
    ...
    while counter < try_perturb:
        ...
        if not sample_rotation:
            perturbed_adsorbate = perturb_adsorbate(adsorbate, scale=perturb_scale, force_rotation=False)
        else:
            perturbed_adsorbate = perturb_adsorbate(adsorbate, scale=perturb_scale, force_rotation=rot_counter)
            rot_counter += 1

        new_slab, perturbed_adsorbate = assign_fragment(new_slab, perturbed_adsorbate)
        new_slab = _place_adsorbate(new_slab, perturbed_adsorbate, n_vector, coord, surf_hvector=h_vector)
        ...
        if mfd > fragment_overlap_thr:
            _update_atoms_info(new_slab, smile, site_coord, site_connectivity, site_topology, n_vector, h_vector, site_formula)
            out_trj.append(new_slab)
            break
...
def _update_atoms_info(atoms, smile: str, site_coord, site_connectivity, site_topology,
                       n_vector, h_vector, site_formula) -> None:
    """
...
def perturb_adsorbate(adsorbate: Atoms, scale: float = 0.2, force_rotation: bool = False) -> Atoms:
    """
    ...
    if adsorbate[0].symbol == 'Cl':
        ...
        raise ValueError('Adsorbate must originate from a marked smiles (starting with Cl or S)')
...
def _trilaterate(P1: np.ndarray, P2: np.ndarray, P3: np.ndarray, r1: float, r2: float, r3: float) -> tuple:
    """
...
def _get_pbc_mean(atoms: Atoms, targets: list, marker: str = 'X') -> np.ndarray:
    ...
    if marker in atoms.get_chemical_symbols():
        raise ValueError(f'Marker: {marker} in use in atoms object. Please use a different marker.')
    ...
    for c in itertools.combinations(xlist, len(targets)):
        ...
        vlist = [plist[pair[0]] - plist[pair[1]] for pair in itertools.combinations(range(len(targets)), 2)]
...
def get_nvector(atoms: Atoms) -> np.ndarray:
    ...
    if atom1.symbol == 'Cl':
        ...
        atom3 = atoms[2]  # Atom directly attached to the site (atom following S1S in SMILES)
...
def _get_starting_grid(atoms: Atoms, precision: float, marker: str = 'He') -> Atoms:
    ...
    if _check_a_b_vectors(atoms):
        ...
        if atoms.cell[1][0] < 0:
            range_x = np.arange(atoms.cell[1][0], atoms.cell[0][0] + np.abs(atoms.cell[1][0]), precision)
    ...
    for x, y in [(x, y) for x in range_x for y in range_y]:
        ...
        if any(grid.get_scaled_positions(wrap=False)[-1] > 1.01) or any(grid.get_scaled_positions(wrap=False)[-1] < -0.01):
            del grid[-1]
    return grid


def _drop_marker(grid: Atoms, atoms: Atoms, marker_index: int, touch_sphere_size: float = 3) -> Atoms:
    """
    ...
    while dist > 0.1:
        dist = min([np.linalg.norm(a.position - b.position) for b in atoms]) - touch_sphere_size
...
def attach_fragment(atoms: Atoms, site_dict: dict, fragment: Atoms, n_rotation: float, height: float = None) -> Atoms:
    """
...
def drop_fragment_to_z_zero(atoms: Atoms) -> None:
    ...
    if atoms[0].symbol == 'Cl':
        ...
        raise ValueError("Unsupported fragment type. The fragment must start with 'Cl' or 'S'.")
...
def swing_fragment(atoms, fragment_index, site, resolution=10, mode = 'optimize', span_angle=50, overlap_thr=0):
    """
    ...
    if mode.lower() == 'optimize':
        for swing in swings:
            f=fragment.copy()
            f.rotate(swing, site['h_vector'], center=site['coordinates'], rotate_cell=False)
            xatoms = slab.copy()+f
        ...
        if mdf > overlap_thr:
            ...
            print(f'Failed to generate for: overlap_thr={overlap_thr}, maximal_distance={mdfs[-1][-1]}')

    if mode.lower() != 'optimize':
        raise ValueError(f"Mode {mode} not yet implemented... Please set mode='optimize'.")


def swirl_fragment(atoms, fragment_index, site, resolution=10, mode = 'optimize', overlap_thr=0.):
    """
    Optimizes the orientation of a fragment relative to a slab to minimize overlap and distance.
...
def split_slab_from_target_fragment(atoms, fragment_index):
    ...
    if not hasattr(atoms, 'arrays') or 'fragments' not in atoms.arrays:
        raise ValueError("The 'atoms' object must have an 'arrays' attribute with 'fragments' key.")
