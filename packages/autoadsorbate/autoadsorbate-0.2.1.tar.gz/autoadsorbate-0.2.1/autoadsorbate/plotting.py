...
def get_gaussian_vector(e, std = 0.05, e_min = -0.2, e_max = 3, resolution=0.01, normalize=False, normalize_mode='integral'):
...
def get_gaussian_vectors(energies, std = 0.05, e_min = -0.2, e_max = 3,
                         resolution=0.01, normalize=True, normalize_mode='integral'):
    energy_range = np.linspace(e_min, e_max, int((e_max-e_min)/resolution))
    energy_values = np.zeros(len(energy_range))
    for e in energies:
...
def energy_descriptor_from_slice(df_slice, column='energy_calibrated', std = 0.05,
                                 e_min = 'auto', e_max = 'auto', resolution='auto',
                                 normalize=True, normalize_mode='integral'):
    ...
    energy_range, energy_values = get_gaussian_vectors(
        df_slice[column].values, std = std, e_min = e_min, e_max = e_max, resolution=resolution,
        normalize=normalize, normalize_mode=normalize_mode
        )
...
def get_fragment_center(atoms, fragment_index):
    a = atoms.copy()
    a = a[[atom.index for atom in atoms if a.arrays['fragments'][atom.index] == fragment_index]]
    center = []
    for i in [0,1,2]:
        center.append((np.max(a.positions[:,i]) - np.min(a.positions[:,i]))*0.5 + np.min(a.positions[:,i]))
