...
struct ClassMeasure
{
	...
	float probability;		//!< occurrence probability of the class
...
public:
	...
	//! Struct for histogram positions like peaks, valley and thresholds
	struct HistPos
	{
		int idx;				//!< index of the histogram position
	...
	//!        will be forced to odd values, setting the value to histbins will end in a size of 3
	...
	//!        setting the value to histbins will end in a size of 3
	//! @param threshold_x a double. Peaks are valid if x is equal or greater this threshold.
	//! @param threshold_y a double. Peaks are valid if y is equal or greater this threshold.
