...
class SweptSineSignal:
    ...
    def __init__(self, params=None) -> None:
        ...
        if params is None:
            params = {}
        smprteidx = int(params["smprteidx"]) if "smprteidx" in params else 4
        ...
        sglszeidx = int(params["sglszeidx"]) if "sglszeidx" in params else 3
        ...
        self.frqstt = (
            params["frqstt"] if "frqstt" in params else _DEFAULT_START_FREQUENCY
        )
        self.frqstp = (
            int(params["frqstp"]) if "frqstp" in params else _DEFAULT_STOP_FREQUENCY
        )
        ...
        self.sgllvl = params["sgllvl"] if "sgllvl" in params else _PEAK_LEVEL_FS
        if self.sgllvl > 0:
            raise ValueError(
        ...
        self.antslcdur = (
            ...
            else _DEFAULT_ANTE_SILENCE_DURATION
        )
        self.pstslcdur = (
            params["pstslcdur"]
            if "pstslcdur" in params
            else _DEFAULT_POST_SILENCE_DURATION
        )
        self.ss_rtetyp = params["ss_rtetyp"] if "ss_rtetyp" in params else "log"
    ...
    def save(self, /, path=None) -> None:
        ...
        p = Path(
            "./"
            + _DEFAULT_OUTPUT_SUBFOLDER
            + "/"
            + _DEFAULT_SS_SIGNAL_FILENAME
        ) if path is None else Path(path)
        d = p.parent
        if not d.exists():
            d.mkdir(parents=True)
        sp.io.wavfile.write(p, self.smprte, self.ss_sglvec)
...
class ImpulseResponseSignal:
    ...
    def __init__(self, params=None) -> None:
        ...
        self.frqstt = params.get("frqstt", _DEFAULT_START_FREQUENCY)
        self.frqstp = params.get("frqstp", _DEFAULT_STOP_FREQUENCY)
        ...
        self.sgllvl = params.get("sgllvl", _PEAK_LEVEL_FS)
        if self.sgllvl > 0:
            raise ValueError(
                "Argument for the signal level, sgllvl, should be negative"
                " or zero dB FS."
            )
        self.ss_rtetyp = params.get("ss_rtetyp", "log")
        if self.ss_rtetyp != "log" and self.ss_rtetyp != "lin":
            raise ValueError(
                "Argument for the sweep sine rate type parameter, ss_rtetyp,"
                " should be 'log', for logarithmic, or 'lin' for linear."
    ...
    def save(self, /, path=None):
        ...
        p = Path(
                "./" + _DEFAULT_OUTPUT_SUBFOLDER
                + "/" + _DEFAULT_IRSSIGNAL_FILENAME
            ) if path is None else Path(path)
        d = p.parent
        if not d.exists():
            d.mkdir(parents=True)
        sp.io.wavfile.write(p, self.smprte, self.irssglvec)
