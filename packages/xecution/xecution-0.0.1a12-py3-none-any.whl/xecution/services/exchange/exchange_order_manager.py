...
import logging
from xecution.common.enums import Exchange, KlineType, Mode, OrderSide, Symbol
...
class BinanceOrderManager():
    ...
    async def get_open_orders(self, symbol: Symbol):
    ...
    async def place_order(self, order_config: OrderConfig):
        # 檢查是否已有未成交訂單，避免重複下單
        open_orders = await self.get_open_orders(order_config.symbol)
        if open_orders:
            logging.info(f"Binance ({self.market_type}): Existing open orders for {order_config.symbol}: {open_orders}")
            return None
        # 調整訂單數量與價格精度
