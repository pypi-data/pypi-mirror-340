...
from xecution.common.enums import Exchange, KlineType, Mode
...
from xecution.models.order_update import OrderUpdate
...
class BinanceHelper:
    ...
    @staticmethod
    def get_websocket_user_data_base_url(kline_topic: KlineTopic , mode: Mode):
        """
        根據 KlineType 決定使用的 WebSocket base URL
        """
        base = LiveConstants.Binance if mode == Mode.Live else TestnetConstants.Binance
        kline_type = kline_topic.klineType
        return  (
            base.WEBSOCKET_SPOT_USER_DATA_URL if kline_type == KlineType.Binance_Spot
            else base.WEBSOCKET_FUTURES_USER_DATA_URL
        )
