    ...
    with torch.no_grad():
        if hasattr(model, "fit"):
            if is_tuple_params:
                ...
                for batch in val_loader:
                    outputs, y = _fit_y_dict(model, batch, device, is_parallel)
                    loss, logits = acc_loss_logits(outputs, y)
                    total_loss += loss
                    total += cal_count(y)
                    correct += cal_correct(logits, y, threshold)
...
def do_fit_acc(model, batch, optimizer, device, is_parallel: bool):
    outputs, y = _fit_y(model, batch, device)
    loss, logits = acc_loss_logits(outputs, y)
    optimizer.zero_grad()
    loss.backward()
...
def train_epoch_base_acc(model, train_loader, optimizer, device, is_tuple_params, is_parallel: bool):
    ...
    if hasattr(model, "fit"):
        if is_tuple_params:
            for batch in train_loader:
                loss, count, correct = do_fit_acc(model, batch, optimizer, device, is_parallel)
                total_loss += loss
                total += count
