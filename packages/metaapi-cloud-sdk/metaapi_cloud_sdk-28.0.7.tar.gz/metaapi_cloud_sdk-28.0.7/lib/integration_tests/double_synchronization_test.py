    ...
    async def test_not_corrupt(self):
        ...
        if token:
            if not os.path.exists('.metaapi'):
                os.mkdir('.metaapi')
            api = MetaApi(token, {'application': 'MetaApi', 'domain': 'agiliumtrade.gfyt.agiliumlabs.cloud'})
            ...
            await asyncio.gather(*[account.deploy(), account_copy.deploy()])
            await asyncio.gather(*[account.wait_connected(), account_copy.wait_connected()])
            ...
            api._metaapi_websocket_client.remove_all_listeners()
