...
class TestPacketOrderer:
    ...
    async def test_no_sequence_number(self):
        ...
        packet_without_sn = {'type': 'authenticated', 'connectionId': 'accountId', 'accountId': 'accountId'}
    ...
    async def test_filter_prev_sync_packets_without_specifications(self):
        """Should filter out packets from previous synchronization attempt that doesn't include the start."""
        one_of_previous_packets = {
            'type': 'positions',
            'sequenceTimestamp': 1603124267188,
            'sequenceNumber': 15,
    ...
    async def test_call_out_of_order_if_timeout(self):
        ...
        packet_orderer._sequence_number_by_instance['accountId:0:ps-mpa-1'] = 1
        packet_orderer._packets_by_instance['accountId:0:ps-mpa-1'] = [timed_out_packet, not_timed_out_packet]
        await asyncio.sleep(1)
        out_of_order_listener.on_out_of_order_packet.assert_called_once()
        args_list = out_of_order_listener.on_out_of_order_packet.call_args_list[0].args
        assert args_list[0] == 'accountId'
    ...
    async def test_not_call_out_of_order_if_not_timeout(self):
        ...
        out_of_order_listener.on_out_of_order_packet = MagicMock()
        timed_out_packet = {'accountId': 'accountId', 'sequenceNumber': 11, 'packet': {}, 'receivedAt': date}
        not_timed_out_packet = {
            'accountId': 'accountId',
            'sequenceNumber': 15,
            'packet': {},
        ...
        packet_orderer._sequence_number_by_instance['accountId:0'] = 1
        packet_orderer._packets_by_instance['accountId:0'] = [not_timed_out_packet, timed_out_packet]
        await asyncio.sleep(1)
    ...
    async def test_not_call_out_of_order_if_before_sync_start(self):
        ...
        out_of_order_listener.on_out_of_order_packet = MagicMock()
        out_of_order_packet = {'accountId': 'accountId', 'sequenceNumber': 11, 'packet': {}, 'receivedAt': date}

        # There were no synchronization start packets
        if 'accountId' in packet_orderer._sequence_number_by_instance:
            del packet_orderer._sequence_number_by_instance['accountId:0']

        packet_orderer._packets_by_instance['accountId:0'] = [out_of_order_packet]
        await asyncio.sleep(1)
        out_of_order_listener.on_out_of_order_packet.assert_not_called()
    ...
    async def test_maintain_fixed_queue(self):
        ...
        packet_orderer._wait_list_size_limit = 1
        second_packet = {
            'type': 'prices',
            'sequenceTimestamp': 1603124267180,
            'sequenceNumber': 14,
        ...
        assert len(packet_orderer._packets_by_instance['accountId:0:ps-mpa-1']) == 1
        assert packet_orderer._packets_by_instance['accountId:0:ps-mpa-1'][0]['packet'] == second_packet
        packet_orderer.restore_order(third_packet)
        assert len(packet_orderer._packets_by_instance['accountId:0:ps-mpa-1']) == 1
        assert packet_orderer._packets_by_instance['accountId:0:ps-mpa-1'][0]['packet'] == third_packet

    @pytest.mark.asyncio
    async def test_count_start_packets_with_no_sync_id_as_out_of_order(self):
        """Should count start packets with undefined synchronizationId as out-of-order."""
        start_packet = {
            'type': 'synchronizationStarted',
            'sequenceTimestamp': 1603124267198,
            'sequenceNumber': 16,
        ...
        assert len(packet_orderer._packets_by_instance['accountId:0:ps-mpa-1']) == 1
        assert packet_orderer._packets_by_instance['accountId:0:ps-mpa-1'][0]['packet'] == start_packet
