...
from .provisioning_profile_api import ProvisioningProfileApi
from ..clients.metaapi.provisioning_profile_client import ProvisioningProfileClient, ProvisioningProfileDto
...
class MockClient(ProvisioningProfileClient):
    ...
    async def upload_provisioning_profile_file(
        self, provisioning_profile_id: str, file_name: str, file: Union[str, memoryview]
    ):
        pass
...
class TestProvisioningProfileApi:
    @pytest.mark.asyncio
    async def test_retrieve_provisioning_profiles_with_infinite_scroll_pagination(self):
        """Should retrieve provisioning profiles."""

        api_version = '1'
        client.get_provisioning_profiles = AsyncMock(return_value=[{'_id': 'id'}])
        profiles = await api.get_provisioning_profiles_with_infinite_scroll_pagination(
            {'version': 4, 'status': 'new', 'limit': 1}
        )
        for p in profiles:
            assert p.id == 'id'
            assert isinstance(p, ProvisioningProfile)

        client.get_provisioning_profiles.assert_called_with({'version': 4, 'status': 'new', 'limit': 1}, api_version)
    ...
    async def test_retrieve_provisioning_profiles_with_infinite_scroll_pagination(self):
        ...
        api_version = '2'
        client.get_provisioning_profiles = AsyncMock(return_value=[{'_id': 'id'}])
        profiles = await api.get_provisioning_profiles_with_classic_pagination(
            {'version': 4, 'status': 'new', 'limit': 1}
        )
    ...
    async def test_retrieve_provisioning_profile(self):
        ...
        client.get_provisioning_profile = AsyncMock(
            return_value={
                '_id': 'id',
                'name': 'name',
                'version': 4,
    ...
    async def test_create_provisioning_profile(self):
        ...
        profile = await api.create_provisioning_profile(
            {'name': 'name', 'version': 4, 'brokerTimezone': 'EET', 'brokerDSTSwitchTimezone': 'EET'}
        )
        assert profile.id == 'id'
        assert profile.name == 'name'
        assert profile.version == 4
        ...
        assert profile.broker_dst_switch_timezone == 'EET'
        assert isinstance(profile, ProvisioningProfile)
        client.create_provisioning_profile.assert_called_with(
            {'name': 'name', 'version': 4, 'brokerTimezone': 'EET', 'brokerDSTSwitchTimezone': 'EET'}
        )
