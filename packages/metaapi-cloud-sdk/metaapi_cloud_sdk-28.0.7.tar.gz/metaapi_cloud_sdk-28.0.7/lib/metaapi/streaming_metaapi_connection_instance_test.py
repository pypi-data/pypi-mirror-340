...
class MockClient(MetaApiWebsocketClient):
    ...
    async def wait_synchronized(
        self, account_id: str, instance_index: str, application_pattern: str, timeout_in_seconds: float
    ):
        pass
...
class MockAccount(MetatraderAccount):
    ...
    def account_regions(self) -> dict:
        return {'vint-hill': 'accountId', 'new-york': 'accountIdReplica'}
...
async def run_around_tests():
    ...
    terminal_state = TerminalState('accountId', MagicMock(), MagicMock())
    ...
    client_api_client.get_hashing_ignored_field_lists = AsyncMock(
        return_value={
...
class TestStreamingMetaApiConnection:
    ...
    async def test_close_position_partially(self):
        ...
        assert actual == trade_result
        client.trade.assert_called_with(
            'accountId', {'actionType': 'POSITION_PARTIAL', 'positionId': '46870472', 'volume': 0.9}, None, 'regular'
        )
    ...
    async def test_close_position(self):
        ...
        assert actual == trade_result
        client.trade.assert_called_with(
            'accountId', {'actionType': 'POSITION_CLOSE_ID', 'positionId': '46870472'}, None, 'regular'
        )
    ...
    async def test_close_positions_by_symbol(self):
        ...
        assert actual == trade_result
        client.trade.assert_called_with(
            'accountId', {'actionType': 'POSITIONS_CLOSE_SYMBOL', 'symbol': 'EURUSD'}, None, 'regular'
        )
    ...
    async def test_cancel_order(self):
        ...
        assert actual == trade_result
        client.trade.assert_called_with(
            'accountId', {'actionType': 'ORDER_CANCEL', 'orderId': '46870472'}, None, 'regular'
        )
