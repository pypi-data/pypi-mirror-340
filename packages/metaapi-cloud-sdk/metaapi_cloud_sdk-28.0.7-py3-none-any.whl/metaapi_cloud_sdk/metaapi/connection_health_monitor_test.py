...
async def run_around_tests():
    with patch('lib.metaapi.connection_health_monitor.asyncio.sleep', new=lambda x: sleep(x / 300)):
        with patch('lib.metaapi.connection_health_monitor.uniform', new=MagicMock(return_value=30)):
            global connection
            connection = MockConnection()
            global health_monitor
            health_monitor = ConnectionHealthMonitor(connection)
            health_monitor._quotes_healthy = True
            global prices
            prices = [
                {'symbol': 'EURUSD', 'brokerTime': broker_times[0]},
                {'symbol': 'EURUSD', 'brokerTime': broker_times[1]},
            ]
            yield
...
class TestConnectionHealthMonitor:
    ...
    async def test_return_100_uptime(self):
        ...
        await health_monitor.on_symbol_price_updated('vint-hill:1:ps-mpa-1', prices[0])
        await sleep(0.2)
    ...
    async def test_show_as_healthy_if_not_in_session(self):
        ...
        with freeze_time(start_time) as frozen_datetime:
            await health_monitor.on_symbol_price_updated('vint-hill:1:ps-mpa-1', prices[1])
            frozen_datetime.tick(60)
            await sleep(0.2)
            assert health_monitor.health_status['quoteStreamingHealthy']
