...
api: RpcMetaApiConnectionInstance = None
connection: RpcMetaApiConnection = None
...
async def run_around_tests():
    ...
    connection.application = 'RPC'
    connection.connect = AsyncMock()
    connection.close = AsyncMock()
    connection.wait_synchronized = AsyncMock()
    connection.account = account
    global api
    api = RpcMetaApiConnectionInstance(client, connection)
    yield
...
class TestRpcMetaApiConnection:
    ...
    async def test_wait_synchronized(self):
        ...
        connection.wait_synchronized.assert_called_once()
    ...
    async def test_retrieve_account_information(self):
        ...
        actual = await api.get_account_information({'refreshTerminalState': True})
        assert actual == account_information
        client.get_account_information.assert_called_with('accountId', {'refreshTerminalState': True})
    ...
    async def test_retrieve_positions(self):
        ...
        actual = await api.get_positions({'refreshTerminalState': True})
        assert actual == positions
        client.get_positions.assert_called_with('accountId', {'refreshTerminalState': True})
    ...
    async def test_retrieve_position_by_id(self):
        ...
        actual = await api.get_position('46214692', {'refreshTerminalState': True})
        assert actual == position
        client.get_position.assert_called_with('accountId', '46214692', {'refreshTerminalState': True})
    ...
    async def test_retrieve_orders(self):
        ...
        actual = await api.get_orders({'refreshTerminalState': True})
        assert actual == orders
        client.get_orders.assert_called_with('accountId', {'refreshTerminalState': True})
    ...
    async def test_retrieve_order_by_id(self):
        ...
        actual = await api.get_order('46871284', {'refreshTerminalState': True})
        assert actual == order
        client.get_order.assert_called_with('accountId', '46871284', {'refreshTerminalState': True})
    ...
    async def test_create_market_buy_order(self):
        ...
        await api.connect()
        trade_result = {'error': 10009, 'description': 'TRADE_RETCODE_DONE', 'orderId': 46870472}
        client.trade = AsyncMock(return_value=trade_result)
        actual = await api.create_market_buy_order(
            'GBPUSD', 0.07, 0.9, 2.0, {'comment': 'comment', 'clientId': 'TE_GBPUSD_7hyINWqAlE'}
        )
        assert actual == trade_result
        client.trade.assert_called_with(
            'accountId',
            {
    ...
    async def test_create_market_buy_order_with_relative_sl_tp(self):
        ...
        actual = await api.create_market_buy_order(
            'GBPUSD',
            0.07,
            {'value': 0.1, 'units': 'RELATIVE_PRICE'},
    ...
    async def test_create_market_sell_order(self):
        ...
        trade_result = {'error': 10009, 'description': 'TRADE_RETCODE_DONE', 'orderId': 46870472}
        client.trade = AsyncMock(return_value=trade_result)
        actual = await api.create_market_sell_order(
            'GBPUSD', 0.07, 0.9, 2.0, {'comment': 'comment', 'clientId': 'TE_GBPUSD_7hyINWqAlE'}
        )
        assert actual == trade_result
        client.trade.assert_called_with(
    ...
    async def test_create_limit_buy_order(self):
        ...
        trade_result = {'error': 10009, 'description': 'TRADE_RETCODE_DONE', 'orderId': 46870472}
        client.trade = AsyncMock(return_value=trade_result)
        actual = await api.create_limit_buy_order(
            'GBPUSD', 0.07, 1.0, 0.9, 2.0, {'comment': 'comment', 'clientId': 'TE_GBPUSD_7hyINWqAlE'}
        )
        assert actual == trade_result
        client.trade.assert_called_with(
            'accountId',
    ...
    async def test_create_limit_sell_order(self):
        ...
        trade_result = {'error': 10009, 'description': 'TRADE_RETCODE_DONE', 'orderId': 46870472}
        client.trade = AsyncMock(return_value=trade_result)
        actual = await api.create_limit_sell_order(
            'GBPUSD', 0.07, 1.0, 0.9, 2.0, {'comment': 'comment', 'clientId': 'TE_GBPUSD_7hyINWqAlE'}
        )
        assert actual == trade_result
        client.trade.assert_called_with(
    ...
    async def test_create_stop_buy_order(self):
        ...
        trade_result = {'error': 10009, 'description': 'TRADE_RETCODE_DONE', 'orderId': 46870472}
        client.trade = AsyncMock(return_value=trade_result)
        actual = await api.create_stop_buy_order(
            'GBPUSD', 0.07, 1.0, 0.9, 2.0, {'comment': 'comment', 'clientId': 'TE_GBPUSD_7hyINWqAlE'}
        )
        assert actual == trade_result
        client.trade.assert_called_with(
    ...
    async def test_create_stop_sell_order(self):
        ...
        trade_result = {'error': 10009, 'description': 'TRADE_RETCODE_DONE', 'orderId': 46870472}
        client.trade = AsyncMock(return_value=trade_result)
        actual = await api.create_stop_sell_order(
            'GBPUSD', 0.07, 1.0, 0.9, 2.0, {'comment': 'comment', 'clientId': 'TE_GBPUSD_7hyINWqAlE'}
        )
        assert actual == trade_result
        client.trade.assert_called_with(
    ...
    async def test_create_stop_limit_buy_order(self):
        ...
        trade_result = {'error': 10009, 'description': 'TRADE_RETCODE_DONE', 'orderId': 46870472}
        client.trade = AsyncMock(return_value=trade_result)
        actual = await api.create_stop_limit_buy_order(
            'GBPUSD', 0.07, 1.5, 1.4, 0.9, 2.0, {'comment': 'comment', 'clientId': 'TE_GBPUSD_7hyINWqAlE'}
        )
        assert actual == trade_result
        client.trade.assert_called_with(
            'accountId',
    ...
    async def test_create_stop_limit_sell_order(self):
        ...
        trade_result = {'error': 10009, 'description': 'TRADE_RETCODE_DONE', 'orderId': 46870472}
        client.trade = AsyncMock(return_value=trade_result)
        actual = await api.create_stop_limit_sell_order(
            'GBPUSD', 0.07, 1.0, 1.1, 2.0, 0.9, {'comment': 'comment', 'clientId': 'TE_GBPUSD_7hyINWqAlE'}
        )
        assert actual == trade_result
        client.trade.assert_called_with(
            'accountId',
    ...
    async def test_modify_position(self):
        ...
        client.trade.assert_called_with(
            'accountId',
            {'actionType': 'POSITION_MODIFY', 'positionId': '46870472', 'stopLoss': 2.0, 'takeProfit': 0.9},
            'RPC',
            'regular',
        )
    ...
    async def test_close_position_partially(self):
        ...
        assert actual == trade_result
        client.trade.assert_called_with(
            'accountId', {'actionType': 'POSITION_PARTIAL', 'positionId': '46870472', 'volume': 0.9}, 'RPC', 'regular'
        )
    ...
    async def test_close_position(self):
        ...
        assert actual == trade_result
        client.trade.assert_called_with(
            'accountId', {'actionType': 'POSITION_CLOSE_ID', 'positionId': '46870472'}, 'RPC', 'regular'
        )
    ...
    async def test_close_positions_by_symbol(self):
        ...
        assert actual == trade_result
        client.trade.assert_called_with(
            'accountId', {'actionType': 'POSITIONS_CLOSE_SYMBOL', 'symbol': 'EURUSD'}, 'RPC', 'regular'
        )
    ...
    async def test_modify_order(self):
        ...
        client.trade.assert_called_with(
            'accountId',
            {'actionType': 'ORDER_MODIFY', 'orderId': '46870472', 'openPrice': 1.0, 'stopLoss': 2.0, 'takeProfit': 0.9},
            'RPC',
            'regular',
        )
    ...
    async def test_cancel_order(self):
        ...
        assert actual == trade_result
        client.trade.assert_called_with(
            'accountId', {'actionType': 'ORDER_CANCEL', 'orderId': '46870472'}, 'RPC', 'regular'
        )
    ...
    async def test_retrieve_latest_server_time(self):
        ...
        await api.connect()
        server_time = {'time': date('2022-01-01T00:00:00.000Z'), 'brokerTime': '2022-01-01 02:00:00.000Z'}
