...
class TestHttpClient:
    ...
    async def test_load(self):
        """Should load HTML page from google.com"""
    ...
    async def test_not_found(self):
        ...
        try:
            await http_client.request(opts)
    ...
    async def test_timeout(self):
        ...
        http_client = HttpClient(0.001, {'retries': 2, 'minDelayInSeconds': 0.05, 'maxDelayInSeconds': 0.2})
        try:
            await http_client.request(opts)
    ...
    async def test_return_error_on_retry_limit_exceeded(self):
        ...
        http_client = HttpClient(60, {'retries': 2})
        try:
            await http_client.request(opts)
    ...
    async def test_not_count_retrying_too_many_requests_exception(self):
        ...
        http_client = HttpClient(60, {'retries': 1})
        response = await http_client.request(opts)
        assert response == 'response'
        assert respx.get(test_url).call_count == 3
    ...
    async def test_return_error_if_retry_after_time_too_long(self):
        ...
        http_client = HttpClient(60, {'maxDelayInSeconds': 3, 'longRunningRequestTimeoutInMinutes': 0.25})
        respx.get(test_url).mock(
            side_effect=[
    ...
    async def test_return_error_if_timed_out_to_retry(self):
        ...
        http_client = HttpClient(60, {'maxDelayInSeconds': 2, 'retries': 3, 'longRunningRequestTimeoutInMinutes': 0.1})
        try:
            await http_client.request(opts)
            raise Exception('TimeoutException is expected')
        except Exception as err:
