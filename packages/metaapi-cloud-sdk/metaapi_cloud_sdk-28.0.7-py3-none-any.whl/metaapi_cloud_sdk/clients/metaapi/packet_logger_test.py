...
async def run_around_tests():
    with patch('lib.clients.metaapi.packet_logger.asyncio.sleep', new=lambda x: sleep(x / 50)):
        ...
        packets = {
            ...
            'prices': {
                ...
                'instanceIndex': 7,
                'prices': [
                    {'symbol': 'EURUSD', 'bid': 1.18, 'ask': 1.19},
                    {'symbol': 'USDJPY', 'bid': 103.222, 'ask': 103.25},
                ],
                'accountId': 'accountId',
                'sequenceNumber': 1,
                'sequenceTimestamp': 100000,
...
class TestPacketLogger:
    ...
    async def test_record_short_specifications(self):
        ...
        await sleep(0.04)
        result = await packet_logger.read_logs('accountId')
        assert json.loads(result[0]['message']) == {
            'type': 'specifications',
            'sequenceNumber': 1,
            'sequenceTimestamp': 100000,
            'instanceIndex': 7,
    ...
    async def test_stop_price_if_sn_doesnt_match(self):
        """Should stop price packet sequence if price sequence number doesn't match."""
        packet_logger.log_packet(packets['prices'])
        packet_logger.log_packet(change_sn(packets['prices'], 2))
        packet_logger.log_packet(change_sn(packets['prices'], 3))
        packet_logger.log_packet(change_sn(packets['prices'], 4))
    ...
    async def test_read_logs_within_bounds(self):
        ...
        with freeze_time(start_time) as frozen_datetime:
            ...
            packet_logger.log_packet(packets['accountInformation'])
            await sleep(0.04)
            result = await packet_logger.read_logs(
                'accountId', date('2020-10-10 00:30:00.000'), date('2020-10-10 01:30:00.000')
            )
            assert len(result) == 5
