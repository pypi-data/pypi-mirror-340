...
from .provisioning_profile_client import ProvisioningProfileClient
...
class TestProvisioningProfileClient:
    ...
    async def test_retrieve_many(self):
        ...
        expected = [{'_id': 'id', 'name': 'name', 'version': 4, 'status': 'active'}]
        rsps = respx.get(f'{PROVISIONING_API_URL}/users/current/provisioning-profiles').mock(
            return_value=Response(200, json=expected)
        )
        profiles = await provisioning_client.get_provisioning_profiles({'version': 5, 'status': 'active'})
        assert (
    ...
    @respx.mock
    @pytest.mark.asyncio
    async def test_retrieve_provisioning_profiles_from_api_using_exact_api_version(self):
        """Should retrieve provisioning profiles from API using exact api version."""
        expected = [{'_id': 'id', 'name': 'name', 'version': 4, 'status': 'active'}]
        api_version = '1'
        rsps = respx.get(f'{PROVISIONING_API_URL}/users/current/provisioning-profiles').mock(
            return_value=Response(200, json=expected)
        )
        profiles = await provisioning_client.get_provisioning_profiles({'version': 4, 'status': 'active'}, api_version)
        assert (
            rsps.calls[0].request.url
            == f'{PROVISIONING_API_URL}/users/current/provisioning-profiles?version=4&status=active'
        )
        assert rsps.calls[0].request.headers['auth-token'] == 'header.payload.sign'
        assert rsps.calls[0].request.headers['api-version'] == api_version
        assert profiles == expected
    ...
    async def test_not_retrieve_profiles_with_account_token(self):
        ...
        provisioning_client = ProvisioningProfileClient(http_client, domain_client)
        try:
            await provisioning_client.get_provisioning_profiles({'version': 5, 'status': 'active'})
            pytest.fail()
        except Exception as err:
            assert (
                err.__str__()
                == 'You can not invoke get_provisioning_profiles method, because you '
    ...
    async def test_retrieve_one(self):
        ...
        expected = {'_id': 'id', 'name': 'name', 'version': 4, 'status': 'active'}
        rsps = respx.get(f'{PROVISIONING_API_URL}/users/current/provisioning-profiles/id').mock(
            return_value=Response(200, json=expected)
        )
        profile = await provisioning_client.get_provisioning_profile('id')
        assert rsps.calls[0].request.url == f'{PROVISIONING_API_URL}/users/current/provisioning-profiles/id'
        assert rsps.calls[0].request.headers['auth-token'] == 'header.payload.sign'
    ...
    async def test_not_retrieve_profile_with_account_token(self):
        ...
        try:
            ...
            assert (
                err.__str__()
                == 'You can not invoke get_provisioning_profile method, because you '
                + 'have connected with account access token. Please use API access token from '
                + 'https://app.metaapi.cloud/api-access/generate-token page to invoke this method.'
            )
    ...
    async def test_create(self):
        ...
        expected = {'id': 'id'}
        profile = {'name': 'name', 'version': 4}
        rsps = respx.post(f'{PROVISIONING_API_URL}/users/current/provisioning-profiles').mock(
            return_value=Response(200, json=expected)
        )
    ...
    async def test_not_create_profile_with_account_token(self):
        ...
        try:
            ...
            assert (
                err.__str__()
                == 'You can not invoke create_provisioning_profile method, because you '
                + 'have connected with account access token. Please use API access token from '
                + 'https://app.metaapi.cloud/api-access/generate-token page to invoke this method.'
            )
    ...
    async def test_upload_file(self):
        ...
        rsps = respx.put(f'{PROVISIONING_API_URL}/users/current/provisioning-profiles/id/servers.dat').mock(
    ...
    async def test_not_upload_file_with_account_token(self):
        ...
        try:
            ...
            assert (
                err.__str__()
                == 'You can not invoke upload_provisioning_profile_file method, because you '
                + 'have connected with account access token. Please use API access token from '
                + 'https://app.metaapi.cloud/api-access/generate-token page to invoke this method.'
            )
    ...
    async def test_delete(self):
        ...
        rsps = respx.delete(f'{PROVISIONING_API_URL}/users/current/provisioning-profiles/id').mock(
    ...
    async def test_not_delete_with_account_token(self):
        ...
        try:
            ...
            assert (
                err.__str__()
                == 'You can not invoke delete_provisioning_profile method, because you '
                + 'have connected with account access token. Please use API access token from '
                + 'https://app.metaapi.cloud/api-access/generate-token page to invoke this method.'
            )
    ...
    async def test_update(self):
        ...
        rsps = respx.put(f'{PROVISIONING_API_URL}/users/current/provisioning-profiles/id').mock(
            return_value=Response(200)
        )
    ...
    async def test_not_update_with_account_token(self):
        ...
        try:
            ...
            assert (
                err.__str__()
                == 'You can not invoke update_provisioning_profile method, because you '
                + 'have connected with account access token. Please use API access token from '
                + 'https://app.metaapi.cloud/api-access/generate-token page to invoke this method.'
            )
