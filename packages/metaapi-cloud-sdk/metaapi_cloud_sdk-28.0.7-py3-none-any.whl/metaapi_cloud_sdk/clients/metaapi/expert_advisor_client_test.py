...
from .expert_advisor_client import ExpertAdvisorClient
...
class TestExpertAdvisorClient:
    ...
    async def test_retrieve_many(self):
        ...
        expected = [{'expertId': 'my-ea', 'period': '1H', 'symbol': 'EURUSD', 'fileUploaded': False}]
        rsps = respx.get(f'{PROVISIONING_API_URL}/users/current/accounts/id/expert-advisors').mock(
            return_value=Response(200, json=expected)
        )
        profiles = await expert_advisor_client.get_expert_advisors('id')
        assert rsps.calls[0].request.url == f'{PROVISIONING_API_URL}/users/current/accounts/id/expert-advisors'
    ...
    async def test_not_retrieve_advisors_with_account_token(self):
        ...
        try:
            ...
            assert (
                err.__str__()
                == 'You can not invoke get_expert_advisors method, because you '
    ...
    async def test_retrieve_one(self):
        ...
        expected = {'expertId': 'my-ea', 'period': '1H', 'symbol': 'EURUSD', 'fileUploaded': False}
        rsps = respx.get(f'{PROVISIONING_API_URL}/users/current/accounts/id/expert-advisors/my-ea').mock(
            return_value=Response(200, json=expected)
        )
        profiles = await expert_advisor_client.get_expert_advisor('id', 'my-ea')
        assert rsps.calls[0].request.url == f'{PROVISIONING_API_URL}/users/current/accounts/id/expert-advisors/my-ea'
        assert rsps.calls[0].request.headers['auth-token'] == 'header.payload.sign'
        assert profiles == expected
    ...
    async def test_not_retrieve_advisor_with_account_token(self):
        ...
        try:
            ...
            assert (
                err.__str__()
                == 'You can not invoke get_expert_advisor method, because you '
                + 'have connected with account access token. Please use API access token from '
                + 'https://app.metaapi.cloud/api-access/generate-token page to invoke this method.'
            )
    ...
    async def test_delete(self):
        ...
        rsps = respx.delete(f'{PROVISIONING_API_URL}/users/current/accounts/id/expert-advisors/my-ea').mock(
    ...
    async def test_not_delete_advisor_with_account_token(self):
        ...
        try:
            ...
            assert (
                err.__str__()
                == 'You can not invoke delete_expert_advisor method, because you '
                + 'have connected with account access token. Please use API access token from '
                + 'https://app.metaapi.cloud/api-access/generate-token page to invoke this method.'
            )
    ...
    async def test_update(self):
        ...
        rsps = respx.put(f'{PROVISIONING_API_URL}/users/current/accounts/id/expert-advisors/my-ea').mock(
            return_value=Response(200)
        )
    ...
    async def test_not_update_advisor_with_account_token(self):
        ...
        try:
            ...
            assert (
                err.__str__()
                == 'You can not invoke update_expert_advisor method, because you '
                + 'have connected with account access token. Please use API access token from '
                + 'https://app.metaapi.cloud/api-access/generate-token page to invoke this method.'
            )
    ...
    async def test_upload_file(self):
        ...
        rsps = respx.put(f'{PROVISIONING_API_URL}/users/current/accounts/id/expert-advisors/my-ea/file').mock(
    ...
    async def test_not_upload_file_with_account_token(self):
        ...
        with patch('__main__.open', new=mock_open(read_data='test')) as file:
            ...
            try:
                ...
                assert (
                    err.__str__()
                    == 'You can not invoke upload_expert_advisor_file method, because you '
                    + 'have connected with account access token. Please use API access token from '
                    + 'https://app.metaapi.cloud/api-access/generate-token page to invoke this method.'
                )
