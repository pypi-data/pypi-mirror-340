...
from .historical_market_data_client import HistoricalMarketDataClient
...
async def run_around_tests():
    ...
    global get_url_stub
    respx.get('https://mt-provisioning-api-v1.agiliumtrade.agiliumtrade.ai/users/current/regions').mock(
        return_value=Response(200, json=['vint-hill', 'us-west'])
    )
    get_url_stub = respx.get(
...
class TestHistoricalMarketDataClient:
    ...
    async def test_download_candles(self):
        ...
        rsps = respx.get(
            f'{market_data_client_api_url}/users/current/accounts/accountId/historical-market-data/'
            'symbols/AUDNZD/timeframes/15m/candles'
        ).mock(return_value=Response(200, json=expected))

        candles = await client.get_historical_candles(
            'accountId', 'vint-hill', 'AUDNZD', '15m', date('2020-04-07T03:45:00.000Z'), 1
        )
        expected[0]['time'] = date(expected[0]['time'])
        assert (
            rsps.calls[0].request.url == f'{market_data_client_api_url}/users/current/accounts/accountId/'
    ...
    async def test_download_candles_with_special_characters(self):
        ...
        candles = await client.get_historical_candles(
            'accountId', 'vint-hill', 'GBPJPY#', '15m', date('2020-04-07T03:45:00.000Z'), 1
        )
        expected[0]['time'] = date(expected[0]['time'])
        assert (
            rsps.calls[0].request.url == f'{market_data_client_api_url}/users/current/accounts/accountId/'
    ...
    async def test_download_ticks(self):
        ...
        rsps = respx.get(
            f'{market_data_client_api_url}/users/current/accounts/accountId/historical-market-data/'
            'symbols/AUDNZD/ticks'
        ).mock(return_value=Response(200, json=expected))
        ticks = await client.get_historical_ticks(
            'accountId', 'vint-hill', 'AUDNZD', date('2020-04-07T03:45:00.000Z'), 0, 1
        )
        expected[0]['time'] = date(expected[0]['time'])
        assert (
            rsps.calls[0].request.url == f'{market_data_client_api_url}/users/current/accounts/accountId/'
    ...
    async def test_download_ticks_with_special_characters(self):
        ...
        rsps = respx.get().mock(return_value=Response(200, json=expected))
        ticks = await client.get_historical_ticks(
            'accountId', 'vint-hill', 'GBPJPY#', date('2020-04-07T03:45:00.000Z'), 0, 1
        )
        expected[0]['time'] = date(expected[0]['time'])
        assert (
            rsps.calls[0].request.url == f'{market_data_client_api_url}/users/current/accounts/accountId/'
