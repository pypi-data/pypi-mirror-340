...
async def check_positions(streaming_connection, rpc_connection):
    return {
        'local': len(streaming_connection.terminal_state.positions),
...
async def run_around_tests():
    global api
    api = MetaApi(token, {'application': 'MetaApi', 'domain': 'agiliumtrade.agiliumlabs.cloud'})
    yield
        ...
        if token and login:
            ...
            if not profile:
                profile = await api.provisioning_profile_api.create_provisioning_profile(
                    {'name': server_name, 'version': 4, 'brokerTimezone': 'EET', 'brokerDSTSwitchTimezone': 'EET'}
                )
