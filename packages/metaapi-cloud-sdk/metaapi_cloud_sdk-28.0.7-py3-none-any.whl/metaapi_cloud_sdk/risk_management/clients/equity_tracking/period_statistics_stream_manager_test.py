...
from .equity_tracking_client import EquityTrackingClient
...
listener: PeriodStatisticsListener = None
account = MagicMock()
sync_listeners: List[SynchronizationListener] = None
results = {}
...
async def run_around_tests():
    ...
    global listener
    listener = Listener('accountId', 'tracker1')
...
class TestPeriodStatisticsStreamManager:
    ...
    async def test_add_listener_and_request_events(self):
        ...
        listener_id = create_task(
            period_statistics_stream_manager.add_period_statistics_listener(listener, 'accountId', 'tracker1')
        )
        await sleep(0.05)
    ...
    async def test_process_price_events(self):
        ...
        await sync_listeners[0].on_symbol_price_updated(
    ...
    async def test_process_price_events_within_tracker_limits(self):
        ...
        listener_id = create_task(
            period_statistics_stream_manager.add_period_statistics_listener(listener, 'accountId', 'tracker2')
        )
        await sleep(0.05)
        updated_stub.assert_any_call(results)
        updated_stub.assert_called_once()
    ...
    async def test_process_balance_deals(self):
        ...
        await sync_listeners[0].on_deal_added(
            'vint-hill:1:ps-mpa-1',
            {
    ...
    @pytest.mark.asyncio
    async def test_process_balance_deals_on_multiple_trackers(self):
        """Should process balance deals on multiple trackers."""
        updated_stub_2 = MagicMock()
        completed_stub_2 = MagicMock()
        tracker_completed_stub_2 = MagicMock()
        connected_2 = MagicMock()
        disconnected_stub_2 = MagicMock()
        error_stub_2 = MagicMock()

        updated_stub_3 = MagicMock()
        completed_stub_3 = MagicMock()
        tracker_completed_stub_3 = MagicMock()
        connected_3 = MagicMock()
        disconnected_stub_3 = MagicMock()
        error_stub_3 = MagicMock()

        class Listener2(PeriodStatisticsListener):
            async def on_period_statistics_updated(self, period_statistics_event: List[PeriodStatistics]):
                updated_stub_2(period_statistics_event)

            async def on_period_statistics_completed(self):
                completed_stub_2()

            async def on_tracker_completed(self):
                tracker_completed_stub_2()
        ...
        class Listener3(PeriodStatisticsListener):
            async def on_period_statistics_updated(self, period_statistics_event: List[PeriodStatistics]):
                updated_stub_3(period_statistics_event)

            async def on_period_statistics_completed(self):
                completed_stub_3()

            async def on_tracker_completed(self):
                tracker_completed_stub_3()
        ...
        listener_2 = Listener2('accountId', 'tracker2')
        listener_3 = Listener3('accountId', 'tracker1')
        listeners_ids = await asyncio.gather(
            period_statistics_stream_manager.add_period_statistics_listener(listener, 'accountId', 'tracker1'),
            period_statistics_stream_manager.add_period_statistics_listener(listener_2, 'accountId', 'tracker2'),
            period_statistics_stream_manager.add_period_statistics_listener(listener_3, 'accountId', 'tracker1'),
        )
        await sleep(0.05)
        ...
        track_listeners = period_statistics_stream_manager.get_tracker_listeners('accountId', 'tracker1')

        assert track_listeners == {listeners_ids[0]: listener, listeners_ids[2]: listener_3}
        ...
        updated_stub_2.assert_called_once()
        assert updated_stub.call_count == 2
        assert updated_stub_3.call_count == 2
        ...
        assert updated_stub.call_count == 2
        assert updated_stub_3.call_count == 2
        updated_stub.assert_called_with(
            [
                {
                    'endBrokerTime': '2020-05-12 11:59:59.999',
                    'exceededThresholdType': None,
        ...
        updated_stub_2.assert_called_once()
        assert updated_stub.call_count == 3
        assert updated_stub_3.call_count == 3
        updated_stub.assert_any_call(
            [
                {
                    'endBrokerTime': '2020-05-12 11:59:59.999',
        ...
        updated_stub_2.assert_called_once()
        completed_stub_2.assert_not_called()
        assert updated_stub.call_count == 4
        completed_stub.assert_called_once()
        assert updated_stub_3.call_count == 4
        completed_stub_3.assert_called_once()
        ...
        await sync_listeners[1].on_symbol_price_updated(
            'vint-hill:1:ps-mpa-1',
            {
                'symbol': 'EURUSD',
                'bid': 1.02273,
        ...
        assert updated_stub_2.call_count == 2
        assert updated_stub.call_count == 4
        assert updated_stub_3.call_count == 4
        ...
        await sync_listeners[1].on_deal_added(
            'vint-hill:1:ps-mpa-1',
            {
                'clientId': 'TE_GBPUSD_7hyINWqAlE',
                'commission': -0.25,
        ...
        assert updated_stub_2.call_count == 3
        assert updated_stub.call_count == 4
        assert updated_stub_3.call_count == 4
        updated_stub.assert_any_call(
            [
        ...
        assert updated_stub_2.call_count == 4
        completed_stub_2.assert_called_once()
        assert updated_stub.call_count == 4
        completed_stub.assert_called_once()
        assert updated_stub_3.call_count == 4
        completed_stub_3.assert_called_once()
        ...
        assert len(sync_listeners) == 2
        connection.close.assert_called_once()

    @pytest.mark.asyncio
    async def test_remove_sync_listeners_when_all_tracker_listeners_removed(self):
        """Should remove sync listeners when all tracker listeners are removed."""
        connection.remove_synchronization_listener = MagicMock()
        updated_stub_2 = MagicMock()
        completed_stub_2 = MagicMock()
        tracker_completed_stub_2 = MagicMock()
        connected_2 = MagicMock()
        ...
        listener_2 = Listener2('accountId', 'tracker2')
        listener_3 = Listener3('accountId', 'tracker1')
        listener_id = await period_statistics_stream_manager.add_period_statistics_listener(
            listener, 'accountId', 'tracker1'
        )
        listener_id_2 = await period_statistics_stream_manager.add_period_statistics_listener(
            listener_2, 'accountId', 'tracker2'
        )
        listener_id_3 = await period_statistics_stream_manager.add_period_statistics_listener(
            listener_3, 'accountId', 'tracker1'
        )
        await sleep(0.05)
        ...
        period_statistics_stream_manager.remove_period_statistics_listener(listener_id)
        connection.remove_synchronization_listener.assert_not_called()
        connection.close.assert_not_called()

        period_statistics_stream_manager.remove_period_statistics_listener(listener_id_3)
        connection.remove_synchronization_listener.assert_called_once()
        connection.close.assert_not_called()

        period_statistics_stream_manager.remove_period_statistics_listener(listener_id_2)
        assert connection.remove_synchronization_listener.call_count == 2
        connection.close.assert_called_once()
    ...
    async def test_retry_on_sync_error(self):
        ...
        with patch(
            'lib.risk_management.clients.equity_tracking.period_statistics_stream_manager.asyncio.sleep',
    ...
    async def test_not_rewrite_record_exceeded_event(self):
        ...
        get_tracker_stub.return_value = {
            'name': 'trackerName1',
            '_id': 'tracker1',
            'absoluteDrawdownThreshold': 500,
    ...
    async def test_track_connection_state(self):
        ...
        await asyncio.sleep(0.05)
        await sync_listeners[0].on_deals_synchronized('vint-hill:1:ps-mpa-1', 'syncId')
        await asyncio.sleep(0.05)
        assert connected_stub.call_count == 1
        await sync_listeners[0].on_deals_synchronized('vint-hill:1:ps-mpa-1', 'syncId')
        await asyncio.sleep(0.05)
        assert connected_stub.call_count == 1
        await sync_listeners[0].on_disconnected('vint-hill:1:ps-mpa-1')
        await asyncio.sleep(0.05)
    ...
    async def test_send_an_update_event_if_a_new_deal_arrived(self):
        ...
        await asyncio.gather(
            sync_listeners[0].on_deal_added('vint-hill:1:ps-mpa-1', deal_balance),
            connection.history_storage.on_deal_added('vint-hill:1:ps-mpa-1', deal_balance),
        )
        await asyncio.sleep(0.05)
        assert updated_stub.call_count == 1
        await asyncio.gather(
            sync_listeners[0].on_deal_added('vint-hill:1:ps-mpa-1', deal),
        ...
        await asyncio.sleep(0.05)
        await asyncio.gather(
            sync_listeners[0].on_deal_added('vint-hill:1:ps-mpa-1', deal2),
            connection.history_storage.on_deal_added('vint-hill:1:ps-mpa-1', deal2),
        )
        assert updated_stub.call_count == 2
        await asyncio.gather(
            sync_listeners[0].on_deal_added('vint-hill:1:ps-mpa-1', deal3),
