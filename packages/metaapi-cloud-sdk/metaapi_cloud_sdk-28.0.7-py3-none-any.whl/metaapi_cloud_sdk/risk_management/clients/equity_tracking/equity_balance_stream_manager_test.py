...
listener: EquityBalanceListener = None
account = MagicMock()
...
class TestEquityBalanceStreamManager:
    ...
    async def test_retry_on_sync_error(self):
        ...
        with patch(
            'lib.risk_management.clients.equity_tracking.equity_balance_stream_manager.asyncio.sleep',
    ...
    async def test_handle_two_listeners_at_same_time(self):
        """Should handle two listeners at the same time."""
        call_stub2 = MagicMock()
        ...
        listener_ids = await asyncio.gather(
            equity_balance_stream_manager.add_equity_balance_listener(listener, 'accountId'),
            equity_balance_stream_manager.add_equity_balance_listener(listener2, 'accountId'),
        )
        await sleep(0.05)
        await sync_listeners[0].on_account_information_updated(
            'vint-hill:1:ps-mpa-1', {'equity': 10500, 'balance': 9000}
        ...
        call_stub.assert_any_call({'equity': 10500, 'balance': 9000})
        call_stub2.assert_any_call({'equity': 10500, 'balance': 9000})
        equity_balance_stream_manager.remove_equity_balance_listener(listener_ids[0])
        await sleep(0.05)
        connection.close.assert_not_called()
        equity_balance_stream_manager.remove_equity_balance_listener(listener_ids[1])
        await sleep(0.05)
    ...
    async def test_wait_until_sync_for_second_listener(self):
        ...
        listener2 = Listener2('accountId')
        listener_promise = create_task(
            equity_balance_stream_manager.add_equity_balance_listener(listener2, 'accountId')
        )
        await sleep(0.05)
        await sync_listeners[0].on_account_information_updated(
            'vint-hill:1:ps-mpa-1', {'equity': 10400, 'balance': 9000}
        )
        call_stub2.assert_any_call({'equity': 10400, 'balance': 9000})
        asyncio.create_task(sync_listeners[0].on_deals_synchronized('new-york:0:ps-mpa-1', 'id'))
    ...
    async def test_track_connection_state(self):
        ...
        await sync_listeners[0].on_deals_synchronized('vint-hill:1:ps-mpa-1', 'id')
        await sleep(0.05)
