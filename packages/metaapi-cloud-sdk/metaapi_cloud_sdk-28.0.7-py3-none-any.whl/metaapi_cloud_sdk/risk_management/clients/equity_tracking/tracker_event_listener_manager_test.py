...
async def run_around_tests():
    ...
    async def get_event_func(arg1, arg2):
        opt1 = {
            ...
            'method': 'GET',
            'qs': {'previousSequenceNumber': 1, 'accountId': 'accountId', 'trackerId': 'trackerId', 'limit': 1000},
        }
        opt2 = {
            'url': '/users/current/tracker-events/stream',
            'headers': {'auth-token': token, 'api-version': '1'},
            'method': 'GET',
            'qs': {'previousSequenceNumber': 1, 'accountId': 'accountId', 'trackerId': 'trackerId2', 'limit': 1000},
        }
        opt3 = {
            'url': '/users/current/tracker-events/stream',
            'headers': {'auth-token': token, 'api-version': '1'},
            'method': 'GET',
            'qs': {'previousSequenceNumber': 3, 'accountId': 'accountId', 'trackerId': 'trackerId', 'limit': 1000},
        }
        if arg1 == opt1 or arg1 == opt2:
...
class TestTrackerEventListenerManager:
    ...
    @pytest.mark.asyncio
    async def test_add_multiple_tracker_event_listeners(self):
        """Should add multiple tracker event listeners."""
        call_stub_2 = MagicMock()
        error_stub_2 = MagicMock()
        call_stub_3 = MagicMock()
        error_stub_3 = MagicMock()

        class Listener2(TrackerEventListener):
            async def on_tracker_event(self, tracker_event):
                call_stub_2(tracker_event)

            async def on_error(self, error: Exception):
                error_stub_2(error)

        class Listener3(TrackerEventListener):
            async def on_tracker_event(self, tracker_event):
                call_stub_3(tracker_event)

            async def on_error(self, error: Exception):
                error_stub_3(error)

        listener_2 = Listener2('accountId', 'trackerId2')
        listener_3 = Listener3('accountId', 'trackerId')
        id = tracker_event_listener_manager.add_tracker_event_listener(listener, 'accountId', 'trackerId', 1)
        id_2 = tracker_event_listener_manager.add_tracker_event_listener(listener_2, 'accountId', 'trackerId2', 1)
        id_3 = tracker_event_listener_manager.add_tracker_event_listener(listener_3, 'accountId', 'trackerId', 1)
        await sleep(2.2)
        assert call_stub.call_count == 4
        assert call_stub_2.call_count == 2
        assert call_stub_3.call_count == 4
        assert call_stub.call_args_list[0].args[0] == expected[0]
        assert call_stub.call_args_list[1].args[0] == expected[1]
        assert call_stub.call_args_list[2].args[0] == expected2[0]
        assert call_stub.call_args_list[3].args[0] == expected2[1]
        tracker_event_listener_manager.remove_tracker_event_listener(id)
        tracker_event_listener_manager.remove_tracker_event_listener(id_2)
        tracker_event_listener_manager.remove_tracker_event_listener(id_3)
