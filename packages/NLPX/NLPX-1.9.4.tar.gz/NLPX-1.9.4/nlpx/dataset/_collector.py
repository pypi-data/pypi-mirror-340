	...
	def __call__(self, examples: List[Tuple[List[int], int]]) -> Tuple[torch.Tensor]:
		...
		if self.return_sequence_length:
			...
			return (torch.tensor(np.array(ids), dtype=torch.long), 
		   			torch.tensor(np.array(sequence_lengths), dtype=torch.int32), 
					labels)
		
		ids = self.pad_func(tokens, max_length, **params)
		if isinstance(ids, Tuple):
			ids = ids[0]
		return torch.tensor(np.array(ids), dtype=torch.long), labels
...
class PaddingLongTensorCollector:
	...
	def __call__(self, batch: List[Tuple]) -> Tuple[torch.LongTensor]:
		batch = (self.pad_func(x) if isinstance(x[0], List) else x for x in zip(*batch))
		return tuple(torch.tensor(x if isinstance(x, np.ndarray) else np.array(x), dtype=torch.long) for x in batch)
