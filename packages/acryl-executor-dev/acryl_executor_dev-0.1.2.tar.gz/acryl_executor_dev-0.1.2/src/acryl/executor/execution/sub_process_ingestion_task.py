...
class SubProcessIngestionTask(Task):
    ...
    async def execute(self, args: dict, ctx: ExecutionContext) -> None:
        ...
        # --- Install custom plugin into the freshly minted venv ---
        venv_dir = os.path.join(self.tmp_dir, f"venv-{plugin}-{ctx.exec_id}")
        venv_python = os.path.join(venv_dir, "bin", "python")

        install_cmd = [venv_python, "-m", "pip", "install", "-e", "/plugins/json-ingester"]
        logger.info(f"Installing plugin with: {' '.join(install_cmd)}")
        ...
        if install_process.returncode != 0:
            raise RuntimeError("Failed to install plugin into venv")

        # --- End of custom plugin installation ---
