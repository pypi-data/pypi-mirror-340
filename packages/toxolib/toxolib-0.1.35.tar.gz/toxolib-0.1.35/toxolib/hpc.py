...
if ! command -v conda &> /dev/null; then
    ...
    if [ -d "$HOME/miniconda3/bin" ]; then
        ...
        log_message "WARNING: No conda installation found and compute node may not have internet access."
...
if ! command -v snakemake &> /dev/null; then
    log_message "Snakemake not found. Installing snakemake..."
    pip install snakemake 2>&1 | tee -a "$DEBUG_LOG" || {
        log_message "ERROR: Failed to install snakemake with pip. Trying conda..."
        if command -v conda &> /dev/null; then
            conda install -y -c bioconda snakemake 2>&1 | tee -a "$DEBUG_LOG" || {
                log_message "ERROR: Failed to install snakemake. Cannot proceed."
                exit 1
            }
        else
            log_message "ERROR: Cannot install snakemake. No conda or pip available."
            exit 1
        fi
    }
fi

log_message "Using Snakemake: $(which snakemake 2>/dev/null || echo 'Not found')"
log_message "Snakemake version: $(snakemake --version 2>/dev/null || echo 'Unknown')"
