...
class HPCConnection:
    ...
    def close_connection(self, force=False):
        ...
        # If no_persist is False and not forcing, keep the connection alive
        if not force and not self.no_persist and hostname and username:
            # Check if this connection is stored in the session
            existing_ssh, existing_sftp = get_connection(hostname, username)
            if existing_ssh and self.ssh is existing_ssh:
                # This connection is stored in the session, don't close it
                logging.info(f"Connection to {hostname} will be kept alive for future commands")
                return
        ...
        if hostname and username:
            ...
            # Also clear session credentials if we're closing the connection
            from .session import clear_credentials
            clear_credentials(hostname, username)
