...
def main():
    ...
    hpc_pwd_parser.add_argument(
        ...
        help="Keep the connection open and start an interactive shell after executing the command"
    )
    ...
    if args.command == "hpc-setup":
        ...
        # If keep-open is specified, start an interactive shell
        if hasattr(args, 'keep_open') and args.keep_open:
            print("\nStarting interactive shell. Type 'exit' or 'quit' to end the session.")
            hpc_conn.interactive_shell()
            
        # Close connection
        hpc_conn.close_connection()
        sys.exit(0)
        ...
        # Close connection when shell exits
        hpc_conn.close_connection()
        sys.exit(0)
