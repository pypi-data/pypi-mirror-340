...
class HPCConnection:
    ...
    def interactive_shell(self):
        """Start an interactive shell session with the HPC
        
        This keeps the connection open and allows running multiple commands
        until the user explicitly exits by typing 'exit' or 'quit'.
        
        Returns:
            bool: True if the session was successful, False otherwise
        """
        if not self.ssh:
            logging.error("No SSH connection. Please set up connection first.")
            return False
        ...
        try:
            ...
            if not current_dir:
                logging.error("Failed to get current directory")
                return False
                
            print(f"\n\033[1mConnected to HPC. Interactive shell started.\033[0m")
            ...
            while True:
                ...
                prompt = f"\033[32m{self.config.get('username')}@{self.config.get('hostname')}:\033[34m{current_dir}\033[0m$ "
                user_input = input(prompt)
                
                # Check for exit command
                if user_input.lower() in ['exit', 'quit']:
                    print("Closing HPC connection...")
                    break
                ...
                if user_input.startswith('cd '):
                    ...
                    new_dir = self.change_directory(path)
                    if new_dir:
                        current_dir = new_dir
                    continue
                ...
                try:
                    _, stdout, stderr = self.ssh.exec_command(user_input)
                    ...
                    print(f"\033[31mError executing command: {e}\033[0m")
            ...
            logging.error(f"Error in interactive shell: {e}")
            return False
            
    def _print_shell_help(self):
        """Print help information for the interactive shell"""
        help_text = """
...
\033[1mOther Commands:\033[0m
  Any valid shell command will be executed on the HPC.
  Examples:
