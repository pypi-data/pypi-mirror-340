...
if command -v conda &> /dev/null; then
    ...
    if ! conda info &>/dev/null; then
        log_message "Running conda init..."
        conda init bash
        source ~/.bashrc
    fi
fi
...
if command -v conda &> /dev/null; then
    ...
    conda activate $ENV_NAME 2>&1 | tee -a "$DEBUG_LOG" || {
        log_message "WARNING: Failed to activate conda environment. Using system Python instead."
    }
