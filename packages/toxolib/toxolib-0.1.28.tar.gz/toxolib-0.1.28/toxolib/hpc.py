...
def generate_job_script(snakemake_file, output_dir, kraken_db_dir, corn_db_dir, job_name="toxolib_job", partition="normal", threads=32, memory=200, time="144:00:00"):
    """Generate a SLURM job script for running Snakemake on HPC with enhanced logging
...
log_message "Loading anaconda3 module..."
module load anaconda3
if [ $? -ne 0 ]; then
    log_message "ERROR: Failed to load anaconda3 module"
    exit 1
fi

# Create conda environment if it doesn't exist
CONDA_ENV_DIR="{output_dir}/conda_env"
log_message "Setting up conda environment at $CONDA_ENV_DIR"
if [ ! -d "$CONDA_ENV_DIR" ]; then
    log_message "Creating conda environment..."
    conda env create -f {output_dir}/environment.yml -p $CONDA_ENV_DIR 2>&1 | tee -a "$DEBUG_LOG"
    if [ $? -ne 0 ]; then
        log_message "ERROR: Failed to create conda environment"
        exit 1
    fi
...
log_message "Using Snakemake: $(which snakemake)"
log_message "Snakemake version: $(snakemake --version 2>&1)"

# Check for database directories
log_message "Checking database directories..."
grep -E "KRAKEN2_DB_DIR|corn_db" {snakemake_file} >> "$DEBUG_LOG"
...
log_message "Starting Snakemake workflow..."
log_message "Command: snakemake -s {os.path.basename(snakemake_file)} --cores $SLURM_CPUS_PER_TASK --verbose"

# Run Snakemake with detailed logging
snakemake -s {os.path.basename(snakemake_file)} --cores $SLURM_CPUS_PER_TASK --verbose 2>&1 | tee -a "$SNAKEMAKE_LOG"
SNAKEMAKE_EXIT_CODE=$?
...
def retrieve_job_logs(job_id, local_dir=None, log_type="all", tail=50, config_file=None, no_persist=False):
    ...
    try:
        ...
        if not log_files or log_files[0] == '':
            # Try to find standard SLURM output files
            logging.info(f"No log files found in {log_dir}, searching for SLURM output files...")
            _, stdout, _ = hpc.ssh.exec_command(f"find ~/ -name '*{job_id}*.out' -o -name '*{job_id}*.err' | sort")
            slurm_logs = stdout.read().decode().strip().split('\n')
            
            if not slurm_logs or slurm_logs[0] == '':
                # Also check in the standard SLURM output directory
                _, stdout, _ = hpc.ssh.exec_command(f"find /scratch/slurm/{job_id}* -type f 2>/dev/null | sort")
                scratch_logs = stdout.read().decode().strip().split('\n')
                
                if scratch_logs and scratch_logs[0] != '':
                    log_files = scratch_logs
                    log_dir = "/scratch/slurm"
                    logging.info(f"Found logs in scratch directory: {log_files}")
                else:
                    logging.error(f"No log files found for job {job_id}")
                    return False
            else:
