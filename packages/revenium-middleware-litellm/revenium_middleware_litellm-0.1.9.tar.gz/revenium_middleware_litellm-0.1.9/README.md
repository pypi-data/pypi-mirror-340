...
Simply export your `REVENIUM_API_KEY` (and other relevant Revenium environment variables) and import the middleware module *before* your first call to `litellm.completion`. Your LiteLLM calls will be metered automatically:

```python
import litellm
import os

# Ensure Revenium environment variables (e.g., REVENIUM_API_KEY) are set
# os.environ["REVENIUM_API_KEY"] = "YOUR_REVENIUM_API_KEY"
...
2. Ensure Revenium environment variables (e.g., `REVENIUM_API_KEY`) are set where the proxy runs.
