...
class CoingeckoUniversalQuoteTool(BaseTool):
    ...
    def _run(self, token_id: str = None, chain_id: str = None, amount: float = 1.0):
        """
        Fetches the price for a token or chain.

        Args:
            token_id (str, optional): The token ID (e.g., "bitcoin").
            chain_id (str, optional): The blockchain ID (e.g., "ethereum").
        ...
        try:
            if token_id:
                ...
                endpoint = f"{self._base_url}/simple/price?ids={chain_id}&vs_currencies=usd"
            else:
                return {"error": "Invalid input. Provide either a token_id or chain_id."}

            response = requests.get(endpoint)
            response.raise_for_status()  # Raise an error for bad responses (4xx, 5xx)
            data = response.json()

            price = data.get(token_id or chain_id, {}).get("usd")

            if price is not None:
                return {"token": token_id or chain_id, "price": price}
            else:
...
if __name__ == "__main__":
    ...
    # You can add more tests if needed, for example with chain_id
    # chain_result = tool_instance.run(chain_id="ethereum", amount=1.0)
    # print("Result for Ethereum price query:", chain_result)
