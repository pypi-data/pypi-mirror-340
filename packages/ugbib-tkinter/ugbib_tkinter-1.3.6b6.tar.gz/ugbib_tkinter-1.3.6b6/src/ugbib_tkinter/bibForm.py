...
class InfoEntry(tk.Entry):
    """InfoEntry - DISABLED Entry Widget zum Anzeigen von Werten
    
        InfoEntry dient ausschließlich zum Anzeigen von Werten aus der Datenbank.
        ...
        Es verhält sich genau wie ttk.Enter. Nur wird als Standard
        state=tk.DISABLED gesetzt, und die Vorder- und Hintergrundfarbe wird
        anders gesetzt.
    """
    _styleInitialized = False

    def __init__(self, parent=None, **kwargs):
        if not __class__._styleInitialized:
            style = ttk.Style()
            style.configure(
                'InfoEntry.TEntry',
        ...
        kwargs.setdefault('disabledforeground', 'black')
        kwargs.setdefault('disabledbackground', '#e0e0e0')
        # kwargs['style'] = 'InfoEntry.TEntry'
...
class BearbVonAm(tk.Frame):
    def __init__(self, parent):
        ...
        self.entBearbAm = InfoEntry(self, width=15, font=(None, 6))
...
class Form():
    ...
    def addWidget(self, colName, widget, typ, label=None):
        ...
        #
        if type(widget) in (ttk.Entry, InfoEntry, ttk.Combobox):
