...
from .log_util import get_main_tracebak, local_trace
from .log_wrapper import get_error_log, get_service_log_str, get_process_time, request_context
...
def register_log_exception(app: FastAPI):
    ...
    def log_and_response(request, exc, default_code=500, status_code=500):
        ...
        get_error_log(header_str=header_str, status_code=status_code, code=code, msg=msg, module_line=module_line)
        ...
        get_service_log_str(module_line, process_time=process_time, log_time_start=time.time(),status_code=status_code, code=code,
    ...
    def exception_handle(request: Request, exc: Exception):
        ...
        if "status_code" in exc.__dict__:
            ...
            new_status_code = 500
            new_code = 500
        return log_and_response(request, exc, default_code=new_status_code, status_code=new_code)
    ...
    def http_exception_handle(request: Request, exc: HTTPException):
        ...
        if "status_code" in exc.__dict__:
            ...
            new_code = 500
        return log_and_response(request, exc, default_code=new_code, status_code=new_code)

    @app.exception_handler(BaseHTTPException)
    def base_http_exception_handle(request: Request, exc: BaseHTTPException):
    ...
    def assert_exception_handle(request: Request, exc: AssertionError):
        # 处理断言错误
        if "status_code" in exc.__dict__:
            new_code = exc.__dict__.get("status_code")
        else:
    ...
    def validation_exception_handle(request: Request, exc: ValidationError):
        # 处理验证错误
        if "status_code" in exc.__dict__:
            new_code = exc.__dict__.get("status_code")
        else:
