...
from fastapi import (
    FastAPI,
    Request,
    Response, HTTPException, Depends
)

from sthg_fastapi_logs.enumerate import BusinessCode, HttpStatusCode, RESULT_SUCCESS
from sthg_fastapi_logs.log_util import create_loggers, TraceID, local_trace,  get_module_line, \
    get_main_tracebak, clean_error_message
from sthg_fastapi_logs.utils.code_utils import handle_method_status, handle_business_code
from sthg_fastapi_logs.utils.exception_utils import CustomException
from sthg_fastapi_logs.utils.response_utils import get_process_time, get_request_data,get_response_data, logs_set_header
        ...
        def wrapper(*args, **kwargs):
            if default_config['access']['enable']:
                ...
                try:
                    ...
                    access_info['return_desc'] = HttpStatusCode.get_description(500)
                    ...
                    get_access_log_str(**access_info)
                    raise
                try:
                    process_time = get_process_time(start_time, time.time())
                    log_time_start = time.time()
                    header_str = logs_set_header(request=request)
                    ...
                    access_info['process_time'] = process_time
                    access_info['status_code'] = handle_method_status(200)
                    access_info['code'] = handle_business_code(200)
                    access_info['return_desc'] = HttpStatusCode.get_description(200)
                    access_info['msg'] = msg
                    access_info['moduler_func_line'] = moduler_func_line
                    access_info['respData'] = new_response if new_response else '-'
                    access_info['log_time_start'] = log_time_start
                    get_access_log_str(**access_info)
                except Exception as exc:
            ...
            if default_config['server']['enable']:
                ...
                try:
                    if asyncio.iscoroutinefunction(func):
                        ...
                        response = func(*args, **kwargs)
                    new_response = response
                    reData = get_response_data(new_response)
                except Exception as exc:
                    error_logger.error('日志处理异常 :{}'.format(str(exc)))
                    raise
                try:
                    ...
                    get_service_log_str(moduler_func_line, process_time, log_time_start, RESULT_SUCCESS, "OK",
...
def class_decorator(func):
    ...
    def wrapper(*args, **kwargs):
        if default_config['server']['enable']:
            ...
            get_service_log_str(moduler_func_line, process_time, log_time_start, "SUCCESS", 'OK', '业务处理成功',
                                      msg, params, reData)
            return response
        return func(*args, **kwargs)
...
def get_access_log_str(**access_kwargs):
    try:
...
def get_error_log(header_str, status_code, code, msg, module_line="-"):
    try:
        t1 = time.time()
...
def request_params(request):
    try:
        ...
        if content_type and 'application/json' in content_type:
            body_data = asyncio.wait_for(request.body(), timeout=5)
            body_data = json.dumps(asyncio.run(body_data).decode())
        elif content_type and 'multipart/form-data' in content_type:
            if 'multipart/form-data; boundary=' in content_type:
                form = asyncio.run(asyncio.wait_for(request.form(), timeout=5))
                has_file = False
