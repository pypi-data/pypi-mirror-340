...
at::Tensor FarthestPointSamplingCpu(const at::Tensor &points,
    ...
    for (int64_t n = 0; n < N; ++n) {
        ...
        std::fill(selected_points_mask.begin(), selected_points_mask.end(),
                  false);
        std::fill(dists.begin(), dists.end(),
        ...
        for (int64_t k = 1; k < batch_k; ++k) {
            ...
            for (int64_t p = 0; p < lengths_a[n]; ++p) {
                ...
                // If the distance of this point to the last selected point is
                // closer than the distance to any of the previously selected
                // points, then update this distance
                if (dist2 < dists[p]) {
                    dists[p] = dist2;
                }
            }
