...
def extract_references_in_text(text):
...
def separate_word_and_number(word):
...
def process_words(words):
    ...
    return [separate_word_and_number(word).split() for word in clean_list_of_strings(words)]
...
def extract_references(text, filter_allegati=False):
    words = extract_references_in_text(text)
    cleaned_words = process_words(words)
