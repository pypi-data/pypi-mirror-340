...
class BaseRouter:
    def __init__(self, path=None, method=None, title=None, description=None, tags=None, fields=None, table=None, schema=None, config: vic.Config = None):
        """
        Root 根节点（/）
        """
        self.path = path
        ...
        self.schema = schema
        if config is None:
            raise vie.MError("RouterClass.Init", "Config 不能为空")
    ...
    def db_model(self):
        ...
        if self.table is None:
            raise vie.MError("RouterClass.DBModel", "Table 不能为空")
        if self.schema is None:
            raise vie.MError("RouterClass.DBModel", "Schema 不能为空")
        return DBOrm(self.table, self.schema, self.config)
