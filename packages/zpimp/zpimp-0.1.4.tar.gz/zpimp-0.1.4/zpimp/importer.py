    ...
    try:
        ...
        for line in content.splitlines():
            if line.startswith("```python"):
                ...
                if in_code_block:
                    python_code_blocks.append(current_code_block.strip())
                in_code_block = False
            elif in_code_block:
        ...
        if python_code_to_execute:
            local_namespace = {}
            try:
                exec(python_code_to_execute, globals(), local_namespace)
                for name, obj in local_namespace.items():
                    if callable(obj):
                        defined_functions[name] = obj
            except Exception as e:
                print(f"Ошибка при выполнении кода из '{full_notebook_path}': {e}")
                traceback.print_exc()

        return defined_functions

    except FileNotFoundError:
        raise FileNotFoundError(f"Файл не найден: {full_notebook_path}")
    except Exception as e:
        print(f"НЕПРЕДВИДЕННАЯ ОШИБКА при чтении '{full_notebook_path}':")
        traceback.print_exc()
        return {}
