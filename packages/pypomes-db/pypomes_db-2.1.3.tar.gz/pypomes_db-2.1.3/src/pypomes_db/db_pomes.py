...
def db_select(errors: list[str] | None,
    ...
    are specified with equal values, then exactly that number of tuples must be returned by the query.
...
def db_insert(errors: list[str] | None,
    ...
    If provided, *id_column* holds the name of the the *identity column* for the table, that is, a column
    whose values are generated by the database itself.
...
def db_update(errors: list[str] | None,
    ...
    :param committable: whether to commit operation upon errorless completion
    :param logger: optional logger
    :return: the values of *return_cols*, the number of updated tuples, or *None* if error
    """
    # initialize the local errors list
    op_errors: list[str] = []
