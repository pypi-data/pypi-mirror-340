...
def RegisterWindowClass(classname: str, msgCBK = None, args = None, kwargs = None) -> _wintype.WNDCLASSEXW:
    ...
    wndclass.style          = _wincon.CS_HREDRAW | _wincon.CS_VREDRAW
...
@_overload
def SetCursor(curpath: str) -> int: ...

def SetCursor(cursor: int | str) -> int:
    if isinstance(cursor, str):
        hcursor = _user32.LoadCursorFromFileW(cursor)
    else: hcursor = _user32.LoadCursorW(0, cursor)
    previous_hcursor = _user32.SetCursor(hcursor)
...
def SetWinEventHook(
    ...
    def wrappedcbk(hWinEventHook, event, hwnd, idObject, idChild, dwEventThread, dwmsEventTime):
        if not inwindow or hwnd == inwindow:
            callback(hWinEventHook, event, hwnd, idObject, idChild, dwEventThread, dwmsEventTime) or wineventproc
    wineventproc = _wintype.WINEVENTPROC(wrappedcbk)
    return _user32.SetWinEventHook(
        eventStart, eventEnd, 0, wineventproc, inprocess, inthread, _wincon.WINEVENT_OUTOFCONTEXT)  # hEventHook

def UnhookWinEvent(hEventHook) -> bool:
    return bool(_user32.UnhookWinEvent(hEventHook))     # success
...
def AskFile(hownerwnd=0, title='打开', initdir: str=..., filter='All Files: *.*', *filters: str):
    ...
    _comdlg32.GetOpenFileNameW(ofn.ref)
    return pathbuffer.value and _os.path.abspath(pathbuffer.value)
# endregion
