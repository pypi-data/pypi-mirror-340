...
class RegisterUserBody(BaseModel):
    ...
    def check_sessionToken(cls, value: str) -> str:
        ...
        if not match(r"^[0-9a-z]{25}$", value):
            raise ValueError("sessionToken不合法喵,只能有小写字母和数字")
...
async def register_user(
    ...
    boby:RegisterUserBody = Body(None)
):
    """
    注册新用户
    ...
    if boby is None:
        boby = RegisterUserBody()
        
    session_token = boby.sessionToken
    user_id = boby.objectId

    await db.create_user(session_token, user_id)  # 移除不必要的时间参数
    return JSONResponse({"sessionToken": session_token, "objectId": user_id},status_code=201)
