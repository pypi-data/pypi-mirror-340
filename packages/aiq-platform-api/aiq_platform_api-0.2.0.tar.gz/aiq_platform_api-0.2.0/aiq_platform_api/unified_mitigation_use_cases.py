...
def list_mitigation_rules(
    client: AttackIQRestClient, limit: Optional[int] = None
) -> int:
    """Lists unified mitigation rules."""
    logger.info(f"Listing unified mitigations (limit: {limit if limit else 'None'})...")
    count = 0
    try:
        mitigations = UnifiedMitigationUtils.list_mitigations(client)
        for rule in itertools.islice(mitigations, limit):
            count += 1
            logger.info(
...
def list_project_associations(
    ...
    logger.info(
        f"Listing unified mitigation project associations (limit: {limit if limit else 'None'})..."
    )
    count = 0
    try:
        associations = UnifiedMitigationProjectUtils.list_associations(client)
        for assoc in itertools.islice(associations, limit):
            count += 1
            logger.info(
...
def list_mitigations_with_relations(
    ...
    logger.info(
        f"Listing unified mitigations with relations (limit: {limit if limit else 'None'})..."
    )
    count = 0
    try:
        mitigations = (
            UnifiedMitigationWithRelationsUtils.list_mitigations_with_relations(client)
        )
        for rule in itertools.islice(mitigations, limit):
            count += 1
            logger.info(
