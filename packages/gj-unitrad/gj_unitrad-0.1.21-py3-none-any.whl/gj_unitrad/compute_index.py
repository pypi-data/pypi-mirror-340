...
class ComputeIndex:
    ...
    def __init__(self):
        ...
        self.config_data = None  # config配置文件内容
        self.is_prompt = 1  # 是否将结果图片弹窗
        self.result_path = None  # 结果文件路径
        self.is_prompt = None  # 是否将结果图片弹窗
    ...
    def compute_index(self, data, yield_file_path):
        """
        计算各类成交、持仓指标
        :return:
        """
        ...
        # 计算期初权益
        self.begin_period = self.compute_begin_period()
    ...
    def run(self, config_data, position_file, yield_file_path, result_path, is_prompt):
        ...
        if not data.empty:
            ...
            # 计算各类成交、持仓指标
            data = self.compute_index(data, yield_file_path)
