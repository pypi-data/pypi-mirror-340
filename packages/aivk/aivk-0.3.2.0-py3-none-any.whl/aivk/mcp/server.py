...
def mount_aivk_root_dir(path: str = None):
    ...
    try:
        ...
        try:
            ...
            # 如果当前事件循环已经在运行，使用create_task而不是run
            if loop.is_running():
                # 创建任务并等待执行完成
                loop.create_task(AivkFS.mount(path))
                steps.append("4. 使用当前事件循环创建挂载任务")
            else:
                # 如果没有运行的事件循环，仍可以使用asyncio.run
                asyncio.run(AivkFS.mount(path))
                steps.append("4. 使用asyncio.run执行挂载")
        except RuntimeError as e:
            # 如果无法获取事件循环，尝试使用替代方法
            steps.append(f"4. 事件循环管理错误: {str(e)}")
            # 直接调用同步版本或特殊处理
            AivkFS.mount_sync(path)  # 假设有同步版本的挂载函数
            steps.append("   使用同步方法替代执行挂载")
        ...
        # 显示挂载时间
        steps.append(f"8. 挂载完成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
