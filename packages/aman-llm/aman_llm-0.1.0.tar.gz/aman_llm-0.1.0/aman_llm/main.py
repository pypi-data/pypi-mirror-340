...
shell_tool = Agent(
    name="Shell Agent",
    role="Executes shell commands, automates terminal workflows, and provides information about the environment.",
...
file_writer = Agent(
    name="File Agent",
    role="Writes files to the filesystem.",
    model=Gemini(id="gemini-2.0-flash"),
    tools=[FileTools()],
    instructions=[
        '''
...
editor.print_response(user_query, stream=True)

# -*- Print the messages in the memory
pprint([m.model_dump(include={"role", "content"}) for m in editor.memory.messages]) 