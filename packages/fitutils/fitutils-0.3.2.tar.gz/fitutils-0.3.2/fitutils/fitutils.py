...
class FitRes(object):
    ...
    def _calc_params(self):
        ...
        if self._fitter.__class__.__name__ == 'LinFitXY':
            _dy = _sigxy(self.popt[0], self._data.dx, self._data.dy)
        else:
            _dy = self._data.dy
        self.chi2 = np.sum(
            (self._data.y - self._fitter.func(self._data.x, *self.popt))**2 / _dy**2)
        self.residus = self._data.y - \
            self._fitter.func(self._data.x, *self.popt)
        self.residus_norm = self.residus / _dy
    ...
    def plot(self, xplot, style=None, draw_hull=True, marker='.',
        ...
        if style == True or style.lower() == 'simple':
            ...
            ax[1].errorbar(self._data.x, self.residus,
                           xerr=self._data.dx, yerr=self._data.dy,
