...
class TulgaTTS:
    ...
    async def _synthesize_internal(self, text: str, voice_name: str) -> bytes | None:
        ...
        try:
            ...
            turn_message = await self.client.chat.send_message(char_id, chat_id, text)
            turn_id = turn_message.turn_id
            primary_candidate = turn_message.get_primary_candidate()

            if not primary_candidate:
                logging.error(f"No primary candidate found for message with fixed char ID '{char_id}'.")
                return None
            ...
            speech_bytes = await self.client.utils.generate_speech(
                ...
                voice_id=voice_id # Use the selected voice_id
            )
            return speech_bytes
