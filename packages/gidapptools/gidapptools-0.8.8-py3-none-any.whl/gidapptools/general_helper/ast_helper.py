...
def ast_from_file(in_file_path: Union[str, os.PathLike, Path]) -> ast.Module:
    in_file_path = Path(in_file_path).resolve()
    with in_file_path.open("r", encoding='utf-8', errors='ignore') as f:
        return ast.parse(f.read(), filename=in_file_path)
