...
def load_zipcode_mapping(env: Union[SparkEnv, PandasEnv]):
    if isinstance(env, SparkEnv):
        ...
        account_url = "https://{account_name}.blob.{endpoint}".format(
            account_name=azure_storage_account_name, endpoint="core.windows.net"
        )
        blob_service = BlobServiceClient(
            account_url=account_url,
            credential=azure_storage_sas_token.lstrip('?')
        )
        blob = blob_service.get_blob_client(
            container=azure_storage_container_name,
            blob=azure_storage_relative_path.lstrip('/')
        ).download_blob()

        return read_csv(StringIO(blob.readall()))
