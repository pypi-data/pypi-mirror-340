...
def _round_bin_edge(edge: float, bin_width: float, decimals: int = 2, direction: str = "nearest") -> np.float:
    ...
    if direction == "nearest":
        if bin_width >= 1:
            return np.float_(np.around(edge, decimals=decimals))
        else:
            return np.float_(np.around(edge, decimals=log_magnitude))
    else:
        round_func = np.ceil if direction == 'up' else np.floor
        mult = 10 ** log_magnitude
        return np.around(round_func(edge * mult) / mult, decimals=log_magnitude)  # type: ignore


def get_dataset_bins(cv_splits, X, y, y_full=None):
    """
    Compute bins for cross validated data.

    :param X:
    :param y:
    :param y_full: Full raw data, used only for time series CV splits.
    """
