...
from . import _scoring_utilities, constants, utilities
from ._metric_base import Metric, NonScalarMetric, ScalarMetric
...
class ExplainedVariance(RegressionMetric, ScalarMetric):
    ...
    def compute(self):
        ...
        ret = sklearn.metrics.explained_variance_score(
            self._y_test, self._y_pred, sample_weight=self._sample_weight, multioutput="uniform_average"
        )
    ...
    def aggregate(scores: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for bin_index in range(n_bins):
            ...
            if bin_count == 0:
                ...
                bin_var = _scoring_utilities.total_variance(split_counts, split_means, split_vars)
