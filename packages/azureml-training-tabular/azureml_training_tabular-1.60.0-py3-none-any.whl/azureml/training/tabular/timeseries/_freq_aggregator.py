...
class _ColumnTypeAggDetectionResult:
    """
    The class to hold the column types for the purpose of frequency aggregation.
    ...
    def __init__(
        self, numeric_columns: Set[Any], date_columns: Set[Any], categoric_columns: Set[Any], detection_failed: bool
    ) -> None:
        """
        Constructor.
...
def aggregate_dataset(
    ...
    column_types: Optional[_ColumnTypeAggDetectionResult] = None,
    ...
    if (
        time_series_config.freq is not None
        and dataset_freq is not None
        and _get_frequency_nanos(time_series_config.freq) < _get_frequency_nanos(dataset_freq)
    ):
        forecast_freqstr = (
            time_series_config.freq if isinstance(time_series_config.freq, str) else time_series_config.freq.freqstr
        )
    ...
    if not force_aggregation and (dataset_freq is None or dataset_freq == to_offset(time_series_config.freq)):
        ...
        special_columns = (
            ...
            else copy.deepcopy(time_series_config.time_series_id_column_names)
        )
            ...
            if time_series_config.time_series_id_column_names:
                ...
                for grain, X_one in X_groupby:
                    ...
                    data_grid = pd.date_range(
                        min_time, X_one[time_series_config.time_column_name].max(), freq=time_series_config.freq
                    )
    ...
    if X_groupby is None:
        ...
        if (
            ...
            and TimeSeriesInternal.DUMMY_GRAIN_COLUMN not in time_series_config.data_x.columns
        ):
...
def _aggregate_one_grain(
    X_one: pd.DataFrame,
    numeric_columns: Set[Any],
    datetime_columns: Set[Any],
    time_series_config: TimeSeriesDataConfig,
    time_series_id: Optional[GrainType] = None,
) -> pd.DataFrame:
    """
    ...
    cat_columns = (
        all_columns
        - special_column_names.union(numeric_columns, datetime_columns)
    ...
    target_agg_fun = cast(
        str, time_series_config.target_aggregation_function
    )  # type: Union[str, Callable[[Any], float]]
    if target_agg_fun == AggregationFunctions.SUM:
        target_agg_fun = sum_with_nan
    X_return = _resample_numeric_features(
        X_one, time_series_config, {TimeSeriesInternal.DUMMY_TARGET_COLUMN}, [target_agg_fun]
    )  # type: pd.DataFrame
    agg_target_col_name = set(X_return.columns) - {time_series_config.time_column_name}
    X_return.rename({list(agg_target_col_name)[0]: TimeSeriesInternal.DUMMY_TARGET_COLUMN}, axis=1, inplace=True)
    # Additional regressors are optional.
    # Aggregate numeric columns if any.
    if numeric_columns:
        X_one_num = _resample_numeric_features(
            X_one, time_series_config, numeric_columns, AggregationFunctions.ALL
        )  # type: pd.DataFrame
    ...
    if cat_columns or datetime_columns:
        ...
        index_df = _resample_numeric_features(
            index_df, time_series_config, {_INDEX}, [AggregationFunctions.MIN, AggregationFunctions.MAX]
        )
        index_df.dropna(subset=[_INDEX_MIN, _INDEX_MAX], inplace=True)
        if cat_columns:
            X_one_cat = _resample_cat_features(
                index_df, X_one, time_series_config.time_column_name, list(cat_columns)
            )  # type: pd.DataFrame
            X_return = X_return.merge(X_one_cat, on=time_series_config.time_column_name)
        if datetime_columns:
            X_one_dt = _resample_datetime_features(
                index_df, X_one, time_series_config, datetime_columns
            )  # type: pd.DataFrame
    ...
    if time_series_config.time_series_id_column_names:
        grain_ord = 0
        time_series_id_iterable = (
            time_series_id
            if isinstance(time_series_id, tuple) or isinstance(time_series_id, list)
            else (time_series_id,)
        )
        for grain in time_series_config.time_series_id_column_names:
...
def _resample_numeric_features(
    X_one: pd.DataFrame,
    time_series_config: TimeSeriesDataConfig,
    numeric_columns: Set[Any],
...
def _resample_datetime_features(
    ...
    **Note:** This function drops grains
    ...
    X_one_date_mode = _resample_cat_features(
        index_ranges, X_one_date, time_series_config.time_column_name, list(datetime_columns)
    )
...
def _resample_cat_features(
    index_ranges: pd.DataFrame, X_one_cat: pd.DataFrame, time_column_name: str, categorical_columns: List[Any]
) -> pd.DataFrame:
    """
    ...
    **Note:** This function drops grains
    ...
    for i in range(len(index_ranges)):
        ...
        cat_index = np.arange(index_ranges[_INDEX_MIN].iloc[i], index_ranges[_INDEX_MAX].iloc[i] + 1).astype(int)
...
def _replace_categories_with_mode(
    X_one: pd.DataFrame,
    indices: np.ndarray,
...
def get_columns_before_agg(columns: List[Any]) -> Set[str]:
    ...
    suffix = re.compile("|".join(["(_{}$)".format(suffix) for suffix in agg_functions]))
    return {col if not isinstance(col, str) else suffix.sub("", col) for col in columns}


def get_column_types(
    columns_train: List[Any], columns_test: List[Any], time_column_name: str, grain_column_names: Optional[List[str]]
) -> _ColumnTypeAggDetectionResult:
    """
    ...
    if columns_agg.intersection(columns):
        return _ColumnTypeAggDetectionResult(set(), set(), set(), detection_failed=True)
    ...
    if columns_agg:
        return _ColumnTypeAggDetectionResult(set(), set(), set(), detection_failed=True)
    return _ColumnTypeAggDetectionResult(numeric_columns, date_columns, categorical_columns, detection_failed=False)
