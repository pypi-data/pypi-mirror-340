...
def get_period_offsets_from_dates(reference_index, time_index, freq, misalignment_action="warn"):
    """
    ...
    Contract.assert_true(freq is not None, ForecastingUtilConstants._TIMESERIES_FREQUENCY_NOT_SET, log_safe=True)
    ...
    if is_iterable_but_not_string(reference_index):
        ...
        reference_index = pd.DatetimeIndex(np.repeat(reference_index, len(time_index)))
    ...
    if any(ref not in date_grid for ref in reference_index):
        msg = "get_period_offsets_from_dates: " + "Reference dates are misaligned. " + "Expected dates on grid {0}"
    ...
    if any(dt not in date_grid for dt in time_index):
        msg = (
            "get_period_offsets_from_dates: " + "Time index dates are misaligned. " + "Expected dates on grid {0}"
        ).format(time_index)
        if misalignment_action == "warn":
...
def make_groupby_map(df, groupby_cols_or_indices):
    ...
    """
    if groupby_cols_or_indices is None or len(groupby_cols_or_indices) == 0:
        from .time_series_data_frame import TimeSeriesDataFrame

        # Return an identity mapping when there are no groupby columns
        return pd.Series(TimeSeriesDataFrame.identity_grain_level(), index=df.index)
    ...
    helper_dict = {"gby_" + col: get_col_or_index(col) for col in groupby_cols_or_indices}
    ...
    if len(groupby_cols) == 1:
        ...
        series_map = pd.Series(map(tuple, helper_df[groupby_cols].values), index=helper_df.index)
