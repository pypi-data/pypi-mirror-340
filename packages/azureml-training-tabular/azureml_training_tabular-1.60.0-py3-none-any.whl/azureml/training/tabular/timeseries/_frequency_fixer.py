...
def fix_data_set_regularity_may_be(
    ...
    if aggregation_enabled:
        ...
        # We have to do the validations if we are running in v2 as in this case we are not protected
        # automl_base_settings validations.
        if automl_settings.featurization and not isinstance(automl_settings.featurization, str):
            config = automl_settings.featurization
            if isinstance(config, FeaturizationConfig):
                config = config.__dict__
            automl_settings._validate_timeseries_featurization_settings(config)
