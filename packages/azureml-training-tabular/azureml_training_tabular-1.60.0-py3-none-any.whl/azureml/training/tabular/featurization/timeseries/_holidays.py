...
class Holidays:
    ...
    def __init__(self):
        ...
        holidays_path = os.path.join("featurization", "timeseries", "_data", "holidays.7z")
        self.holiday_data_ref = package_resources.files(PACKAGE_NAME) / holidays_path
        with package_resources.as_file(self.holiday_data_ref) as holidays_data_path:
            self.holidays_df = _holiday_data_loader(holidays_data_path)

        mappings_cr = Series(self.holidays_df.CountryCode.values, index=self.holidays_df.CountryOrRegion)
        mappings_cc = Series(self.holidays_df.CountryOrRegion.values, index=self.holidays_df.CountryCode)
    ...
    def update_holidays_by_adding_window(self) -> None:
        ...
        holiday_resource_name = os.path.join("featurizer", "transformer", "timeseries",
                                             "publicholidays", "data", "holidays_origin.7z")
        holiday_ref = package_resources.files(PACKAGE_NAME) / holiday_resource_name
        with package_resources.as_file(holiday_ref) as holiday_origin_dataPath:
            with lzma.open(holiday_origin_dataPath, "rb") as f:
                df_origin = pickle.loads(f.read())

        window_resource_name = os.path.join("featurizer", "transformer", "timeseries",
                                            "publicholidays", "data", "holiday_effect_window.7z")

        window_ref = package_resources.files(PACKAGE_NAME) / window_resource_name
        with package_resources.as_file(window_ref) as window_dataPath:
    ...
    def get_holidays_dates(
        ...
        """
        date_col_name = "Date"
        if date_col_name in self.holidays_df.columns:
            if country_code:
                if self.holidays_dates_country_code != country_code:
                    self.holidays_dates = dict.fromkeys(
                        self.holidays_df[self.holidays_df.CountryCode == country_code][date_col_name].tolist(),
                        None,
                    )
                ...
                if self.holidays_dates_country != country_or_region:
                    self.holidays_dates = dict.fromkeys(
                        self.holidays_df[self.holidays_df.CountryOrRegion == country_or_region][
                            date_col_name
                        ].tolist(),
                        None,
                    )
    ...
    def get_holidays_in_range(self, start_date: date, end_date: date, country_code: str = "US") -> pd.DataFrame:
        ...
        rsdf_country = self.holidays_df
        if country_code is not None:
            rsdf_country = self.holidays_df[self.holidays_df.CountryCode == country_code]
        else:
            return rs
    ...
    def get_holidays_in_range_by_country_or_region(
        ...
        if country_or_region is not None:
            rsdf_country = self.holidays_df[self.holidays_df.CountryOrRegion == country_or_region]
        else:
            return rs
        rs_date = rsdf_country[(rsdf_country.Date >= start_date) & (rsdf_country.Date <= end_date)]
        rs_date.drop(columns=["CountryOrRegion", "CountryCode"], inplace=True)
