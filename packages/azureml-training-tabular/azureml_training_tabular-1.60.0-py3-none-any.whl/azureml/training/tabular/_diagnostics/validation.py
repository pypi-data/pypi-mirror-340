...
from .error_definitions import ArgumentBlankOrEmpty, InvalidArgumentType
...
class Validation:
    ...
    def validate_non_empty(value: Any, name: str, reference_code: Optional[str] = None) -> None:
        ...
        if is_empty:
            logger.error(
                "Argument validation failed. Expected argument {} to have a valid and " "non-empty value.".format(name)
            )
            raise AzureMLError.create(
                ArgumentBlankOrEmpty, argument_name=name, target=name, reference_code=reference_code
            )
    ...
    @staticmethod
    def validate_type(
        value: Any, name: str, expected_types: Union[type, Tuple[type, ...]], reference_code: Optional[str] = None
    ) -> None:
        """
        ...
        if not isinstance(value, expected_types):
            logger.error(
                "Argument validation failed. Expected argument {} of type {}, but is of type {}.".format(
