...
class ExperimentTypeApiTest(ApiBase, unittest.TestCase):
    ...
    def test_create_experiment_type_api(self):
        __unique_name = Generators.generate_unique_name('ExperimentTypeApi')
        ...
        #
        received = self.__create_entry_api(experiment_type, expect)
        ...
        #
        self.__create_error_entry_uk_api(experiment_type)

        #
        # Get entry by name
        #
        self.__get_all_entries_by_name_api(experiment_type_name, expect)

        #
        # Get entry by ID
        #
        self.__get_entry_by_id_api(experiment_type_id, expect)

        #
        # Put entry information (update some fields should succeed)
        #
        self.__update_entry_api(experiment_type_id, expect)

        #
        # Delete entry (should succeed)
        # (test purposes only to keep the DB clean)
        #
        self.__delete_entry_by_id_api(experiment_type_id)
    ...
    def __update_entry_api(self, entry_id, expect):
        ...
        response = self.client_api.update_experiment_type_api(
            entry_id,
            experiment_type_upd
        )
