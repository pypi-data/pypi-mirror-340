...
This library (metadata_api) is a client for the RESTful APIs exposed
by the European XFEL Metadata Catalogue Web Application - myMdC
(https://in.xfel.eu/metadata).

*Repository:*

- https://git.xfel.eu/ITDM/metadata_api
...
2. Make metadata_api library available in your python environment
 ...
 - `metadata_api` with the sources;
 - `metadata_api-4.1.0.dist-info/` with Wheels configuration files.
...
1. All tests continue passing successfully (to validate that run *pytest*)::
    ...
    pytest metadata_api/tests

    # When running all tests against the standard http application
    OAUTHLIB_INSECURE_TRANSPORT=1 pytest metadata_api/tests

    # Run all tests and get information about coverage for all files inside metadata_api package
    pytest --cov metadata_api/tests --cov-report term-missing
...
pypi-AgEIcHlwaS5vcmcCJGFlMDA3ODE5LTUxMTUtNDEwZC05YTE4LTM2Y2NhMDM1ZDA4NQACKlszLCJhNTgzNTg4YS04MTE0LTQ0M2MtODZmNS00ODMzNzg1YjBlMWIiXQAABiAIxm9nPIelB_TuD_NW4bOnL-oYYXFIkoY5KcKmOMZ2lA