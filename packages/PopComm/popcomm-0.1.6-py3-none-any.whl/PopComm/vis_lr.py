...
def circle_plot(filtered_lr, edge_width="count", node_colors=None, show_self_interactions=True, cutoff = 0, save_path=None):
    """
    ...
    - cutoff: Minimum edge weight to display.
    - save_path: If provided, saves the figure to the given path (e.g., 'output.png' or 'output.pdf').
    """
...
def dot_plot(filtered_lr, top_n=5, axis="LR-SR", size_scale=1500, min_size=5, selected_LR=None, figsize=(12, 8), save_path=None):
    """
    ...
    filtered_lr["log_pval"] = -np.log10(filtered_lr["p_val"].replace(0, np.nan))
    filtered_lr["log_pval"] = filtered_lr["log_pval"].fillna(15)
