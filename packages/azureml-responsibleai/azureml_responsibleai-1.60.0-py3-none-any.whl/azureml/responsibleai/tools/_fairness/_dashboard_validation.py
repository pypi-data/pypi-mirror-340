...
from azureml.responsibleai.common._errors.error_definitions import DashboardValidationError
...
def _check_serialisation_size(dash_dict, max_bytes):
    ...
    if dash_str_bytes > max_bytes:
        msg = _build_message(_DASHBOARD_UPLOAD_TOO_LARGE, dash_str_bytes, max_bytes)
        raise UserErrorException._with_error(AzureMLError.create(DashboardValidationError, full_message=msg))
...
def validate_dashboard_dictionary(dashboard_dict, max_serialization_bytes):
    ...
    _logger.info("Validation against schema complete")
    ...
    for sf in dashboard_dict[FDK.SENSITIVE_FEATURES]:
        ...
        if len(sf_vector) != len_y_true:
            msg = _build_message(_BAD_SF_LENGTH, sf_name, len(sf_vector), len_y_true)
            raise UserErrorException._with_error(AzureMLError.create(DashboardValidationError, full_message=msg))

        sf_classes = sf[FDK.SENSITIVE_FEATURE_CLASSES]
        num_classes = 1 + max(sf_vector)
        if len(sf_classes) != num_classes:
            msg = _build_message(_BAD_SF_GROUPS, sf_name, len(sf_classes), num_classes)
            raise UserErrorException._with_error(AzureMLError.create(DashboardValidationError, full_message=msg))

    len_metrics_d1 = len(dashboard_dict[FDK.METRICS])
    if len_metrics_d1 != num_sf:
        msg = _build_message(_BAD_METRICS_DIM1, len_metrics_d1, num_sf)
        raise UserErrorException._with_error(AzureMLError.create(DashboardValidationError, full_message=msg))

    for metrics_arr in dashboard_dict[FDK.METRICS]:
        if len(metrics_arr) != num_y_pred:
            msg = _build_message(_BAD_METRICS_DIM2, len(metrics_arr), num_y_pred)
            raise UserErrorException._with_error(AzureMLError.create(DashboardValidationError, full_message=msg))
