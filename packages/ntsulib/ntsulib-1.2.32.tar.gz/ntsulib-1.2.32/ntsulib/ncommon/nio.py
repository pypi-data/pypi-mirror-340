...
def getProjectPath(layer: int = 0, auto: Auto_Mode = Auto_Mode.normal) -> str:
    """
    获取项目路径（智能适配PyInstaller/Nuitka/开发环境）
    ...
    if getattr(sys, 'frozen', False):
        ...
        if is_pyinstaller_packaged():
            base_path = sys._MEIPASS  # 返回临时解压目录

        # 2. Nuitka打包
        elif is_nuitka_packaged():
            # 获取.exe真实路径（处理短路径和符号链接）
    ...
    if auto == Auto_Mode.disable:
        ...
        def is_project_dir(path: str) -> bool:
            ...
            normal_markers = [
                ...
                'setup.py', 'Pipfile', 'README.md'
            ]

            # 高敏感标记（high模式）
            high_markers = [
                '.idea', '.vscode', '.venv',
                'venv', 'env', '.projectroot'
            ]
