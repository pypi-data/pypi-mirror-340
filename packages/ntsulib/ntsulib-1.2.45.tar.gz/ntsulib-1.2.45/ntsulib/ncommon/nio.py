...
def copy_file_to(
    ...
    """
    Copy a file from source to destination, with two modes:
    - When file_name is provided: paths are directories and file_name is the filename
    - When file_name is None: paths are complete file paths

    Args:
        becopyed_file_path: Source path (either directory or full file path)
        new_file_path: Destination path (either directory or full file path)
        cover: Whether to overwrite if destination exists (True=overwrite, False=skip)
        file_name: Optional filename to append to directory paths
    """
    # Expand any ~ in paths
    src_path = os.path.expanduser(becopyed_file_path)
    dst_path = os.path.expanduser(new_file_path)

    # Handle file_name mode (directory paths)
    if file_name is not None:
        # Validate paths are directories
        if not os.path.isdir(src_path):
            raise ValueError(f"Source path must be a directory when file_name is provided: {src_path}")
        if not os.path.isdir(dst_path):
            raise ValueError(f"Destination path must be a directory when file_name is provided: {dst_path}")

        # Construct full paths
        src_path = os.path.join(src_path, file_name)
        dst_path = os.path.join(dst_path, file_name)
    else:
        # Validate paths are files (not directories)
        if os.path.isdir(src_path):
            raise ValueError(f"Source path must be a file when file_name is None: {src_path}")
        if os.path.isdir(dst_path):
            raise ValueError(f"Destination path must be a file when file_name is None: {dst_path}")

    # Check source exists and is a file
    if not os.path.exists(src_path):
        raise FileNotFoundError(f"Source file does not exist: {src_path}")
    if not os.path.isfile(src_path):
        raise ValueError(f"Source path is not a regular file: {src_path}")

    # Check if destination exists
    if os.path.exists(dst_path):
        if not cover:
            return  # Skip if not covering
        if os.path.isdir(dst_path):
            raise ValueError(f"Destination exists but is a directory: {dst_path}")
