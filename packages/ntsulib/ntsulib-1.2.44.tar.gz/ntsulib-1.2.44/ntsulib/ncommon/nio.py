...
def copy_file_to(becopyed_file_path: str, new_file_path: str, cover: bool):
    """
    Copy a file from source path to destination path, creating directories if needed.

    Args:
        becopyed_file_path: Path to the source file (can contain ~ for home directory)
        new_file_path: Destination path for the copied file
        cover: Whether to overwrite if destination file exists (True=overwrite, False=skip)
    """
    # Expand any ~ in paths to absolute paths
    ...
    # Check if destination is a directory (invalid case)
    if os.path.isdir(dst_path):
        raise ValueError(f"Destination path is a directory: {dst_path}")
    # Check if destination file exists
    if os.path.exists(dst_path):
        if not cover:
            return  # Skip if cover is False
