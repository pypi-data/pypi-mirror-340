...
class CfgStack (object):
  ...
  def _load (self):
    if not isinstance (self.fname, str):
      raise ValueError (f"Name: {self.fname} is not a string")
    for d in self.dirs:
      for ext in self.exts:
        if self.fname[0] in ("/", "."):
          f = Path (f"{self.fname}{ext}")
        else:
          f = Path (d / f"{self.fname}{ext}")
        # print (self.dirs, d, f, self.exts, ext)
        if f.is_file ():
          try:
    ...
    raise IOError (f"CfgStack: Cannot find/parse file for {self.fname}")
