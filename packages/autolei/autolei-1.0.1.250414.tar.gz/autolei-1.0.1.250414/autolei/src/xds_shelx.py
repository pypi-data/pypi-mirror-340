...
def convert_to_shelx(input_path: str, xconv_folder: str = "merge") -> None:
    ...
    if not hkl_list:
        messagebox.showerror("Error", "No HKL files found.")
        return
    if len(hkl_list) == 1:
...
def write_xconv(input_path: str, result_list: list, relative_path: str = "merge") -> None:
...
def output_content_P4P(file_folder: str, file_name: str = "1.P4P", multi: bool = True) -> None:
    ...
    line_append = [f"TITLE  AutoXDSGUI P4P export at {datetime.now().strftime('%a %b %d %H:%M:%S %Y')}\n",
                   f"CHEM \n",
...
def update_after_prep(input_path: str, ins_path: str, pcf_path: str, info_dict: dict, single: bool = False) -> None:
    ...
    Arguments:
        ...
        single (bool): Indicates if only a single dataset is included. Defaults to False.
    ...
    if not single:
        ...
        wavelength = update_pcf_file(input_path, pcf_path, info_dict, single)
...
def update_pcf_file(input_path: str, pcf_path: str, info_dict: dict, single: bool = False) -> float:
...
def update_ins(
    ...
    Arguments:
        ...
        reference (str, optional): Scattering factors reference. Defaults to None.
        olex2 (bool, optional): Indicates if the file is in OLEX2 format. Defaults to True.
