...
Overview:
    ...
    statistics, enabling comprehensive analysis. It supports tasks such as parsing indexing and scaling files, processing
    spot and integration data, and generating metadata summaries for large datasets.
...
def analysis_xscale_lp(xscale_lp: str) -> dict:
    ...
    info_dict["unit_cell"], info_dict["unit_cell_esd"], _ = get_avg_esd_cell(
        None, multi=True, mode="list", folder_list=[os.path.dirname(path) for path in info_dict["input"]])
    return info_dict
...
def extract_run_result(xds_dir: str, engine: str = engine_hkl_analysis) -> dict:
    def extract_run_result(xds_dir: str, engine: str = 'default_engine') -> dict:
...
def extract_cluster_result(
    cluster_dir: str, engine: str = 'default_engine', merge: bool = True, reso: float = None, output: bool = False
) -> dict:
...
if __name__ == "__main__":
    print(extract_run_result("/mnt/c/experiment_4_155/SMV"))
