...
from .image_io import head_opener
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Batch convert MRC files to 32-bit floating point monochrome TIFF with various compression methods.")
    ...
    parser.add_argument('-o', '--overwrite', action='store_true', help='Overwrite existing TIFF files if set.')
...
def process_folders(
    ...
    for folder in tqdm(process_list, desc="Processing Folders", unit="folder"):
        ...
        if os.path.exists(output_folder) and tiff_files_existing and not overwrite:
            print(
                f"\nSkipping '{folder}' as '{save_subfolder}' with TIFF files already exists. Use --overwrite to overwrite.")
            continue
