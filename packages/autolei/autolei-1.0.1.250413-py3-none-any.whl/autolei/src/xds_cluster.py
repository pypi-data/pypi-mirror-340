...
def extract_dendrogram(input_path: str, interactive: bool = True, callback: callable = None, work_folder : str = None) -> float:
...
def plot_dendrogram(z: np.ndarray, ccs: SimpleNamespace, input_path: str, interactive: bool = True,
    ...
    Args:
        ...
        ccs (SimpleNamespace): Contains the input data (and possibly other metadata).
    ...
    # Prepare the labels. If a crystal name starts with 'a', remove the first character.
    labels = np.array([
    ...
    latest_cutoff = [initial_distance]  # Mutable holder for the latest cutoff
    ...
    if interactive:
        ...
        # Define the button-click event handler for adjusting the dendrogram.
        def onclick(event):
            nonlocal hline
            if event.ydata is not None:
                ...
                # Remove old dendrogram lines and redraw the dendrogram.
        ...
        # Connect the matplotlib canvas with the onclick event.
        ...
        def open_xdspicker():
            ...
            if os.path.exists(xdspicker_excel_path):
                ...
                messagebox.showerror("Cannot find xdspicker.xlsx at the specified input path.")

        open_button = tk.Button(top, text="Open xdspicker.xlsx", command=open_xdspicker)
        open_button.pack(side=tk.TOP, pady=5)
        ...
        top.grab_set()  # Make the window modal (optional)
        return None
...
def merge(input_path: str,
    ...
    # Ensure input_path and _filter are provided
    if not input_path:
    ...
    # Read the Excel file and drop fully empty rows
    try:
    ...
    if "SG" in df.columns:
        ...
        if len(unique_sg) > 1:
            if not messagebox.askyesno("Inconsistent Space Groups",
                                       f"Inconsistent values in SG column: {unique_sg}. Do you wish to continue?"):
                return
    ...
    def get_hkl_path(row: pd.Series) -> str:
    ...
    def get_resolution(row: pd.Series) -> float:
    ...
    # Only keep rows with a non-null 'Path' that does not contain spaces.
    ...
    with open(inp_file_path, "w") as inp_file:
        # Use the "Space group" column if available; otherwise, fall back to "SG"
        ...
        for _, row in df_valid.iterrows():
            ...
            inp_file.write(f"CRYSTAL_NAME=a{row['No.']}\n")
...
if __name__ == "__main__":
    ...
    pprint(parse_xscale_lp("/mnt/c/SU-100/merge/dis1-cls1/XSCALE.LP"))
