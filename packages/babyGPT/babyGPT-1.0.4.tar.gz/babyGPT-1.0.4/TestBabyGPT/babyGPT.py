...
__date__      = '2025-April-13'   
__url__       = 'https://engineering.purdue.edu/kak/distGPT/babyGPT-1.0.4.html'
...
INTRODUCTION:
    ...
    the prompting script in the Examples directory. 
...
THE MAJOR COMPONENTS of babyGPT:
    ...
    bounded vocabulary --- meaning the set of words that the language model must
    understand. This is done by using a split-and-merge approach in which you start
    by considering each different word in your corpus as a sequence of the most basic
    symbols, which can be ASCII characters as in the WordPiece tokenizer or the
    individual bytes, as in the BPE (Byte Pair Encoding) tokenizer.  Subsequently,
    you form subwords by, first, merging the most basic constituents like the bytes
    and, then, merging smaller subwords into longer subwords, on the basis of the
    frequencies of the merged subwords vis-a-vis the frequencies of the components
    that were merged. The merging process continues until you have reached the
    specified vocabulary size.  What this logic implies is that if a long word in the
    corpus occurs sufficiently frequently, it will be represented by a single token.
    On the other hand, a relatively short word that occurs rarely in the original
    corpus could be decomposed into shorter tokens.  It is in this manner that, with
    the WordPiece tokenizer, the BERT LLM has a vocabulary of around 30,000 tokens
    and, with the BPE tokenizer, the GPT-3 has a vocabulary of 50,000 tokens. Without
    such tokenization, as my Lecture 13 slides show, the size of the vocabulary would
    grow continuously with the the size of the corpus.  And, if a language modeler,
    is ingesting terabytes of text, the vocabulary of the words it sees could run
    into millions.  It is not possible to devise the probability-based logic for
    next-word prediction if your underlying vocabulary is unbounded.
    ...
    PromptResponder must use its next-token prediction ability to keep on generating
    the tokens until it reaches the end-of-sentence token or until it has generated,
    say, N sentences through this process.
...
USAGE:
    ...
    corpus, you would need to construct an instance of the main babyGPT class and
    some supporting class as follows:
...
class babyGPT(object):
    ...
    class TrainTokenizer:
        ...
        def __init__(self, corpus_directory, target_vocab_size=50000):
            ...
            ##  Since we are assuming utf-8 encoding of the text corpus, the integer index
            ##  values (the ordinal values) between 0 and 127 will correspond to the chars
            ##  in the ASCII code and those between 128 and 255 to the other chars that are 
            ##  latin1 extensions of ASCII.  So the first available index for a new token
            ##  produced by a merge rule is 256:
        ...
        def train_tokenizer(self):
            ...
            ##  This dict stores the mappings from the characters that can be mapped to single integers.  We can
            ##  initialize this dictionary with the Latin-1 mappings from the individual chars to the int values
            ##  between 0 and 255.   IMPORTANT:  Subsequently, as we scan the text in our corpus and discover a 
            ##  new character (which, for example, could be an emoji) that maps to a single integer value, we will
            ##  add that entry to this dictionary.
        ...
        while True:
            ...
            if iter % 100 == 99:    
                ...
                for i in random.sample( range(dataloader.batch_size), 4 ): 
                    ...
                    print()
                    FILE_for_training_results.write("ground-truth: %s" % str(detokenizer( ' '.join(token_sequences_in_batch[i]) )))
                    FILE_for_training_results.write("\n")
                    FILE_for_training_results.write("   predicted: %s" % str(predicted_tokens[i]))
                    FILE_for_training_results.write("\n")
                    FILE_for_training_results.write(" detokenized: %s" % str(detokenizer( dataloader.tokenizer.decode(predicted_indexes[i],skip_special_tokens=True))))
                    FILE_for_training_results.write("\n")
