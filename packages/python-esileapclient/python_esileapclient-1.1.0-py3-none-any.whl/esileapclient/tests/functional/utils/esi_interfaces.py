...
def lease_create(client, node_uuid, lessee, parse=True, fail_ok=False, **kwargs):
    valid_flags = ("resource_type", "start_time", "end_time", "name", "properties")
    return _execute(
        client,
        cmd="lease create",
        valid_flags=valid_flags,
        kwargs=kwargs,
        args=(node_uuid, lessee),
        parse=parse,
        fail_ok=fail_ok,
    )
