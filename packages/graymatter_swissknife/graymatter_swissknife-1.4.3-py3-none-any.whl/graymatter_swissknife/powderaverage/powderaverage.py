...
def powder_average(dwi_path, bvals_path, delta_path, mask_path, out_path, debug=False):
    ...
    compute_powder_average = False
    # If one or more files are missing, compute the powder average
    # Check powder average file
    if not os.path.isfile(powder_average_filename):
        compute_powder_average = True
    # Check b-values file
    if not os.path.isfile(updated_bvals_path):
        compute_powder_average = True
    # Check delta file
    if not os.path.isfile(updated_delta_path):
        compute_powder_average = True
    # Check mask file
    if not os.path.isfile(updated_mask_path):
        compute_powder_average = True
        
    if compute_powder_average:
        ...
        logging.info("Powder average already computed, skipping this step. If you want to recompute it, please delete the existing files.")
...
def normalize_sigma(dwi_path, lowb_noisemap_path, bvals_path, out_path):
    ...
    # Check if the normalized sigma file is already present
    if not os.path.isfile(normalized_sigma_filename):
        ...
        logging.info("Normalized sigma map saved !")

    else:

        logging.info("Normalized sigma map already computed, skipping this step. If you want to recompute it, please delete the existing file.")
