...
def xgboost_powered_nls_loop(xgboost_model, target_signal, microstruct_model, acq_param, nls_param_lim, initial_gt=None):
    """
    ...
    if not microstruct_model.has_noise_correction:
        optim_fun = lambda x: np.sum(np.square(xgboost_model.predict(x) - target_signal))
    else:
        optim_fun = lambda x: np.sum(np.square(rice_mean(xgboost_model.predict(x[:-1]), x[-1]) - target_signal))
...
def xgboost_powered_nls_parallel(xgboost_model, signal, N, microstruct_model, acq_param, nls_param_lim, max_nls_verif=5, initial_gt=None, n_cores=-1):
    """
    ...
    x_parallel = Parallel(n_jobs=n_cores)(
        delayed(xgboost_powered_nls_loop_verified)(
            xgboost_model, signal[irunning], microstruct_model, acq_param, nls_param_lim, initial_gt[irunning], max_nls_verif
        )
...
def xgboost_powered_nls_loop_verified(xgboost_model, target_signal, microstruct_model, acq_param, nls_param_lim, initial_gt=None, max_nls_verif=5):
    """
    Verification of each NLS loop max_nls_verif times.
    :param target_signal: signal for all b-values (shells) and diffusion times (Deltas)
    :param microstruct_model: microstructure model
    ...
    """
    nls_loop_return = xgboost_powered_nls_loop(xgboost_model, target_signal, microstruct_model, acq_param, nls_param_lim, initial_gt)
    ...
    while touch_border(x_sol, nls_param_lim, microstruct_model.n_params) and iter_nb < max_nls_verif:
        nls_loop_return = xgboost_powered_nls_loop(xgboost_model, target_signal, microstruct_model, acq_param, nls_param_lim, initial_gt=None)
        x_sol, x_0 = nls_loop_return[0], nls_loop_return[1]
        iter_nb += 1
