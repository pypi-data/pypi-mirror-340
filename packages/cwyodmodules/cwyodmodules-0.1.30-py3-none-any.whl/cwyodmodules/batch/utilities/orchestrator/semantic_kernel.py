    ...
    async def orchestrate(
        ...
        with tracer.start_as_current_span("SemanticKernelOrchestrator.orchestrate.start"):
            ...
            if finish_reason == FinishReason.TOOL_CALLS:
                # Check that result.items exists and is a list with at least one function call
                if hasattr(temp_result, "items") and isinstance(temp_result.items, list) and temp_result.items and hasattr(temp_result.items[0], "name"):
                    function_name = temp_result.items[0].name
                    logger.info(f"{function_name} function detected")
                    function = self.kernel.get_function_from_fully_qualified_function_name(
                        function_name
                    )
                    arguments = json.loads(temp_result.items[0].arguments)
                    with tracer.start_as_current_span(
