    ...
    async def orchestrate(
        ...
        with tracer.start_as_current_span("SemanticKernelOrchestrator.orchestrate.start"):
            ...
            if finish_reason == FinishReason.TOOL_CALLS:
                ...
                arguments = temp_result.metadata.get("arguments", {})
                logger.info(f"Inside function arguments: {arguments}")
                with tracer.start_as_current_span(
                ...
                if isinstance(answer_value, list):
                    answer_instance = answer_value[0]
                else:
                    answer_instance = answer_value

                answer_text = answer_instance.answer
                source_documents = answer_instance.source_documents
                question_text = answer_instance.question
