    ...
    async def orchestrate(
        ...
        with tracer.start_as_current_span("SemanticKernelOrchestrator.orchestrate.start"):
            ...
            with tracer.start_as_current_span("SemanticKernelOrchestrator.invoke"):
                ...
                logger.info("Invoking orchestrate function: %s", orchestrate_function)
                logger.info(f"temp Result from orchestrate function: {temp_result}")
            ...
            if (result.finish_reason == finish_reason_check) or doc_refs:
                ...
                # # Check if answer contains document references like [doc1]
                # doc_refs = re.findall(r"\[doc(\d+)\]", result.content)
                
                # if doc_refs and hasattr(self, "search_documents"):
                #     # If references exist but no function call was made, trigger a search
                #     logger.info(f"Document references found in direct response: {doc_refs}")
                #     with tracer.start_as_current_span("SemanticKernelOrchestrator.retrieve_missing_documents"):
                #         # Call search function to get documents
                #         search_answer = await self.kernel.invoke(
                #             function_name="search_documents",
                #             plugin_name="Chat",
                #             question=user_message
                #         )
                #         source_documents = search_answer.value.source_documents
                # else:
                #     source_documents = []
