...
async def audio_to_text_single(audio: str, save_dir: Optional[str] = None,
                               save_to_clipboard: bool = True, open_dir: bool = False) -> Optional[str]:
    """
    [单个任务处理]将 "flac", "m4a", "mp3", "wav" 格式的音频转化为文字
    :param audio: 音频文件的路径
    :param save_dir: 保存结果到指定的文件夹，默认为 None，不保存
    :param save_to_clipboard: 是否将结果复制到剪贴板，默认为 True
    ...
    if save_to_clipboard:
        pyperclip.copy(result.to_txt())
        print("文案已复制到剪贴板，Windows用户可以使用 Win+V 查看！", flush=True)
