...
def test_conflict_error(databases: DatabaseCollection, root):
    ...
    os.environ["_SNOWFLAKE_PRINT_VERBOSE_STACK_TRACE"] = 'true'
    root.parameters(refresh=True)
    try:
        with pytest.raises(ConflictError) as exc_info:
            databases.create(new_db_def, mode=CreateMode.error_if_exists)
        assert f"Error Message: object '{test_database_name}' already exists" not in str(exc_info.value)
    finally:
        database.drop()
...
def test_not_found_error(databases: DatabaseCollection, root):
    ...
    with pytest.raises(NotFoundError) as exc_info:
        databases[test_database_name].fetch()
    assert (f"Error Message: database '{test_database_name}' does not exist or not authorized."
