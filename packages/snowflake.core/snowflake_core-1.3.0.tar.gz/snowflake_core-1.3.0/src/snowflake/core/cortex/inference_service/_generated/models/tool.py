...
class Tool(BaseModel):
    ...
    def from_dict(cls, obj: dict) -> Tool:
        ...
        _obj = Tool.parse_obj({
            "tool_spec":
            ToolToolSpec.from_dict(obj.get("tool_spec"))
            if obj.get("tool_spec") is not None else None,
        })
...
class ToolModel():
    ...
    def _to_model(self):
        return Tool(tool_spec=self.tool_spec._to_model()
                    if self.tool_spec is not None else None, )

    @classmethod
    def _from_model(cls, model) -> ToolModel:
        return ToolModel(tool_spec=ToolToolSpecModel._from_model(
            model.tool_spec) if model.tool_spec is not None else None, )
