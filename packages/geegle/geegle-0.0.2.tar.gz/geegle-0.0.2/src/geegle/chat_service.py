...
class ChatService:    
    ...
    async def search_tavily(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.cache_service.cache_search_results(query, "tavily", results)
            return results
    ...
    async def search_exa(self, query: str, max_results: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            response.raise_for_status()
            results = response.json()
            await self.cache_service.cache_search_results(query, "exa", results)
            return results
        except requests.ConnectionError as e:
            if cached_results:
                self.console.print("[yellow]Connection error, using cache[/yellow]")
