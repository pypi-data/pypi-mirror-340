...
async def test_lengths():
    ...
    assert len(list(ApiError)) == len(list(ApiErrorIdentifier)), f"ApiError ({len(list(ApiError))}) and ApiErrorIdentifier ({len(list(ApiErrorIdentifier))}) do not have the same number of elements"
    assert len(list(ApiError)) == len(list(HttpStatusMapper._mapping.keys())), f"ApiError ({len(list(ApiError))}) and HttpStatusMapper ({len(list(HttpStatusMapper._mapping.keys()))}) do not have the same number of elements"
...
async def test_fallback():
    ...
    assert ApiError.NOT_EXISTING_ERROR == ApiError.UNKNOWN_ERROR, "Fallback error is not used"
