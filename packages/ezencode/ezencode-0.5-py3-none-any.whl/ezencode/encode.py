...
class Encode(object):
    ...
    @classmethod
    def binary_stream(cls: type['Encode'], /) -> Generator[str | int]:
        for character in cls.character_stream():
            try:
                yield maps.Maps.generate_map(0)[ord(character)]
            except KeyError:
                raise Exception(f"Cannot encode character; '{character}'")

    @classmethod
    def symbol_stream(cls: type['Encode'], /) -> Generator[str | int]:
    ...
    @classmethod
    def encoded(cls: type['Encode'], /) -> str:
