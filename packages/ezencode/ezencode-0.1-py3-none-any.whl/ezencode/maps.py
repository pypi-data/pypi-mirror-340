"""SEA (Simple Encoding Algorithm) mapping module."""
...
class Maps(object):
    ...
    @classmethod
    def ascii_stream(cls: type['Maps'], /) -> Generator[str]:
        for character in range(0, 256):
            if character in range(0, 32) or character in range(127, 161):
                yield hex(character)
            else:
                yield chr(character)
    ...
    @classmethod
    def generate_map(cls: type['Maps'], map_type: int, /) -> dict[str, str]:
