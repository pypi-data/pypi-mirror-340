from falconfoundry.model import *


class FoundryFunction:
    """
    Represents a FoundryFunction.
    """

    _instance: ['FoundryFunction', None] = None
    ...
    def instance(
        ...
        """
        Fetch the singleton instance of the :class:`FoundryFunction`, creating one if one does not yet exist.
        ...
        :returns: :class:`FoundryFunction` singleton.
        """
        if FoundryFunction._instance is None:
            FoundryFunction._instance = FoundryFunction(
                module=module,
                config=config,
        ...
        return FoundryFunction._instance
    ...
    def __init__(
        ...
        if self._config is None:
            if config_loader is None:
                from falconfoundry.config_loader import ConfigLoader
                from falconfoundry.config_loader_fs import FileSystemConfigLoader
                config_loader = ConfigLoader(FileSystemConfigLoader())
            self._config = config_loader.load()
        if self._loader is None:
            from falconfoundry.loader import Loader
            self._loader = Loader()
        if self._router is None:
            from falconfoundry.router import Router
            self._router = Router(self._config)
        if self._runner is None:
            from falconfoundry.runner import Runner
            from falconfoundry.runner_http import HTTPRunner
    ...
    def handler(self, method: str, path: str):
        ...
        def call(func):
            from falconfoundry.router import Route
