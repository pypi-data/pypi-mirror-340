from falconfoundry.model import FoundryRequest, FoundryRequestParams, FoundryResponse
from dataclasses import dataclass, fields, is_dataclass
from typing import Dict


def response_to_dict(r: FoundryResponse) -> Dict:
    """
    Converts a :class:`FoundryResponse` to a dictionary.
    :param r: :class:`FoundryResponse` instance to convert.
...
def dict_to_request(d: Dict) -> FoundryRequest:
    """
    Converts a dictionary to a :class:`FoundryRequest`.
    :param d: Dictionary instance to attempt to map.
    :return: :class:`FoundryRequest` instance populated by the given dictionary.
    """
    req = dict_to_dataclass(d, FoundryRequest())
    req.params = dict_to_dataclass(d.get('params', None), FoundryRequestParams())
...
def dict_to_dataclass(d: [Dict, None], dc) -> [None, dataclass]:
    """
