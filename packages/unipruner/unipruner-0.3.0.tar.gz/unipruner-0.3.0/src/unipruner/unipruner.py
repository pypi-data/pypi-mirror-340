...
def log_metrics(logger, model, universal=False):
    ...
    if universal:
        logger.info(f"Starting universe: [oG: {G}, R: {R}, T: {T}, uM: {uM}, gr: {gr}, bP: {bP}]")
    else:
        logger.info(f"Resulting model: [G: {G}, R: {R}, T: {T}, uM: {uM}, gr: {gr}, bP: {bP}]")
...
def gapfill_on_media(logger, model, universe, expcon, media, varprec, reference, exclude_orphans):
    ...
    for medium in media: 
        ...
        df_B[f'{medium}'] = None
        logger.info(f"Gap-filling on medium '{medium}'...")
...
def focused_suggestions_on_media(logger, model, universe, expcon, media, focus, reference, exclude_orphans):
    
    
    # get metabolites mids on which to focus: 
    if focus == '-':
        return 0    
    focus_mids = focus.split(',')
    modeld_mids = [m.id for m in model.metabolites]


    # get involved media:
    if media == '-':
        logger.info(f"No media provided: focused gap-filling will be skipped.")
        return 0
    media = media.split(',')
    # at least 1 medium must exist:
    if any([i in expcon['media'].columns for i in media])==False:
    ...
    for medium in media: 
        ...
        for mid in focus_mids:
            ...
            if not (mid in modeld_mids and mid.endswith('_c')):
                logger.info(f"Cytosolic metabolite defined with --focus is not included: '{mid}'.")
                continue
...
def grow_on_media(logger, model, expcon, media, fva):
    ...
    for medium in media: 
        ...
        df_G[f'{medium}'] = None
        logger.info(f"Growing on medium '{medium}'...")
        ...
        df_G.loc["max(obj)", f'{medium}'] = res_fba
...
def synthesize_on_media(logger, model, expcon, media, synth):
    ...
    for medium in media: 
        ...
        # create dedicated column con the excel output
        df_S[f'{medium}'] = None
        logger.info(f"Testing synthesis on medium '{medium}'...")
...
def write_excel_model(model, filepath, df_B, df_G, df_S):
...
def unipruner(args, logger): 
    ...
    write_excel_model(model, f'{model.id}.xlsx', df_B, df_G, df_S)  
    ...
    response = focused_suggestions_on_media(logger, model, universe, expcon, args.gap_fill, args.focus, args.reference, args.exclude_orphans)
    if response==1: return 1
