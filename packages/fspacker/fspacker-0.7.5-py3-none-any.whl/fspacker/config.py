...
EMBED_FILE_NAME = f"python-{PYTHON_VER}-embed-{MACHINE_CODE}.zip"
# python 镜像
...
# 路径定义
DIR_CACHE: pathlib.Path = get_cache_dir()
DIR_EMBED: pathlib.Path = get_cache_dir() / "embed-repo"
DIR_LIBS_REPO: pathlib.Path = get_libs_dir()
PATH_EMBED_FILE: pathlib.Path = DIR_EMBED / EMBED_FILE_NAME
PATH_CONFIG: pathlib.Path = get_cache_dir() / "config.json"

for directory in (DIR_CACHE, DIR_EMBED, DIR_LIBS_REPO):
