...
class ProjectFolderPacker(BaseSpecPacker):
    ...
    def pack(self):
        for directory in (self.dest_dir,):
            logging.info(f"创建文件夹: [purple]{directory.name} [bold purple]:heavy_check_mark:")
...
class ExeEntryPacker(BaseSpecPacker):
    ...
    def pack(self):
        ...
        gui_mode = self.project_info.is_gui_project()

        exe_filename = "gui.exe" if gui_mode else "console.exe"
        src_exe_path = config.DIR_ASSETS / exe_filename
        assert src_exe_path.exists()
        dst_exe_path = self.dest_dir / f"{name}.exe"

        logging.info(f"打包目标类型: {'[green bold]窗口' if gui_mode else '[black bold]控制台'}[/]")
        logging.info(
...
class SourceResPacker(BaseSpecPacker):
    ...
    def _valid_file(self, filepath: pathlib.Path) -> bool:
        return all(x not in str(filepath) for x in config.IGNORE_ENTRIES)
    ...
    def pack(self):
        ...
        for entry in source_folder.iterdir():
            ...
            if entry.is_file():
                ...
                if entry.stem not in config.IGNORE_ENTRIES:
