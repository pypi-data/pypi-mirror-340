...
from finetune.parquet.schemas.schemas import SeedPropmts, FilterResult
...
class finetune_tools:
    ...
    def _save_dict_to_parquet(self, dictionary):
        """
        将Dict转换到parquet
        """
    ...
    @inject
    def filter_parquet_instructions_invoke(self, parquet_file, filter_parquet_instructions: str):
        """
        Parquet 问题集的过滤器
        """
        data = self._read_parquet_to_dict(parquet_file)
        result_rows = []

        def _probabilistic_save(rows, p=0.01):
            r = random.random()
            if r <= p:
        ...
        def _filter_invoke(item) -> BaseModel:
            ...
            prompt_template = ChatPromptTemplate.from_messages([
                ("system", "{system_prompt}"),
                ("user", "请根据我的筛选要求对TOPIC进行筛选，看TOPIC是否符合我的要求;"
