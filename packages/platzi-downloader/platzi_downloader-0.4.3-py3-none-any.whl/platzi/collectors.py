...
from .models import Chapter, Resource, ResourceType, TypeUnit, Unit, Video
from .utils import extract_video_description, get_m3u8_url, get_subtitles_url, slugify
...
async def get_unit(context: BrowserContext, url: str) -> Unit:
    ...
    try:
        ...
        if await page.locator(TYPE_SELECTOR).count() == 0:
            ...
            # Extract video description, resources and recommended readings from content
            description, resources_data, readings_data = extract_video_description(content)
            
            # Convert resources data to Resource objects
            resources = []
            if resources_data:
            ...
            # Convert readings data to Resource objects
            readings = []
            if readings_data:
                for r in readings_data:
                    readings.append(Resource(
                        name=r["name"], 
                        url=r["url"],
            ...
            video = Video(
                ...
                recommended_readings=readings if readings else None
            )
