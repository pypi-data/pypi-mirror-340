...
class DataBloomContext:
        ...
        try:
            ...
            if source.startswith("postgresql/"):
                ...
                try:
                    ...
                    logger.info(f"JDBC URL: {api_jdbc_url}")
                except Exception as e:
                ...
                try:
                    # Read from MySQL
                    if table.upper().startswith("SHOW") or table.upper().startswith("SELECT"):
                        # Handle SQL queries
                        df = spark.read \
                            .format("jdbc") \
                            .option("url", api_jdbc_url) \
    ...
    def run_sql_query(self, source: str, database: str, stmt: str) -> pd.DataFrame:
        ...
        try:
            ...
            engine = self.create_sqlalchemy_engine(source, database)
