...
import uvicorn
app=FastAPI()
from .lazy_table import Table
from .lazy_tree import Tree
...
async def home():
    ...
    col.appendChild(Table({'id':'table','style':{'width':'100%'}},
    ...
    col.appendChild(Table({'id':'table2'},{'radios':True,'headers':['col1','col2'],
                    'colWidths':[100,100]}))
    js=doc.body.Script()
    ls=[]
    ...
    col.appendChild(Tree({'id':'tree'},{'headers':['firstlayer','secondlayer','thirdLayer'],'colorSelectors':True}))
    js=doc.body.Script()
    js.addVar('data',[
