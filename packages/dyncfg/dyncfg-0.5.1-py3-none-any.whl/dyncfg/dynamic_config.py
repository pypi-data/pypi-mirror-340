...
class DynamicConfig:
    ...
    def create_from_dict(cls, config_data: Dict[str, Dict[str, Any]], default_section: str = "Default"):
        ...
        # Step 3: Ensure lock is appropriate (should already be RLock from __init__)
        if not isinstance(instance._lock, threading.RLock):
            logger.warning("Instance lock was not RLock after init in create_from_dict, resetting.")
