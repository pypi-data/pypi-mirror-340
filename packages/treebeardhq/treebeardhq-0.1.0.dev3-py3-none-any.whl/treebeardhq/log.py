...
class Log:
    ...
    def _prepare_log_data(message: str, data: Optional[Dict] = None, **kwargs) -> Dict[str, Any]:
        ...
        # loop through data and convert any datetime to ts
        for key, value in log_data.items():
            if isinstance(value, datetime):
                log_data[key] = int(value.timestamp())
            if isinstance(value, dict):
                # include only keys that end in id
                log_data[key] = {k: v for k,
                                 v in value.items() if k.endswith("id")}
            if isinstance(value, list):
                del log_data[key]
                # only include the count
                log_data[f"{key}_count"] = len(value)
            # if it's an object
            if isinstance(value, object):
                del log_data[key]
                # see if it has an ID field
                if hasattr(value, "id"):
                    log_data[f"{key}_id"] = value.id
                log_data[f"{key}_type"] = str(type(value))
