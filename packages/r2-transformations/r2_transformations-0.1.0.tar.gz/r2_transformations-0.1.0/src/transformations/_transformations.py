...
from transformations import _types as types
...
def _clip_within_precision(
    number: float, low: float, high: float, precision: float = _TOL
):
...
def _rmat_to_euler_xzx(rmat: types.RotationMatrix) -> types.EulerArray["XZX"]:
  ...
  if rmat[0, 0] < 1.0:
    if rmat[0, 0] > -1.0:
      z = np.arccos(_clip_within_precision(rmat[0, 0], -1.0, 1.0))
...
def _rmat_to_euler_yxy(rmat: types.RotationMatrix) -> types.EulerArray["YXY"]:
  ...
  if rmat[1, 1] < 1.0:
    if rmat[1, 1] > -1.0:
      x = np.arccos(_clip_within_precision(rmat[1, 1], -1.0, 1.0))
...
def _rmat_to_euler_yzy(rmat: types.RotationMatrix) -> types.EulerArray["YZY"]:
  ...
  if rmat[1, 1] < 1.0:
    if rmat[1, 1] > -1.0:
      z = np.arccos(_clip_within_precision(rmat[1, 1], -1.0, 1.0))
...
def _rmat_to_euler_zxz(rmat: types.RotationMatrix) -> types.EulerArray["ZXZ"]:
  ...
  if rmat[2, 2] < 1.0:
    if rmat[2, 2] > -1.0:
      x = np.arccos(_clip_within_precision(rmat[2, 2], -1.0, 1.0))
...
def _rmat_to_euler_zyz(rmat: types.RotationMatrix) -> types.EulerArray["ZYZ"]:
  ...
  if rmat[2, 2] < 1.0:
    if rmat[2, 2] > -1.0:
      y = np.arccos(_clip_within_precision(rmat[2, 2], -1.0, 1.0))
...
def axisangle_to_euler(
    axisangle: types.AxisAngleArray, ordering: str = "XYZ"
) -> types.SomeEulerArray:
...
def euler_to_axisangle(
    euler_vec: types.SomeEulerArray, ordering: str = "XYZ"
) -> types.AxisAngleArray:
...
def quat_diff_passive(
    source_quat: types.QuatArray, target_quat: types.QuatArray
) -> types.QuatArray:
...
def quat_exp(quat: types.QuatArray, tol: float = _TOL) -> types.QuatArray:
  ...
  return np.stack(
      [a_new[..., 0], v_new[..., 0], v_new[..., 1], v_new[..., 2]], axis=-1
  )
...
# LINT.IfChange
def quat_rotate(
    quat: types.QuatArray, vec: types.PositionArray
) -> types.PositionArray:
...
def quat_between_vectors(
    source_vec: types.PositionArray, target_vec: types.PositionArray
) -> types.QuatArray:
  ...
  if np.linalg.norm(source_vec) == 0 or np.linalg.norm(target_vec) == 0:
    raise ValueError(
        "Source or target vector is a 0 vector; cannot compute "
        "rotation for a vector with no direction."
    )

  dot_product_normalized = np.dot(
...
# LINT.IfChange
def quat_slerp(
    quat0: types.QuatArray, quat1: types.QuatArray, fraction: float
) -> types.QuatArray:
  ...
  if d < 0.0:
    ...
    d = -d
    q1 *= -1.0
  angle = np.arccos(_clip_within_precision(d, -1.0, 1.0))
  if abs(angle) < _TOL:
    return q0
...
def quat_to_euler(
    quat: types.QuatArray, ordering: str = "XYZ"
) -> types.SomeEulerArray:
...
def pos_quat_to_hmat(
    pos: types.PositionArray, quat: types.QuatArray
) -> types.HomogeneousMatrix:
...
# LINT.IfChange
def integrate_quat(
    quat: types.QuatArray, vel: types.AngVelArray
) -> types.QuatArray:
...
def rotation_x_axis(
    theta: np.ndarray, full: bool = False
) -> Union[types.RotationMatrix, types.HomogeneousMatrix]:
...
def rmat_to_axisangle(rmat: types.RotationMatrix) -> types.AxisAngleArray:
  ...
  """
  theta = np.arccos(_clip_within_precision((np.trace(rmat) - 1) / 2, -1.0, 1.0))
...
def rmat_to_euler(
    rmat: types.RotationMatrix, ordering: str = "ZXZ"
) -> types.SomeEulerArray:
...
# LINT.IfChange
def mat_to_quat(
    mat: Union[types.RotationMatrix, types.HomogeneousMatrix]
) -> types.QuatArray:
...
def rot6_to_rmat(rot6: np.ndarray) -> np.ndarray:
  ...
  if np.allclose(np.cross(xu, yu), 0.0, atol=tol):
...
def velocity_transform(
    ht: types.HomogeneousMatrix, vel: Optional[types.ArrayLike] = None
) -> np.ndarray:
  ...
  tv = np.vstack(
      [np.hstack([r, pcross.dot(r)]), np.hstack([np.zeros((3, 3)), r])]
  )
...
def force_transform(
    ht: types.HomogeneousMatrix, wrench: Optional[types.ArrayLike] = None
) -> np.ndarray:
  ...
  tw = np.vstack(
      [np.hstack([r, np.zeros((3, 3))]), np.hstack([pcross.dot(r), r])]
  )
...
def rotate_vec6(
    mat: Union[types.RotationMatrix, types.HomogeneousMatrix], vec6: np.ndarray
) -> np.ndarray:
...
def integrate_hmat(
    hmat: types.HomogeneousMatrix,
    twist: types.Twist,
    rotate_twist_to_hmat: bool = True,
) -> types.HomogeneousMatrix:
...
def postheta_to_matrix_2d(pose: np.ndarray) -> types.HomogeneousMatrix2d:
  ...
  ct = np.cos(pose[2])
  st = np.sin(pose[2])
  return np.array([[ct, -st, pose[0]], [st, ct, pose[1]], [0.0, 0.0, 1.0]])
...
def velocity_transform_2d(
    ht: types.HomogeneousMatrix2d, vel: Optional[types.ArrayLike] = None
) -> np.ndarray:
  ...
  tv = np.vstack([tv, np.array([0.0, 0.0, 1.0])])
...
def force_transform_2d(
    ht: types.HomogeneousMatrix2d,
