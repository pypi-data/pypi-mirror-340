...
class ExposeMotivation(CarolusTheme, AxesUtils, GanttUtils, MinimalSlideTemplate):
    ...
    def construct(self):
        ...
        axes.move_to([0.5, -0.25, 0])

        x_label = self.term_text("Product Variety", font_size = self.font_size_small)
        x_label.next_to(axes.x_axis.get_end(), m.DOWN, buff=0.25)

        y_label = self.term_text("Product Volume \n per Variant", font_size=self.font_size_small)
        ...
        curve_color = self.blue_bright
        dot_color = self.yellow
...
if __name__ == '__main__':
    ExposeMotivation.save_sections_without_cache()