...
import nacl.public
import sr25519
from bip39 import bip39_generate, bip39_to_mini_secret, bip39_validate
...
__all__ = ["DeriveJunction", "Keypair", "KeypairType", "MnemonicLanguageCode"]
...
class DeriveJunction:
    ...
    def from_derive_path(cls, path: str, is_hard=False):
        """
        Return DeriveJunction instance from derive path

        :param path: Derive path
        :param is_hard: Hard derivation if True
        :return:
        """
        if path.isnumeric():
...
def scale_encode_string(string: str) -> bytes:
    """
    Encode a string to SCALE
    ...
    if length < 64:
        ...
        raise NotImplementedError(
            "SCALE compact encoding only implemented for len < 64"
        )

    return encoded_len + string_in_bytes


def derive_ed25519_hard_junction(seed: bytes, cc: bytes) -> bytes:
    """
    Derive a hard junction

    Same as rust function:

        use codec::Encode;
        ("Ed25519HDKD", secret_seed, cc).using_encoded(sp_crypto_hashing::blake2_256)

    :param seed: Mini-secret seed
    :param cc: Chain code of path junction
    :return:
    """
    assert len(seed) == 32
    assert len(cc) == 32
...
class Keypair:
    def __init__(
        ...
        seed_hex: Optional[Union[str, bytes]] = None,
        ...
        if private_key:
            ...
            if type(private_key) is str:
                private_key = bytes.fromhex(private_key.replace("0x", ""))  # type: ignore
        ...
        if type(public_key) is str:
            public_key = bytes.fromhex(public_key.replace("0x", ""))  # type: ignore
        ...
        self.ss58_format: int = ss58_format  # type: ignore

        self.public_key: bytes = public_key  # type: ignore

        self.ss58_address: str = ss58_address  # type: ignore

        self.private_key: bytes = private_key  # type: ignore

        self.mnemonic = None

    @classmethod
    def generate_mnemonic(
        cls, words: int = 12, language_code: str = MnemonicLanguageCode.ENGLISH
    ) -> str:
        """
        ...
        language_code: The language to use, valid values are: 'en', 'zh-hans', 'zh-hant', 'fr', 'it', 'ja', 'ko', 'es'.
    ...
    def validate_mnemonic(
        cls, mnemonic: str, language_code: str = MnemonicLanguageCode.ENGLISH
    ) -> bool:
        """
    ...
    def create_from_mnemonic(
        ...
        if crypto_type == KeypairType.ECDSA:
            raise ValueError("ECDSA not supported")
        ...
        keypair.mnemonic = mnemonic  # type: ignore
    ...
    def create_from_seed(
        ...
        if type(seed_hex) is str:
            seed_hex = bytes.fromhex(seed_hex.replace("0x", ""))  # type: ignore
    ...
    def create_from_uri(
        ...
        """
        Creates Keypair for specified suri in format: `[mnemonic]/[soft-path]//[hard-path]`

        Parameters
        ----------
        suri: str
            Secret URI including mnemonic and derivation path.
        ss58_format: int
            Substrate address format
        crypto_type: int
            Use KeypairType.SR25519 or KeypairType.ED25519
        language_code: MnemonicLanguageCode
            Language of the mnemonic phrase
        ...
        suri_parts = suri_regex.groupdict()  # type: ignore
        ...
        if suri_parts["password"]:
            raise NotImplementedError(
                f"Passwords in suri not supported for crypto_type '{crypto_type}'"
            )
        ...
        if suri_parts["path"] != "":
            derived_keypair.derive_path = suri_parts["path"]  # type: ignore
            ...
            for junction in derive_junctions:
                if crypto_type == KeypairType.SR25519:
                    ...
                    if not junction.is_hard:
                        raise NotImplementedError(
                            "Soft derivation not supported for ED25519"
                        )
                    # get derived seed from parent seed and chaincode
                    seed = derive_ed25519_hard_junction(seed, junction.chain_code)

                    # generate keypair
                    child_pubkey, child_privkey = crypto_sign_seed_keypair(seed)

                else:
                    raise NotImplementedError(
                        f"Derivation for crypto type {crypto_type} not supported"
                    )
    ...
    def sign(self, data: Union[ScaleBytes, bytes, str]) -> bytes:
        ...
        if type(data) is ScaleBytes:
            data = bytes(data.data)  # type: ignore
        elif data[0:2] == "0x":
            data = bytes.fromhex(data[2:])  # type: ignore
        elif type(data) is str:
            data = data.encode()  # type: ignore
    ...
    def verify(
        self, data: Union[ScaleBytes, bytes, str], signature: Union[bytes, str]
    ) -> bool:
        """
        ...
        if not verified:
            ...
            verified = crypto_verify_fn(
                signature, b"<Bytes>" + data + b"</Bytes>", self.public_key  # type: ignore
            )
    ...
    def encrypt_message(
        ...
        if not self.private_key:
            raise ValueError("No private key set to encrypt")
        if self.crypto_type != KeypairType.ED25519:
            raise ValueError("Only ed25519 keypair type supported")
    ...
    def decrypt_message(
        ...
        if not self.private_key:
            raise ValueError("No private key set to decrypt")
        if self.crypto_type != KeypairType.ED25519:
            raise ValueError("Only ed25519 keypair type supported")
        private_key = nacl.bindings.crypto_sign_ed25519_sk_to_curve25519(
            self.private_key + self.public_key
