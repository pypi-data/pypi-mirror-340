...
class _AdbAutomlExperiment:
    ...
    def _should_retry(self, output, exception, current_retry):
        ...
        if pipeline_dto is not None:
            ...
            self.jasmine_client_exception_count = 0
            should_retry = ((pipeline_dto.pipeline_spec is None or pipeline_dto.pipeline_spec == '')
                            and not pipeline_dto.is_experiment_over)
            should_retry = (should_retry
                            and ((pipeline_dto.childrun_id is not None or pipeline_dto.childrun_id == '')
                                 and not self._is_special_run(pipeline_dto.childrun_id)))
