...
from hestia_earth.validation.validators.cycle import (
    ...
    validate_primary_product_input
)
...
def test_validate_primary_product_input_valid():
    # no products should be valid
    assert validate_primary_product_input({}, {}) is True

    with open(f"{fixtures_folder}/primary-product-as-input/valid.json") as f:
        cycle = json.load(f)
    assert validate_primary_product_input(cycle, cycle.get('site')) is True


def test_validate_primary_product_input_invalid():
    with open(f"{fixtures_folder}/primary-product-as-input/invalid.json") as f:
        cycle = json.load(f)
    assert validate_primary_product_input(cycle, cycle.get('site')) == {
        'level': 'error',
        'dataPath': '',
