...
class LocalServer:
    ...
    def _clean_procs(self):
        ...
        with self.apt_lock:
            ...
            for proc in self.apt.values():
                try:
                    proc.wait(10)
                    log.info(f"Proc p_uid={p_uid}:proc.pid={proc.pid} wait has completed.")

                except subprocess.SubprocessError as spe:
                    log.warning(f"wait on puid={proc.props.p_uid} failed: {spe}")

                except Exception as ex:
                    log.warning(f"Got exception {ex} while waiting for proc to exit.")
    ...
    def create_process_local_channel(self, msg: dmsg.SHCreateProcessLocalChannel) -> None:
        ...
        log.info(f"Received and Created a channel via SHCreateProcessLocalChannel with cuid={ch.cuid}")
        send_fli_response(resp_msg, msg.respFLI)
    ...
    def destroy_process_local_channel(self, msg: dmsg.SHDestroyProcessLocalChannel) -> None:
        ...
        log.info(f"Received an SHDestroyProcessLocalChannel for cuid={msg.cuid}")
                ...
                with self.apt_lock:
                    try:
                        ...
                        try:
                            ...
                            log.debug(f"Got exception while flushing stderr: {ex}")

                        try:
                            proc.props.stdout_connector.flush()
                        except Exception as ex:
                            log.debug(f"Got exception while flushing stdout: {ex}")
    ...
    def watch_oom(self):
        ...
        log.info(f"{warn_bytes_env=} {warn_pct_env=} {warn_pct=}")
        ...
        log.info(f"{critical_bytes_env=} {critical_pct_env=} {critical_pct=}")
        ...
        try:
            while not self.check_shutdown():
                ...
                if mem_utilization >= critical_pct:
                    critical_msg = f"OOM CRITICAL: Free memory on node {parms.this_process.index} with hostname {self.hostname} is less than {int(100-mem_utilization)+1}% with {mem_bytes} bytes available. Runtime is coming down.\n"
                    self.be_in.send(
                        dmsg.SHFwdOutput(
                            tag=get_new_tag(),
                            idx=parms.this_process.index,
                    ...
                    warning_msg = f"\nOOM WARNING: Free memory on node {parms.this_process.index} with hostname {self.hostname} is less than {int(100-mem_utilization)+1}% with {mem_bytes} bytes still available.\n"
            ...
            tb = traceback.format_exc()
            self._abnormal_termination(f"LS OOM daemon exception: {ex}\n{tb}")
