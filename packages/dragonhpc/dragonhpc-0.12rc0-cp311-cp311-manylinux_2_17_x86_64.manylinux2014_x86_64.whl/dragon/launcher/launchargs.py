...
for backend node-to-node communication. By default, the high speed transport
agent (hsta) is selected. Currently supported agents are: {', '.join([ta.value
for ta in TransportAgentOptions])}"""
...
def get_parser():
    ...
    parser.set_defaults(
        transport=TransportAgentOptions.HSTA,
