...
def get_idxs(
    ...
    res_idxs: npt.NDArray[np.integer] = np.searchsorted(
        a=self_x, v=np.atleast_1d(times), side="right"
    )
    # Fix up any overrun
...
class PPolyPiecewiseConstantNextLeftClosed:
    ...
    def antidifferentiate(
        ...
        """
        res = antidifferentiate_piecewise_constant(self=self, domain_start=domain_start)

        return res


discrete_to_continuous_piecewise_constant_next_left_closed = partial(
    discrete_to_continuous_piecewise_constant,
    piecewise_constant_like=PPolyPiecewiseConstantNextLeftClosed,
)
