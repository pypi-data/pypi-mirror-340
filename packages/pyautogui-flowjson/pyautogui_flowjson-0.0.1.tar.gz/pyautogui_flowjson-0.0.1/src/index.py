...
from .dtypes import index as types
...
parser.add_argument("--isDebug", action="store_true", help="Whether to enable debug")
args = parser.parse_args()  # args 是 Namespace 类型的 使用 . 获取属性
args.isDebug and utils.printDebug(args)


async def analyzeShot(shot: Optional[types.Shot]):
    if shot is None:
    ...
    # 尝试 获取图片绝对路径
    targetImagePath = utils.getAbsPath(
        pydash.get(screenShot, URI_KEY), args.imagesDirPath
    )
    return await utils.findOnScreen(
        targetImagePath,
        **arguments,
...
async def analyzeImage(image: Optional[types.Image]):
    if image is None:
        return {"isPass": True}
        ...
        (
            asyncio.gather(
                ...
                *[
                    ...
                    analyzeShot(v)
                    # 通用转换为数组
                    for v in utils.toArr(pydash.get(image, "includes"))
                ]
            )
        ),
        (
            asyncio.gather(
                *[analyzeShot(v) for v in utils.toArr(pydash.get(image, "excludes"))]
            )
        ),
    )
...
async def analyzeCondition(condition: Optional[types.Condition]):
    if condition is None:
        return {"isPass": True}
    imageRes = await analyzeImage(pydash.get(condition, "image"))
    args.isDebug and utils.printDebug('pydash.get(condition, "image")', pydash.get(condition, "image"))
    args.isDebug and utils.printDebug("imageRes", imageRes)
    if pydash.get(condition, "run") is None:
        return {
            "position": pydash.get(imageRes, "position"),
            "isPass": pydash.get(imageRes, "isPass"),
        }
    runRes = utils.execRun(
        utils.toArr(pydash.get(condition, "run")), space={**globals(), **locals()}
    )
    args.isDebug and utils.printDebug("runRes", runRes)
    return {
        "position": pydash.get(imageRes, "position"),
        "isPass": runRes,
    }
...
async def execJobs(
    jobs: Optional[List[types.Job]], flowMode: Optional[types.FlowMode] = None
):
    jobs: List[types.Job] = utils.toArr(jobs)
    if len(jobs) == 0:
        return
    if flowMode == types.FlowMode.parallel.value:
...
async def execJob(job: types.Job):
    ...
    if pydash.get(conditionRes, "isPass"):
        if pydash.get(job, "action") is not None:
            ...
            actionArguments: Union[dict, list] = pydash.get(job, "action.arguments", {})
        ...
        if pydash.get(job, "loop") is None:
            ...
            while loopIndex <= loopMax:
                args.isDebug and utils.printDebug(f"循环{loopIndex}次")
                await execJobs(pydash.get(job, "jobs"), pydash.get(job, "flowMode"))
...
async def execWorkflows(workflows: types.Workflows):
...
# F11 F12 退出进程
def on_press(key):
    args.isDebug and utils.printDebug(f"按下 {key}")
...
async def main():
    ...
    workflowsPath = utils.getAbsPath(args.workflowJsonAddress, args.jsonDirPath)
    content = await utils.readFile(workflowsPath)
    # 使用 json5 解析读取到的内容
    workflows: types.Workflows = json5.loads(content)

    args.isDebug and utils.printDebug(
        json.dumps(
            workflows,
            indent=2,
        )
