...
# dtypes.index 与 py自带的模块冲突 更名为dtypes表示 domain types
from dtypes.index import Shot, Image, Condition, Job, FlowMode, Workflows
...
async def analyzeShot(shot: Optional[Shot]):
    ...
    # 尝试 获取图片绝对路径
    targetImagePath = getAbsPath(pydash.get(screenShot, URI_KEY), args.imagesDirPath)
