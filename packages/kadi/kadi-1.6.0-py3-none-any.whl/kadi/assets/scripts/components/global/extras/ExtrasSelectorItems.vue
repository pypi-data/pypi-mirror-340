...
export default {
  ...
  mounted() {
    this.initializeSelection(this.extras, this.initialSelection);
  },
  methods: {
    initializeSelection(extras, selection) {
      for (let i = 0; i < extras.length; i++) {
        ...
        if (key in selection) {
          ...
          if (Object.keys(selection[key]).length > 0) {
            this.initializeSelection(extra.value, selection[key]);
          } else {
            this.checkExtras(extra, true);
          }
        }
      }
    },
    checkExtras(extras, checked, disabled = false) {
      for (const extra of kadi.utils.asArray(extras)) {
        ...
        if (kadi.utils.isNestedType(extra.type)) {
          this.checkExtras(extra.value, checked, true);
        }
      }
    },
  },
