...
class SageMakerConnectionMagic(Magics):
            ...
            if code_blocks[i].startswith(GET_IPYTHON_SHELL):
                ...
                try:
                    ...
                    match language_enum:
                        ...
                        case _:
                            if self._is_all_scala_comment(code_blocks[i]):
                                continue
                            # Session might be timeout or terminated with error
                            self._create_session_if_not_exist(session_manager, connection_name)
    ...
    def _is_all_scala_comment(self, code_block) -> bool:
        try:
            SCALA_LANGUAGE = tslanguage(tsscala.language())
            parser = tsparser(SCALA_LANGUAGE)
            tree = parser.parse(bytes(code_block, "utf8"))
            return all((node.type == "comment" or node.type == "block_comment") for node in tree.root_node.children)
        except Exception as e:
            self.logger.warning(f"Cannot determine if scala code only contains comments: {e}. Default to false.")
            return False
