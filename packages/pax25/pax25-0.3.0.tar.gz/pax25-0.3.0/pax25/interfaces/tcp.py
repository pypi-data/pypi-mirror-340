...
class TCPInterface(Interface[TCPSettings]):
    ...
    def connection_closed(self, connection: "TCPConnection") -> None:
        ...
        """
        print("I ran!", self.station.name)
        if self.shutting_down:
            print("SHUTTING DOWN!", self.station.name)
            # Do nothing, this will be cleaned up elsewhere.
            return
        if connection.inbound or connection.force_close:
            print("REMOVING!", self.station.name)
            self.connections.remove(connection)
            return
        # Need to retry.
        print("RETRYING!", self.station.name)
...
class TCPConnection:
    ...
    async def write_loop(self) -> None:
        ...
        """
        print("Entered write loop", self.interface.station.name)
        try:
            while item := await self.send_queue.get():
                ...
                assert writer
                try:
                    print("writing", self.interface.station.name)
                    writer.write(kiss_command(command, 0, frame.assemble()))
                    print("wrote", self.interface.station.name, frame)
                    await writer.drain()
                except OSError:
                    # Awaiting will kill Cancel this task before it finishes.
                    asyncio.ensure_future(self.close())
