...
class SpatialLocalisation(BaseComposite):
    ...
    Args:
        composite (RiskComponentComposite): The composite.
    ...
    def _compute_domain_and_areas(self, level: LevelComposite, periods: list):
        ...
        # limit research to touching areas
        # to avoid selection of areas with no relation with axis
        touching_areas = (self.domain * areas).dropna(dim="id", how="all").id.data
        ...
        if self.default_localisation:
            self.areas["areaName"] = (
                ["id"],
                [f"{self._('très localisé')} {self.areas['areaName'][0].item()}"]
                + self.areas["areaName"].values[1:].tolist(),
            )
