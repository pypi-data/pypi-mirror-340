...
class BaseReducer(BaseGeo):
    ...
    @cached_property
    def parent_data(self) -> Optional[xr.DataArray | xr.Dataset]:
        return self.parent.compute(geo_id=self.geo_id, force=True)
    ...
    def compute(self, **_kwargs) -> Dict | List[Dict]:
        """
    ...
    def times(self) -> List[Datetime]:
        return [Datetime(d) for d in self.parent_data["valid_time"].to_numpy()]
