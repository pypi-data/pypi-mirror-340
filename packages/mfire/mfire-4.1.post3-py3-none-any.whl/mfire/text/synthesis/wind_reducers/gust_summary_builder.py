...
class GustSummaryBuilder(BaseSummaryBuilderMixin, BaseParamSummaryBuilder):
    ...
    CACHED_EXCEPTIONS: tuple[Exception] = (WindSynthesisError, KeyError, ValueError)
    TIMEZONE: str = "Europe/Paris"
    HAZARD_NAMES: list[str] = ["Vent", "Rafales"]
    FIELD_NAME: str = "RAF__HAUTEUR10"
    ...
    def __init__(self, compo: SynthesisModule, dataset: xr.Dataset):
        ...
        self.units: str = self._get_composite_units(compo, self.PARAM_NAME)
        ...
        # Get risk infos
        self.risk_infos: dict[str, float | int | bool | str] = self._get_risk_infos(
            compo
        )
        self._adapt_pm_sep()
    ...
    def _set_zone_max(self, rg_max_key: str):
        ...
        values: tuple = _("en plaine", "fr"), _("sur les hauteurs", "fr")
    ...
    def _sg_max_descriptors(self, reference_datetime: Datetime) -> str:
        ...
        while bound_inf >= self.FORCE_MIN:
            ...
            if percent >= self.INTERVAL_PERCENT:
                if bound_inf == bound_inf_init:
                    ...
                    case = "2"
                self._summary.update({"bound_inf": bound_inf, "bound_sup": bound_sup})
                break
        ...
        if case == "":
            ...
            self._summary["bound_inf"] = bound_inf

        self._summary.update(
            {
                "units": self.units,
                "period": self._find_gust_period(bound_inf).describe(
                    reference_datetime, self.TIMEZONE, "fr"
                ),
                "sg_max": self.sg_max,
            }
        )
    ...
    def _process_sg_max_is_greater(
        ...
        if self.pm_separation is True:
            bound_inf: int = self._summary["bound_inf"]
            self._summary.update(
                {
            ...
            if (sg_case in ["1", "3"] and self.rg_max <= self.FORCE_MIN) or (
                sg_case == "2" and self.rg_max <= self._summary["bound_sup"]
            ):
                suffix = "_s"
    ...
    def _process_rg_max_is_greater(self, sg_case: str) -> str:
        if sg_case == "1" and self.rg_max <= self._summary["bound_sup"]:
            return sg_case
    ...
    def _compute_case_and_fill_summary(self, reference_datetime: Datetime) -> str:
        """Compute the case and fill the summary."""
