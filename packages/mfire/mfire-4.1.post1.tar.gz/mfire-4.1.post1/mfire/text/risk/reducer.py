...
from mfire.utils.string import concatenate_string, decapitalize, split_var_name
...
class RiskReducerStrategySnow(RiskReducerStrategy):
    def compute(self):
        ...
        intensity = (
            self.parent.parent.params["NEIPOT3__SOL"].compute() * spatial_risk_da
        )
        intensity = unit_conversion(intensity, "cm")

        if self.parent.is_multizone:
            localisation = self.parent.localisation.all_name
        else:
            localisation = self.parent.parent.alt_area_name(self.parent.geo_id)
    ...
    def process_period(self):
        ...
        self.reduction["periods"] = self.parent.parent.period_describer.describe(
            periods
        )
...
class RiskReducerStrategyMonozone(RiskReducerStrategy):
    ...
    def compute_infos(self, infos: List[xr.Dataset] | List[xr.DataArray]) -> Dict:
        ...
        Args:
            infos (List[xr.Dataset] | List[xr.DataArray]): List of DataArray or
        ...
        if self.parent.final_risk_max_level > 0:
            ...
            for key, bloc in self.reduction.items():
                ...
                if (
                    ...
                    or bloc["level"] != self.parent.final_risk_max_level
                ):
                    continue
    ...
    def compute(self) -> dict:
        ...
        Returns:
            List | dict: Reduced risk as a list and a dictionary containing
                information for each block.
        """
        final_risk_da = self.final_risk_da

        self.reduction = CentroidTemplateRetriever.read_file(
            TemplateRetriever.path_by_name(
                "risk/monozone_generic", self.parent.language
            ),
            index_col=["0", "1", "2", "3", "4"],
