...
class WeatherReducer(SynthesisReducer):
    ...
    def _manage_lpn_inconstistencies(self):
        ...
        consistencies_map = ~Wwmf.is_snow(self.parent_data["wwmf"]) | (
            self.parent_data["lpn"] < self.parent.altitude("lpn")
        )
        if not consistencies_map.all():
            LOGGER.warning(
                "Some inconsistencies with snow and LPN were found and fixed"
            )
            self.parent_data["wwmf"] = self.parent_data["wwmf"].where(
    ...
    def _compute_densities(self):
        ...
        for i in range(1, len(self.times)):
            data_for_fixed_time: xr.DataArray = self.parent_data.wwmf.sel(
    ...
    def _pre_process(self):
        ...
        if self.parent_data["wwmf"].units == "w1":
            self.parent_data["wwmf"] = from_w1_to_wwmf(self.parent_data["wwmf"])

        # Replace current codes with nebulosity
        replacing_codes = {72: 71, 73: 70, 78: 77, 82: 81, 83: 80}
        for old, new in replacing_codes.items():
            self.parent_data["wwmf"] = self.parent_data["wwmf"].where(
                self.parent_data["wwmf"] != old, other=new
            )

        if "lpn" in self.parent_data:
    ...
    def _process_localisation(
        ...
        Returns:
            Tuple[Optional[str], bool]: Tuple containing the best localized area name or
                None if not possible and a boolean indicating if it's  a default
                localisation (a less bad area)
        """
        if not wwmfs:
            return None, False
        ...
        if geos_descriptive.size == 0:
            return geos_data.altAreaName.item(), True

        iol = compute_iol(geos_descriptive, precipitation_map)
        return (
            concatenate_string(
                iol[0].areaName.values, last_delimiter=f" {self._('et')} "
            )
            if iol[0].sum().data / geos_data_size < 0.9
    ...
    def _process_lpn(self) -> List[Dict]:
        ...
        lpn = int(
            round_to_closest_multiple(
                self.parent_data["lpn"]
                .where(Wwmf.is_snow(self.parent_data["wwmf"]))
                .min()
                .item(),
                100,
            )
