...
class RiskComponentComposite(AbstractComponentComposite):
    ...
    def levels_of_risk(self, level: float) -> List[LevelComposite]:
        """
    ...
    def level_event_couples(
        ...
        """Generate the level/event couples for which events have the input field."""
        for level in sorted(self.levels, key=lambda level: level.level, reverse=True):
            for event in level.events:
    ...
    @staticmethod
    def _is_plain_mountain_separation(level_event_couples: list, geo_id: str) -> bool:
        """Check if there is a separation between the plain and the mountain."""
        for _, event in level_event_couples:
    ...
    def get_risk_infos(
        ...
        Args:
            ...
            valid_time (slice): A time slice object representing the valid time
                range to consider.
            data_types (list[str]): Data types as a list which can be [], ['min'],
                ['max'] or ['min', 'max'].
            with_details (Optional[bool]): True to have details, else False

        Returns:
            dict[str, float | int | bool]: A dictionary with the risk's infos.
        """
...
class SynthesisComponentComposite(AbstractComponentComposite):
    ...
    def _compute(self, **_kwargs) -> Optional[xr.DataArray | xr.Dataset]:
        """
        ...
        """
        return xr.merge([weather.compute(force=True) for weather in self.weathers])
