...
class RiskBuilderFactory(RiskBuilder, BaseCompositeFactory):
    geo_id: str = "geo_id"
    parent: BaseCompositeFactory = RiskComponentCompositeFactory()
...
class _RepValueBuilderBaseFactory(RepValueBuilder, BaseCompositeFactory):
    geo_id: str = "geo_id"
    parent: RiskComponentCompositeFactory = RiskComponentCompositeFactory()
...
class RepValueBuilderFactory(_RepValueBuilderBaseFactory):
    reducer_class: type = lambda **kwargs: RepValueReducerFactory(
        phenomenon_factory="phen", **kwargs
    )


class FFRepValueReducerFactory(_RepValueReducerBaseFactory, FFRepValueReducer):
    test_var_name: ClassVar[str] = "FF__HAUTEUR"


class FFRepValueBuilderFactory(_RepValueBuilderBaseFactory, FFRepValueBuilder):
...
class TemperatureRepValueBuilderFactory(
    _RepValueBuilderBaseFactory, TemperatureRepValueBuilder
):
    reducer_class: type = TemperatureRepValueReducerFactory


class FFRafRepValueReducerFactory(_RepValueReducerBaseFactory, FFRafRepValueReducer):
    test_var_name: ClassVar[str] = "RAF__HAUTEUR10"


class FFRafRepValueBuilderFactory(_RepValueBuilderBaseFactory, FFRafRepValueBuilder):
...
class AccumulationRepValueBuilderFactory(
    _RepValueBuilderBaseFactory, AccumulationRepValueBuilder
):
...
class SnowRepValueBuilderFactory(_RepValueBuilderBaseFactory, SnowRepValueBuilder):
...
class PrecipitationRepValueBuilderFactory(
    _RepValueBuilderBaseFactory, PrecipitationRepValueBuilder
):
...
class RainRepValueBuilderFactory(_RepValueBuilderBaseFactory, RainRepValueBuilder):
...
class LpnRepValueBuilderFactory(_RepValueBuilderBaseFactory, LpnRepValueBuilder):
...
class AltitudeRepValueReducerFactory(AltitudeRepValueReducer, BaseCompositeFactory):
    parent: RiskComponentComposite = RiskComponentCompositeFactory()
    geo_id: str = "geo_id"
