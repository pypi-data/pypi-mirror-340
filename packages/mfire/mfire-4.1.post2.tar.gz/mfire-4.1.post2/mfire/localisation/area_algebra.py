...
def _iou_retained_ids(
    ...
    return (
        geos_descriptive.where(
            iou >= iou_max * MARGIN_THRESHOLD, drop=True
        ).id.values.tolist(),
        iou_max < MIN_IOU_THRESHOLD,
    )
...
def _iol_retained_ids(
    ...
    remainings = remainings.sortby(remainings.sum(SPACE_DIM), ascending=True)
...
def _clean_inclusions(
    ...
    sorted_areas = sorted_areas.sortby(sorted_areas.sum(SPACE_DIM), ascending=False)
...
def compute_iol(
    ...
    Returns:
        Tuple[xr.DataArray, bool]: Tuple with the best area and a boolean indicating
            if it's a default area or not
    """
