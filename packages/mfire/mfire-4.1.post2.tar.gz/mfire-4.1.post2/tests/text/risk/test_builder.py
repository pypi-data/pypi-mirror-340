...
class TestRiskBuilder:
    ...
    def test_compute_simple_ME(self, reduction, expected):
        assert (
            RiskBuilderFactory(
                parent=RiskComponentCompositeFactory(hazard_name="ME_XXX"),
                reduction_factory=reduction,
...
class TestRiskBuilderMonozone:
    ...
    def test_post_process_precip(self, final_risk_max, assert_equals_result):
        ...
        for language in builder.iter_languages():
            builder.reducer = RiskReducerFactory(
                final_risk_max_level_factory=final_risk_max,
                reduction={
...
class TestRiskBuilderMultizone:
    ...
    def test_template(self, template_name_factory, assert_equals_result):
        ...
        builder = RiskBuilderFactory(
            is_multizone_factory=True, template_name_factory=template_name_factory
        )
        result = {}
        for language in builder.iter_languages():
            result[language] = {}
