    ...
    class TokenPayload(BaseModel):
        ...
        def set_iat_and_exp(cls, values:dict):
            ...
            if not iat:
                ...
                if not isinstance(datetime, iat):
                    values["iat"] = datetime.fromisoformat(iat)
                else:
                    values["iat"] = iat
            exp = values.get("exp", None)
            if not exp:
                ...
                if not isinstance(datetime, exp):
                    values["exp"] = datetime.fromisoformat(exp)
                else:
                    values["exp"] = exp
            return values
