...
class reward_货船(Quest_Task):
    ...
    def task_flow(self):
        ...
        # 节点坐标
        coordinate_node_货船 = {(239,133):"res/dtws/reward_task/货船_小地图1.bmp",
                                (207,119):"res/dtws/reward_task/货船_小地图2.bmp",
                                (175, 115):"res/dtws/reward_task/货船_小地图3.bmp",}

        coor_dict_start = (239,133)
        coor_dict_end = (134, 120)
        ...
        if self.map_name and self.map_name in map_name_list :
            ...
            if not condition_1 and not condition_2: #未锁定目标体
                ...
                if "商盟" in self.map_name: #根据坐标判断
                    value_x_end, value_y_end = coor_dict_end
                ...
                if self.coord and all(isinstance(x, int) for x in self.coord[:2]):
                    for key, value in coordinate_node_货船.items():
...
dic_reward_tasks={
    ...
    "image": {
        ...
        "res/dtws/reward_task/不赦死囚.bmp":(371, 136, 973, 696, 0.8, 22, 176),  # 场景
        "res/dtws/reward_task/出口.bmp": (997,545,1151,670, 0.8, -18, -15),
        "res/dtws/reward_task/出口_货船.bmp": (997,545,1151,670, 0.8, -18, -15),
        },
    "yolo":{
        # "豪猪":(0,0,0,0),
        # "蜀山":True,
...
#资源合并,保持开启
quest_task_货船_merged_dict=merge_dicts(dic_reward_tasks,quest_task_merged_dict) #todo:更改名称

#todo:任务信息,是否需要去除背景色,调试是打开
task_background_scope=(1177,251,1395,552)
task_货船_text_alternative_parameters=task_background_scope #todo,更改名称
...
def run(target_name_list):
    ...
    # todo,注意参数名称更改
    task=reward_货船(py,win_hwnd[0],quest_task_货船_merged_dict,True,100,task_货船_text_alternative_parameters,quest_task_word_standard_dict,target_name_list=target_name_list)
    res=task.run(classes=2,running_time=1)
    print(res)

target_name_list=["疾风强盗","巨狼",]
