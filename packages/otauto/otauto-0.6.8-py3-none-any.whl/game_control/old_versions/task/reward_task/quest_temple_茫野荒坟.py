...
class reward_茫野荒坟(Quest_Task):
    ...
    def task_flow(self):
        ...
        # 节点坐标
        coordinate_node_茫野荒坟 = {(47,62):"res/dtws/reward_task/荒坟_1.bmp",
                                (56,78):"res/dtws/reward_task/荒坟_2.bmp",
                                (80, 77):"res/dtws/reward_task/荒坟_3.bmp",
                                (78, 70):"res/dtws/reward_task/荒坟_4.bmp",}

        coor_dict_start = {"茫野荒坟": (56,50)}
        coor_dict_end = {"茫野荒坟": (79, 75)}
        ...
        if self.map_name and self.map_name in map_name_list :
            ...
            if not condition_1 and not condition_2: #未锁定目标体
                ...
                if self.coord and all(isinstance(x, int) for x in self.coord[:2]):
                    for key, value in coordinate_node_茫野荒坟.items():
...
dic_reward_tasks={
    ...
    "image": {
        ...
        "res/dtws/reward_task/出口.bmp": (497,473,593,573, 0.8, 43, -24),
        },
    "yolo":{
        # "豪猪":(0,0,0,0),
        # "蜀山":True,
...
#资源合并,保持开启
quest_task_茫野荒坟_merged_dict=merge_dicts(dic_reward_tasks,quest_task_merged_dict) #todo:更改名称

#todo:任务信息,是否需要去除背景色,调试是打开
task_background_scope=(1177,251,1395,552)
task_茫野荒坟_text_alternative_parameters=task_background_scope #todo,更改名称
...
def run(target_name_list):
    ...
    # todo,注意参数名称更改
    task=reward_茫野荒坟(py,win_hwnd[0],quest_task_茫野荒坟_merged_dict,True,100,task_茫野荒坟_text_alternative_parameters,quest_task_word_standard_dict,target_name_list=target_name_list)
