...
class TaskMain(TaskLogic):
        ...
        if self.map_name in ["血衣镇"]:
            ...
            for key, value in self.task_info.items():
                if "血衣首领" in key and "0" in key:
                    self.find_data_from_keys_list_click(["0"],self.task_info,delay_time=10)
                    self.mouse_move_scope(929, 263, 1163, 479)
    ...
    def type_guide(self):
        if self.find_data_from_keys_list_click(["resource/images_info/main_task/任务对话.png"],self.image_data,delay_time=3):
            ...
            if self.find_data_from_keys_list(["技能（完成）"],self.word_acquire_data):
                ...
                if self.find_data_from_keys_list_click(["确定"],self.word_handle_data,delay_time=1):
                    logger.info("技能学习完成")
                elif self.find_data_from_keys_list_click(["resource/images_info/other/技能升级.png"],self.image_data,delay_time=1):
        ...
        if not self.role_running or self.map_differences:
            for key, value in self.task_info.items():
                if "集" in key and "0" in key:
                    for i in range(3):
                        if self.find_data_from_keys_list_click(["木料"], self.yolo_data, delay_time=10):
                            logger.info("采集中")
                    self.find_data_from_keys_list_click(["0"],self.task_info,delay_time=10)
                    self.mouse_move_scope(929, 263, 1163, 479)
            ...
            try:
                for task_name, value in self.task_info.items():
                    if is_task_incomplete(task_name, keywords):
                        self.perform_task_action(["未"], delay_time=10, mouse_x=mouse_x, mouse_y=mouse_y)
                    elif is_task_complete(task_name, keywords):
                        self.perform_task_action(finish_keywords, delay_time=10, mouse_x=mouse_x, mouse_y=mouse_y)
    ...
    def task_details(self):
        ...
        # 奖励中心
        if self.find_data_from_keys_list_click(["福利中心"],self.word_handle_data,delay_time=2,action=3):
