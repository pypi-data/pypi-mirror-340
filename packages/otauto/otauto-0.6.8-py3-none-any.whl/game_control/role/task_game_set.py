...
class TaskGameSet(TaskLogic):
    ...
    def task_快捷键取消(self):
        ...
        if self.node_counter>=5:
            ...
            if res_dict: #{2: (607, 369)}
                h=25 #格子之间的高度
                # self. queue_message({"interval":38})
                ...
                try:
                    self.mouse_drag(877, 284, 882, 400,step=20)  # 拖动
                    time.sleep(1)
                    self.node_flag = True
    ...
    def task_游戏设置(self):
        ...
        if self.node_counter>=5:
            ...
            if not self.set_flag:
                ...
                self.queue_message({"interval": 13})
    ...
    #     logger.error(f"npc收藏:{self.node_set}")
    #     if "系统菜单界面" in self.interface_info:
    #         self.key_press("esc",delay_time=2)
    #         return True
    #
    #     if self.find_data_from_keys_list_click(["定"], self.word_handle_data, delay_time=2):
    #         return True
    #
    #     if self.node_counter>=12 :
    #         if "主界面" in self.interface_info:
    #             self.interface_closes()
    #             if self.find_data_from_keys_list_click(['resource/images_info/other/快速搜索_关闭.png'], self.image_data, delay_time=1):
    #                 return True
    #             self.node_current = "task_基础技能"
    ...
    #         logger.info("地图界面")
    #         if self.node_set == {"功能1","功能2", "任务", "活动1" ,"活动2"}:
    #             logger.info("npc收藏完成,界面关闭")
    #             self.node_counter = 15  # 重置节点计数器
    #             self.mouse_left_click(1053,177,delay_time=1) #收藏界面关闭
    #             self.key_press("M",delay_time=2) # 退出地图界面
    #             return True
    #
    #         elif self.find_data_from_keys_list(["自动寻路"],self.word_handle_data) or self.find_data_from_keys_list(["resource/images_info/other/自动寻路.png"],self.image_data):
    #             if "功能1" not in self.node_set:
    #                 logger.error("爵位领取人收藏")
    #                 # 调用处理函数
    #                 if not self.node_menu_flag :
    #                     if self.find_data_from_keys_list_click(["功能"],self.word_acquire_data, delay_time=1,action=2):
    #                         self.node_menu_flag=True
    #                     return True
    #                 elif self.node_menu_flag:
    #                     self.handle_node("功能1", 26,  ["爵位领取人"])
    #                     if "功能1" in self.node_set:
    #                         self.node_menu_flag=False
    #                     return True
    #
    #             elif "功能2" not in self.node_set :
    #                 logger.error("悬赏令发布使收藏")
    #                 # 调用处理函数
    #                 if not self.node_menu_flag :
    #                     if self.find_data_from_keys_list_click(["功能"],self.word_acquire_data, delay_time=1,action=2):
    #                         self.node_menu_flag=True
    #                     return True
    #                 elif self.node_menu_flag:
    #                     self.handle_node("功能2", 25, ["购买悬赏令"])
    #                     if "功能2" in self.node_set:
    #                         self.node_menu_flag = False
    #                     return True
    #
    #             elif "任务" not in self.node_set:
    #                 logger.error("西域商团青城山接引人收藏")
    #                 # 调用处理函数
    #                 if not self.node_menu_flag :
    #                     if self.find_data_from_keys_list_click(["resource/images_info/other/任务.png"],self.image_data, delay_time=1,action=2):
    #                         self.node_menu_flag=True
    #                     return True
    #                 elif self.node_menu_flag:
    #                     self.handle_node("任务", 19,  ["西域商团青城山接引人"])
    #                     if "任务" in self.node_set:
    #                         self.node_menu_flag = False
    #                     return True
    #             elif "活动1" not in self.node_set:
    #                 logger.error("任务领取人收藏")
    #                 if not self.node_menu_flag:
    #                     if self.find_data_from_keys_list_click(["传送"], self.word_acquire_data,x3=76,y3=9, delay_time=1,action=2):
    #                         self.node_menu_flag = True
    #                     return True
    #                 elif self.node_menu_flag:
    #                     self.handle_node("活动1", 24,  ["义军任务领取人"])
    #                     if "活动1" in self.node_set:
    #                         self.node_menu_flag = False
    #                     return True
    #             elif "活动2" not in self.node_set:
    #                 logger.error("任务领取人收藏")
    #                 self.handle_node("活动2", 24,  ["白屏寨唐军"])
    #                 if "活动2" in self.node_set:
    #                     self.node_menu_flag = False
    #                 return  True
    #
    ...
    #             self.mouse_left_click_scope(1011, 573, 1067, 580) # 点击快速搜索
    #             time.sleep(2)
    #             return True
    #
    #
    #     elif "主界面" in self.interface_info and len(self.node_list)<3:
    #         self.key_press("M",delay_time=2) # 打开地图
    #         self.queue_screenshot_sync()
    #         return True
    #
    #     self.node_counter+=1
    ...
    def task_元宝购买(self):
        ...
        if self.node_counter>=5:
            ...
            if self.追星_flag and self.龙睛_flag and self.药箱_flag:
                ...
                self.queue_message({"interval":35})
        ...
        if self.node_counter>=5:
            ...
            try:
                ...
                if res_dict:
                    ...
                    point=(scope_list[0][0]+14,scope_list[0][1]+12)
                if point:
                    self.queue_message({"interval":75})
    ...
    def task_details(self):
        ...
        if self.find_data_from_keys_list(["返回"], self.word_handle_data):
            self.ls_progress = "task_finish"
            return True
