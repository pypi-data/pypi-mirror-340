...
class Explore(TaskLogic):
    ...
    def task_医生模块(self):
        ...
        """
        try:
           #  res_image=self.find_data_from_keys_list(["resource/images_info/other/空血槽.png"],self.image_data)
           #  logger.error(f"======================={res_image}")
           #  if res_image:
           #      for key,value in res_image.items():
           #          scope_list=value["scope"]
           #          for scope in scope_list:
           #              x1, y1, x2, y2, _ = scope
           #
           #  """
           # {'resource/images_info/other/空血槽.png': {'scope': [(142, 226, 179, 243, 0.822), (143, 226, 180, 243, 0.835), (144, 226, 181, 243, 0.837), (145, 226, 182, 243, 0.852), (146, 226, 183, 243, 0.879), (147, 226, 184, 243, 0.904), (148, 226, 185, 243, 0.939), (149, 226, 186, 243, 0.967), (150, 226, 187, 243, 0.982), (151, 226, 188, 243, 0.993), (152, 226, 189, 243, 0.982), (153, 226, 190, 243, 0.954), (154, 226, 191, 243, 0.919), (155, 226, 192, 243, 0.879), (156, 226, 193, 243, 0.834)], 'model': 0, 'enable': True, 'unique': True}}
           #
           #  """
    ...
    def task_移动_astar(self):
        ...
        if res_list:
            ...
            if converted_points:
                time_queue = max(len(converted_points) * 0.4-5, 1)
                ...
                self.key_press("1",delay_time=1)  # 攻击
                self.queue_screenshot_sync() # 截图
                return "task_finish"
        else:
    ...
    def task_移动模块(self):
        if "主界面" in self.interface_info:
            ...
            if len(self.point_list) >= 4 or self.astar_fail_num>=3:
                self.astar_fail_num=0 # 失败次数初始化
    ...
    def task_general(self):
        ...
        if self.find_data_from_keys_list(["连续按|空格|挣脱控制"],self.word_handle_data):
            for i in range(3, 6):
                self.key_press("spacebar", delay_time=0.3)
    ...
    def task_on_hook(self,team_mode:bool=False):
        ...
        if team_mode and self.role_sect not in ["少林"]:
            ...
            if self.captain_leave_flag:  # 队长不在附近
                ...
                if self.role_sect in ["百花医"]:  # 医生 ,加血,协助攻击
                    if self.task_医生模块():
                        self.key_press("f2", delay_time=0.5)  # 选择队长
                        self.key_press("kp0", delay_time=1)  # 选择跟随
                        for i in range(random.randint(1, 2)):
                            self.key_press("8", delay_time=0.5)
                        for i in range(random.randint(1, 2)):
                            self.key_press("9", delay_time=1.5)
                ...
                if captain_info['interactor'] in ["5"]:
                    ...
                    self.queue_interval(1)
                    self.key_press("f2", delay_time=0.2)  # 选择队长
                    self.key_press("kp0", delay_time=0.4)  # 选择跟随
                    self.key_press("F", delay_time=1)  # 协助攻击
                    self.node_current = "task_刷怪模块"  # 移动节点
                    return True

        if self.target_info and self.target_info['name'] in ["屠狼牙","贱剑客","纳魂尊者","灭魂灵兽","摄魂玄女"]:
        ...
        if self.find_data_from_keys_list_click(["resource/images_info/main_task/任务对话.png"], self.image_data,
            ...
            if "屠狼牙" in self.boss_name or "贱剑客" in self.boss_name or self.boss_name == {"灭魂灵兽","摄魂玄女","纳魂尊者"}:
                self.node_current = "task_离开模块"
                self.point_list = []  # 重置坐标列表
                self.boss_flag = True  # boss标识
                if team_mode:
