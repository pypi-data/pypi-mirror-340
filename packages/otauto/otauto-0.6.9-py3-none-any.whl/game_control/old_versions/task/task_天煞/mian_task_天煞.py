...
from task.task_天煞.quest_task_凤鸣山_天煞 import *
from task.task_天煞.quest_task_血衣镇_天煞 import *
from task.task_天煞.reward_task_template_天煞 import * #悬赏任务
from task.camp_task_template import camp_task_word_standard_dict
from task.task_天煞.quest_task_屠狼洞_天煞 import *
from game_role.role_奖励领取 import * #奖励领取
from game_role.role_技能学习 import * #技能学习
from game_role.role_技能配置_天煞 import * #技能配置
...
class Mian_Task_天煞(TaskModule): #凌云主线任务
    ...
    def task_拜见童敢(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:童敢","拜见:童敢(已完成)"],"dic_word_ocr",x3=83,y3=8):
                pass
            elif self.get_value_from_key_operation("拜见:童敢(未完成)","dic_word_ocr",x3=35,y3=8):
                pass
    ...
    def task_熟悉武器(self):
        ...
        if self.get_value_from_key("【主线】黄沙野兽", "dic_word_ocr"):
            self.node_current = "task_黄沙野兽"
            return "task_finish"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:童敢","打倒:荒漠武者(已完成)"],"dic_word_ocr",x3=50,y3=5):
                logger.info("任务交付")
            elif (self.target_info and self.target_info["目标体"] == "荒漠武者")  or self.find_ppocr_word_op("荒漠武者", 629, 71, 757, 94):
                logger.info("寻找到荒漠武者,开始攻击")
                self.target_info.update({"目标体": "荒漠武者"}) #手动更新目标信息
                skill_num=random.randint(1, 2)
                self.key_press_op(f"{skill_num}",delay_time=2)

            elif self.get_value_from_key_operation("打倒:荒漠武者(/5)","dic_word_ocr",x3=40,y3=8):
                logger.info("寻找荒漠武者中")
    ...
    def task_黄沙野兽(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:童敢","打倒:黄沙刀客(已完成)"],"dic_word_ocr",x3=83,y3=8):
                logger.info("任务交付")
            elif (self.target_info and self.target_info["目标体"] == "黄沙刀客")  or self.find_ppocr_word_op("黄沙刀客", 629, 71, 757, 94):
                logger.info("寻找到黄沙刀客,开始攻击")
                self.target_info.update({"目标体": "黄沙刀客"}) #手动更新目标信息
                skill_num=random.randint(1, 2)
                self.key_press_op(f"{skill_num}",delay_time=2)
            elif self.get_value_from_key_operation("打倒:黄沙刀客(/5)","dic_word_ocr",x3=38,y3=8):
                logger.info("寻找黄沙刀客中")
    ...
    def task_熟悉药品(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:童敢","了解:杂货商(完成)"],"dic_word_ocr",x3=83,y3=8):
                logger.info("任务交付")
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_value_from_key("学习:技能(完成)","dic_word_ocr"):
                ...
                if self.py.ocr_vnc_click("确",629,441,695,465,delay_time=1):
                    logger.info("技能学习完成")
                elif self.py.mutil_colors_vnc_click(colors,786, 432, 835, 483,delay_time=1,x3=13,y3=-14):
                    logger.info("点击技能图标")

        elif "游戏主界面" in self.game_interface[0]:
            if self.get_items_from_all_keys_operation(["交付人:童敢","学习:技能(完成)"],"dic_word_ocr",x3=83,y3=8):
                logger.info("任务交付")
    ...
    def task_新的任务(self):
        ...
        if self.get_value_from_key("【主线】夺回长枪", "dic_word_ocr"):
            self.node_current = "task_夺回长枪"
            return "task_finish"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:萨可", "寻找:萨可(完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
            elif self.get_value_from_key_operation("寻找:萨可(未完成)", "dic_word_ocr", x3=50, y3=8):
                logger.info("寻找萨可中")
    ...
    def task_夺回长枪(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:萨可","寻物:长枪(已完成)"],"dic_word_ocr",x3=38,y3=4):
                logger.info("任务交付")

            elif (self.target_info and self.target_info["目标体"] == "盗马贼首")  or self.find_ppocr_word_op("盗马贼首",635, 65, 716, 97):
                logger.info("寻找到盗马贼首,开始攻击")
                self.target_info.update({"目标体": "盗马贼首"}) #手动更新目标信息
                skill_num=random.randint(1, 3)
                self.key_press_op(f"{skill_num}",delay_time=2)

            elif self.get_value_from_key_operation("寻物:长枪(0/1)","dic_word_ocr",x3=38,y3=4):
                logger.info("寻找盗马贼首中")
    ...
    def task_驿站的功用(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:萨可", "开通:驿站(外)(完成"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
    ...
    def task_能力的考察(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:萨可", "上等木料(已完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
            else:
                if self.coord[-1] and self.get_value_from_key("采集:上等木料(/3)", "dic_word_ocr"):
                    if self.get_yolo_click("木料",delay_time=5):
                        logger.info("采集中")
                    elif self.coord and 300<self.coord[0]<350 and 70<self.coord[1]<110:
                        self.get_value_from_key_operation("采集光圈","dic_mutil_colors",delay_time=5)
    ...
    def task_收服武将(self):#todo.血衣镇副本没有写
        ...
        if self.get_value_from_key("【主线】卓兰的求助", "dic_word_ocr"):
            self.node_current = "task_卓兰的求助"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:萨可", "打倒:血衣首领(已完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
    ...
    def task_卓兰的求助(self):
        ...
        if self.get_value_from_key("【主线】挑战契丹主力", "dic_word_ocr"):
            self.node_current = "task_挑战契丹主力"
            return "task_finish"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:卓兰", "寻找:卓兰(完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
            elif self.get_value_from_key_operation("寻找:卓兰(未完成)", "dic_word_ocr", x3=50, y3=8):
                logger.info("寻找:卓兰中")

    def task_挑战契丹主力(self):
        """
        1,寻找强盗
        2,攻击强盗
        3,交付任务
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:卓兰","寻物:契丹令牌(已完成)"],"dic_word_ocr",x3=83,y3=8):
                logger.info("任务交付")

            elif (self.target_info and self.target_info["目标体"] == "契丹士兵") or self.find_ppocr_word_op("契丹士兵",597, 71, 791, 92):
                logger.info("寻找到契丹士兵,开始攻击")
                self.target_info.update({"目标体": "契丹士兵"}) #手动更新目标信息
                skill_num = random.randint(1, 2)
                self.key_press_op(f"{skill_num}", delay_time=2)

            elif self.get_value_from_key_operation("寻物:契丹令牌(/2)","dic_word_ocr",x3=38,y3=8):
                logger.info("寻找契丹士兵中")
    ...
    def task_突如其来的隋军(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:卓兰", "找人:龙虎禁军(完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
    ...
    def task_危机(self):
        ...
        if self.get_value_from_key("【主线】奇异的雕", "dic_word_ocr"):
            self.node_current = "task_奇异的雕"
            return "task_finish"
    ...
    def task_奇异的雕(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:卓兰","打倒:雕(已完成)"],"dic_word_ocr",x3=83,y3=8):
                logger.info("任务交付")

            elif (self.target_info and self.target_info["目标体"] == "雕")  or self.find_ppocr_word_op("雕",597, 71, 791, 92):
                logger.info("寻找到雕,开始攻击")
                self.target_info.update({"目标体": "雕"}) #手动更新目标信息
                skill_num = random.randint(1, 2)
                self.key_press_op(f"{skill_num}", delay_time=2)

            elif self.get_value_from_key_operation("打倒:雕(/5)","dic_word_ocr",x3=52,y3=4):
    ...
    def task_使用道具商城(self):
        ...
        if self.get_value_from_key("【主线】为天下霸道之路", "dic_word_ocr"):
            self.node_current = "task_天下霸道之路"
            return "task_finish"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:卓兰", "购买:青龙丹(完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
            elif self.get_value_from_key("购买:青龙丹(未完成)", "dic_word_ocr"):
                self.get_value_from_key_operation("res/dtws/main_task/商城.bmp","dic_image_hand",delay_time=2)

    def task_天下霸道之路(self):
        """
        1,对话界面点击对话
        2,寻路
        3,【主线】谁动了乞丐的馒头,该节点完成
...
#notice,待修改
        task_word_key=["【主线】继续主线","【主线】拜见童敢","【主线】熟悉武器","【主线】黄沙野兽","【主线】熟悉药品","【主线】门派的技能",
                       "【主线】新的任务","【主线】夺回长枪","【主线】驿站的功用","【主线】能力的考察","【主线】收服武将","【主线】卓兰的求助",
                       "【主线】挑战契丹主力","【主线】突如其来的隋军","【主线】危机","【主线】奇异的雕","【主线】使用道具商城","【主线】天下霸道之路",
...
dic_reward_tasks={
    ...
    "mutil_colors":{
        ...
        "了解:杂货商(未完成)": {"colors": {"ed140f":(1339, 278),
            ...
            "打倒:荒漠武者(/5)":{"colors":{"2df9f9":(1253, 276),
                                    "27d8d8":(1285, 283),
                                    "21b7b7":(1298, 288),
                                    "ed140f":(1334, 287),
                                    },
                        "scope":(1205, 275, 1348, 289),
                        "tolerance":25},
...
public_res:公共资源
"""
reward_merged_dict_天煞=merge_dicts(dic_reward_tasks,public_res)
...
task_天煞_text_alternative_parameters=task_background_scope
...
"""
#notice,待修改
task_天煞_word_standard_dict={"继续":"【主线】继续主线",
                            "拜见":"【主线】拜见童敢","童":"拜见:童敢(未完成)","敢":"拜见:童敢(完成)","童敢":"交付人:童敢",
                            "器":"【主线】熟悉武器","漠":"打倒:荒漠武者(/5)","荒":"打倒:荒漠武者(已完成)",
                            "野":"【主线】黄沙野兽","黄":"打倒:黄沙刀客(/5)","沙":"打倒:黄沙刀客(已完成)",
                            "药品":"【主线】熟悉药品","杂":"了解:杂货商(未完成)","货":"了解:杂货商(完成)",
                            "派":"【主线】门派的技能","技":"学习:技能(未完成)","能":"学习:技能(完成)",
                            "新":"【主线】新的任务","萨":"寻找:萨可(未完成)","可":"寻找:萨可(完成)","萨可":"交付人:萨可",
                            "夺":"【主线】夺回长枪","长":"寻物:长枪(0/1)","枪":"寻物:长枪(已完成)",
                            "功":"【主线】驿站的功用","站":"开通:驿站(外)(未完","驿":"开通:驿站(外)(完成",
                            "考":"【主线】能力的考察","木":"采集:上等木料(/3)","料":"采集:上等木料(已完成)",
                            "将":"【主线】收服武将","衣":"打倒:血衣首领(0/1)","领":"打倒:血衣首领(已完成)",
                            "助":"【主线】卓兰的求助","卓":"寻找:卓兰(未完成)","兰":"寻找:卓兰(完成)","卓兰":"交付人:卓兰",
                            "契":"【主线】挑战契丹主力","契丹":"寻物:契丹令牌(/2)","令牌":"寻物:契丹令牌(已完成)",
                            "其":"【主线】突如其来的隋军","禁":"找人:龙虎禁军(未完成)","虎":"找人:龙虎禁军(完成)",
                            "机":"【主线】危机",
                            "奇":"【主线】奇异的雕","打":"打倒:雕(/5)","倒":"打倒:雕(已完成)",
                            "商":"【主线】使用道具商城","青":"购买:青龙丹(未完成)","丹":"购买:青龙丹(完成)",
                            "路":"【主线】天下霸道之路","明":"寻找:明月(未完成)","月":"寻找:明月(完成)","明|月":"交付人:明月",
...
# 倒序排列
task_天煞_word_standard_dict_order = dict(sorted(task_天煞_word_standard_dict.items(), reverse=True))
