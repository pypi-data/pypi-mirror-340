...
from task.task_凌云.quest_task_凤鸣山_凌云 import *
from task.task_凌云.quest_task_血衣镇_凌云 import *
from task.task_凌云.reward_task_template_凌云 import * #悬赏任务
from task.camp_task_template import camp_task_word_standard_dict
from task.task_凌云.quest_task_屠狼洞_凌云 import *
from game_role.role_奖励领取 import * #奖励领取
from game_role.role_技能学习 import * #技能学习
from game_role.role_技能配置_凌云 import * #技能配置
from game_role.role_装备进阶 import * #装备进阶

"""
更新日志:2024-12-29 16:44:38
更新内容:凌云寨升级主线
"""

class Mian_Task_凌云(TaskModule): #凌云主线任务
    ...
    def  role_promotion(self):
        ...
        if award_flag and skills_learn_flag and skills_set_flag and equipment_evolution_flag:
            self.mouse_left_click_op(803,832,delay_time=1)
            return "task_finish"
    ...
    def task_拜见闻拓(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:闻拓","拜见:闻拓(已完成)"],"dic_word_ocr",x3=83,y3=8):
                pass
            elif self.get_value_from_key_operation("拜见:闻拓(未完成)","dic_word_ocr",x3=35,y3=8):
                pass
    ...
    def task_熟悉武器(self):
        ...
        if self.get_value_from_key("【主线】黄羊肉", "dic_word_ocr"):
            self.node_current = "task_黄羊肉"
            return "task_finish"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:闻拓","打倒:跳兔(已完成)"],"dic_word_ocr",x3=50,y3=5):
                logger.info("任务交付")
            elif (self.target_info and self.target_info["目标体"] == "跳兔")  or self.get_value_from_key("跳兔", "dic_mutil_colors"):
                logger.info("寻找到跳兔,开始攻击")
                self.target_info.update({"目标体": "跳兔"}) #手动更新目标信息
                skill_num=random.randint(1, 2)
                self.key_press_op(f"{skill_num}",delay_time=2)

            elif self.get_value_from_key_operation("打倒:跳兔(/5)","dic_word_ocr",x3=40,y3=8):
                logger.info("寻找跳兔中")
    ...
    def task_黄羊肉(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:闻拓","寻物:黄羊肉(已完成)"],"dic_word_ocr",x3=83,y3=8):
                logger.info("任务交付")
            elif (self.target_info and self.target_info["目标体"] == "黄羊")  or self.get_value_from_key("黄羊", "dic_mutil_colors"):
                logger.info("寻找到黄羊,开始攻击")
                self.target_info.update({"目标体": "黄羊"}) #手动更新目标信息
                skill_num=random.randint(1, 2)
                self.key_press_op(f"{skill_num}",delay_time=2)
            elif self.get_value_from_key_operation("寻物:黄羊肉(/2)","dic_word_ocr",x3=38,y3=8):
                logger.info("寻找黄羊中")
    ...
    def task_熟悉药品(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:闻拓","了解:杂货商(完成)"],"dic_word_ocr",x3=83,y3=8):
                logger.info("任务交付")
    ...
    def task_门派的技能(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_value_from_key("学习:技能(完成)","dic_word_ocr"):
                ...
                colors = {'40cb00': (646,350),  '39d951': (646,357),'48f973': (646,359)}
                if self.py.ocr_vnc_click("确",629,441,695,465,delay_time=1):
                    logger.info("技能学习完成")
                elif self.py.mutil_colors_vnc_click(colors,634, 299, 670, 343,delay_time=1,x3=13,y3=-14):
                    logger.info("点击技能图标")

        elif "游戏主界面" in self.game_interface[0]:
            if self.get_items_from_all_keys_operation(["交付人:闻拓","学习:技能(完成)"],"dic_word_ocr",x3=83,y3=8):
                logger.info("任务交付")
    ...
    def task_新的任务(self):
        ...
        if self.get_value_from_key("【主线】危险的血狼", "dic_word_ocr"):
            self.node_current = "task_危险的血狼"
            return "task_finish"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:谢红鸾", "寻找:谢红鸾(完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
            elif self.get_value_from_key_operation("寻找:谢红鸾(未完成)", "dic_word_ocr", x3=50, y3=8):
                logger.info("寻找谢红鸾中")
    ...
    def task_危险的血狼(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:谢红鸾","打倒:血狼(已完成)"],"dic_word_ocr",x3=83,y3=12):
                logger.info("任务交付")

            elif (self.target_info and self.target_info["目标体"] == "血狼")  or self.find_ppocr_word_op("血狼",635, 65, 716, 97):
                logger.info("寻找到血狼,开始攻击")
                self.target_info.update({"目标体": "血狼"}) #手动更新目标信息
                skill_num=random.randint(1, 2)
                self.key_press_op(f"{skill_num}",delay_time=2)

            elif self.get_value_from_key_operation("打倒:血狼(/10)","dic_word_ocr",x3=38,y3=8):
                logger.info("寻找血狼中")
    ...
    def task_驿站的功用(self):
        ...
        if self.get_value_from_key("【主线】能力的考察(一)", "dic_word_ocr"):
            self.node_current = "task_能力的考察_1"
            return "task_finish"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:谢红鸾", "开通:驿站(外)(完成"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
            elif self.get_value_from_key_operation("开通:驿站(外)(未完", "dic_word_ocr", x3=50, y3=8):
                logger.info("前往驿站中")

    def task_能力的考察_1(self):
        """
        1,对话界面点击对话
        ...
        if self.get_value_from_key("【主线】能力的考察(二)", "dic_word_ocr"):
            self.node_current = "task_能力的考察_2"
            return "task_finish"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:谢红鸾", "采集:香芒草(已完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
            else:
                if self.coord[-1] and self.get_value_from_key("采集:香芒草(/3)", "dic_word_ocr"):
                    coord = self.get_value_from_key("采集光圈", "dic_mutil_colors")
                    logger.info(f"{coord}")
                    if coord:
                        self.mouse_move_op(coord[0][0], coord[0][1], delay_time=1)
                        self.mouse_left_click_op(coord[0][0], coord[0][1], delay_time=8)

                    else:
                        self.get_value_from_key_operation("采集:香芒草(/3)", "dic_word_ocr", x3=30, y3=3)

    def task_能力的考察_2(self):
        """
        1,对话界面点击对话
        2,木材的识别,这里取巧,直接点击了任务附近.
        2,【主线】收服武将,该节点完成
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:谢红鸾", "寻物:密码信(已完成)"], "dic_word_ocr", x3=83, y3=12):
                logger.info("任务交付")
            elif (self.target_info and self.target_info["目标体"] == "突厥密探") or self.find_ppocr_word_op("密探",651, 62, 767, 93):
                logger.info("寻找到突厥密探,开始攻击")
                self.target_info.update({"目标体": "突厥密探"})  # 手动更新目标信息
                skill_num = random.randint(1, 2)
                self.key_press_op(f"{skill_num}", delay_time=2)
    ...
    def task_收服武将(self):#todo.血衣镇副本没有写
        ...
        if self.get_value_from_key("【主线】前往阿依族", "dic_word_ocr"):
            self.node_current = "task_前往阿依族"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:谢红鸾", "打倒:血衣首领(已完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
    ...
    def task_前往阿依族(self):
        ...
        if self.get_value_from_key("【主线】寻找阿玛拉", "dic_word_ocr"):
            self.node_current = "task_寻找阿玛拉"
            return "task_finish"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:娜米露", "寻找:娜米露(完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
            elif self.get_value_from_key_operation("寻找:娜米露(未完成)", "dic_word_ocr", x3=50, y3=8):
                logger.info("寻找:娜米露中")
    ...
    def task_寻找阿玛拉(self):
        ...
        if self.get_value_from_key("【主线】枭军先锋", "dic_word_ocr"):
            self.node_current = "task_枭军先锋"
            return "task_finish"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:阿玛拉", "寻找:阿玛拉(完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
            elif self.get_value_from_key_operation("寻找:阿玛拉(未完成)", "dic_word_ocr", x3=50, y3=8):
                logger.info("寻找:阿玛拉中")
    ...
    def task_枭军先锋(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:阿玛拉","打倒:枭军先锋(已完成)"],"dic_word_ocr",x3=83,y3=8):
                logger.info("任务交付")

            elif (self.target_info and self.target_info["目标体"] == "枭军先锋") or self.find_ppocr_word_op("先锋",597, 71, 791, 92):
                logger.info("寻找到枭军先锋,开始攻击")
                self.target_info.update({"目标体": "枭军先锋"}) #手动更新目标信息
                skill_num = random.randint(1, 2)
                self.key_press_op(f"{skill_num}", delay_time=2)

            elif self.get_value_from_key_operation("打倒:枭军先锋(/10)","dic_word_ocr",x3=38,y3=8):
                logger.info("寻找枭军先锋中")
    ...
    def task_突如其来的隋军(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:阿玛拉", "找人:龙虎禁军(完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")

            elif self.get_ocr_vnc_click("龙虎禁军", 645, 174, 806, 202, x3=-57, y3=423, delay_time=1):
                logger.info("对话中")
    ...
    def task_危机(self):
        ...
        if self.get_value_from_key("【主线】战士的荣耀", "dic_word_ocr"):
            self.node_current = "task_战士的荣耀"
            return "task_finish"
    ...
    def task_战士的荣耀(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:阿玛拉","打倒:枭军大将(已完成)"],"dic_word_ocr",x3=83,y3=8):
                logger.info("任务交付")

            elif (self.target_info and self.target_info["目标体"] == "枭军大将")  or self.find_ppocr_word_op("枭军",597, 71, 791, 92):
                logger.info("寻找到枭军大将,开始攻击")
                self.target_info.update({"目标体": "枭军大将"}) #手动更新目标信息
                skill_num = random.randint(1, 2)
                self.key_press_op(f"{skill_num}", delay_time=2)

            elif self.get_value_from_key_operation("打倒:枭军大将(0/1)","dic_word_ocr",x3=38,y3=8):
                logger.info("寻找枭军大将中")
    def task_使用道具商城(self):
        ...
        if self.get_value_from_key("【主线】为壮志凌云之路", "dic_word_ocr"):
            self.node_current = "task_壮志凌云之路"
            return "task_finish"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:阿玛拉", "购买:青龙丹(完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
            elif self.get_value_from_key("购买:青龙丹(未完成)", "dic_word_ocr"):
                self.get_value_from_key_operation("res/dtws/main_task/商城.bmp","dic_image_hand",delay_time=2)

    def task_壮志凌云之路(self):
        """
        1,对话界面点击对话
        2,寻路
        3,【主线】谁动了乞丐的馒头,该节点完成
    ...
    def handle_task(self): #notice,待修改
        task_methods = {
            "task_继续主线":self.task_继续主线,
            'task_拜见闻拓': self.task_拜见闻拓,
            'task_熟悉武器': self.task_熟悉武器,
            "task_黄羊肉": self.task_黄羊肉,
            "task_熟悉药品": self.task_熟悉药品,
            "task_门派的技能": self.task_门派的技能,
            "task_新的任务": self.task_新的任务,
            "task_危险的血狼": self.task_危险的血狼,
            "task_驿站的功用": self.task_驿站的功用,
            "task_能力的考察_1": self.task_能力的考察_1,
            "task_能力的考察_2": self.task_能力的考察_2,
            "task_收服武将": self.task_收服武将,
            "task_前往阿依族": self.task_前往阿依族,
            "task_寻找阿玛拉": self.task_寻找阿玛拉,
            "self.task_枭军先锋": self.task_枭军先锋,
            "task_突如其来的隋军": self.task_突如其来的隋军,
            "task_危机": self.task_危机,
            "task_战士的荣耀": self.task_战士的荣耀,
            "task_使用道具商城": self.task_使用道具商城,
            "task_壮志凌云之路": self.task_壮志凌云之路,
...
#notice,待修改
        task_word_key=["【主线】继续主线","【主线】拜见闻拓","【主线】熟悉武器","【主线】黄羊肉","【主线】熟悉药品","【主线】门派的技能",
                       "【主线】新的任务","【主线】危险的血狼","【主线】驿站的功用","【主线】能力的考察(一)","【主线】收服武将","【主线】前往阿依族",
                       "【主线】枭军先锋","【主线】突如其来的隋军","【主线】危机","【主线】战士的荣耀","【主线】使用道具商城","【主线】壮志凌云之路",
                       ...
                       "【主线】乌部神老","【主线】突变","【主线】伤","【主线】能力的考察(二)","【主线】寻找阿玛拉"] # 主线任务关键词

        if not self.get_value_from_key("主线","dic_word_ocr") :
            task_name=self.find_ppocr_op(1174, 247, 1342, 277)
            logger.debug(f"{task_name}")

        if self.get_value_from_key(task_word_key[1],"dic_word_ocr"): #【主线】拜见宁婉儿
            self.task_拜见闻拓()
        ...
        elif self.get_value_from_key(task_word_key[39],"dic_word_ocr"): #【主线】竹海的强盗
            self.task_寻找阿玛拉()
        ...
        elif self.get_value_from_key(task_word_key[38],"dic_word_ocr"): #【主线】能力考察
            self.task_能力的考察_2()

        elif self.get_value_from_key(task_word_key[0],"dic_word_ocr"): #【主线】继续主线
            self.task_继续主线()
...
dic_reward_tasks={
    ...
    "image": {
        ...
        "res/dtws/main_task/香芒草.png":(382, 157, 1133, 661,0.8),#香芒草
        "res/dtws/main_task/香芒草_1.png":(382, 157, 1133, 661,0.8),#香芒草
        },
    "yolo":{
        # "豪猪":(0,0,0,0),
        # "蜀山":True,
    ...
    "mutil_colors":{
            "交付人:闻拓":{"colors":{"ecece4":(1209,322),
                                    "dedbd6":(1230,326),
                                    "f38100":(1266,320),
                                    "ea8519":(1281,332),
                                    },
                        "scope":(1201, 291, 1298, 333),
                        "tolerance":25},
            ...
            "打倒:跳兔(/5)":{"colors":{"ed140f":(1305, 279),
                                    "27d8d8":(1274, 285),
                                    "33d6d0":(1255, 283),
                                    "f0e8df":(1206, 288),
                                    },
                        "scope":(1200, 276, 1318, 309),
                        "tolerance":25},

            "黄羊":{"colors":{"c8e243":(666,103),
                            "cde535":(673,115),
                            "c7d647":(686,108),
                            "cce742":(684,113),
                            },
            ...
            "【主线】继续主线":{"colors":{"fff438":(1203,295),
                                        "f5f630":(1232,298),
                                        "e9820c":(1251,294),
                                        "f48d00":(1294,294),
                                        },
                                "scope":(1176, 251, 1307, 281),
                                "tolerance":25},

            "【主线】能力的考察(一)":{"colors":{"fec86a":(1326, 266),
                                            "dcad5c":(1319, 267),
                                            "edbb63":(1247, 266),
                                            "876b39":(1224, 266),
                                            },
                                    "scope":(1179, 252, 1356, 277),
                                    "tolerance":25},

            "【主线】能力的考察(二)":{"colors":{"fec86a":(1328, 262),
                                            "dcad5c":(1325, 271),
                                            "edbb63":(1259, 270),
                                            "987840":(1233, 260),
                                            },
            ...
            "采集光圈":{"colors":{"aae663":(802, 425),
                                "ade767":(803, 427),
                                "88d240":(806, 428),
                                        },
                                "scope":(394, 262, 1183, 726),
                                "tolerance":30},
...
public_res:公共资源
"""
reward_merged_dict_凌云=merge_dicts(dic_reward_tasks,public_res)
...
task_凌云_text_alternative_parameters=task_background_scope
...
"""
#notice,待修改
task_凌云_word_standard_dict={"继续":"【主线】继续主线",
                            "拜见":"【主线】拜见闻拓","闻":"拜见:闻拓(未完成)","拓":"拜见:闻拓(完成)","闻拓":"交付人:闻拓",
                            "器":"【主线】熟悉武器","兔":"打倒:跳兔(/5)","跳":"打倒:跳兔(已完成)",
                            "肉":"【主线】黄羊肉","黄":"寻物:黄羊肉(/2)","羊":"寻物:黄羊肉(已完成)",
                            "药品":"【主线】熟悉药品","杂":"了解:杂货商(未完成)","货":"了解:杂货商(完成)",
                            "派":"【主线】门派的技能","技":"学习:技能(未完成)","能":"学习:技能(完成)",
                            "新":"【主线】新的任务","鸾":"寻找:谢红鸾(未完成)","红":"寻找:谢红鸾(完成)","谢":"交付人:谢红鸾",
                            "险":"【主线】危险的血狼","血":"打倒:血狼(/10)","血|狼":"打倒:血狼(已完成)",
                            "功":"【主线】驿站的功用","站":"开通:驿站(外)(未完","驿":"开通:驿站(外)(完成",
                            "考":"【主线】能力的考察(一)","香":"采集:香芒草(/3)","芒":"采集:香芒草(已完成)",
                            "察":"【主线】能力的考察(二)","密|码":"寻物:密码信(0/1)","码|信":"寻物:密码信(已完成)",
                            "将":"【主线】收服武将","衣":"打倒:血衣首领(0/1)","领":"打倒:血衣首领(已完成)",
                            "助":"【主线】前往阿依族","娜":"寻找:娜米露(未完成)","米":"寻找:娜米露(完成)","露":"交付人:娜米露",
                            "阿玛拉":"【主线】寻找阿玛拉","阿":"寻找:阿玛拉(未完成)","玛":"寻找:阿玛拉(完成)","拉":"交付人:阿玛拉",
                            "枭":"【主线】枭军先锋","枭军":"打倒:枭军先锋(/10)","锋":"打倒:枭军先锋(已完成)",
                            "其":"【主线】突如其来的隋军","禁":"找人:龙虎禁军(未完成)","虎":"找人:龙虎禁军(完成)",
                            "机":"【主线】危机",
                            "荣":"【主线】战士的荣耀","大":"打倒:枭军大将(0/1)","大将":"打倒:枭军大将(已完成)",
                            "商":"【主线】使用道具商城","青":"购买:青龙丹(未完成)","丹":"购买:青龙丹(完成)",
                            "路":"【主线】壮志凌云之路","明":"寻找:明月(未完成)","月":"寻找:明月(完成)","明|月":"交付人:明月",
...
# 倒序排列
task_凌云_word_standard_dict_order = dict(sorted(task_凌云_word_standard_dict.items(), reverse=True))
...
#     # todo,注意参数名称更改
#     task=Mian_Task_凌云(py,win_hwnd[0],reward_merged_dict_主线,True,1200,task_主线_text_alternative_parameters,task_主线_word_standard_dict_order)
#     res=task.run()
