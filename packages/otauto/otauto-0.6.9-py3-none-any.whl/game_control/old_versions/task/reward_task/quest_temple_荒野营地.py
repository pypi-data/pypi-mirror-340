...
class reward_荒野营地(Quest_Task):
    ...
    def task_flow(self):
        ...
        coor_dict_start = {"荒野营地": (66,22)}
        coor_dict_end = {"荒野营地": (52, 54)}
        ...
        if self.map_name and self.map_name in map_name_list :
            ...
            if not condition_1 and not condition_2: #未锁定目标体
                ...
                if self.coord and all(isinstance(x, int) for x in self.coord[:2]):
                    for key, value in coordinate_node_荒野营地.items():
...
dic_reward_tasks={
    ...
    "image": {
        ...
        "res/dtws/reward_task/出口.bmp": (497,473,593,573, 0.8, 7, -10),
        },
    "yolo":{
        # "豪猪":(0,0,0,0),
        # "蜀山":True,
...
#资源合并,保持开启
quest_task_荒野营地_merged_dict=merge_dicts(dic_reward_tasks,quest_task_merged_dict) #todo:更改名称

#todo:任务信息,是否需要去除背景色,调试是打开
task_background_scope=(1177,251,1395,552)
task_荒野营地_text_alternative_parameters=task_background_scope #todo,更改名称
...
def run(target_name_list):
    ...
    # todo,注意参数名称更改
    task=reward_荒野营地(py,win_hwnd[0],quest_task_荒野营地_merged_dict,True,100,task_荒野营地_text_alternative_parameters,quest_task_word_standard_dict,target_name_list=target_name_list)
