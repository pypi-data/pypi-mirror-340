...
class reward_北隅冰原(Quest_Task):
    ...
    def task_flow(self):
        ...
        # 节点坐标
        coordinate_node_北隅冰原 = {(82,59):"res/dtws/reward_task/冰原_1.bmp",
                                    (110,64):"res/dtws/reward_task/冰原_2.bmp",
                                    (122,93):"res/dtws/reward_task/冰原_3.bmp",
                                    (95,89):"res/dtws/reward_task/冰原_4.bmp",}

        coor_dict_start = {"北隅冰原": (75,58)}
        coor_dict_end = {"北隅冰原": (98, 119)}
        ...
        if self.map_name and self.map_name in map_name_list :
            ...
            if not condition_1 and not condition_2: #未锁定目标体
                ...
                if self.coord and all(isinstance(x, int) for x in self.coord[:2]):
                    for key, value in coordinate_node_北隅冰原.items():
...
#资源合并,保持开启
quest_task_北隅冰原_merged_dict=merge_dicts(dic_reward_tasks,quest_task_merged_dict) #todo:更改名称

#todo:任务信息,是否需要去除背景色,调试是打开
task_background_scope=(1177,251,1395,552)
task_北隅冰原_text_alternative_parameters=task_background_scope #todo,更改名称
...
def run(target_name_list):
    ...
    # todo,注意参数名称更改
    task=reward_北隅冰原(py,win_hwnd[0],quest_task_北隅冰原_merged_dict,True,100,task_北隅冰原_text_alternative_parameters,quest_task_word_standard_dict,target_name_list=target_name_list)
