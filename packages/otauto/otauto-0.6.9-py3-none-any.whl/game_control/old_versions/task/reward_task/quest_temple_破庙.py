...
class reward_破庙(Quest_Task):
    ...
    def task_flow(self):
        ...
        # 节点坐标
        coordinate_node_破庙 = {(74,43):"res/dtws/reward_task/破庙_小地图1.bmp",
                                (75,62):"res/dtws/reward_task/破庙_小地图2.bmp",
                                (76, 83):"res/dtws/reward_task/破庙_小地图3.bmp",}

        coor_dict_start = {"破庙": (76,44)}
        coor_dict_end = {"破庙": (77, 112)}
        ...
        if self.map_name and self.map_name in map_name_list :
            ...
            if condition_1 or condition_2: #已锁定目标体
                ...
                if self.get_items_from_all_keys([key_info[0], key_info[1]], "dic_mutil_colors"):  # 已锁定目标体
                    self.py.find_image_vnc_click("res/dtws/reward_task/蜀山普通攻击.bmp",1198,808,1247,858,delay_time=1)
                    self.task_攻击目标()
            ...
            if not condition_1 and not condition_2: #未锁定目标体
                ...
                if self.coord and all(isinstance(x, int) for x in self.coord[:2]):
                    for key, value in coordinate_node_破庙.items():
...
dic_reward_tasks={
    ...
    "image": {
        ...
        "res/dtws/reward_task/出口.bmp": (419,459,616,603, 0.8, 17, -17),
        },
    "yolo":{
        # "豪猪":(0,0,0,0),
        # "蜀山":True,
...
#资源合并,保持开启
quest_task_破庙_merged_dict=merge_dicts(dic_reward_tasks,quest_task_merged_dict) #todo:更改名称

#todo:任务信息,是否需要去除背景色,调试是打开
task_background_scope=(1177,251,1395,552)
task_破庙_text_alternative_parameters=task_background_scope #todo,更改名称
...
def run(target_name_list):
    ...
    # todo,注意参数名称更改
    task=reward_破庙(py,win_hwnd[0],quest_task_破庙_merged_dict,True,100,task_破庙_text_alternative_parameters,quest_task_word_standard_dict,target_name_list=target_name_list)
