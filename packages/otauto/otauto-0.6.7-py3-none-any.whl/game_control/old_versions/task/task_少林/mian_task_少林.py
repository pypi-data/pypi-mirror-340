...
from task.task_少林.quest_task_凤鸣山_少林 import *
from task.task_少林.quest_task_血衣镇_少林 import *
from task.task_少林.reward_task_template_少林 import * #悬赏任务
from task.camp_task_template import camp_task_word_standard_dict
from task.task_少林.quest_task_屠狼洞_少林 import *
from game_role.role_奖励领取 import * #奖励领取
from game_role.role_技能学习 import * #技能学习
from game_role.role_技能配置_少林 import * #技能配置
...
class Mian_Task_少林(TaskModule): #凌云主线任务
    ...
    def task_拜见师兄(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:明尘","寻找:明尘(已完成)"],"dic_word_ocr",x3=83,y3=8):
                pass
            elif self.get_value_from_key_operation("寻找:明尘(未完成)","dic_word_ocr",x3=35,y3=8):
                pass
    ...
    def task_熟悉武器(self):
        ...
        if self.get_value_from_key("【主线】打倒野猪王", "dic_word_ocr"):
            self.node_current = "task_打倒野猪王"
            return "task_finish"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:明尘","打倒:野猪(已完成)"],"dic_word_ocr",x3=50,y3=5):
                logger.info("任务交付")
            elif (self.target_info and self.target_info["目标体"] == "野猪")  or self.find_ppocr_word_op("野猪", 629, 71, 757, 94):
                logger.info("寻找到野猪,开始攻击")
                self.target_info.update({"目标体": "野猪"}) #手动更新目标信息
                self.key_press_op("1",delay_time=1)

            elif self.get_value_from_key_operation("打倒:野猪(/5)","dic_word_ocr",x3=40,y3=8):
                logger.info("寻找野猪中")
    ...
    def task_打倒野猪王(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:明尘","打倒:野猪王(已完成)"],"dic_word_ocr",x3=83,y3=8):
                logger.info("任务交付")
            elif self.find_ppocr_word_op("已完成",1179, 249, 1361, 311):
                self.get_value_from_key_operation("交付人:明尘","dic_word_ocr",x3=53,y3=8)
            elif (self.target_info and self.target_info["目标体"] == "野猪王")  or self.find_ppocr_word_op("野猪王", 629, 71, 757, 94):
                logger.info("寻找到野猪王,开始攻击")
                self.target_info.update({"目标体": "野猪王"}) #手动更新目标信息
                self.key_press_op("1",delay_time=2)
            elif self.get_value_from_key_operation("打倒:野猪王(0/1)","dic_word_ocr",x3=38,y3=8):
                logger.info("寻找野猪王中")
    ...
    def task_熟悉药品(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:明尘","了解:杂货商(完成)"],"dic_word_ocr",x3=83,y3=8):
                logger.info("任务交付")
    ...
    def task_门派的技能(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_value_from_key("学习:技能(完成)","dic_word_ocr"):
                ...
                colors = {"ffffc9":(803, 455), "36ca00":(802, 461),"48fb76":(804, 468)}
                if self.py.ocr_vnc_click("确",629,441,695,465,delay_time=1):
                    logger.info("技能学习完成")
                elif self.py.mutil_colors_vnc_click(colors,476, 500, 508, 528,delay_time=1,x3=13,y3=-14):
                    logger.info("点击技能图标")

        elif "游戏主界面" in self.game_interface[0]:
            if self.get_items_from_all_keys_operation(["交付人:明尘","学习:技能(完成)"],"dic_word_ocr",x3=83,y3=8):
                logger.info("任务交付")
    ...
    def task_新的任务(self):
        ...
        if self.get_value_from_key("【主线】夺回佛珠", "dic_word_ocr"):
            self.node_current = "task_夺回佛珠"
            return "task_finish"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:明悔", "寻找:明悔(完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
            elif self.get_value_from_key_operation("寻找:明悔(未完成)", "dic_word_ocr", x3=50, y3=8):
                logger.info("寻找明悔中")
    ...
    def task_夺回佛珠(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:明悔","寻物:佛珠(已完成)"],"dic_word_ocr",x3=38,y3=4):
                logger.info("任务交付")

            elif self.find_ppocr_word_op("已",1179, 249, 1361, 311):
                self.get_value_from_key_operation("交付人:明悔","dic_word_ocr",x3=53,y3=8)

            elif (self.target_info and self.target_info["目标体"] == "蒙面刀")  or self.find_ppocr_word_op("蒙面刀",635, 65, 716, 97):
                logger.info("寻找到蒙面刀,开始攻击")
                self.target_info.update({"目标体": "蒙面刀"}) #手动更新目标信息
                skill_num=random.randint(1, 3)
                self.key_press_op(f"{skill_num}",delay_time=2)

            elif self.get_value_from_key_operation("寻物:佛珠(0/1)","dic_word_ocr",x3=38,y3=4) or self.find_ppocr_word_click("佛珠",1175, 241, 1328, 314,x3=50,y3=4):
                logger.info("寻找蒙面刀中")
    ...
    def task_驿站的功用(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:明悔", "开通:驿站(外)(完成"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
            elif self.get_value_from_key_operation("开通:驿站(外)(未完成", "dic_word_ocr", x3=50, y3=8):
                logger.info("前往驿站中")
            elif self.get_ocr_vnc_click("未完成",1174, 243, 1408, 327):
                logger.info("前往驿站中")
    ...
    def task_能力的考察(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:明悔", "上等木料(已完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
            else:
                if self.coord[-1] and (self.get_value_from_key("采集:上等木料(/3)", "dic_word_ocr") or self.find_ppocr_word_op("0/3",1180, 243, 1360, 321)):
                    if self.get_yolo_click("木料",delay_time=5):
                        logger.info("采集中")

                    elif self.get_value_from_key_operation("采集:上等木料(/3)", "dic_word_ocr", x3=50, y3=3) or self.find_ppocr_word_click("0/3",1180, 243, 1360, 321) :
                        pass
    ...
    def task_收服武将(self):#todo.血衣镇副本没有写
        ...
        if self.get_value_from_key("【主线】明悟的求助", "dic_word_ocr"):
            self.node_current = "task_明悟的求助"
        ...
        if "对话界面" in self.game_interface[0]:
        ...
        elif self.map_name and "血衣镇" in self.map_name:
            ...
            if self.get_items_from_all_keys_operation(["交付人:明悔", "打倒:血衣首领(已完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
    ...
    def task_明悟的求助(self):
        ...
        if self.get_value_from_key("【主线】夺回舍利子", "dic_word_ocr"):
            self.node_current = "task_夺回舍利子"
            return "task_finish"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:明悟", "寻找:明悟(完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
            elif self.get_value_from_key_operation("寻找:明悟(未完成)", "dic_word_ocr", x3=50, y3=8):
                logger.info("寻找:明悟中")
    ...
    def task_夺回舍利子(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:明悟","寻物:舍利子(已完成)"],"dic_word_ocr",x3=83,y3=8):
                logger.info("任务交付")

            elif (self.target_info and self.target_info["目标体"] == "盗墓贼") or self.find_ppocr_word_op("盗墓贼",597, 71, 791, 92):
                logger.info("寻找到盗墓贼,开始攻击")
                self.target_info.update({"目标体": "盗墓贼"}) #手动更新目标信息
                skill_num = random.randint(1, 2)
                self.key_press_op(f"{skill_num}", delay_time=2)

            elif self.get_value_from_key_operation("寻物:舍利子(0/1)","dic_word_ocr",x3=38,y3=8):
                logger.info("寻找盗墓贼中")
    ...
    def task_突如其来的隋军(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:明悟", "找人:龙虎禁军(完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
    ...
    def task_危机(self):
        ...
        if self.get_value_from_key("【主线】明达的挑战", "dic_word_ocr"):
            self.node_current = "task_明达的挑战"
            return "task_finish"
    ...
    def task_明达的挑战(self):
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:明悟","打倒:明达(已完成)"],"dic_word_ocr",x3=83,y3=8):
                logger.info("任务交付")

            elif (self.target_info and self.target_info["目标体"] == "明达")  or self.find_ppocr_word_op("明达",597, 71, 791, 92):
                logger.info("寻找到明达,开始攻击")
                self.target_info.update({"目标体": "明达"}) #手动更新目标信息
                skill_num = random.randint(1, 2)
                self.key_press_op(f"{skill_num}", delay_time=2)

            elif self.get_value_from_key_operation("打倒:明达(0/1)","dic_word_ocr",x3=52,y3=4):
                self.mouse_move_op(1160,309)
                logger.info("寻找明达中")
    def task_使用道具商城(self):
        ...
        if self.get_value_from_key("【主线】为拯救天下之路", "dic_word_ocr"):
            self.node_current = "task_拯救天下之路"
            return "task_finish"
        ...
        if "对话界面" in self.game_interface[0]:
            ...
            if self.get_items_from_all_keys_operation(["交付人:明悟", "购买:青龙丹(完成)"], "dic_word_ocr", x3=83,
                                                      y3=8):
                logger.info("任务交付")
            elif self.get_value_from_key("购买:青龙丹(未完成)", "dic_word_ocr"):
                self.get_value_from_key_operation("res/dtws/main_task/商城.bmp","dic_image_hand",delay_time=2)

    def task_拯救天下之路(self):
        """
        1,对话界面点击对话
        2,寻路
        3,【主线】谁动了乞丐的馒头,该节点完成
...
#notice,待修改
        task_word_key=["【主线】继续主线","【主线】拜见师兄","【主线】熟悉武器","【主线】打倒野猪王","【主线】熟悉药品","【主线】门派的技能",
                       "【主线】新的任务","【主线】夺回佛珠","【主线】驿站的功用","【主线】能力的考察","【主线】收服武将","【主线】明悟的求助",
                       "【主线】夺回舍利子","【主线】突如其来的隋军","【主线】危机","【主线】明达的挑战","【主线】使用道具商城","【主线】拯救天下之路",
        ...
        elif self.get_value_from_key(task_word_key[7],"dic_word_ocr") or self.find_ppocr_word_op("夺回佛珠",1175, 241, 1328, 314): #【主线】剑客的挑战
            self.task_夺回佛珠()
...
dic_reward_tasks={
    ...
    "mutil_colors":{
        ...
        "了解:杂货商(未完成)": {"colors": {"ed140f":(1339, 278),
                                   "2df9f9":(1320, 288),
                                   "27d8d8":(1302, 279),
                                   "2ae8e8":(1284, 277),
                                   },
                        "scope": (1201, 270, 1388, 294),
                        "tolerance": 25},

            "交付人:明尘":{"colors":{"ff8800":(1291, 296),
                                    "ee7f00":(1266, 297),
                                    "f0e8df":(1245, 303),
                                    "d0c9c1":(1209, 303),
                                    },
                        "scope":(1194, 274, 1320, 296),
                        "tolerance":25},

            "交付人:明悔": {"colors": {"ff8800":(1291, 302),
                                       "ee7f00":(1287, 300),
                                       "aa5b00":(1265, 302),
                                       "f0e8df":(1235, 304),
                                       },
                            "scope": (1201, 291, 1300, 307),
                            "tolerance": 25},
            ...
            "采集:上等木料(/3)":{"colors":{"ed140f":(1334, 287),
                                        "2df9f9":(1305, 288),
                                        "21b7b7":(1289, 283),
                                        "27d8d8":(1256, 287),
                                        },
                                        "scope":(1204, 275, 1352, 291),
                                        "tolerance":25},

            "打倒:野猪(/5)":{"colors":{"ed140f":(1306, 287),
                                    "2ae8e8":(1267, 287),
                                    "2df9f9":(1267, 280),
                                    "27d8d8":(1251, 287),
                                    },
            ...
            # "【主线】夺回佛珠": {"colors": {"edbb63":(1299, 271),
            #                               "dcad5c":(1282, 268),
            #                               "fec86a":(1243, 265),
            #                               },
            #                    "scope": (1175, 253, 1316, 279),
            #                    "tolerance": 25},
...
public_res:公共资源
"""
reward_merged_dict_少林=merge_dicts(dic_reward_tasks,public_res)
...
task_少林_text_alternative_parameters=task_background_scope
...
"""
#notice,待修改
task_少林_word_standard_dict={"继续":"【主线】继续主线",
                            "拜见":"【主线】拜见师兄","明|尘":"寻找:明尘(未完成)","尘":"寻找:明尘(完成)","明尘":"交付人:明尘",
                            "器":"【主线】熟悉武器","野猪":"打倒:野猪(/5)","猪":"打倒:野猪(已完成)",
                            "野":"【主线】打倒野猪王","猪王":"打倒:野猪王(0/1)","野猪王":"打倒:野猪王(已完成)",
                            "药品":"【主线】熟悉药品","杂":"了解:杂货商(未完成)","货":"了解:杂货商(完成)",
                            "派":"【主线】门派的技能","技":"学习:技能(未完成)","能":"学习:技能(完成)",
                            "新":"【主线】新的任务","明|悔":"寻找:明悔(未完成)","悔":"寻找:明悔(完成)","明悔":"交付人:明悔",
                            "夺":"【主线】夺回佛珠","长":"寻物:佛珠(0/1)","枪":"寻物:佛珠(已完成)",
                            "功":"【主线】驿站的功用","站":"开通:驿站(外)(未完成","驿":"开通:驿站(外)(完成",
                            "考":"【主线】能力的考察","木":"采集:上等木料(/3)","料":"采集:上等木料(已完成)",
                            "将":"【主线】收服武将","衣":"打倒:血衣首领(0/1)","领":"打倒:血衣首领(已完成)",
                            "助":"【主线】明悟的求助","明|悟":"寻找:明悟(未完成)","悟":"寻找:明悟(完成)","明悟":"交付人:明悟",
                            "舍":"【主线】夺回舍利子","利":"寻物:舍利子(0/1)","舍|利":"寻物:舍利子(已完成)",
                            "其":"【主线】突如其来的隋军","禁":"找人:龙虎禁军(未完成)","虎":"找人:龙虎禁军(完成)",
                            "机":"【主线】危机",
                            "挑战":"【主线】明达的挑战","打":"打倒:明达(0/1)","倒":"打倒:明达(已完成)",
                            "商":"【主线】使用道具商城","青":"购买:青龙丹(未完成)","丹":"购买:青龙丹(完成)",
                            "路":"【主线】拯救天下之路","明":"寻找:明月(未完成)","月":"寻找:明月(完成)","明|月":"交付人:明月",
...
# 倒序排列
task_少林_word_standard_dict_order = dict(sorted(task_少林_word_standard_dict.items(), reverse=True))
