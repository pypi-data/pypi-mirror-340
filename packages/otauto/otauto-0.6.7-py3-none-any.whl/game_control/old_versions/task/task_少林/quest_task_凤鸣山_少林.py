...
class Quest_Task_凤鸣山(Quest_Task):
    ...
    def task_flow(self):
        ...
        if self.map_name and self.map_name in "凤鸣山":#副本内
            if self.get_ocr_vnc_click("离开",549, 583, 648, 632,delay_time=10):
                logger.info("退出副本")
                ...
                if  self.get_items_from_keys([key_info[0],key_info[1]],"dic_mutil_colors"):#已锁定目标体
                    ...
                    if self.task_目标识别() == "task_fail":  # 识别目标体失败
                        ...
                        if self.map_name == "凤鸣山":
                            ...
                            res_num = self.way_finding_node_op(coordinate_node_dispose, debug=True, range_num=5,
                                                               record_num=4)
                            if res_num == 1:
                                self.node_finish_flag = True
                                return True
