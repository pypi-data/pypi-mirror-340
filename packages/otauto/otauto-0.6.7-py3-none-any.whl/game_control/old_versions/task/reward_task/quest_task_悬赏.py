...
class reward_悬赏(Quest_Task):
    ...
    def task_data_荒野营地(self):
        ...
        return {
            ...
            "find_way_node": [(66,23),(67,31),(71,38),(77,40),(82,50),(81,62),(77,69),(70,78),(62,73),(56,68),(57,61),(53,54)],
    ...
    def task_flow(self):
        ...
        map_name_list =["蜀南竹海","白屏寨","少林","天煞盟","青城山","塔林",]
            ...
            if condition_1 or condition_2: #已锁定目标体
                ...
                if self.coord and all(isinstance(x, int) for x in self.coord[:2]): #根据坐标判断
                    ...
                    if self.find_way_node: #判断节点是否存在
                        ...
                        res_num = self.way_finding_node_op(coordinate_node_dispose,debug=True, range_num=5,record_num=4,option=2)
                        logger.debug("节点寻路中")
                        if res_num == 1:
...
scene_scope=(371, 136, 973, 696, 0.8,)
dic_reward_tasks={
    "word": {
...
#todo:文字模糊匹配,调试时开启
quest_task_word_standard_dict={"疑犯":"疑犯(0/1)","犯":"疑犯(已完成)","交付":"点击交付"}
...
def run():
    ...
    # todo,注意参数名称更改
    task=reward_悬赏(py,win_hwnd[0],quest_task_悬赏_merged_dict,True,100,task_悬赏_text_alternative_parameters,quest_task_word_standard_dict)
