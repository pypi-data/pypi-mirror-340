...
class reward_南荒毒沼(Quest_Task):
    ...
    def task_目标识别(self):
        ...
        if "游戏主界面" in self.game_interface[0]:
            ...
            if self.target_position_map : #目标体在小地图出现
                ...
                if check_proximity(self.target_position_map,self.role_position_map,threshold=10):
    ...
    def task_攻击目标(self):
        ...
        self.ls_progress = "任务中"
        self.node_current = "task_攻击目标"
        logger.info(f"目标名称:{self.target_name_list}")

        self.node_coords=[] #清空节点坐标

        if not self.get_items_from_keys(["目标体等级","目标体血条"],"dic_mutil_colors") or not self.find_yolo(["红色名称"]):
            self.attack_a_target_flag=False
            return "task_finish"

        elif "游戏主界面" in self.game_interface[0]:
            if self.target_name_list is None:
                logger.error("没有设置目标体名称,请检查")

            if self.attack_a_target_flag :
                logger.info(f"已锁定目标,开始攻击")
                self.start_attack()

            elif self.target_info and self.target_info["目标体"] in self.target_name_list:
                logger.info(f"寻找到{self.target_info},开始攻击")
                self.start_attack()

            elif self.get_items_from_all_keys(["目标体等级", "目标体红名"], "dic_mutil_colors"):
                self.start_attack()

            elif self.get_items_from_all_keys(["目标体等级", "目标体血条"], "dic_mutil_colors"):
                self.start_attack()

            elif self.get_items_from_all_keys(["目标体红名", "目标体血条"], "dic_mutil_colors"):
                self.start_attack()
    ...
    def task_flow(self):
        ...
        if self.get_ocr_vnc_click("确定",670,440,737,465) or  self.map_name and self.map_name in map_name_list :
            ...
            if "游戏主界面" in self.game_interface[0]:
                ...
                if self.get_value_from_key_operation("出口","dic_word_hand",delay_time=15,mouse_button=3):
                    logger.info("点击地图出口")
                elif self.get_value_from_key_operation("res/dtws/reward_task/出口.bmp","dic_image_hand",delay_time=15,mouse_button=3):
                    logger.info("点击地图出口")
...
#资源合并,保持开启
quest_task_南荒毒沼_merged_dict=merge_dicts(dic_reward_tasks,quest_task_merged_dict) #todo:更改名称

#todo:任务信息,是否需要去除背景色,调试是打开
task_background_scope=(1177,251,1395,552)
task_南荒毒沼_text_alternative_parameters=task_background_scope #todo,更改名称
...
def run(target_name_list):
    ...
    # todo,注意参数名称更改
    task=reward_南荒毒沼(py,win_hwnd[0],quest_task_南荒毒沼_merged_dict,True,100,task_南荒毒沼_text_alternative_parameters,quest_task_word_standard_dict,target_name_list=target_name_list)
    res=task.run(classes=2,running_time=1)
    print(res)

target_name_list=["云瘴妖花","巨狼",]
