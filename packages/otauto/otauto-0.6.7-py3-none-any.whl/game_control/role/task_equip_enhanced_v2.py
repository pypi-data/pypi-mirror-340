...
class TaskEquipEnhanced(TaskLogic):
    ...
    def task_装备强化(self):
        ...
        if self.node_counter >=8 or self.err_num >= 3:  # 如果超过5次,则放弃任务
            self.node_current = "task_强化完成"
            self.node_counter = 0  # 重置节点计数器
            ...
            if self.ready_flag:
                ...
                if "背包界面" in self.interface_info:
                    if not self.organize_backpack_flag:
                        ...
                        self.eqiup_numpy=self.vnc_sync.capture_full_screen_as_numpy() # 获取当前屏幕截图
                        self.last_color_block_list = self.find_color_black(*self.scope_背包)
                ...
                if "装备强化界面" in self.interface_info or self.find_data_from_keys_list(["欢迎光临"],
                    ...
                    if self.num_enhance >= 4:
                        ...
                        return True
                    elif self.find_data_from_keys_list_click(["resource/images_info/other/强化.png"], self.image_data, delay_time=3):
                        self.node_counter = 0  # 重置节点计数器
                        ...
                        return True
                            ...
                            ["resource/images_info/other/强化_无装备.png"], self.image_data, x3=267, y3=95,
                            delay_time=1):
                        logger.warning("装备强化界面,无装备")
                        self.node_counter = 0  # 重置节点计数器
                        self.num_enhance = 0  # 重置次数
                        self.ready_flag = True  # 重置标志位
                        return True
                    ...
                    if differences and difference_ls: # 都存在的情况下
                        ...
                        if res_ls:
                            ...
                            if len(difference_ls)==1:
                                self.equip_position=difference_ls[0][0]+15,difference_ls[0][1]+15 # 已脱装备的位置计算为准
...
equip_enhanced_data={
    ...
    "image": {
        ...
        r"resource/images_info/other/强化.png": {
            "scope": (664, 617, 763, 662),
            "con": 0.8,
            "enable": True,
            "unique": True,
            "model": 1,
        },
        r"resource/images_info/other/强化_无装备.png": {
            "scope": (563, 525, 621, 590),
            "con": 0.8,
            "enable": True,
            "unique": True,
            "model": 1,
