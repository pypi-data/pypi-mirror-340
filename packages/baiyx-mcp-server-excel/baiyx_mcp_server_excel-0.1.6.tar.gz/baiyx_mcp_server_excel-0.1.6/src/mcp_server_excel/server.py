...
class ExcelServer:
    ...
    def setup_tools(self):
        ...
        def read_sheet_data(file_path: str, sheet_name: str, range: Optional[str] = None) -> ExcelData:
            ...
            try:
                ...
                # 如果是空数据，返回空列表
                if len(data) == 0:
                    return ExcelData(data=[], range=range or "")
                
                return ExcelData(
                    data=data,
                    range=range or f"A1:{get_column_letter(len(df.columns))}{len(data)}"
                )
        ...
        def write_sheet_formula(file_path: str, sheet_name: str, range: str, formulas: List[str]) -> bool:
            ...
            try:
                ...
                # 解析起始单元格
                start_col = column_index_from_string(range[0])
                start_row = int(range[1:])
                ...
                raise Exception(f"写入公式失败: {str(e)}")
    ...
    def _parse_range(self, range_str: str) -> tuple[str, int, str, int]:
        ...
        try:
            ...
            raise Exception(f"Invalid range format. Expected 'A1:C10', got '{range_str}': {str(e)}")
